...
- **No `color` prop** — `OIcon` has no color prop. Icons render in `currentColor` by default, which means they inherit the text color of their parent automatically. **Set color on the parent element** using semantic tokens, not on the icon itself.
  ...
  > **Visibility trap**: If the parent element carries a muted or light token (e.g. `tw:text-text-secondary` = grey-500, or no explicit text color at all), the icon will render in that same light grey and become nearly invisible — especially prominent after migrating from Quasar, where icons were explicitly coloured. If an icon looks washed-out after migration, check what text color the parent is resolving to and upgrade it to `tw:text-text-primary` or an appropriate semantic token. **Never fix this with a hardcoded colour class on the icon.** Fix the parent's token.
...
- [ ] **For every `q-btn`: noted `flat`/`outline`/`unelevated`/`no-border` props and mapped correctly** — flat→ghost, outline→outline, non-flat unelevated→outline or secondary, `no-border`→ghost
- [ ] **`o2-primary-button` moved from container to OButton `variant="primary"`** (if applicable)
- [ ] **OButtonGroup outer border preserved** — added `el-border` class if original `q-btn-group` had a visible border
...
- [ ] **Text and icon colours are visibly readable** — open in browser and confirm no labels or icons look washed-out / too light. Inactive tabs, ghost buttons, and `OIcon` inside muted parents are the most common failure points (see **Common Pitfalls** below)
- [ ] **Git diff visual review** — run `git diff main -- <file>` and for EVERY removed `q-btn` line, verify the replacement OButton has an equivalent variant. Especially watch for: `outline` prop → `variant="outline"`, `color="primary"` non-flat → `variant="primary"` or keep primary context via parent.
...
| `icon="add"`                            | `<template #icon-left><q-icon name="add" size="16px" /></template>` | Named slot `#icon-left` / `#icon-right`                                             |
| `color="primary"`                       | (drop it)                                                           | Variant controls color via tokens                                                   |
| `flat`                                  | `variant="ghost"`                                                   | Flat = no border, no bg                                                             |
| `flat color="primary"`                  | `variant="ghost-primary"`                                           | Flat with primary text color                                                        |
| `flat color="negative"`                 | `variant="ghost-destructive"`                                       | Flat with destructive red color                                                     |
| `outline`                               | `variant="outline"`                                                 | Border + transparent bg                                                             |
| (plain unelevated, no flat)             | `variant="secondary"` or `variant="outline"`                        | See border rule below                                                               |
| `class="o2-primary-button"`             | `variant="primary"`                                                 | Legacy app class → primary variant                                                  |
| `class="o2-secondary-button"`           | `variant="outline"`                                                 | Legacy app class → secondary/outline                                                |
| `dense`                                 | `size="sm"`                                                         |                                                                                     |
| `round flat icon="..."`                 | `variant="ghost" size="icon-circle"`                                | Round icon-only button                                                              |
| `round flat dense size="sm" icon="..."` | `variant="ghost" size="icon-circle"`                                | Compact round icon button                                                           |
...
| `no-caps`                               | (drop it)                                                           | Global CSS already sets text-transform:none for all q-btn, OButton never uppercases |
| `unelevated`, `push`, `glossy`          | (drop it)                                                           | Not applicable                                                                      |

**⚠ Critical: Border preservation rule**

In this project, `quasar-overrides.scss` adds `.q-btn:before { border: 1px solid $input-field-border-color }` to ALL non-flat `q-btn`. Flat buttons have it removed via `.q-btn--flat:before { border: none }`.

- `q-btn` (no `flat`) → had a visible border → use `variant="outline"` (or `variant="secondary"` for unelevated buttons with a background)
- `q-btn flat` → had no border → use `variant="ghost"`
- `q-btn class="no-border"` → had no border (explicit) → use `variant="ghost"`
- `q-btn outline` → had Quasar outline style → use `variant="outline"`

**Never migrate a non-flat `q-btn` to `variant="ghost"` — that removes the border which was visible in production.**

**⚠ Critical: `o2-primary-button` on container vs button**

When `o2-primary-button` class was on a `q-btn-group` container (not on `q-btn` itself), the inner `q-btn` was visually primary because the group bg showed through flat/transparent children. When migrating:

- Migrate the **inner `q-btn`** to `OButton variant="primary"` (not the container)
- Remove `o2-primary-button` from the `OButtonGroup` wrapper
...
**⚠ Critical: OButtonGroup has NO outer border**

`q-btn-group` in this project gets `border: 1px solid $input-field-border-color` from `quasar-overrides.scss`. `OButtonGroup` is a plain flex div — it has NO outer border. If the original group had a visible border, add `class="el-border"` to the OButtonGroup, or ensure children use `variant="outline"` (each button has its own border).

**⚠ Critical: Never leave `q-btn` inside `OButtonGroup`**

All direct OButton slot children must be `OButton` (not `q-btn`). A group with `q-btn` inside will have no border management or size unification. Migrate ALL inner `q-btn` before considering the group migration complete.

**Height rule**: Never add `tw:h-[*]` to `OButtonGroup`. Height is determined by children's `size` prop.
...
| **Non-flat `q-btn` → `variant="ghost"` loses the border**                     | In this project all non-flat q-btn have `.q-btn:before { border: 1px solid }`. Flat buttons have no border. Map: `flat` → `ghost`, non-flat → `outline` or `secondary`. Never use `ghost` for a button that was not flat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **`o2-primary-button` on container instead of on button**                     | When a `q-btn-group` had `class="o2-primary-button"`, the inner buttons inherited the primary bg. After migrating to `OButtonGroup`, put `variant="primary"` on the **inner `OButton`**, not on the group wrapper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Missing outer border after replacing q-btn-group with OButtonGroup**        | `q-btn-group` had a CSS border from `quasar-overrides.scss`. `OButtonGroup` has none. Add `class="el-border"` to OButtonGroup, or ensure inner OButton children use `variant="outline"` so each button has its own border.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **`ghost-destructive` disabled button drops its red color**                   | The disabled state of `ghost-primary` and `ghost-destructive` uses `tw:disabled:opacity-60` (not `tw:disabled:text-text-disabled`). This deliberately preserves the red/primary color while signaling disabled via opacity. If a `ghost-destructive` button looks neutral-grey when disabled, the component token is wrong — do NOT paper over it with inline style; fix the variant class in `OButton.vue`.                                                                                                                                                                                                                                                                                                          |
| **Quasar spacing classes (`q-ml-md`, `q-mr-sm`, `q-ml-sm`) on O2 components** | O2 components must not carry Quasar spacing classes. Replace with Tailwind equivalents on the component's `class` attr or the parent layout: `q-ml-sm` → `tw:ml-2`, `q-ml-md` → `tw:ml-4`, `q-mr-sm` → `tw:mr-2`, `q-mr-md` → `tw:mr-4`. Prefer removing margin entirely and letting the parent `tw:gap-*` or `tw:space-x-*` handle spacing uniformly.                                                                                                                                                                                                                                                                                                                                                                |
...
| **Text / icon colours look too faint or invisible after migration**           | This is the most common visual regression after migrating tabs and buttons. Root causes: (1) Inactive `OTab` labels use `--color-tabs-inactive-text` = `text-secondary` (grey-500) — intentionally muted, but can look washed-out. (2) Ghost `OButton` uses `--color-button-ghost-text` = `text-secondary`. (3) `OIcon` renders in `currentColor` — a light parent means a light icon. **Fix order**: first ensure the correct O2 `variant` / `size` is set; then verify the parent element's text token is strong enough; if the token value itself is the problem, update `--color-tabs-inactive-text` (or the relevant token) in `component.css` + `dark.css`. **Never paper over with hardcoded colour classes.** |
