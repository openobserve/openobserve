...
<template>
  <q-layout view="hHh lpR fFf" :class="[
    ...
    <q-header :class="[store?.state?.theme == 'dark' ? 'dark-mode' : 'bg-white']">
      <q-toolbar>
        <div class="flex relative-position q-mr-sm" v-if="(config.isEnterprise == 'true' &&
    ...
    ">
          <span v-if="store.state.zoConfig.hasOwnProperty('custom_logo_text') &&
    store.state.zoConfig?.custom_logo_text != ''
    " class="text-h6 text-bold q-pa-none cursor-pointer q-mr-sm" @click="goToHome">{{
    store.state.zoConfig.custom_logo_text }}</span>
          <img v-if="store.state.zoConfig.hasOwnProperty('custom_logo_img') &&
    store.state.zoConfig?.custom_logo_img != null
    ...
    store.state.organizationData.quotaThresholdMsg
        ...
        <div data-test="navbar-organizations-select" class="q-mx-sm current-organization">
