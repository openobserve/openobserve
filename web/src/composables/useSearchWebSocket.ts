...
const useSearchWebSocket = () => {
  ...
  const onMessage = (response: any) => {
    ...
    traces[response.content.trace_id]?.message?.forEach((handler: any) => handler(response));
  };
  ...
  const onClose = async (response: any, _socketId: string) => {
    ...
    if (shouldRetry && socketFailureCount.value < maxSearchRetries) {
      ...
      setTimeout(() => {
        Object.keys(traces).forEach((traceId) => {
          if (
            (traces[traceId].socketId === _socketId && traces[traceId].isInitiated) ||
            !traces[traceId].socketId
          ) {
            // Don't send error event when retry is happening
            traces[traceId]?.close.forEach((handler: any) =>
              handler({
                ...response,
                type: "close",
                code: 1000,
  ...
  const onError = async (response: any) => {
    if (response.type === "error") {
      ...
      if (response.content.code === 401) {
        ...
        const isReadOnly =
          socketMeta.value[traces[response.content.trace_id]?.socketId as string]?.isReadOnly;
    ...
    traces[response.content.trace_id]?.error?.forEach((handler: any) => handler(response));
    closeSearchTrace(response.content.trace_id, response);
  };
  ...
  const sendSearchMessageBasedOnRequestId = (data: any) => {
    try {
      ...
      if (
        inactiveSocketId.value &&
        traces[data.content.trace_id]?.socketId === inactiveSocketId.value
      ) {
  ...
  const cancelSearchQueryBasedOnRequestId = (payload: { trace_id: string; org_id: string }) => {
    ...
    const socket = _webSocket!.getWebSocketBasedOnSocketId(_socketId as string);
