...
describe("useAiChat", () => {
  ...
  describe("registerAiChatHandler", () => {
    ...
    it("should register a non-function handler", () => {
      ...
      expect(() =>
        aiChatComposable.registerAiChatHandler(nonFunctionHandler),
      ).not.toThrow();
    });
    ...
    it("should register undefined as handler", () => {
      expect(() =>
        aiChatComposable.registerAiChatHandler(undefined),
      ).not.toThrow();
    });
    ...
    it("should register an object as handler", () => {
      ...
      expect(() =>
        aiChatComposable.registerAiChatHandler(objectHandler),
      ).not.toThrow();
    });
  ...
  describe("getContext", () => {
    ...
    it("should handle async handler that rejects", async () => {
      ...
      await expect(aiChatComposable.getContext()).rejects.toThrow(
        "Handler failed",
      );
    });
  });
  ...
  describe("fetchAiChat", () => {
    ...
    it("should make API call with correct URL and body when model is provided", async () => {
      ...
      const result = await aiChatComposable.fetchAiChat(
        mockMessages,
        model,
        orgId,
      );
    ...
    it("should handle different organization IDs correctly", async () => {
      ...
      await aiChatComposable.fetchAiChat(
        mockMessages,
        "gpt-4",
        "different-org",
      );
