...
const dashboardPanelData = reactive({ ...getDefaultDashboardPanelData() });
...
const useMetricsExplorer = () => {
  ...
  const addXAxisItem = (row: any) => {
    ...
    if (
      !dashboardPanelData.data.fields.x.find((it: any) => it.column == row.name)
    ) {
      dashboardPanelData.data.fields.x.push({
        label: !dashboardPanelData.data.customQuery
          ? generateLabelFromName(row.name)
          : row.name,
        alias: !dashboardPanelData.data.customQuery
          ? "x_axis_" + (dashboardPanelData.data.fields.x.length + 1)
          : row.name,
        column: row.name,
        color: null,
        aggregationFunction:
  ...
  const addYAxisItem = (row: any) => {
    ...
    if (
      !dashboardPanelData.data.fields.y.find((it: any) => it.column == row.name)
    ) {
      dashboardPanelData.data.fields.y.push({
        label: !dashboardPanelData.data.customQuery
          ...
          : row.name,
        alias: !dashboardPanelData.data.customQuery
          ? "y_axis_" + (dashboardPanelData.data.fields.y.length + 1)
          : row.name,
        column: row.name,
  ...
  const getNewColorValue = () => {
    const YAxisColor = dashboardPanelData.data.fields.y.map(
      (it: any) => it.color
    );
  ...
  const updateArrayAlias = () => {
    dashboardPanelData.data.fields.x.forEach(
      (it: any, index: any) =>
        (it.alias = !dashboardPanelData.data.customQuery
          ? "x_axis_" + (index + 1)
          : it.column)
    );
    dashboardPanelData.data.fields.y.forEach(
      (it: any, index: any) =>
        (it.alias = !dashboardPanelData.data.customQuery
          ? "y_axis_" + (index + 1)
          : it.column)
  ...
  const removeXAxisItem = (name: string) => {
    const index = dashboardPanelData.data.fields.x.findIndex(
  ...
  const removeYAxisItem = (name: string) => {
    const index = dashboardPanelData.data.fields.y.findIndex(
      (it: any) => it.column == name
    );
    if (index >= 0) {
  ...
  const removeFilterItem = (name: string) => {
    const index = dashboardPanelData.data.fields.filter.findIndex(
      (it: any) => it.column == name
    );
    if (index >= 0) {
  ...
  const removeXYFilters = () => {
    if (promqlMode.value || dashboardPanelData.data.customQuery == false) {
      dashboardPanelData.data.fields.x.splice(
        0,
        dashboardPanelData.data.fields.x.length
      );
      dashboardPanelData.data.fields.y.splice(
        0,
        dashboardPanelData.data.fields.y.length
      );
      dashboardPanelData.data.fields.filter.splice(
        0,
        dashboardPanelData.data.fields.filter.length
      );
    }
  };
      ...
      dashboardPanelData.meta.stream.customQueryFields.forEach(
        (it: any, index: number) => {
          ...
          const currentFieldType =
            index < dashboardPanelData.data.fields.x.length ? "x" : "y";
          // Get the current field based on its index and whether it's an x or y field
          const field =
            index < dashboardPanelData.data.fields.x.length
  ...
  const updateXYFieldsOnCustomQueryChange = (oldCustomQueryFields: any) => {
    ...
    if (oldArray.length == newArray.length) {
      ...
      if (changedIndex.length == 1) {
        ...
        let fieldIndex = dashboardPanelData.data.fields.x.findIndex(
          (it: any) => it.alias == oldName
        );
        ...
        if (fieldIndex >= 0) {
          ...
          fieldIndex = dashboardPanelData.data.fields.y.findIndex(
            (it: any) => it.alias == oldName
          );
          if (fieldIndex >= 0) {
            const newName = newArray[changedIndex[0]]?.name;
