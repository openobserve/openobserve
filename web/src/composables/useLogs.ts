...
const useLogs = () => {
  ...
  const handleSearchResponse = (
    ...
    if(payload.type === "search" && response?.type === "search_response_hits") {
      ...
      handleStreamingHits(payload, response, payload.isPagination, !(response.content?.streaming_aggs || searchObj.data.queryResults.streaming_aggs) && (searchPartitionMap[payload.traceId].partition > 1 || isChunkedHits));
      return;
    }
    ...
    if(payload.type === "search" && response?.type === "search_response_metadata") {
      ...
      handleStreamingMetadata(payload, response, payload.isPagination, !response.content?.streaming_aggs && searchPartitionMap[payload.traceId].partition > 1);
      return;
    }
