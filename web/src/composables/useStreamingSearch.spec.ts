...
const WorkerConstructor = vi.fn().mockImplementation(() => {
  console.log(
    "WorkerConstructor called, returning mockWorker with postMessage:",
    typeof mockWorker.postMessage,
  );
  return mockWorker;
});
...
describe("useHttpStreaming", () => {
  ...
  describe("initiateStreamConnection", () => {
    ...
    it.skip("should successfully initiate a stream connection", async () => {
      ...
      const streamPromise = httpStreaming.fetchQueryDataWithHttpStream(
        mockData,
        mockHandlers,
      );
    ...
    it.skip("should cancel the stream and clean up resources", async () => {
      ...
      console.log(
        "Before fetchQueryDataWithHttpStream, window.Worker:",
      ...
      console.log(
        "WorkerConstructor calls:",
        WorkerConstructor.mock.calls.length,
      );
      console.log(
        "mockWorker.postMessage calls:",
        mockWorker.postMessage.mock.calls.length,
      );
      console.log(
        "All postMessage calls:",
        JSON.stringify(mockWorker.postMessage.mock.calls),
      );

      // Mock AbortController and add it to abortControllers
      const abortFn = vi.fn();
      const mockAbortController = { abort: abortFn };
      httpStreaming.abortControllers.value[traceId] =
    ...
    it.skip("should abort all controllers, send 'closeAll' to worker, clear traceMap, and reset activeStreamId", async () => {
      ...
      const abortFn = vi.fn();
      const mockAbortController = { abort: abortFn };
      httpStreaming.abortControllers.value[traceId1] =
        mockAbortController as any;
      httpStreaming.abortControllers.value[traceId2] =
        mockAbortController as any;
    ...
    it("should convert a response with results and other fields into the expected WebSocket response format", () => {
      ...
      const result = httpStreaming.convertToWsResponse(
        traceId,
        mockResponse,
        type,
      );
  ...
  describe("WebSocket response converters", () => {
    ...
    describe("convertToWsEventProgress", () => {
      it("should convert progress response with percent", () => {
        ...
        const result = httpStreaming.convertToWsEventProgress(
          traceId,
          response,
          "progress",
        );
    ...
    describe("wsMapper", () => {
      it("should map types to the correct converter function", () => {
        ...
        expect(httpStreaming.wsMapper["progress"]).toBe(
          httpStreaming.convertToWsEventProgress,
        );
        expect(httpStreaming.wsMapper["error"]).toBe(
          httpStreaming.convertToWsError,
        );
        expect(httpStreaming.wsMapper["end"]).toBe(
          httpStreaming.convertToWsEnd,
        );
      });
    });
  });
