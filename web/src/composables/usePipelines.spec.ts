...
describe("usePipelines", () => {
  ...
  it("should fetch used streams list successfully", async () => {
    ...
    expect(mockPipelinesService.getPipelineStreams).toHaveBeenCalledWith(
      "test-org",
    );
  ...
  it("should handle single pipeline destination", async () => {
    const mockResponse = {
      data: [{ name: "single-destination", url: "http://single.com" }],
    };
  ...
  it("should handle async operations correctly", async () => {
    ...
    mockPipelinesService.getPipelineStreams.mockResolvedValue(
      mockStreamResponse,
    );
  ...
  it("should create independent instances", () => {
    ...
    expect(instance1.getPipelineDestinations).not.toBe(
      instance2.getPipelineDestinations,
    );
  });
