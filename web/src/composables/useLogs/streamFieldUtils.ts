...
import { getHistogramTitle } from "@/composables/useLogs/histogramUtils";
...
export const loadStreamFields = async (streamName: string) => {
  try {
    if (streamName != "") {
...
export const getStreamList = async (selectStream: boolean = true) => {
  try {
    const store = useStore();
...
export const loadStreamLists = async (selectStream: boolean = true) => {
  try {
...
export const hasInterestingFieldsInLocal = (streamName: string) => {
  const localInterestingFields: any = useLocalInterestingFields();
  return (
...
export const createFieldIndexMapping = async () => {
...
export const filterHitsColumns = () => {
  const store = useStore();
