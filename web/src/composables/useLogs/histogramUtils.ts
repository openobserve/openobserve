...
export const getHistogramTitle = () => {
  try {
...
export const resetHistogramWithError = (
  errorMsg: string,
  errorCode: number = 0,
) => {
  searchObj.data.histogram = {
...
export const generateHistogramSkeleton = () => {
  if (
    searchObj.data.queryResults.hasOwnProperty("aggs") &&
    searchObj.data.queryResults.aggs
  ) {
...
export const setMultiStreamHistogramQuery = (queryReq: any) => {
  const store = useStore();
...
export const isHistogramEnabled = (searchObj: any) => {
  return (
