...
describe("useSelectAutocomplete.ts Comprehensive Coverage", () => {
  ...
  describe("FilterFn - Mixed Data Types", () => {
    ...
    it("should handle arrays with null and undefined values", () => {
      const options = ref(["valid string", { name: "Valid Object" }, ""]);
      const searchKey = "name";
      ...
      expect(mockUpdate).toHaveBeenCalledTimes(1);
      expect(filteredOptions.value).toEqual(["valid string", { name: "Valid Object" }]);
    });

    it("should handle numbers in mixed array", () => {
      const options = ref(["item1", "123", "456", { name: "123", type: "number" }]);
      const searchKey = "name";
      ...
      expect(mockUpdate).toHaveBeenCalledTimes(1);
      expect(filteredOptions.value).toEqual(["123", { name: "123", type: "number" }]);
    });
  });
