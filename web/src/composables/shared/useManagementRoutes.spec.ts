...
describe("useManagementRoutes", () => {
  ...
  describe("Enterprise Routes", () => {
    ...
    it("should have exactly 20 children routes when enterprise is enabled", () => {
      const routes = useManagementRoutes();
      expect(routes[0].children).toHaveLength(21); // 5 base (incl. alert_sources redirect) + modelPricing (+ editor) + llmProviders + genAiAgentMapping + 12 enterprise (incl. passwordPolicy)
    });
  });
  ...
  describe("Combined Enterprise and Cloud Routes", () => {
    ...
    it("should have exactly 21 children routes when both enterprise and cloud are enabled", () => {
      const routes = useManagementRoutes();
      expect(routes[0].children).toHaveLength(22); // 5 base (incl. alert_sources redirect) + modelPricing (+ editor) + llmProviders + genAiAgentMapping + 12 enterprise (incl. passwordPolicy) + 1 cloud
    });
  ...
  describe("Edge Cases and Error Handling", () => {
    it("should handle isEnterprise as string 'false'", () => {
      ...
      config.isCloud = "false";
      const routes = useManagementRoutes();
      expect(routes[0].children).toHaveLength(6); // syntheticsLocations ships in OSS; passwordPolicy, model_pricing (+ editor), gen_ai_agent_mapping and llm_providers are enterprise/cloud-only; alert_sources redirect is always present
    });
    ...
    it("should handle isEnterprise as non-string truthy value", () => {
      ...
      config.isCloud = "false";
      const routes = useManagementRoutes();
      expect(routes[0].children).toHaveLength(6); // syntheticsLocations ships in OSS; passwordPolicy, model_pricing (+ editor), gen_ai_agent_mapping and llm_providers are enterprise/cloud-only; alert_sources redirect is always present, since config comparison is strict "true"
    });
