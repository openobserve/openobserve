...
describe("usePermissions", () => {
  ...
  describe("Initial State Tests", () => {
    ...
    it("should initialize usersState with correct default values", () => {
      ...
      expect(typeof permissionsComposable.usersState.getOrgUsers).toBe(
        "function",
      );
    });
    ...
    it("should initialize serviceAccountsState with correct default values", () => {
      expect(
        permissionsComposable.serviceAccountsState.service_accounts_users,
      ).toEqual([]);
      expect(
        typeof permissionsComposable.serviceAccountsState.getServiceAccounts,
      ).toBe("function");
    });
  });
  ...
  describe("State Reactivity Tests", () => {
    it("should maintain reactivity when permissionsState.permissions is modified", async () => {
      ...
      expect(permissionsComposable.permissionsState.permissions).toHaveLength(
        1,
      );
      expect(permissionsComposable.permissionsState.permissions[0]).toEqual(
        mockPermission,
      );
    });
  ...
  describe("getServiceAccounts Function Tests", () => {
    it("should successfully fetch service accounts", async () => {
      ...
      const result =
        await permissionsComposable.serviceAccountsState.getServiceAccounts(
          "test_org",
        );
    ...
    it("should handle error when fetching service accounts fails", async () => {
      ...
      await expect(
        permissionsComposable.serviceAccountsState.getServiceAccounts(
          "test_org",
        ),
  ...
  describe("State Sharing Tests", () => {
    it("should share the same reactive state between instances (singleton pattern)", () => {
      ...
      const mockPermission: Resource = {
        resource_name: "shared_test",
      } as Resource;
      instance1.permissionsState.permissions.push(mockPermission);

      expect(instance2.permissionsState.permissions).toHaveLength(1);
      expect(instance2.permissionsState.permissions[0].resource_name).toBe(
        "shared_test",
      );
      expect(instance1.permissionsState.permissions).toEqual(
        instance2.permissionsState.permissions,
      );
    });
    ...
    it("should maintain state consistency across multiple instances", () => {
      ...
      instance1.permissionsState.permissions = [
        { resource_name: "test_resource" } as Resource,
      ];
  ...
  describe("Complex State Operations Tests", () => {
    it("should handle multiple permissions being added and removed", () => {
      const permission1: Resource = {
        resource_name: "resource1",
        resource_id: "1",
      } as Resource;
      const permission2: Resource = {
        resource_name: "resource2",
        resource_id: "2",
      } as Resource;
      const permission3: Resource = {
      ...
      permissionsComposable.permissionsState.permissions =
        permissionsComposable.permissionsState.permissions.filter(
          (p) => p.resource_id !== "2",
        );
      expect(permissionsComposable.permissionsState.permissions).toHaveLength(
        2,
      ...
      expect(
        permissionsComposable.permissionsState.permissions.find(
          (p) => p.resource_id === "2",
        ),
      ).toBeUndefined();
    });
    ...
    it("should handle complex selectedResources operations", () => {
      ...
      expect(
        Object.keys(permissionsComposable.permissionsState.selectedResources),
      ).toHaveLength(3);
      ...
      expect(
        "resource2" in permissionsComposable.permissionsState.selectedResources,
      ).toBe(false);
    });
    ...
    it("should maintain state consistency after reset operations", () => {
      ...
      permissionsComposable.permissionsState.permissions = [
        { resource_name: "test" } as Resource,
      ];
      ...
      permissionsComposable.usersState.users = [
  ...
  describe("Edge Cases and Additional Scenarios", () => {
    ...
    it("should maintain type safety with Resource interface", () => {
      ...
      const retrievedResource =
        permissionsComposable.permissionsState.permissions[0];
    ...
    it("should handle concurrent async operations for users and service accounts", async () => {
      ...
      expect(serviceAccountsResult).toEqual([
        { id: 1, name: "service_account" },
      ]);
    });
    ...
    it("should handle empty string organization identifiers gracefully", async () => {
      ...
      await expect(
        permissionsComposable.usersState.getOrgUsers(""),
      ).resolves.toEqual([]);
      await expect(
        permissionsComposable.serviceAccountsState.getServiceAccounts(""),
      ).resolves.toEqual([]);
    ...
    it("should maintain immutability of original state objects", () => {
      ...
      const modifiedPermissions = [
        ...originalPermissions,
        { resource_name: "new_resource" } as Resource,
      ];
