...
const useRateLimiter = () => {
  ...
  const getRoleLimitsByOrganization = async (
    orgId: string,
    rolename: string,
    interval: string = "second",
  ) => {
    try {
      ...
      const response = await rateLimiterService.getRoleLimits(
        orgId,
        rolename,
        interval,
      );
  ...
  const getModulesToDisplay = async (orgId: string) => {
    try {
      ...
      const modulesToDisplay = response.data
        .map((role: any) => ({
          label: role,
          value: role,
        }))
