...
describe("useSearchInputUsingRegex", () => {
  ...
  describe("Main composable function", () => {
    it("should initialize with copied options", () => {
      const { filteredOptions } = useSearchInputUsingRegex(
        mockOptions,
        searchKey,
        searchRegex,
      );
    ...
    it("should return all expected methods", () => {
      const result = useSearchInputUsingRegex(
        mockOptions,
        searchKey,
        searchRegex,
      );
  ...
  describe("filterFn", () => {
    it("should reset to all options when value is empty string", () => {
      const { filteredOptions, filterFn } = useSearchInputUsingRegex(
        mockOptions,
        searchKey,
        searchRegex,
      );
    ...
    it("should filter options based on regex match", () => {
      ...
      expect(
        filteredOptions.value.some((option: any) => option.name === "John Doe"),
      ).toBe(true);
      expect(
        filteredOptions.value.some(
          (option: any) => option.name === "Bob Johnson",
        ),
      ).toBe(true);
    });
  ...
  describe("Integration tests", () => {
    ...
    it("should maintain reactive updates", () => {
      ...
      expect(filteredOptions.value).not.toContain({
        name: "New Person",
        id: 5,
      });
    });
  ...
  describe("Watch function coverage", () => {
    it("should update filteredOptions when they are empty and options change", async () => {
      ...
      expect(
        filteredOptions.value.some((item: any) => item.name === "New Person"),
      ).toBe(true);
    });
  ...
  describe("FilterFn branch coverage", () => {
    ...
    it("should handle primitive options that are not strings", () => {
      const primitiveOptions = ref(["string item", 123, true, false, 0]);
      const { filteredOptions, filterFn } = useSearchInputUsingRegex(
        primitiveOptions,
        "name",
        searchRegex,
  ...
  describe("Regex edge cases", () => {
    it("should handle invalid regex patterns gracefully", () => {
      ...
      const { filterFn } = useSearchInputUsingRegex(
        mockOptions,
        searchKey,
        invalidRegex,
      );
