...
describe("AddAlert Component", () => {
  ...
  describe("When user fills form and clicks submit", async () => {
    ...
    beforeEach(async () => {
      mock
        .onPost(
          "http://localhost:5080/api/default_organization_01/k8s_json/alerts?type=logs",
        )
        .reply((config) => {
      ...
      mock
        .onGet(
          "http://localhost:5080/api/default_organization_01/streams?type=logs",
        )
        .reply((config) => {
          return [
            200,
    ...
    it("should fill the form with the correct data and api should be called", async () => {
      ...
      await wrapper.find('[data-test="add-alert-submit-btn"]').trigger("click");
      await wrapper.find('[data-test="add-alert-form"]').trigger("submit");
      console.log(wrapper.vm.formData,'formdata')
      await flushPromises();
      await vi.advanceTimersByTime(500);
      await flushPromises();
