...
export const createTestWrapper = (
  ...
  return mount(component, {
    ...
    // Add attachTo option to ensure proper DOM mounting
...
export const mockUseIngestion = () => {
  vi.mock("@/composables/useIngestion", () => ({
    default: vi.fn(() => {
      // Create reactive refs to avoid Vue composition API warnings
      const endpoint = { value: mockEndpoint };
...
Access Key: [BASIC_PASSCODE]`,
        securityDocURLs: {
          ...
          googleworkspace:
            "https://short.openobserve.ai/security/google-workspace",
        },
...
export const componentCategories = {
  ...
  devops: ["Jenkins", "Ansible", "Terraform", "GithubActions"],
  servers: ["Nginx", "Apache", "IIS"],
  messagequeues: ["Kafka", "RabbitMQ", "Nats"],
  languages: [
    "Python",
    "DotNetLogs",
    "DotNetTracing",
...
export const commonAssertions = {
  ...
  shouldHandlePropsCorrectly: (wrapper: any, props: TestProps) => {
    ...
    if (props.currOrgIdentifier && wrapper.vm.currOrgIdentifier !== undefined) {
      result =
        result && wrapper.vm.currOrgIdentifier === props.currOrgIdentifier;
    }
