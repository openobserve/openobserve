...
console.log = (...args) => {
  ...
  if (
    message.includes("error parsing sql query") ||
...
console.warn = (...args) => {
  ...
  if (typeof message === "string") {
    if (
      message.includes('Failed setting prop "prefix" on <q-input-stub>') ||
      message.includes("Cannot set property prefix of [object Element]") ||
      message.includes(
        'Failed setting prop "prefix" on <q-select-stub>: value undefined is invalid',
      ) ||
      message.includes(
      ...
      message.includes(
        'setup() return property "$q" should not start with "$"',
      ) ||
      message.includes(
      ...
      message.includes(
        "Unhandled error during execution of beforeUpdate hook",
      ) ||
...
console.error = (...args) => {
  ...
  const isExpectedError = expectedErrorPrefixes.some((prefix) =>
