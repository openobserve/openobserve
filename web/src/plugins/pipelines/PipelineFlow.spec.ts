  ...
  messages: {
    en: {
      pipeline: {
        ...
        emptyCanvas:
          "Drag and drop nodes from the sidebar to start building your pipeline",
...
vi.mock("@vue-flow/core", () => ({
  ...
  useVueFlow: () => ({
    ...
    setViewport: vi
      .fn()
      .mockImplementation((params) => mockSetViewport(params)),
  }),
}));
...
describe("PipelineFlow.vue", () => {
  ...
  it("should render DropzoneBackground component", () => {
    wrapper = mountComponent();
    expect(wrapper.findComponent({ name: "DropzoneBackground" }).exists()).toBe(
      true,
    );
  });
  ...
  it("should have correct data test attributes", () => {
    wrapper = mountComponent();
    expect(wrapper.find('[data-test="pipeline-flow-container"]').exists()).toBe(
      true,
    );
  });
  ...
  it("should hide warning text when dirtyFlag is false", () => {
    wrapper = mountComponent();
    const warningText = wrapper.find(
  ...
  it("should reset dirtyFlag when currentSelectedPipeline changes", async () => {
    ...
    wrapper.vm.pipelineObj.currentSelectedPipeline = {
      nodes: [{ id: "new" }],
      edges: [],
    };
    await nextTick();
