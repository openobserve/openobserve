...
export default function useDragAndDrop() {
  ...
  function onConnect(connection: any) {
    ...
    const isConnectionAlreadyAvailable =
      pipelineObj.currentSelectedPipeline.edges.find(
  ...
  function addNode(newNode: any) {
      ...
      if (pipelineObj.userSelectedNode == null) {
        pipelineObj.currentSelectedPipeline.edges =
          pipelineObj.currentSelectedPipeline.edges.filter(
            (edge: any) =>
              edge.targetNode.id !== pipelineObj.currentSelectedNodeData.id,
          );
      }
    }
    if (
      ...
      !pipelineObj.userSelectedNode?.id
    ) {
      const newEdge = {
    ...
    if (
      ...
      const targetEdgeIfExist = pipelineObj.currentSelectedPipeline.edges.find(
        (previousEdge: any) =>
          previousEdge.targetNode.id === pipelineObj.currentSelectedNodeData.id,
      );
      ...
      if (targetEdgeIfExist) {
        pipelineObj.currentSelectedPipeline.edges =
          pipelineObj.currentSelectedPipeline.edges.filter(
            (edge: any) =>
              edge.targetNode.id !== targetEdgeIfExist.targetNode.id,
          );
      }
    ...
    if (pipelineObj.isEditNode == true && currentSelectedNode.id != "") {
      if (currentSelectedNode) {
        ...
        console.log("[useDnD] Updated node (edit mode):", currentSelectedNode);
      ...
      if (currentSelectedNode) {
        currentSelectedNode.data = { ...currentSelectedNode.data, ...newNode };
        console.log("[useDnD] Created new node:", currentSelectedNode);
    ...
    if (
      newNode.hasOwnProperty("meta") &&
      newNode.meta.hasOwnProperty("append_data")
    ) {
  ...
  function deletePipelineNode(nodeId: any) {
    ...
    pipelineObj.previousNodeOptions = pipelineObj.previousNodeOptions.filter(
      (node: any) => node.id !== nodeId,
    );
  ...
  const resetPipelineData = () => {
    pipelineObj.currentSelectedPipeline = JSON.parse(
      JSON.stringify(defaultPipelineObj),
    );
  ...
  const getInputNodeStream = () => {
    ...
    if (
      inputNode?.data.hasOwnProperty("node_type") &&
      inputNode.data.node_type === "stream"
    ) {
      return (
        inputNode?.data?.stream_name?.value || inputNode.data.stream_name || ""
      );
    } else {
      return null;
    }
