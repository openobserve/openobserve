  ...
  <div>
    ...
    >
      ...
      >
        ...
        >
          <!-- ── HERO SECTION: Pattern Template (moved to top) ── -->
          <div class="tw:mb-[1rem]">
            <div class="tw:flex tw:items-center tw:gap-[0.5rem] tw:mb-[0.375rem] tw:flex-wrap">
              <div class="text-subtitle2 text-weight-medium">
                {{ t("search.patternTemplate") }}
              </div>
              <!-- Inline occurrence + percentage badges -->
              <span
              ...
              <OButton
                ...
                data-test="pattern-detail-copy-sql-btn"
              >
                <template #icon-left>
                  <q-icon name="content_copy" size="0.75rem" />
                </template>
                {{ t("search.patternCopySql") }}
              </OButton>
            </div>
...
const sqlForCurrentPattern = computed(() => {
  if (!props.selectedPattern?.pattern) return "";
  return buildPatternSqlQuery(
    props.selectedPattern.pattern.template ?? "",
    "",
  );
});
...
const copySqlToClipboard = () => {
  try {
    navigator.clipboard.writeText(sqlForCurrentPattern.value);
    $q.notify({
      type: "positive",
