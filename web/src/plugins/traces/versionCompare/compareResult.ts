...
export function buildCompareResult(
  ...
  const volA = kpiA.traceCount / hoursA,
    volB = kpiB.traceCount / hoursB;

  const errCI = proportionDiffCI(kpiA.errorCount, nA, kpiB.errorCount, nB);
  const p50CI = bootstrapDiffCI(
    samplesA.durations,
    samplesB.durations,
    (xs) => percentile(xs, 0.5),
    BOOTSTRAP_ITERS,
    CI_LEVEL,
    seed,
  );
  const p95CI = bootstrapDiffCI(
    samplesA.durations,
    samplesB.durations,
    (xs) => percentile(xs, 0.95),
    BOOTSTRAP_ITERS,
    CI_LEVEL,
    seed + 1,
  );
  const costCI = bootstrapDiffCI(
    samplesA.costs,
    samplesB.costs,
    mean,
    BOOTSTRAP_ITERS,
  ...
  const metrics: MetricResult[] = [
    ...
    },
    {
      ...
      a: errRateA,
      b: errRateB,
      deltaPct: pct(errRateA, errRateB),
      ci: errCI,
      verdict: classifyVerdict(errCI, "up-worse", enoughSample),
      flagged: true,
      associative,
    },
    {
      key: "p50",
      a: percentile(samplesA.durations, 0.5),
      b: percentile(samplesB.durations, 0.5),
      deltaPct: null,
      ci: hasDurationSamples ? p50CI : null,
      verdict: hasDurationSamples
        ? classifyVerdict(p50CI, "up-worse", enoughSample)
        : "insufficient",
      flagged: true,
      associative,
    },
    {
      key: "p95",
      a: percentile(samplesA.durations, 0.95),
      b: percentile(samplesB.durations, 0.95),
      deltaPct: null,
      ci: hasDurationSamples ? p95CI : null,
      verdict: hasDurationSamples
        ? classifyVerdict(p95CI, "up-worse", enoughSample)
        : "insufficient",
      flagged: true,
      associative,
    },
    {
      ...
      a: costPerA,
      b: costPerB,
      deltaPct: pct(costPerA, costPerB),
      ci: hasCostSamples ? costCI : null,
      verdict: hasCostSamples ? classifyVerdict(costCI, "up-worse", enoughSample) : "insufficient",
      flagged: true,
      associative,
    },
    {
      key: "p99",
      a: percentile(samplesA.durations, 0.99),
      b: percentile(samplesB.durations, 0.99),
      deltaPct: null,
      ci: null,
      verdict: "nochange",
      flagged: true,
  ...
  metrics.forEach((m) => {
    if (m.deltaPct === null) m.deltaPct = pct(m.a, m.b);
  });
...
export function buildCompareResultFromEndpoint(
  ...
  const metrics: MetricResult[] = [
    ...
    },
    {
      ...
      a: p50.a,
      b: p50.b,
      deltaPct: pct(p50.a, p50.b),
      ci: p50.insufficient ? null : p50CI,
      verdict: p50.insufficient ? "insufficient" : classifyVerdict(p50CI, "up-worse", enoughSample),
      flagged: true,
      associative,
    },
    {
      ...
      a: p95.a,
      b: p95.b,
      deltaPct: pct(p95.a, p95.b),
      ci: p95.insufficient ? null : p95CI,
      verdict: p95.insufficient ? "insufficient" : classifyVerdict(p95CI, "up-worse", enoughSample),
      flagged: true,
      associative,
    },
    {
      ...
      a: cost.a,
      b: cost.b,
      deltaPct: pct(cost.a, cost.b),
      ci: cost.insufficient ? null : costCI,
      verdict: cost.insufficient
        ? "insufficient"
        : classifyVerdict(costCI, "up-worse", enoughSample),
      flagged: true,
      associative,
    },
    {
