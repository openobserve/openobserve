...
export interface DiffCI { delta: number; lower: number; upper: number; straddlesZero: boolean; }
...
export function proportionDiffCI(errA: number, nA: number, errB: number, nB: number, level = CI_LEVEL): DiffCI {
  const z = zFor(level);
