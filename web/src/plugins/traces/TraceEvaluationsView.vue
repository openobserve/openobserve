...
<template>
  <div class="w-full h-full flex flex-col overflow-y-auto bg-surface-base p-4 gap-4">
    <!-- Loading state -->
    <div v-if="isLoading" class="flex flex-col items-center justify-center h-[60vh] gap-4 text-text-secondary">
      <OSpinner size="xl" />
      <div>{{ $t("traces.evaluations.loading") }}</div>
    </div>

    <!-- Empty state -->
    <div v-else-if="evalData.length === 0" class="flex flex-col items-center justify-center h-[60vh] gap-4 text-text-secondary">
    ...
    <template v-else>
      ...
      <div class="flex justify-end items-center">
        <div class="flex items-center gap-2">
          <span class="text-xs font-medium text-text-secondary">{{ $t("traces.evaluations.templateLabel") }}</span>
          <OSelect
      ...
      <div v-for="(record, idx) in evalData" :key="idx" class="flex flex-col gap-4 w-full">
        ...
        <div class="flex gap-4">
          ...
          <div class="flex flex-col gap-4 w-[66.67%]">
            ...
            <div class="border border-border-default rounded-lg bg-surface-base p-4">
              <div class="flex items-center gap-2 mb-3">
                <OIcon name="rule" size="sm" />
                <div class="text-sm font-semibold text-text-heading">{{ $t("traces.evaluations.issuesAnalysis") }}</div>
              </div>
              ...
              <div v-if="selectedTemplateData?.content" class="mb-4">
                ...
                >
                  ...
                  <div class="mt-2 p-3 bg-border-default rounded-md text-xs leading-relaxed whitespace-pre-wrap text-text-heading">
                    {{ selectedTemplateData.content }}
                  </div>
                  <div class="mt-1 text-[10px] text-text-secondary italic">
              ...
              <div v-if="getWeakestDimension(record)" class="mb-4">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <span class="text-xs font-medium text-text-secondary">{{ $t("traces.evaluations.weakestDimension") }}</span>
                  <OTag type="evalBadge" value="weakest">{{ formatDimLabel(getWeakestDimension(record)!.dimension) }}</OTag>
                </div>
                <div v-if="getWeakestDimension(record)!.reasoning" class="text-sm bg-border-default p-3 rounded-md leading-relaxed">
            ...
            <div class="border border-border-default rounded-lg bg-surface-base p-4">
              <div class="flex items-center gap-2 mb-3">
                ...
                <div class="text-sm font-semibold text-text-heading">{{ $t("traces.evaluations.dimensionReasoning") }}</div>
              </div>
              <div class="grid grid-cols-2 gap-3">
                <div
                  ...
                  :class="isTemplateDimension(dim.dimension) ? 'bg-[rgba(25,118,210,0.05)] border border-[rgba(25,118,210,0.2)] dark:bg-[rgba(96,165,250,0.07)] dark:border-[rgba(96,165,250,0.25)]' : 'bg-border-default'"
                >
                  ...
                  <div class="text-xs leading-relaxed text-text-heading">
                    {{ truncateContent(dim.reasoning, 200) }}
                  </div>
                </div>
                <div v-if="!getDimensionScores(record).some(d => d.reasoning)" class="text-sm italic text-text-secondary">
            ...
            <div class="border border-border-default rounded-lg bg-surface-base overflow-hidden">
              <div class="px-4 py-3 border-b border-border-default bg-border-default/30">
                <div class="flex items-center gap-2">
                  <OIcon name="code" size="sm" />
                  <div class="text-sm font-semibold text-text-heading">{{ $t("traces.evaluations.queryResponse") }}</div>
                </div>
              </div>
          ...
          <div class="flex flex-col gap-4 w-[33.33%]">
            ...
            <div class="border border-border-default rounded-lg bg-surface-base p-4 flex flex-col gap-4">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <OIcon name="analytics" size="sm" />
                  <div class="text-sm font-semibold text-text-heading">{{ $t("traces.evaluations.dimensions") }}</div>
                </div>
                ...
                <div v-if="selectedTemplateData?.dimensions?.length" class="flex items-center gap-1">
                  <span class="text-[10px] text-text-secondary">{{ $t("traces.evaluations.passThreshold") }}</span>
              ...
              <div v-if="selectedTemplateData?.dimensions?.length" class="flex items-center gap-1.5 text-[10px] text-text-secondary">
                <OIcon name="assignment" size="xs" />
                ...
                >
                  <div class="flex justify-between items-center">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-xs font-medium text-text-heading">{{ formatDimLabel(dim.dimension) }}</span>
                    ...
                    <div class="flex items-center gap-1.5">
                      ...
                      <OTag
                        ...
                        class="text-[9px] py-px px-1"
                      />
                      <span class="text-xs font-bold text-text-heading">{{ formatScore(dim.score) }}</span>
                    </div>
                  </div>
                  <div class="h-1.5 w-full bg-border-default rounded-full overflow-hidden">
                    <div
            ...
            <div class="border border-border-default rounded-lg bg-surface-base p-4">
              ...
              <div class="flex flex-col gap-3">
                <div class="grid grid-cols-[80px_1fr] gap-1">
                  <div class="text-[10px] font-bold text-text-secondary uppercase">{{ $t("traces.evaluations.judge") }}</div>
                  <div class="text-xs truncate">{{ getJudgeModel(record) }}</div>
                </div>
                <div class="grid grid-cols-[80px_1fr] gap-1">
                  <div class="text-[10px] font-bold text-text-secondary uppercase">{{ $t("traces.evaluations.time") }}</div>
                  <div class="text-xs">{{ formatTimestampDisplay(record._timestamp) }}</div>
                </div>
                <div class="grid grid-cols-[80px_1fr] gap-1">
                  <div class="text-[10px] font-bold text-text-secondary uppercase">{{ $t("traces.evaluations.confidence") }}</div>
                  <div class="text-xs">{{ formatScore(record.llm_evaluation_judge_confidence) }}</div>
                </div>
                <div class="grid grid-cols-[80px_1fr] gap-1">
                  <div class="text-[10px] font-bold text-text-secondary uppercase">{{ $t("traces.evaluations.status") }}</div>
                  <div>
                    <OTag
                      type="evalStatus"
                ...
                <div v-if="record.is_multi_step" class="grid grid-cols-[80px_1fr] gap-1">
                  <div class="text-[10px] font-bold text-text-secondary uppercase">{{ $t("traces.evaluations.type") }}</div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const DIM_COLOR_CLASSES: Record<string, string> = {
      ...
      groundedness: "text-status-success-text",
