...
function extractContent(content: unknown): string {
  ...
  if (Array.isArray(content)) {
    // OTEL / Anthropic multipart: [{type:"text", text:"..."}, {function_call:...}]
    return content
      .map((part: any) => {
  ...
  if (typeof content === "object") {
    ...
    // Vertex/ADK shape: { parts: [{text}, {function_call}] }
    if (Array.isArray(obj.parts)) return extractContent(obj.parts);
    return ""; // not stringify — too noisy for the chat view
  }
...
/**
 * Heuristic: distinguish a "real" human user message from agent-internal
 * user-role messages that carry tool-feedback, prior-assistant context, or
 * system reminders.
 *
 * Common shapes we want to filter out:
 *   "[Tool schemas cached — use tools_call ...]"
 *   "[tool_result: ...]" / "[function_call: ...]"
 *   "For context:\n[agent_name] said: ..."
 *   "[agent_name] said: ..."   (no preamble)
 *   long multi-paragraph blocks that paraphrase prior assistant output
 */
function looksLikeAgentInjection(text: string): boolean {
  ...
  // Bracketed tool / function envelope.
  if (
  ...
  // "[<agent_name>] said:" — agent paraphrasing previous assistant output.
...
/** A single trace within (potentially) a multi-trace session. */
...
/** Pure helper: turn a flat span list (from one trace_id) into a TraceGroup. */
function buildTraceGroup(spans: any[]): TraceGroup | null {
  ...
  // Historical user-message count (other real user messages in turn 1's input).
  ...
  // Resolve user identity from any common field set by the SDK / backend.
  const userId =
    String(
      rootSpan?.user_email ||
        rootSpan?.user_id ||
