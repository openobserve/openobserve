...
describe("ScriptEditor.vue", () => {
    ...
    return mount(ScriptEditor, {
      ...
      global: {
        ...
        stubs: {
          ...
          "q-icon": {
            template:
              '<div class="icon"><q-tooltip-stub v-if="$slots.default"><slot></slot></q-tooltip-stub></div>',
            props: ["name", "size"],
          },
          "q-tooltip": {
  ...
  describe("Props Handling", () => {
    it("accepts all required props", () => {
      ...
      expect(wrapper.props("file")).toEqual({
        name: "script.py",
        language: "python",
      });
  ...
  describe("Script Input Handling", () => {
    ...
    it("inputScript computed property emits update when set", () => {
      ...
      expect(wrapper.emitted("update:script")?.[0]).toEqual([
        "new script content",
      ]);
    });
  ...
  describe("Edge Cases", () => {
    it("handles undefined file gracefully", () => {
      wrapper = createWrapper({
        file: { name: undefined, language: undefined },
      });
      expect(wrapper.exists()).toBe(true);
    });
    ...
    it("handles very long error messages", () => {
      const longError =
        "A very long error message that might cause layout issues. ".repeat(10);
  ...
  describe("Data Test Attributes", () => {
    it("has correct data-test attributes for testing", () => {
      ...
      const titleSection = wrapper.find(
