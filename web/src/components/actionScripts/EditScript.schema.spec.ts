...
const makeSchema = (isEditing = false, executionDetails = "repeat", cronEdited = true) =>
  makeEditScriptSchema({
    t,
    getIsEditing: () => isEditing,
    getCronError: (cron: string) => (cron === "bad" ? "Invalid cron expression!" : ""),
...
const errorPaths = (
  ...
  const res = makeSchema(isEditing, executionDetails, cronEdited).safeParse(input);
...
describe("EditScript.schema", () => {
  ...
  describe("required scalars", () => {
    ...
    it("requires service_account", () => {
      expect(errorPaths({ ...base(), service_account: "" })).toContain("service_account");
    });
  ...
  describe("codeZip (required only on create)", () => {
    it("is required on create", () => {
      expect(errorPaths({ ...base(), codeZip: null }, false)).toContain("codeZip");
    });

    it("is NOT required on edit", () => {
      expect(errorPaths({ ...base(), codeZip: null }, true)).not.toContain("codeZip");
    });
  });
  ...
  describe("cron (byte-exact main: execution_details on edit + edited-field on create)", () => {
    ...
    it("is required when execution_details is repeat and cron empty", () => {
      expect(errorPaths({ ...base(), cron: "" }, false, "repeat", false)).toContain("cron");
    });

    it("flags an invalid cron when execution_details is repeat", () => {
      expect(errorPaths({ ...base(), cron: "bad" }, false, "repeat", false)).toContain("cron");
    });

    it("accepts a valid cron when execution_details is repeat", () => {
      expect(errorPaths({ ...base(), cron: "0 12 * * *" }, false, "repeat", false)).not.toContain(
        "cron",
      );
    });
    ...
    it("validates a blank cron on create once the field is edited", () => {
      expect(errorPaths({ ...base(), cron: "" }, false, "", true)).toContain("cron");
    });

    it("flags an invalid cron on create once the field is edited", () => {
      expect(errorPaths({ ...base(), cron: "bad" }, false, "", true)).toContain("cron");
    });
    ...
    it("does NOT validate an untouched blank cron on create (main's gap, preserved)", () => {
      expect(errorPaths({ ...base(), cron: "" }, false, "", false)).not.toContain("cron");
    });
    ...
    it("is NOT validated for a 'once' schedule even if the field was edited", () => {
      expect(
        errorPaths({ ...base(), frequencyType: "once", cron: "" }, false, "", true),
      ).not.toContain("cron");
    });
  });
});
