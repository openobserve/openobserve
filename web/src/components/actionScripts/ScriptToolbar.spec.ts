...
describe("ScriptToolbar.vue", () => {
    ...
    return mount(ScriptToolbar, {
      ...
      global: {
        ...
        stubs: {
          "q-icon": {
            template:
              '<div class="q-icon-stub" :name="name"><q-tooltip-stub v-if="$slots.default"><slot></slot></q-tooltip-stub></div>',
            props: ["name", "size"],
          },
          "q-input": {
            template:
              '<input class="q-input-stub" :data-test="$attrs[\'data-test\']" />',
            props: ["modelValue", "label", "readonly", "disable", "rules"],
          },
          "q-btn": {
            template:
              '<button class="q-btn-stub" :data-test="$attrs[\'data-test\']" @click="$emit(\'click\')">{{ label }}</button>',
            props: [
              "label",
              "color",
              "padding",
  ...
  describe("Component Rendering", () => {
    ...
    it("renders action buttons", () => {
      ...
      expect(buttons[0].attributes("data-test")).toBe(
        "add-script-fullscreen-btn",
      );
  ...
  describe("Event Handling", () => {
    ...
    it("handles fullscreen toggle", async () => {
      wrapper = createWrapper();
      const fullscreenBtn = wrapper.find(
