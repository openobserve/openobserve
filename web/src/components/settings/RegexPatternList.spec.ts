...
describe("RegexPatternList.vue Component Logic", () => {
  ...
  beforeEach(() => {
    ...
    convertUnixToDateFormat.mockImplementation((timestamp) => `2022-01-01 ${timestamp}`);
  ...
  it("should handle pagination setPagination when table ref exists", () => {
    ...
    expect(setup.regexPatternListTableRef.value.setPagination).toHaveBeenCalledWith(
...
describe("RegexPatternList.vue - ODrawer Migration", () => {
  ...
  const mountComponent = async (overrides: { isAiChatEnabled?: boolean } = {}) => {
