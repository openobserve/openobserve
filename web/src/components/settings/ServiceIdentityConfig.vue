    ...
    <div v-else>
      ...
      >
        <div class="tw:p-4 text-body2 tw:leading-relaxed">
          ...
          <div class="tw:mb-3">
            <div class="tw:font-semibold text-primary tw:mb-1">
              {{ t("settings.correlation.compoundFqnLabel") }}
            </div>
      ...
      >
        ...
        <div class="fqn-formula-box tw:p-3 tw:rounded tw:overflow-x-auto">
          <div v-if="fqnFormula.envPatterns.length > 0" class="tw:space-y-2">
            ...
            >
              <span
                class="fqn-env-label tw:font-semibold tw:text-sm tw:min-w-[100px]"
                >{{ env.name }}:</span
              >
              <!-- Scope dimensions for this environment -->
              <template
                v-for="(dim, idx) in env.scopeDims"
                :key="'scope-' + dim"
              >
                <q-chip
                  dense
                ...
                >
                  ...
                  <q-tooltip>{{
                    t("settings.correlation.scopeDimension")
                  }}</q-tooltip>
                </q-chip>
                <span
                  v-if="
                    idx < env.scopeDims.length - 1 ||
                    env.workloadDims.length > 0
                  "
                  class="fqn-separator"
                  >/</span
                >
              ...
              <template v-if="env.workloadDims.length > 0">
                <template v-if="env.workloadDims.length === 1">
                  ...
                  >
                    ...
                    <q-tooltip>{{
                      t("settings.correlation.workloadDimension")
                    }}</q-tooltip>
                  </q-chip>
                </template>
                <template v-else>
                  <span class="fqn-text-muted tw:text-xs tw:italic"
                    >{{ t("settings.correlation.firstNonNull") }} (</span
                  >
                  <template
                    v-for="(dim, idx) in env.workloadDims"
                    :key="'workload-' + dim"
                  >
                    <q-chip
                      dense
                    ...
                    <span
                      v-if="idx < env.workloadDims.length - 1"
                      class="fqn-comma"
                      >,</span
                    >
                  </template>
              ...
              <span
                v-if="env.scopeDims.length > 0 && env.workloadDims.length === 0"
                class="fqn-text-muted tw:text-xs tw:italic"
              >
          ...
          <span
            v-if="fqnFormula.envPatterns.length === 0"
            class="fqn-text-muted tw:italic"
          >
        ...
        <div class="tw:p-4">
          <div class="tw:flex tw:gap-4">
            ...
            <div class="tw:flex-1">
              <div class="tw:text-sm tw:font-semibold tw:mb-2">
                {{ t("settings.correlation.priorityOrderLabel") }}
              </div>
              <q-input
              ...
              <q-list bordered class="tw:rounded-lg tw:h-80 tw:overflow-auto">
                ...
                >
                  ...
                  <q-item-section avatar class="tw:min-w-0 tw:mr-3">
                    <q-badge
                      :color="
                        isDimensionScope(item.dim) ? 'primary' : 'secondary'
                      "
                      text-color="white"
                  ...
                  <q-item-section>
                    <q-item-label class="tw:font-medium">
                      ...
                      <span class="tw:font-normal tw:text-xs tw:opacity-70"
                        >({{ item.dim }})</span
                      >
                      <q-badge
                        :color="
                      ...
                      >
                        {{
                          isDimensionScope(item.dim)
                            ? t("settings.correlation.scopeTag")
                            : t("settings.correlation.workloadTag")
                        }}
                      </q-badge>
                    </q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <div class="tw:flex tw:gap-1">
                      ...
                      <q-btn
                        ...
                        icon="arrow_downward"
                        :disable="
                          item.actualIndex === localFqnPriority.length - 1
                        "
                        @click.stop="moveFqnDimensionDown(item.actualIndex)"
                      >
                        <q-tooltip>{{
                          t("settings.correlation.moveDown")
                        }}</q-tooltip>
                      </q-btn>
                      <q-btn
                        flat
                      ...
                      >
                        <q-tooltip>{{
                          t("settings.correlation.removeFromList")
                        }}</q-tooltip>
                      </q-btn>
                    </div>
                  </q-item-section>
                ...
                <q-item
                  v-if="filteredPriorityDimensions.length === 0"
                  class="tw:py-4 tw:text-center tw:text-gray-500"
                >
                  {{
                    prioritySearchQuery
                      ? t("settings.correlation.noMatchingDimensions")
            ...
            <div class="tw:flex-1">
              <div class="tw:text-sm tw:font-semibold tw:mb-2">
                {{ t("settings.correlation.availableDimensions") }}
              </div>
              <q-input
              ...
              <q-list bordered class="tw:rounded-lg tw:h-80 tw:overflow-auto">
                ...
                >
                  <q-item-section>
                    <q-item-label
                      >{{ group.label }}
                      <span class="tw:font-normal tw:text-xs"
                        >({{ group.value }})</span
                      ></q-item-label
                    >
                  </q-item-section>
                ...
                >
                  {{
                    availableSearchQuery
                      ? t("settings.correlation.noMatchingGroups")
          ...
          <div
            v-if="
              ...
              localSemanticGroups.length === 0
            "
            class="tw:mt-2 tw:text-sm tw:text-amber-600 dark:tw:text-amber-400"
          >
...
const filteredPriorityDimensions = computed(() => {
  const items = localFqnPriority.value.map((dim, actualIndex) => ({
    dim,
    actualIndex,
  }));
...
const fqnFormula = computed(() => {
  ...
  const groupedDims = new Map<
    string,
    { scope: string[]; workload: string[] }
  >();
...
const hasFallbackDimensions = computed(() => {
  ...
  return fqnFormula.value.envPatterns.some(
    (env) =>
      (env.name === otherGroupName || env.name === fallbackGroupName) &&
      env.workloadDims.length > 0,
  );
});
...
const addFqnDimension = () => {
  if (
    selectedSemanticGroup.value &&
    !localFqnPriority.value.includes(selectedSemanticGroup.value)
  ) {
