...
describe("FilterCreatorPopup.vue", () => {
    ...
    return mount(FilterCreatorPopup, {
      ...
      global: {
        ...
        stubs: {
          ...
          "q-select": {
            template:
              '<select class="q-select-stub" v-model="modelValue"><option v-for="option in options" :key="option.value" :value="option.value">{{ option.label }}</option></select>',
          ...
          "q-checkbox": {
            template:
              '<input type="checkbox" class="q-checkbox-stub" v-model="modelValue" :value="val" />',
  ...
  describe("Props Handling", () => {
    it("accepts all required props", () => {
      ...
      expect(wrapper.props("operators")).toEqual([
        { label: "equals", value: "=" },
      ]);
