...
color="grey-6" size="md" />
      ...
      <div v-else class="correlation-results">
        ...
        <div class="service-info">
          ...
          <div
            v-if="
              correlationResult.correlationData?.matched_dimensions &&
              Object.keys(correlationResult.correlationData.matched_dimensions)
                .length > 0
            "
...
size="xs" color="positive" />
              ...
              <q-tooltip
                >These stable dimensions were used to find related
                telemetry</q-tooltip
              >
            </div>
            <div class="service-dimensions">
              <q-chip
                v-for="(value, key) in correlationResult.correlationData
                  .matched_dimensions"
                :key="`matched-${key}`"
                size="sm"
                dense
          ...
          <div
            v-if="
              correlationResult.correlationData?.additional_dimensions &&
              Object.keys(
                correlationResult.correlationData.additional_dimensions,
              ).length > 0
            "
...
size="xs" color="grey-7" />
              <span class="dimension-section-title"
                >Additional Filters Available</span
              >
              <q-tooltip
                >These additional dimensions can be used for more specific
                filtering</q-tooltip
              >
            </div>
            <div class="service-dimensions">
        ...
        <div class="correlation-queries">
          <!-- Traces -->
          <div v-if="traceQueries.length > 0" class="query-section">
            <div class="section-header">
              <q-icon name="timeline" color="primary" />
              <span class="section-title"
                >Traces ({{ traceQueries.length }})</span
              >
            </div>
          ...
          <!-- Metrics -->
          <div v-if="metricQueries.length > 0" class="query-section">
            <div class="section-header">
              <q-icon name="show_chart" color="secondary" />
              <span class="section-title"
                >Metrics ({{ metricQueries.length }})</span
              >
            </div>
            <div class="query-items">
              <div
          ...
          <!-- Logs -->
          <div v-if="logQueries.length > 0" class="query-section">
            <div class="section-header">
