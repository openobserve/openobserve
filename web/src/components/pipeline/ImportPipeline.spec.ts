...
describe("ImportPipeline.vue", () => {
    ...
    return mount(ImportPipeline, {
      ...
      global: {
        ...
        stubs: {
          BaseImport: {
            template:
              '<div data-test="base-import"><slot name="output-content" /></div>',
            methods: {
  ...
  describe("updateSqlQuery Function", () => {
    ...
    it("should update sql_query at root level", () => {
      ...
      expect(wrapper.vm.baseImportRef.jsonArrayOfObj[0].sql_query).toBe(
        sqlQuery,
      );
    });
    ...
    it("should update source query_condition sql", () => {
      ...
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].source.query_condition.sql,
      ).toBe(sqlQuery);
    });
    ...
    it("should update nodes with matching criteria", () => {
      ...
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].nodes[0].data.query_condition
          .sql,
      ).toBe(sqlQuery);
    });
  });
  ...
  describe("updatePipelineName Function", () => {
    ...
    it("should update pipeline name", () => {
      ...
      expect(wrapper.vm.baseImportRef.jsonArrayOfObj[0].name).toBe(
        pipelineName,
      );
    });
  });
  ...
  describe("updateFunctionName Function", () => {
    ...
    it("should update function name in correct node", () => {
      ...
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].nodes[0].data.name,
      ).toBe(functionName);
    });

    it("should not update non-function nodes", () => {
      wrapper.vm.baseImportRef.jsonArrayOfObj[0].nodes[0].data.node_type =
        "stream";
      const functionName = "test-function";
      wrapper.vm.updateFunctionName(functionName, 0, 0);
  ...
  describe("updateRemoteDestination Function", () => {
    ...
    it("should update remote destination name", () => {
      ...
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].nodes[0].data
          .destination_name,
      ).toBe(remoteDestination);
    });
  });
  ...
  describe("updateTimezone Function", () => {
        ...
        jsonArrayOfObj: [
          {
            ...
            nodes: [{ data: { node_type: "query", trigger_condition: {} } }],
          },
        ],
        updateJsonArray: vi.fn(),
        isImporting: false,
    ...
    it("should update timezone in source trigger_condition", () => {
      ...
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].source.trigger_condition
          .timezone,
      ).toBe(timezone);
    });
  ...
  describe("updateOrgId Function", () => {
    ...
    it("should update organization id in source", () => {
      ...
      expect(wrapper.vm.baseImportRef.jsonArrayOfObj[0].source.org_id).toBe(
        orgId,
      );
    });
    ...
    it("should update organization id in eligible nodes", () => {
      ...
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].nodes[0].data.org_id,
      ).toBe(orgId);
      expect(
        wrapper.vm.baseImportRef.jsonArrayOfObj[0].nodes[1].data.org_id,
      ).toBe(orgId);
    });
  });
  ...
  describe("Complex Integration Tests", () => {
    ...
    it("should handle multiple pipeline objects", () => {
      ...
      expect(wrapper.vm.baseImportRef.jsonArrayOfObj[1].name).toBe(
        "updated-pipeline2",
      );
    });
