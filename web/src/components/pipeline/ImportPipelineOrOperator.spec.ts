...
describe("ImportPipeline.vue - OR Operator Tests", () => {
  ...
  describe("Import Pipeline with OR Conditions", () => {
    it("should import pipeline with simple OR condition", () => {
      ...
      expect(wrapper.vm.jsonArrayOfObj[0].nodes[0].data.query_condition.conditions.label).toBe(
        "or",
      );
      expect(
        wrapper.vm.jsonArrayOfObj[0].nodes[0].data.query_condition.conditions.items,
      ).toHaveLength(2);
    });
