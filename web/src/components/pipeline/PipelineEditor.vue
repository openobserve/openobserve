...
<template>
  ...
  <ConfirmDialog
    ...
    :message="t('pipeline.error.noImpactConfirm')"
...
const getPipeline = () => {
  ...
  pipelineService
    ...
    .then(async (response) => {
      ...
      if (!_pipeline) {
        q.notify({
          message: t("pipeline.error.notFound"),
          color: "negative",
          position: "bottom",
          timeout: 3000,
        });
...
const savePipeline = async () => {
  ...
  if (pipelineObj.currentSelectedPipeline.name === "") {
    ...
    pipelineNameErrorMessage.value = t("pipeline.error.nameRequired");
    ...
    q.notify({
      message: t("pipeline.error.nameRequired"),
      color: "negative",
      position: "bottom",
      timeout: 3000,
    });
  ...
  if (inputNodeIndex === -1) {
    q.notify({
      message: t("pipeline.error.sourceNodeRequired"),
      color: "negative",
      position: "bottom",
      timeout: 3000,
    });
    if(showJsonEditorDialog.value == true){
      validationErrors.value = [t("pipeline.error.sourceNodeRequired")];
    }
    return;
  } else if (outputNodeIndex === -1) {
    q.notify({
      message: t("pipeline.error.destinationNodeRequired"),
      color: "negative",
      position: "bottom",
      timeout: 3000,
    });
    if(showJsonEditorDialog.value == true){
      validationErrors.value = [t("pipeline.error.destinationNodeRequired")];
    }
    return;
  } else {
  ...
  if (findMissingEdges()) {
    q.notify({
      message: t("pipeline.error.connectAllNodes"),
      color: "negative",
      position: "bottom",
      timeout: 3000,
    });
...
const validatePipeline = () => {
  ...
  if (
    ...
    q.notify({
      message:
        t("pipeline.error.enrichmentTablesScheduledOnly"),
      color: "negative",
      position: "bottom",
      timeout: 2000,
    });
...
const onSubmitPipeline = async () => {
  ...
  saveOperation
    .then(() => {
      ...
      q.notify({
        message: t("pipeline.success.updated"),
        color: "positive",
        position: "bottom",
        timeout: 3000,
      });
        ...
        q.notify({
          message: t("pipeline.success.saved"),
          color: "positive",
          position: "bottom",
          timeout: 3000,
        });
    ...
    .catch((error) => {
      ...
      if (
        ...
        if (error.response.status != 403) {
          ...
          if(showJsonEditorDialog.value == true){
            validationErrors.value = [error.response?.data?.message || t("pipeline.error.savingError")];
          }
        }
      }
    })
...
const openCancelDialog = () => {
  if (
    ...
    confirmDialogMeta.value.message =
      t("pipeline.error.cancelConfirm");
...
const savePipelineJson = async (json: string) => {
  try {
    ...
    validationErrors.value = [t('pipeline.error.invalidJson')];
  }
};
