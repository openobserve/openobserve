...
describe("WorkflowStepResultDrawer", () => {
  ...
  describe("copy", () => {
    it("copies the input records with the input success message", async () => {
      ...
      expect(mockCopy).toHaveBeenCalledWith(JSON.stringify([rec(2)], null, 2), expect.any(Function), {
    ...
    it("copies the output records", async () => {
      ...
      await btn.trigger("click");
      expect(mockCopy).toHaveBeenCalledWith(JSON.stringify([rec(3)], null, 2), expect.any(Function), {
