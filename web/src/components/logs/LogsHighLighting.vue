...
/**
 * Detects semantic type based on value and color for CSS class mapping
 */
function detectSemanticType(text: string, color: string): string {
  // Map color values to semantic types
  if (color === currentColors.value.timestamp) return "timestamp";
  if (color === currentColors.value.numberValue) return "number";
  if (color === currentColors.value.booleanValue) return "boolean";
  if (color === currentColors.value.nullValue) return "null";
  if (color === currentColors.value.objectValue) return "object";

  // Default string type detection
  if (!text || typeof text !== "string") return "string";
...
function createStyledSpan(
  ...
  const semanticType = detectSemanticType(text, color);
  ...
  result += highlightParts
    .map((part) => {
      if (part.isHighlighted) {
        ...
        return `<span class="log-${semanticType}">${escapeHtml(part.text)}</span>`;
      }
    })
    .join("");
...
function colorizeObject(
  ...
  // Add opening brace: "{"
  if (showBraces) {
    parts.push('<span class="log-brace">{</span>');
  }
  ...
  entries.forEach(([key, value], index) => {
    ...
    if (value === null) {
      ...
      if (isLogLineWithMixedContent(value)) {
        ...
        const processedValue = processTextWithHighlights(
          value,
          queryString,
          colors,
          showQuotes,
      ...
      parts.push(
        createStyledSpan(objStr, colors.objectValue, queryString, showQuotes),
      );
    } else {
...
.log-key {
  color: #b71c1c; /* Red for keys */
}

/* Data Types - Light Theme */
.log-string {
  color: #059669; /* Green */
}
...
:global(.body--dark) .log-key {
  color: #fca5a5; /* Light Red */
}

:global(.body--dark) .log-string {
  color: #6ee7b7; /* Light Green */
}

:global(.body--dark) .log-number {
...
:global(.body--dark) .log-boolean {
  color: #a78bfa; /* Light Purple */
}

:global(.body--dark) .log-null {
  color: #9ca3af; /* Light Gray */
}

:global(.body--dark) .log-object {
  color: #d1d5db; /* Light Gray */
}

:global(.body--dark) .log-ip {
  color: #f87171; /* Light Red */
}

:global(.body--dark) .log-url {
  color: #93c5fd; /* Light Blue */
}

:global(.body--dark) .log-email {
  color: #c4b5fd; /* Light Purple */
}

:global(.body--dark) .log-http-method {
  color: #6ee7b7; /* Light Green */
}

:global(.body--dark) .log-status-code {
  color: #f87171; /* Light Red */
}

:global(.body--dark) .log-uuid {
  color: #9ca3af; /* Light Gray */
}

:global(.body--dark) .log-path {
  color: #d1d5db; /* Light Gray */
}

:global(.body--dark) .log-brace,
:global(.body--dark) .log-separator {
