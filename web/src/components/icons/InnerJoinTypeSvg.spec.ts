...
describe("InnerJoinTypeSvg.vue", () => {
  ...
  const createWrapper = (props = {}) => mount(InnerJoinTypeSvg, { global: { plugins: [] }, props });
  ...
  describe("Component Rendering", () => {
    ...
    it("has correct component name", () => {
      wrapper = createWrapper();
      expect(wrapper.vm.$options.name).toBe("InnerJoinTypeSvg");
    });
  ...
  describe("shouldFill prop", () => {
    ...
    it("rect has correct fill-opacity when shouldFill is true", () => {
      wrapper = createWrapper({ shouldFill: true });
      expect(wrapper.find("rect").attributes("fill-opacity")).toBe("0.12");
    });

    it("rect has correct dimensions when shouldFill is true", () => {
      wrapper = createWrapper({ shouldFill: true });
    ...
    it("rect has correct border-radius when shouldFill is true", () => {
      wrapper = createWrapper({ shouldFill: true });
