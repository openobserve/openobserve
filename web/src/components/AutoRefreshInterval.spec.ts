...
describe("AutoRefreshInterval", () => {
    ...
    return mount(AutoRefreshInterval, {
      ...
      global: {
        ...
        stubs: {
          "q-btn-dropdown": {
            template:
              '<div class="q-btn-dropdown"><slot /><slot name="label" /></div>',
          },
          "q-icon": {
          ...
          "q-btn": {
            template:
              '<button @click="$emit(\'click\')" :disabled="disable"><slot /></button>',
  ...
  describe("Function Coverage Tests", () => {
    ...
    it("should handle disabled values through button clicks", async () => {
      ...
      const fiveSecButton = wrapper.find(
        '[data-test="logs-search-bar-refresh-time-5"]',
      );
    ...
    it("should handle valid values through button clicks", async () => {
      ...
      const oneMinButton = wrapper.find(
        '[data-test="logs-search-bar-refresh-time-60"]',
      );
    ...
    it("should test onItemClick through component interaction", async () => {
      ...
      const fiveMinButton = wrapper.find(
        '[data-test="logs-search-bar-refresh-time-300"]',
      );
    ...
    it("should update disabled flags when minRefreshInterval changes", async () => {
      ...
      let oneMinItem = wrapper.vm.refreshTimes
        .flat()
        .find((item: any) => item.value === 60);
      ...
      oneMinItem = wrapper.vm.refreshTimes
        .flat()
        .find((item: any) => item.value === 60);
    ...
    it("should test disabled state through component data", () => {
      ...
      const fiveSecItem = wrapper.vm.refreshTimes
        .flat()
        .find((item: any) => item.value === 5);
      const tenSecItem = wrapper.vm.refreshTimes
        .flat()
        .find((item: any) => item.value === 10);
      const fifteenSecItem = wrapper.vm.refreshTimes
        .flat()
        .find((item: any) => item.value === 15);
      const oneMinItem = wrapper.vm.refreshTimes
        .flat()
        .find((item: any) => item.value === 60);
    ...
    it("should show tooltip for disabled buttons", () => {
      ...
      const disabledItems = wrapper.vm.refreshTimes
        .flat()
