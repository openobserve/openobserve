      ...
      >
        ...
        >
          <div class="folder-item full-width row justify-between no-wrap">
            <span class="folder-name" :title="tab.name">{{ tab.name }}</span>
  ...
  setup(props, { emit }) {
    ...
    const deleteFolder = async () => {
      if (selectedFolderDelete.value) {
        try {
          //delete folder
          await deleteFolderByIdByType(
            store,
            selectedFolderDelete.value,
            "alerts",
          );
    ...
    const filteredTabs = computed(() => {
      ...
      return store.state.organizationData.foldersByType[props.type]?.filter(
        (tab) => {
          return tab.name
            .toLowerCase()
