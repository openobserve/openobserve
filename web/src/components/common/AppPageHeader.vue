    ...
    <div class="flex items-center gap-3.25 min-w-0 h-full flex-1">
      ...
      <template v-if="hasBack">
        <slot name="back">
          <button
            type="button"
            class="inline-flex items-center justify-center shrink-0 w-9.5 h-9.5 rounded-lg bg-surface-subtle text-text-primary transition-colors hover:bg-button-ghost-hover-bg outline-none focus-visible:ring-4 focus-visible:ring-primary-500/25 focus-visible:ring-inset"
            :title="backLabel"
      ...
      <span
        v-else-if="icon"
        class="inline-flex items-center justify-center shrink-0 w-9.5 h-9.5 rounded-lg bg-tabs-active-bg text-tabs-active-text"
        aria-hidden="true"
      >
      ...
      <div class="flex flex-col justify-center min-w-0 shrink-0">
        <h1
          class="text-base! font-semibold! leading-[1.45]! tracking-[-0.02em]! text-text-primary truncate min-h-6"
          :title="title"
