...
<template>
  <span
    ...
    :class="size === 'md' ? 'text-text-body gap-2 text-sm' : 'text-text-secondary gap-1.5'"
    data-test="powered-by-openobserve"
  >
    ...
    <a
      ...
      data-test="powered-by-openobserve-link"
    >
      <img
        v-if="size === 'md'"
        class="block h-8 max-w-37.5"
        :src="
          getImageURL(
            isDark
      ...
      <template v-else>
        ...
        <strong class="text-text-heading font-semibold">{{ brand }}</strong>
      </template>
    </a>
  </span>
</template>
...
// `md` shows the main app logo for page headers; the default `sm` is the compact footer mark, where the wordmark would be unreadable.
withDefaults(defineProps<{ size?: "sm" | "md" }>(), { size: "sm" });

const { t } = useI18nTyped();
const brand = raw("OpenObserve");
