...
describe("VariableCustomValueSelector", () => {
  ...
  describe("Component Setup & Mounting", () => {
    ...
    it("should setup useSelectAutoComplete composable", async () => {
      ...
      const useSelectAutoCompleteModule =
        await import("../../../composables/useSelectAutocomplete");
      expect(
        useSelectAutoCompleteModule.useSelectAutoComplete,
      ).toHaveBeenCalledWith(expect.any(Object), "value");
    });
  ...
  describe("Props & Reactive State", () => {
    ...
    it("should display correct label from variableItem", () => {
      ...
      if (qSelect.exists()) {
        ...
        expect(wrapper.props("variableItem").label).toBe(
          defaultVariableItem.label,
        );
      }
    });
    ...
    it("should fallback to name when label is not available", () => {
      ...
      if (qSelect.exists()) {
        ...
        expect(wrapper.props("variableItem").name).toBe(
          defaultVariableItem.name,
        );
      }
    });
  ...
  describe("Multi-Select Functionality", () => {
    ...
    it("should compute isAllSelected as false when no options are selected", () => {
      ...
      if (
        wrapper.vm.fieldsFilteredOptions &&
        wrapper.vm.fieldsFilteredOptions.length > 0
      ) {
    ...
    it("should handle deselect all functionality", () => {
      mockFilteredOptions.value = defaultVariableItem.options;
      wrapper.vm.selectedValue = defaultVariableItem.options.map(
        (opt) => opt.value,
      );
      wrapper.vm.fieldsFilteredOptions = mockFilteredOptions;
  ...
  describe("Display Value Logic", () => {
    ...
    it("should display single value correctly", () => {
      ...
      const matchingOption = defaultVariableItem.options.find(
        (opt) => opt.value === "production",
      );
      if (matchingOption) {
