...
vi.mock("./AddSettingVariable.vue", () => ({
  default: {
    ...
    template:
      '<div data-test="add-setting-variable-mock">Add Setting Variable</div>',
...
vi.mock("./VariablesDependenciesGraph.vue", () => ({
  default: {
    ...
    template:
      '<div data-test="dependencies-graph-mock">Dependencies Graph</div>',
    props: ["variablesList"],
    emits: ["closePopUp"],
  },
}));
...
vi.mock("vue-draggable-next", () => ({
  VueDraggableNext: {
    ...
    template:
      '<div data-test="dashboard-variable-settings-drag"><slot /></div>',
...
describe("VariableSettings", () => {
  ...
  describe("Variables List Display", () => {
    ...
    it("should render draggable container when variables exist", async () => {
      ...
      expect(
        wrapper.find('[data-test="dashboard-variable-settings-drag"]').exists(),
      ).toBe(true);
    });
  ...
  describe("Add Variable Functionality", () => {
    ...
    it("should show AddSettingVariable component in add mode", async () => {
      ...
      expect(
        wrapper.find('[data-test="add-setting-variable-mock"]').exists(),
      ).toBe(true);
    });
  ...
  describe("Delete Variable Functionality", () => {
    ...
    it("should show confirm dialog component when delete dialog is active", async () => {
      ...
      expect(wrapper.find('[data-test="confirm-dialog-mock"]').exists()).toBe(
        true,
      );
    });
    ...
    it("should handle delete error with 409 status", async () => {
      ...
      expect(
        mockNotifications.showConfictErrorNotificationWithRefreshBtn,
      ).toHaveBeenCalledWith("Conflict error");
  ...
  describe("Drag and Drop Functionality", () => {
    ...
    it("should handle drag end error with 409 status", async () => {
      const vm = wrapper.vm as any;
      const error409 = {
        response: { status: 409, data: { message: "Reorder conflict" } },
      };
      ...
      expect(
        mockNotifications.showConfictErrorNotificationWithRefreshBtn,
      ).toHaveBeenCalledWith("Reorder conflict");
    });
  ...
  describe("Data Loading and Error Handling", () => {
    ...
    it("should handle getDashboard error gracefully", async () => {
      ...
      const vm = wrapper.vm as any;
      await expect(vm.getDashboardData()).rejects.toThrow(
