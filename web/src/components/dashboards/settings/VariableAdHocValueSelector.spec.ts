...
describe("VariableAdHocValueSelector", () => {
  ...
  describe("Component Initialization", () => {
    it("should render correctly with initial props", () => {
      ...
      expect(
        wrapper
          .find('[data-test="dashboard-variable-adhoc-name-selector"]')
          .exists(),
      ).toBe(true);
      expect(
        wrapper
          .find('[data-test="dashboard-variable-adhoc-operator-selector"]')
          .exists(),
      ).toBe(true);
      expect(
        wrapper
          .find('[data-test="dashboard-variable-adhoc-value-selector"]')
          .exists(),
      ).toBe(true);
    });
  ...
  describe("Add Fields Functionality", () => {
    it("should add new field when addFields is called", async () => {
      ...
      const newField =
        wrapper.vm.adhocVariables[wrapper.vm.adhocVariables.length - 1];
  ...
  describe("Remove Fields Functionality", () => {
    ...
    it("should remove field when close button is clicked", async () => {
      const closeButton = wrapper.find(
        '[data-test="dashboard-variable-adhoc-close-0"]',
      );
    ...
    it("should render correct number of close buttons", async () => {
      const closeButtons = wrapper.findAll(
        '[data-test^="dashboard-variable-adhoc-close-"]',
      );
  ...
  describe("Operator Selection", () => {
    ...
    it("should display current operator value", () => {
      ...
      expect(wrapper.vm.adhocVariables[0].operator).toBe(
        defaultProps.modelValue[0].operator,
      );
    });
  });
  ...
  describe("Multiple Variables", () => {
    ...
    it("should handle removal of middle item", async () => {
      const closeButton = wrapper.find(
        '[data-test="dashboard-variable-adhoc-close-1"]',
      );
    ...
    it("should update correct field when input changes", async () => {
      const secondNameInput = wrapper.findAll(
        '[data-test="dashboard-variable-adhoc-name-selector"]',
      )[1];
  ...
  describe("Component Integration", () => {
    it("should integrate with useSelectAutoComplete", () => {
      ...
      expect(wrapper.vm.fieldsFilterFn).toBe(
        mockUseSelectAutoComplete.filterFn,
      );
      expect(wrapper.vm.fieldsFilteredOptions).toBe(
        mockUseSelectAutoComplete.filteredOptions,
      );
    });
