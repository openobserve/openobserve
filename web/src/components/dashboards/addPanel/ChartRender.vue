...
<script lang="ts">
import { defineComponent, onMounted,onUpdated, ref, reactive, nextTick, watch, computed, onActivated } from "vue";
...
export default defineComponent({
    ...
    setup(props) {
        ...
        watch(() => [props.data, props.selectedTimeDate] , () => {
            if (props.data.query) {
                fetchQueryData()
            } else {
        ...
        onMounted(async () => {
            await nextTick()
            if(props.data.type != "table"){
                await Plotly.newPlot(plotRef.value, [{}], {}, {
                    responsive: true,
                    displaylogo: false,
                    displayModeBar: false
                });
        ...
        const fetchQueryData = async () => {
            ...
            if (
                ...
                startISOTimestamp = new Date(timestamps.start_time.toISOString()).getTime() * 1000;
                endISOTimestamp = new Date(timestamps.end_time.toISOString()).getTime() * 1000;
            }
            const query = {
                query: { sql: queryData, sql_mode: "full", start_time: startISOTimestamp, end_time: endISOTimestamp },
            };
        ...
        watch(() => [searchQueryData.data, props.data.type], () => {
        ...
        const renderChart = async () => {
            ...
            traces = yAxisKeys.map((key) => {
                const trace = {
                    name: props.data.fields?.y.find((it)=> it.label == key).column,
                    ...getTraceValuesByChartType(xAxisKey, key),
                    showlegend: props.data.config?.show_legends,
                    marker: {
                        color: props.data.fields?.y.find((it)=> it.label == key).color || "#5960b2",
                        opacity: 0.8,
                    },
                }
                return trace
