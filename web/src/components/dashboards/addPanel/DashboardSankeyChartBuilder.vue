...
<template>
  <div v-if="!promqlMode && dashboardPanelData.data.type == 'sankey'">
    ...
    <div style="display: flex; flex-direction: row" class="tw:pl-3">
      <div class="layout-name tw:whitespace-nowrap tw:min-w-[130px] tw:flex tw:items-center">
        {{ t("panel.source") }}
        <OIcon name="info-outline" size="sm" class="tw:ml-1" />
          <OTooltip :content="Hint" />
      </div>
      <span class="layout-separator tw:flex tw:items-center tw:ml-[2px] tw:mr-[2px]">:</span>
      <div
        class="axis-container droppable scroll tw:flex-1 tw:w-full tw:flex tw:flex-wrap tw:border-transparent tw:border-dashed tw:border-2"
        :class="{
