...
describe("PanelContainer", () => {
    ...
    return mount(PanelContainer, {
      ...
      global: {
        ...
        stubs: {
          ...
          QueryInspector: {
            template: '<div data-test="query-inspector"></div>',
          },
          PanelSchemaRenderer: {
  ...
  describe("Component Rendering", () => {
    it("should render panel container with basic structure", () => {
      ...
      expect(
        wrapper.find('[data-test="dashboard-panel-container"]').exists(),
      ).toBe(true);
      expect(wrapper.find('[data-test="dashboard-panel-bar"]').exists()).toBe(
        true,
      );
    ...
    it("should show drag indicator when not in view-only mode", () => {
      ...
      expect(wrapper.find('[data-test="dashboard-panel-drag"]').exists()).toBe(
        true,
      );
    });
  ...
  describe("Panel Header", () => {
    ...
    it("should show description tooltip on hover when description exists", async () => {
      ...
      await wrapper
        .find('[data-test="dashboard-panel-container"]')
        .trigger("mouseover");

      expect(
        wrapper.find('[data-test="dashboard-panel-description-info"]').exists(),
      ).toBe(true);
    });
  ...
  describe("Panel Controls", () => {
    it("should show fullscreen button on hover when not view-only", async () => {
      ...
      expect(
        wrapper.find('[data-test="dashboard-panel-fullscreen-btn"]').exists(),
      ).toBe(true);
    });
    ...
    it("should open fullscreen view when fullscreen button is clicked", async () => {
      ...
      await wrapper
        ...
        .trigger("mouseover");
      await wrapper
        .find('[data-test="dashboard-panel-fullscreen-btn"]')
        .trigger("click");
  ...
  describe("Error States", () => {
    it("should show error button when error data exists", async () => {
      ...
      expect(wrapper.find('[data-test="panel-error-data"]').exists()).toBe(
        true,
      );
    });
  ...
  describe("Warning States", () => {
    it("should show dependent ad-hoc variable warning", async () => {
      ...
      expect(
        wrapper
          .find('[data-test="dashboard-panel-dependent-adhoc-variable-btn"]')
          .exists(),
      ).toBe(true);
    });
  ...
  describe("Chart Rendering", () => {
    it("should render panel schema renderer for chart types", () => {
      ...
      expect(wrapper.find('[data-test="panel-schema-renderer"]').exists()).toBe(
        true,
      );
    });
    ...
    it("should pass correct props to panel schema renderer", () => {
      ...
      const panelRenderer = wrapper.findComponent(
        '[data-test="panel-schema-renderer"]',
      );
  ...
  describe("Context Menu", () => {
    it("should show dropdown menu in non-view-only mode", () => {
      ...
      const dropdown = wrapper.find(
        '[data-test="dashboard-edit-panel-Test Panel-dropdown"]',
      );
  ...
  describe("Refresh Panel Functionality", () => {
    it("should refresh panel and generate new runId", async () => {
      ...
      expect(wrapper.emitted("refreshPanelRequest")[0]).toEqual([
        mockPanelData.id,
        false,
      ]);
    ...
    it("should not refresh if panel is already loading", async () => {
      ...
      const emitCountBefore =
        wrapper.emitted("refreshPanelRequest")?.length || 0;

      await wrapper.vm.onRefreshPanel(false);
      await wrapper.vm.$nextTick();

      const emitCountAfter =
        wrapper.emitted("refreshPanelRequest")?.length || 0;
    ...
    it("should show refresh button when not view-only", () => {
      ...
      expect(
        wrapper
          .find('[data-test="dashboard-panel-refresh-panel-btn"]')
          .exists(),
      ).toBe(true);
    });
  ...
  describe("Move Panel", () => {
    ...
    it("should emit onMovePanel with correct tab id", async () => {
      ...
      expect(wrapper.emitted("onMovePanel")[0]).toEqual([
        mockPanelData.id,
        targetTabId,
      ]);
    });
  ...
  describe("Download Functionality", () => {
    ...
    it("should hide download options when no queries", () => {
      ...
      expect(
        wrapper
          .find('[data-test="dashboard-panel-download-as-csv-btn"]')
          .exists(),
      ).toBe(false);
      expect(
        wrapper
          .find('[data-test="dashboard-panel-download-as-json-btn"]')
          .exists(),
      ).toBe(false);
    });
  });
  ...
  describe("Variables Data Management", () => {
    ...
    it("should handle array value comparison for variables", async () => {
      const variablesData = {
        values: [
          { name: "testVar", value: ["val1", "val2"], type: "constant" },
        ],
        isLoading: false,
      };
      const currentVariablesData = {
        values: [
          { name: "testVar", value: ["val1", "val3"], type: "constant" },
        ],
        isLoading: false,
      };
      const panelWithVariable = {
  ...
  describe("Cached Data Warning", () => {
    ...
    it("should hide cached data warning when not different", async () => {
      ...
      await wrapper.vm.handleIsCachedDataDifferWithCurrentTimeRangeUpdate(
        false,
      );
      await wrapper.vm.$nextTick();
  ...
  describe("Limit Series Warning", () => {
    it("should show limit series warning message", async () => {
      ...
      expect(
        wrapper
          .find('[data-test="panel-limit-number-of-series-warning"]')
          .exists(),
      ).toBe(true);
    });
  ...
  describe("Event Emissions", () => {
    ...
    it("should emit refresh event from SinglePanelMove", async () => {
      ...
      const panelMove = wrapper.findComponent(
        '[data-test="single-panel-move"]',
      );
