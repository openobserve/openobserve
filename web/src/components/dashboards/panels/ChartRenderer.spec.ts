...
describe("ChartRenderer", () => {
  ...
  describe("Theme Management", () => {
    ...
    it("should cleanup chart on theme change", async () => {
      ...
      expect(vi.mocked(echarts.init).mock.calls.length).toBeGreaterThan(
        initialCallCount,
      );
    });
  });
    ...
    it("should handle data prop changes", async () => {
      const newData = {
        ...
        options: {
          ...
          series: [
            { name: "Bar Series", type: "bar", data: [[1609459200000, 15]] },
          ],
        },
      };
  ...
  describe("Event Handling", () => {
    it("should handle mouse events on container", async () => {
      ...
      expect(mockHoveredSeriesState.setIndex).toHaveBeenCalledWith(
        -1,
        -1,
        -1,
        null,
  ...
  describe("Hover State Management", () => {
    ...
    it("should respond to hover state changes", async () => {
      ...
      expect(mockHoveredSeriesState.value.hoveredSeriesName).toBe(
        "Test Series",
      );
    });
  });
  ...
  describe("Context Menu Events", () => {
    ...
    it("should emit domcontextmenu event when DOM element is right-clicked", async () => {
      ...
      expect(
        wrapper.emitted("domcontextmenu") || wrapper.emitted("contextmenu"),
      ).toBeTruthy();
    });
    ...
    it("should extract Y-axis value correctly from DOM contextmenu", async () => {
      ...
      vi.mocked(mockChart.convertFromPixel).mockReturnValue([
        1609459200000, 85.7,
      ]);
