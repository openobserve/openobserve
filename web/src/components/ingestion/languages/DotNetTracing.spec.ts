...
import DotNetTracing from './DotNetTracing.vue';
...
vi.mock('@/composables/useIngestion', () => ({
  default: vi.fn(() => ({
    ...
    languagesDocURLs: {
      dotnettracing: 'https://short.openobserve.ai/languages/dotnettracing',
    },
  })),
}));
...
describe('DotNetTracing.vue Comprehensive Coverage', () => {
  ...
  describe('Setup Function Tests', () => {
    ...
    it('should replace [STREAM_NAME] with dotnettracing', () => {
      wrapper = createWrapper();
      expect((wrapper.vm as any).content).toContain('dotnettracing');
    });
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with processed name', () => {
      ...
      expect(vm.content).toContain('dotnettracing');
    });
    ...
    it('should produce lowercase stream name', () => {
      wrapper = createWrapper();
      expect((wrapper.vm as any).content).not.toContain('DOTNETTRACING');
    });
  });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass processed content without [STREAM_NAME] to CopyContent', () => {
      ...
      expect(content).toContain('dotnettracing');
    });
    ...
    it('should pass correct href to documentation link', () => {
      ...
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/languages/dotnettracing');
    });
  });
