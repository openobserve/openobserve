...
vi.mock('@/composables/useIngestion', () => ({
  default: vi.fn(() => ({
    ...
    messageQueuesDocURLs: {
      rabbitmq: 'https://short.openobserve.ai/messagequeues/rabbitmq',
    },
  })),
}));
...
describe('RabbitMQ.vue Comprehensive Coverage', () => {
  ...
  describe('Setup Function Tests', () => {
    ...
    it('should replace [STREAM_NAME] with rabbitmq', () => {
      wrapper = createWrapper();
      expect((wrapper.vm as any).content).toContain('rabbitmq');
    });
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with processed name', () => {
      ...
      expect(vm.content).toContain('rabbitmq');
    });
    ...
    it('should produce lowercase stream name', () => {
      wrapper = createWrapper();
      expect((wrapper.vm as any).content).not.toContain('RABBITMQ');
    });
  });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass correct href to documentation link', () => {
      ...
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/messagequeues/rabbitmq');
    });
  });
