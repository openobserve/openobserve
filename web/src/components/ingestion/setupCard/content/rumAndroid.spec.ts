...
/**
 * Unit tests for the pure `rumAndroidCard()` builder function.
 * This module has no Vue component — all assertions are on the returned data
 * structure. No mounting, no store, no i18n.
 *
 * rumAndroid.ts exports:
 *   - rumAndroidCard(subs: RumAndroidCardSubs): RichCardContent
 *   - RUM_ANDROID_SDK_VERSION: string
...
// Mock getImageURL before importing rumAndroid.ts so the builder can resolve
// assets without a real DOM environment.
vi.mock("@/utils/zincutils", () => ({
  getImageURL: vi.fn((p: string) => `/mocked-asset/${p}`),
}));

import rumAndroidCard, {
...
import type { RumAndroidCardSubs } from "./rumAndroid";
import type { RichCardContent } from "../types";

// ── shared substitutions ──────────────────────────────────────────────────────

const BASE_SUBS: RumAndroidCardSubs = {
...
const HTTP_SUBS: RumAndroidCardSubs = {
  ...BASE_SUBS,
  insecureHTTP: true,
};
...
function buildCard(subs: RumAndroidCardSubs = BASE_SUBS): RichCardContent {
  return rumAndroidCard(subs);
}

// ── tests ─────────────────────────────────────────────────────────────────────

describe("rumAndroidCard builder", () => {
  // ── RUM_ANDROID_SDK_VERSION ────────────────────────────────────────────────
  ...
  describe("provider", () => {
    ...
    it("sets runtime to 'Android'", () => {
      const card = buildCard();

      expect(card.provider.runtime).toBe("Android");
    });
  ...
  // ── install step — kotlin/groovy variants ───────────────────────────────────

  describe("install step variants", () => {
    it("has kotlin and groovy variant ids", () => {
      const card = buildCard();
      const install = card.steps.find((s) => s.id === "install")!;
      const ids = install.variants!.map((v) => v.id);

      expect(ids).toEqual(["kotlin", "groovy"]);
    });

    it("kotlin variant code includes the artifact and version", () => {
      const card = buildCard();
      const install = card.steps.find((s) => s.id === "install")!;
    ...
    it("kotlin variant filename is 'build.gradle.kts'", () => {
      ...
      expect(kotlin.code.filename).toBe("build.gradle.kts");
    });

    it("groovy variant filename is 'build.gradle'", () => {
      const card = buildCard();
      const install = card.steps.find((s) => s.id === "install")!;
      const groovy = install.variants!.find((v) => v.id === "groovy")!;

      expect(groovy.code.filename).toBe("build.gradle");
    });
  });
  ...
  describe("detect config", () => {
    it("has streamType 'logs', streamName '_rumdata' and android filter", () => {
      const card = buildCard();
  ...
  describe("init step — raw vs masked", () => {
    ...
    it("masked code contains the masked token, not the raw token", () => {
      const subs: RumAndroidCardSubs = {
        ...BASE_SUBS,
        rumToken: "super-secret-token",
        rumTokenMasked: "supe-****-oken",
      };
      const card = rumAndroidCard(subs);
      const init = card.steps.find((s) => s.id === "init")!;
    ...
    it("raw code contains the full RUM URL and Logs URL", () => {
      const card = buildCard();
      const init = card.steps.find((s) => s.id === "init")!;
    ...
    it("raw code contains OpenObserve.initialize, Rum.enable, Logs.enable and useCustomEndpoint", () => {
      const card = buildCard();
      const init = card.steps.find((s) => s.id === "init")!;
      ...
      expect(init.code!.raw).toContain("useCustomEndpoint");
    });
  });
  ...
  describe("init step — insecureHTTP", () => {
    it("raw code contains allowClearTextHttp when insecureHTTP is true", () => {
      const card = rumAndroidCard(HTTP_SUBS);
      const init = card.steps.find((s) => s.id === "init")!;

      expect(init.code!.raw).toContain("allowClearTextHttp");
    });

    it("raw code does not contain allowClearTextHttp when insecureHTTP is false", () => {
      const card = rumAndroidCard(BASE_SUBS);
      const init = card.steps.find((s) => s.id === "init")!;

      expect(init.code!.raw).not.toContain("allowClearTextHttp");
    });
  });
  ...
  describe("docUrl", () => {
    it("provides the openobserve docs setup URL", () => {
      const card = buildCard();
  ...
  describe("determinism", () => {
    it("calling rumAndroidCard twice with identical subs returns identical output", () => {
      const card1 = rumAndroidCard(BASE_SUBS);
      const card2 = rumAndroidCard(BASE_SUBS);

      expect(JSON.stringify(card1)).toBe(JSON.stringify(card2));
    });

    it("different rumToken values produce different raw init codes", () => {
      const card1 = rumAndroidCard({
        ...BASE_SUBS,
        rumToken: "token-a",
        rumTokenMasked: "tok-a",
      });
      const card2 = rumAndroidCard({
        ...BASE_SUBS,
        rumToken: "token-b",
