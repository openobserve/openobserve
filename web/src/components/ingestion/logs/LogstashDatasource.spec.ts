...
describe("LogstashDatasource.vue Component Logic", () => {
  ...
  it("should work with different endpoint URLs", () => {
    ...
    expect(setup.content).toContain(
      "https://api.example.com/api/test-org/default/_json",
    );
  ...
  it("should handle special characters in organization identifier", () => {
    ...
    expect(setup.content).toContain(
      "/api/org-with-special-chars_123/default/_json",
    );
  });
  ...
  it("should work correctly with HTTPS endpoints", () => {
    ...
    expect(setup.content).toContain(
      "https://secure.example.com/api/test-org/default/_json",
    );
  });
  ...
  it("should work correctly with custom ports", () => {
    ...
    expect(setup.content).toContain(
      "http://localhost:8080/api/test-org/default/_json",
    );
  });
  ...
  it("should integrate properly with store organization data", () => {
    ...
    expect(setup.store.state.selectedOrganization.identifier).toBe("default");
    expect(setup.store.state.selectedOrganization.label).toBe(
      "default Organization",
    );
    expect(setup.store.state.selectedOrganization.user_email).toBe(
      "example@gmail.com",
    );
  ...
  it("should handle different organization subscription types", () => {
    ...
    expect(setup.store.state.selectedOrganization.subscription_type).toBe(
      "premium",
    );
  });
  ...
  it("should handle edge case organization identifiers", () => {
    const edgeCases = [
      ...
      {
        identifier: "very-long-organization-identifier-123456789",
        expected:
          "/api/very-long-organization-identifier-123456789/default/_json",
      },
    ];
