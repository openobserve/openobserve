...
import GoogleWorkspace from './GoogleWorkspace.vue';
...
describe('GoogleWorkspace.vue Comprehensive Coverage', () => {
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with googleworkspace in content', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('googleworkspace');
      expect(vm.content).not.toContain('[STREAM_NAME]');
    });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass the correct googleworkspace doc URL as href', () => {
      wrapper = createWrapper();
      const docLink = wrapper.find('a');
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/security/google-workspace');
    });
  });
