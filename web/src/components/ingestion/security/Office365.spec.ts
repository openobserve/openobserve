...
import Office365 from './Office365.vue';
...
describe('Office365.vue Comprehensive Coverage', () => {
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with office365 in content', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('office365');
      expect(vm.content).not.toContain('[STREAM_NAME]');
    });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass the correct office365 doc URL as href', () => {
      wrapper = createWrapper();
      const docLink = wrapper.find('a');
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/security/office365');
    });
  });
