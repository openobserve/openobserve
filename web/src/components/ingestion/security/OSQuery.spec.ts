...
import OSQuery from './OSQuery.vue';
...
vi.mock('@/composables/useIngestion', () => ({
  default: vi.fn(() => ({
    ...
    securityContent: `HTTP Endpoint: http://localhost:5080/api/test-org/[STREAM_NAME]/_json\nAccess Key: [BASIC_PASSCODE]`,
    securityDocURLs: {
...
describe('OSQuery.vue Comprehensive Coverage', () => {
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with osquery in content', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('osquery');
      expect(vm.content).not.toContain('[STREAM_NAME]');
    });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass the correct osquery doc URL as href', () => {
      wrapper = createWrapper();
      const docLink = wrapper.find('a');
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/security/osquery');
    });
  });
