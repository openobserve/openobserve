...
describe("cassandraCard builder", () => {
  it("builds metadata + JMX step flow", () => {
    ...
    expect(card.detect).toMatchObject({ streamType: "metrics", match: "keyword", streamName: "cassandra" });
    expect(card.steps.map((s) => s.id)).toEqual(["jmx-jar", "install", "configure", "run", "verify"]);
  });
  it("downloads the JMX jar and uses a jmx receiver targeting cassandra", () => {
    const card = cassandraCard(SUBS);
    expect(card.steps.find((s) => s.id === "jmx-jar")!.code!.raw).toContain("opentelemetry-jmx-metrics.jar");
