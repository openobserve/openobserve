...
describe("Databricks.vue", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(zincutils.getImageURL).mockReturnValue(
  ...
  const getGlobalConfig = (props = {}) => ({
    ...
    global: {
      ...
      stubs: {
        CopyContent: {
          template:
            '<div class="copy-content-stub copy-content-container-cls"><slot /></div>',
          props: ["content"],
        },
      },
    },
  ...
  it("should have correct documentation URL", () => {
    ...
    expect(link.attributes("href")).toBe(
      "https://short.openobserve.ai/databricks",
    );
  });
  ...
  it("should accept currOrgIdentifier prop", () => {
    wrapper = mount(
      Databricks,
      getGlobalConfig({ currOrgIdentifier: "test-org-123" }),
    );
  ...
  it("should accept currUserEmail prop", () => {
    wrapper = mount(
      Databricks,
      getGlobalConfig({ currUserEmail: "test@example.com" }),
    );
