...
// CouchDB.vue uses name = "couchDB"; after .replace(" ", "_").toLowerCase() => "couchdb"
...
describe('CouchDB.vue Comprehensive Coverage', () => {
  ...
  describe('Setup Function Tests', () => {
    ...
    it('should process content with stream name replacement', () => {
      // name = "couchDB" => after replace/toLowerCase => "couchdb"
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('couchdb');
    ...
    it('should return correct docURL', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.docURL).toBe('https://short.openobserve.ai/database/couchdb');
    });
  ...
  describe('Content Processing Tests', () => {
    ...
    it('should process name to lowercase', () => {
      ...
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('couchdb');
      expect(vm.content).not.toContain('CouchDB');
    });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass href to documentation link', () => {
      wrapper = createWrapper();
      const docLink = wrapper.find('a');
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/database/couchdb');
    });
  });
  ...
  describe('Edge Cases and Error Handling Tests', () => {
    ...
    it('should handle missing couchdb docURL', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.docURL).toBeDefined();
