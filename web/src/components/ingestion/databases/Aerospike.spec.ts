...
describe("aerospikeCard builder", () => {
  it("builds metadata + step flow (no prepare step)", () => {
    ...
    expect(card.detect).toMatchObject({ streamType: "metrics", match: "keyword", streamName: "aerospike" });
  ...
  it("has a aerospike receiver config with the org's exporter", () => {
    const config = aerospikeCard(SUBS).steps.find((s) => s.id === "configure")!.variants!.find((v) => v.id === "linux-amd64")!.code.raw;
