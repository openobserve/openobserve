...
import Ansible from './Ansible.vue';
...
vi.mock('@/composables/useIngestion', () => ({
  default: vi.fn(() => ({
    ...
    devopsContent: `HTTP Endpoint: http://localhost:5080/api/test-org/[STREAM_NAME]/_json\nAccess Key: [BASIC_PASSCODE]`,
    devopsDocURLs: {
...
describe('Ansible.vue Comprehensive Coverage', () => {
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with ansible in content', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('ansible');
      expect(vm.content).not.toContain('[STREAM_NAME]');
    });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass the correct ansible doc URL as href', () => {
      wrapper = createWrapper();
      const docLink = wrapper.find('a');
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/devops/ansible');
    });
  });
