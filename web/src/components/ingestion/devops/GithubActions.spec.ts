...
import GithubActions from './GithubActions.vue';
...
describe('GithubActions.vue Comprehensive Coverage', () => {
  ...
  describe('Content Processing Tests', () => {
    it('should replace [STREAM_NAME] with githubactions in content', () => {
      wrapper = createWrapper();
      const vm = wrapper.vm as any;
      expect(vm.content).toContain('githubactions');
      expect(vm.content).not.toContain('[STREAM_NAME]');
    });
  ...
  describe('Component Props Passing Tests', () => {
    ...
    it('should pass the correct githubactions doc URL as href', () => {
      wrapper = createWrapper();
      const docLink = wrapper.find('a');
      expect(docLink.attributes('href')).toBe('https://short.openobserve.ai/devops/github-actions');
    });
  });
