...
describe("FrontendRumConfig Component", () => {
  ...
  describe("Configuration Content", () => {
    it("should have default configuration template", () => {
      ...
      expect(wrapper.vm.defaultConfig).toContain(
  ...
  describe("replaceStaticValues Function", () => {
    ...
    it("should replace OPENOBSERVE_SITE placeholder", () => {
      ...
      expect(wrapper.vm.displayConfiguration).not.toContain(
        "<OPENOBSERVE_SITE>",
      );
    ...
    it("should replace OPENOBSERVE_ORGANIZATION_IDENTIFIER placeholder", () => {
      ...
      expect(wrapper.vm.initConfiguration).not.toContain(
        "<OPENOBSERVE_ORGANIZATION_IDENTIFIER>",
      );
      expect(wrapper.vm.displayConfiguration).not.toContain(
        "<OPENOBSERVE_ORGANIZATION_IDENTIFIER>",
    ...
    it("should set insecureHTTP to false for HTTPS endpoints", () => {
      ...
      expect(testWrapper.vm.displayConfiguration).toContain(
        "insecureHTTP: false",
      );
      testWrapper.unmount();
    });
  ...
  describe("Lifecycle Hooks", () => {
    ...
    it("should call replaceStaticValues on onUpdated", async () => {
      ...
      wrapper.vm.store.state.organizationData.rumToken = {
        rum_token: "updated-token",
      };
  ...
  describe("Computed Properties", () => {
    it("should have checkRUMToken computed property", () => {
      ...
      expect(wrapper.vm.checkRUMToken).toEqual(
        wrapper.vm.store.state.organizationData.rumToken,
      );
    });
    ...
    it("should update checkRUMToken when store changes", async () => {
      const testStore = createStore({
        state: () => ({
          organizationData: {
            rumToken: { rum_token: "new-token", key: "new-key" },
          },
          selectedOrganization: { identifier: "test-org" },
          API_ENDPOINT: "https://api.openobserve.ai",
        }),
      ...
      await nextTick();
      expect(testWrapper.vm.checkRUMToken).toEqual({
        rum_token: "new-token",
        key: "new-key",
      });
      testWrapper.unmount();
  ...
  describe("Watch Properties", () => {
    it("should watch checkRUMToken changes", async () => {
      ...
      const testStore = createStore({
        state: () => ({
          organizationData: {
            rumToken: { rum_token: "watched-token", key: "watched-key" },
          },
          selectedOrganization: { identifier: "test-org" },
          API_ENDPOINT: "https://api.openobserve.ai",
        }),
  ...
  describe("Template Rendering", () => {
    ...
    it("should render SanitizedHtmlRenderer components", async () => {
      ...
      const htmlRenderers = wrapper.findAllComponents({
    ...
    it("should handle special characters in tokens", () => {
      const testStore = createStore({
        state: () => ({
          ...mockStoreState,
          organizationData: {
            rumToken: { rum_token: "token-with-@#$%^&*()-chars" },
          },
        }),
        getters: {},
        mutations: {},
      ...
      expect(() => testWrapper.vm.replaceStaticValues()).not.toThrow();
      expect(testWrapper.vm.initConfiguration).toContain(
        "token-with-@#$%^&*()-chars",
      );
      testWrapper.unmount();
    });
