...
vi.mock("@/components/CopyContent.vue", () => ({
  default: {
    name: "ContentCopy",
    template:
      '<div class="content-copy-mock" :data-content="content">{{content}}</div>',
    props: ["content"],
  },
}));
...
describe("OtelConfig.vue", () => {
  ...
  beforeEach(() => {
    ...
    mockB64EncodeStandard.mockReturnValue(
      "dGVzdEBleGFtcGxlLmNvbTpwYXNzY29kZQ==",
    );
  });
  ...
  describe("Component Initialization", () => {
    ...
    it("should render ContentCopy components", () => {
      ...
      const contentCopyComponents = wrapper.findAllComponents({
        name: "ContentCopy",
      });
  ...
  otlphttp/openobserve:
    ...
    it("should use full endpoint URL for HTTP", () => {
      ...
      expect(vm.getOtelHttpConfig).toContain(
        "https://api.example.com:8080/api/test-org",
      );
    });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle special characters in organization identifier", () => {
      wrapper = createWrapper({
        currOrgIdentifier: "org-with-special@chars#123",
      });
      const vm = wrapper.vm;

      expect(vm.getOtelHttpConfig).toContain("/api/org-with-special@chars#123");
      expect(vm.getOtelGrpcConfig).toContain(
        "organization: org-with-special@chars#123",
      );
    });
    ...
    it("should handle special characters in user email", () => {
      ...
      store.state.organizationData.organizationPasscode = "";
      wrapper = createWrapper({
        currUserEmail: "user+test@example-domain.com",
      });
  ...
  describe("defineExpose Integration", () => {
    ...
    it("should expose accessKey computed", () => {
      ...
      expect(exposedData.accessKey).toBe(
        "dGVzdEBleGFtcGxlLmNvbTpwYXNzY29kZQ==",
      );
    });
