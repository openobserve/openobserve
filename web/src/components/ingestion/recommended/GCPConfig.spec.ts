...
describe("GCPConfig.vue Component Logic", () => {
  ...
  it("should work with different endpoint URLs", () => {
    ...
    expect(setup.content).toContain(
      "https://gcp.example.com/gcp/test-org/default/_sub",
    );
  ...
  it("should work correctly with HTTPS endpoints", () => {
    ...
    expect(setup.content).toContain(
      "https://secure-gcp.example.com/gcp/test-org/default/_sub",
    );
  });
  ...
  it("should work correctly with custom ports", () => {
    ...
    expect(setup.content).toContain(
      "http://localhost:8080/gcp/test-org/default/_sub",
    );
  });
  ...
  it("should follow GCP URL pattern", () => {
    ...
    expect(setup.content).toMatch(
      /URL:\s*https?:\/\/.+\/gcp\/.+\/default\/_sub\?API-Key=\[BASIC_PASSCODE\]/,
    );
  });
  ...
  it("should handle special characters in organization identifier", () => {
    ...
    expect(setup.content).toContain(
      "/gcp/gcp-org-with-special-chars_123/default/_sub",
    );
  });
