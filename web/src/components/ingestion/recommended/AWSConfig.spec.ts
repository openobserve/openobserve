...
vi.mock("./AWSIndividualServices.vue", () => ({
  default: {
    ...
    props: ["initialSearch"],
    template:
      '<div class="aws-individual-services-mock">AWS Individual Services</div>',
  },
}));
...
describe("AWSConfig", () => {
  ...
  it("should contain HTTP endpoint in content", () => {
    ...
    expect(content).toContain(
      "http://localhost:5080/aws/test_org/default/_kinesis_firehose",
    );
  });
  ...
  it("should perform complete component initialization workflow", async () => {
    ...
    const quickSetupComponent = wrapper.findComponent({
      name: "AWSQuickSetup",
    });
