...
describe("PendingSubscriptionWarning.vue", () => {
    ...
    router = createRouter({
      ...
      routes: [
        { path: "/", component: { template: "<div>Home</div>" } },
        {
          path: "/plans",
          name: "plans",
          component: { template: "<div>Plans</div>" },
        },
      ],
    });
  ...
  describe("Component Rendering", () => {
    ...
    it("displays warning icon", () => {
      ...
      expect(icon.attributes("color")).toBe(
        "warning row col-8 justify-center q-mt-lg",
      );
    });
  ...
  describe("Text Content", () => {
    it("displays correct warning message", () => {
      ...
      expect(text).toContain(
        "organization's is on pending subscription state.",
      );
    });
