...
const stubs = {
  NoData: { template: '<div data-test="no-data">No Data</div>' },
  OSpinner: {
  ...
  OTable: {
    name: 'OTable',
    props: ['data', 'columns', 'rowKey', 'pagination', 'selection', 'selectedIds', 'showGlobalFilter'],
    emits: ['update:selectedIds', 'row-click'],
    template: '<div data-test="running-queries-table"><slot name="empty" /><template v-for="row in data" :key="row.row_id"><slot name="cell-actions" :row="row" /></template><slot name="bottom" /></div>',
  },
};
...
describe("SummaryList.vue", () => {
  ...
  beforeEach(async () => {
    ...
    // Make isMetaOrg true so OTable is rendered
  ...
  // Test 5: Test isMetaOrg renders table
  ...
  // Test 19: Test selectedIds setter emits update:selectedRows
  it("should emit update:selectedRows when selectedIds is set", () => {
  ...
  // Test 21: Test columns have correct headers (i18n labels)
  ...
  // Test 29: Test "cancel" returned string from setup
  it("should expose 'cancel' string in setup return", () => {
    expect(wrapper.vm["cancel"]).toBe("cancel");
  });
  ...
  // Test 33: Test setup return values present
  it("should return all required values from setup function", () => {
    const setupReturnKeys = [
      "t", "columns", "confirmDeleteAction", "deleteDialog", "pageSize",
      "pageSizeOptions", "showListSchemaDialog", "loadingState",
      "isMetaOrg", "selectedRow", "selectedIds",
      "handleMultiQueryCancel", "getAllUserQueries", "durationFormatter"
    ];
