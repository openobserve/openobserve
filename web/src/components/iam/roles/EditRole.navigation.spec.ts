...
describe("EditRole - summary model [characterization]", () => {
  it("groups grants by resource and entity, keeping staged removals visible", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet", "logs:app:AllowList"]);
    vm.updatePermissionMappings("logs:app:AllowList");
    vm.updatePermissionMappings("metrics:cpu:AllowGet");

    const byResource = vm.grantsByResource;
  ...
  it("drops an entity from heldGrants once its only action is staged for removal", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet", "logs:sys:AllowGet"]);
    vm.updatePermissionMappings("logs:app:AllowGet");

    expect(vm.heldGrants("logs")).toEqual([["sys", [{ action: "AllowGet", state: "saved" }]]]);
  });

  it("describes a type level grant as reaching every stream", async () => {
    const vm = await mountEditRole();
    withSaved(vm, [`stream:${ALL}:AllowGet`]);

    expect(vm.moduleDescription("stream", ["stream", "logs", "metrics"])).toBe(
      i18n.global.t("iam.editRole.summaryReachEveryStream"),
    );
  });

  it("describes a type level grant on a plain module as all of it", async () => {
    const vm = await mountEditRole();
    withSaved(vm, [`dfolder:${ALL}:AllowGet`]);

    expect(vm.moduleDescription("dfolder", ["dfolder", "dashboard"])).toBe(
  ...
  it("lists the resource labels when more than one is granted", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet", "metrics:cpu:AllowGet"]);

    expect(vm.moduleDescription("stream", ["stream", "logs", "metrics"])).toBe("Logs, Metrics");
  });

  it("counts the entities when only one resource is granted and no list is loaded", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet", "logs:sys:AllowGet"]);
  ...
  it("reads N of M once the resource's list is loaded", async () => {
    const vm = await mountEditRole();
    await drillInto(vm, "stream", "metrics");
    withSaved(vm, ["metrics:cpu:AllowGet"]);
  ...
  it("builds a summary card from the rail module it mirrors", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet", "logs:app:AllowList"]);

    const [card] = vm.summaryModules;
    ...
    expect(card.actions.map((action: any) => action.action)).toEqual(["AllowList", "AllowGet"]);
  });

  it("labels each summary action with its column name", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet"]);

    expect(vm.summaryModules[0].actions).toEqual([
      { action: "AllowGet", label: i18n.global.t("iam.get") },
    ]);
  });
});

describe("EditRole - pending changes [characterization]", () => {
  it("keys a change by resource, entity and state, and carries the grant keys", async () => {
    const vm = await mountEditRole();
    vm.updatePermissionMappings("metrics:cpu:AllowGet");
    vm.updatePermissionMappings("metrics:cpu:AllowList");

    expect(vm.pendingChanges).toEqual([
      {
  ...
  it("names a wildcard entity in words rather than as _all_", async () => {
    const vm = await mountEditRole();
    vm.updatePermissionMappings(`dfolder:${ALL}:AllowGet`);
    vm.updatePermissionMappings(`stream:${ALL}:AllowGet`);

    expect(vm.pendingChanges.map((change: any) => change.label)).toEqual([
      i18n.global.t("iam.editRole.scopeAllOf", { module: "Dash Folders" }),
      i18n.global.t("iam.editRole.scopeEveryStream"),
    ]);
  });

  it("splits one entity's additions and removals into two changes", async () => {
    const vm = await mountEditRole();
    withSaved(vm, ["logs:app:AllowGet"]);
    vm.updatePermissionMappings("logs:app:AllowGet");
    vm.updatePermissionMappings("logs:app:AllowPut");

    expect(vm.pendingChanges.map((change: any) => [change.state, change.keys])).toEqual([
  ...
  it("keeps the review drawer open while any change remains", async () => {
    ...
    vm.updatePermissionMappings("metrics:cpu:AllowGet");
    vm.updatePermissionMappings("metrics:mem:AllowGet");
    vm.unsavedDrawerOpen = true;
    await flushPromises();

    vm.updatePermissionMappings("metrics:cpu:AllowGet");
    await flushPromises();

    expect(vm.unsavedDrawerOpen).toBe(true);
  });
});
