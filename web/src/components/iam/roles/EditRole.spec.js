...
vi.mock("@/services/iam", () => ({
  ...
  getRoleUsers: vi.fn(async () => ({
    data: ["u1@example.com", "u2@example.com"],
  })),
...
vi.mock("@/services/pipelines", () => ({
  default: {
    getPipelines: vi.fn(async () => ({
      data: { list: [{ pipeline_id: "p1", name: "P1" }] },
    })),
  },
}));
...
vi.mock("@/services/action_scripts", () => ({
  default: {
    list: vi.fn(async () => ({ data: [{ id: "ac1", name: "AC1" }] })),
  },
}));
...
vi.mock("@/services/saved_views", () => ({
  default: {
    get: vi.fn(async () => ({
      data: { views: [{ view_id: "v1", view_name: "V1" }] },
    })),
  },
}));
vi.mock("@/services/dashboards", () => ({
  default: {
    list: vi.fn(async () => ({
      data: { dashboards: [{ dashboardId: "d1", title: "D1" }] },
    })),
...
vi.mock("@/services/service_accounts", () => ({
  default: {
    list: vi.fn(async () => ({ data: { data: ["svc1@example.com"] } })),
  },
}));
vi.mock("@/services/cipher_keys", () => ({
  default: {
    list: vi.fn(async () => ({ data: { keys: [{ name: "key1" }] } })),
  },
}));
    ...
    global: {
      ...
      stubs: {
        ...
        GroupServiceAccounts: {
          template: '<div data-test="group-service-accounts-stub"></div>',
        },
...
describe("EditRole - basic rendering", () => {
  it("renders page and title with role name", async () => {
    ...
    expect(wrapper.get('[data-test="edit-role-title"]').text()).toContain(
      "Admin",
    );
  });
  ...
  it("shows loading spinner during initial fetch", async () => {
    ...
    await flushPromises();
    expect(
      wrapper.find('[data-test="edit-role-page-loading-spinner"]').exists(),
    ).toBe(true);
  });
});
...
describe("EditRole - permission mappings", () => {
  ...
  it("updateEntityPermission sets entity permission value based on selection", async () => {
    ...
    const resource = {
      entities: [{ name: "app", permission: { AllowGet: { value: false } } }],
    };
...
describe("EditRole - filtering & visibility", () => {
  ...
  it("filterRowsByResourceName returns nested matches only", async () => {
    ...
    const rows = [
      {
        resourceName: "stream",
        entities: [{ resourceName: "logs", entities: [] }],
      },
...
describe("EditRole - entities population", () => {
  it("updateResourceEntities pushes entities and respects displayNameKey", async () => {
    ...
    wrapper.vm.updateResourceEntities(
      "role",
      [],
      [{ name: "Admin" }],
      false,
    ...
    expect(
      wrapper.vm.permissionsState.permissions[0].entities.length,
    ).toBeGreaterThan(0);
  });
...
describe("EditRole - micro validations", () => {
  ...
  it("savePermissionHash builds sets from permissions", async () => {
    ...
    wrapper.vm.permissions = [
      { object: "logs:_all_default", permission: "AllowGet" },
    ];
  ...
  it("getOrgId returns store selected org identifier", async () => {
    const wrapper = await mountEditRole();
    expect(wrapper.vm.getOrgId()).toBe(
  ...
  it("updatePermissionVisibility expands heavy resources children list", async () => {
    const wrapper = await mountEditRole();
    wrapper.vm.heavyResourceEntities = {
      logs: [{ show: true }, { show: false }],
    };
    const permissions = [
      { name: "logs", entities: [], permission: { AllowAll: { value: true } } },
    ];
  ...
  it("setDefaultPermissions organizes resources and filters children from top level", async () => {
    ...
    expect(
      wrapper.vm.permissionsState.permissions.some(
        (r) => r.resourceName === "stream",
      ),
    ).toBe(true);
  });
});
