...
describe("AppRoles - rendering", () => {
  ...
  it.skip("renders the search input", async () => {
    ...
    const wrapper = await mountAppRoles();
    expect(wrapper.find('[data-test="o2-table-global-filter-input"]').exists()).toBe(true);
  });
...
describe("AppRoles - showConfirmDialog", () => {
  ...
  it("builds a blast-radius warning with the live user count", async () => {
    ...
    await flushPromises();
    expect(getRoleUsers).toHaveBeenCalledWith("Admin", store.state.selectedOrganization.identifier);
...
describe("AppRoles - _deleteRole", () => {
  it("calls deleteRole with role_name and org identifier", async () => {
    ...
    await flushPromises();
    expect(deleteRole).toHaveBeenCalledWith("Admin", store.state.selectedOrganization.identifier);
  });
...
describe("AppRoles - bulkDeleteUserRoles", () => {
  it("calls bulkDeleteRoles with correct role names", async () => {
    ...
    expect(bulkDeleteRoles).toHaveBeenCalledWith(store.state.selectedOrganization.identifier, {
      ids: ["Admin", "Viewer"],
    });
  });
  ...
  it("resets confirmBulkDelete on error", async () => {
    vi.mocked(bulkDeleteRoles).mockRejectedValueOnce({
