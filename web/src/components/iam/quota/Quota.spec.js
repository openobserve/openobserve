...
vi.mock("@/services/rate_limit", () => ({
  default: {
    update_batch: vi.fn(async () => ({
      status: 200,
      data: { message: "Saved" },
    })),
    download_template: vi.fn(async () => ({
      status: 200,
      data: { some: "json" },
    })),
    upload_template: vi.fn(async () => ({
      status: 200,
    ...
    global: {
      ...
      stubs: {
        ...
        NoOrganizationSelected: {
          template: '<div data-test="no-org-selected" />',
        },
...
describe.skip("Quota page", () => {
  beforeEach(() => {
    ...
    rateSvc.update_batch.mockResolvedValue({
      status: 200,
      data: { message: "Saved" },
    });
    rateSvc.download_template.mockResolvedValue({
      status: 200,
      data: { some: "json" },
    });
    rateSvc.upload_template.mockResolvedValue({
      status: 200,
      data: { message: "Uploaded" },
    });
  });
  ...
  it("shows organization select and tabs", async () => {
    ...
    expect(wrapper.findAll('[data-test="quota-role-limit-tab"]').length).toBe(
      1,
    );
  });
  ...
  it("updateOrganization fetches API limits when not cached and updates route", async () => {
    ...
    expect(rateLimiter.getApiLimitsByOrganization).toHaveBeenCalledWith(
      "org_a",
    );
  });
  ...
  it("switchTab handles global_rules and resets selectedOrganization for role-limits", async () => {
    ...
    wrapper.vm.selectedOrganization = {
      label: "global rules",
      value: "global_rules",
    };
    await wrapper.vm.updateActiveTab("role-limits");
  ...
  it("saveChanges saves api-limits, refreshes and clears state", async () => {
    ...
    await wrapper.vm.saveChanges();
    expect(rateSvc.update_batch).toHaveBeenCalledWith(
      "org_a",
      { module_1: { get: 10 } },
      "module",
    );
  ...
  it("uploadTemplate handles error and sets uploadError", async () => {
    const wrapper = await mountQuota();
    const notifyStub = vi
      .spyOn(wrapper.vm.$q, "notify")
      .mockImplementation(() => ({}));
    wrapper.vm.selectedOrganization = { label: "Org A", value: "org_a" };
    wrapper.vm.uploadedRules = [
      new Blob([JSON.stringify({ module_1: { get: 1 } })], {
  ...
  it("generateUniqueId concatenates fields", async () => {
    const wrapper = await mountQuota();
    const id = wrapper.vm.generateUniqueId({
      api_group_name: "a",
      api_group_operation: "b",
    });
    expect(id).toBe("a_b");
  });
  ...
  it("updateActiveType switches to table when no changes in json", async () => {
    const wrapper = await mountQuota();
    wrapper.vm.jsonStrToDisplay = JSON.stringify(
      wrapper.vm.transformData(wrapper.vm.apiLimitsRows),
      null,
      2,
    );
    wrapper.vm.editTable = true;
  ...
  it("filterOrganizations updates filteredOrganizations by search", async () => {
    ...
    expect(wrapper.vm.filteredOrganizations).toEqual([
      { label: "Alpha", value: "a" },
    ]);
  });
  ...
  it("filterApiCategoriesToDisplayOptions updates filteredApiCategoryToDisplayOptions", async () => {
    const wrapper = await mountQuota();
    wrapper.vm.apiCategories = [
      { label: "module_1", value: "module_1" },
      { label: "module_2", value: "module_2" },
    ];
    wrapper.vm.filterApiCategoriesToDisplayOptions("module_1", () => {});
    expect(wrapper.vm.filteredApiCategoryToDisplayOptions).toEqual([
      { label: "module_1", value: "module_1" },
    ]);
  });
  ...
  it("filterModulesBasedOnCategory filters role-level rows by selected category", async () => {
    ...
    expect(wrapper.vm.filteredRoleLevelModuleRows).toEqual([
      { module_name: "module_1", list: 1 },
    ]);
