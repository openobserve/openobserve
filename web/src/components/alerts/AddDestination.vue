...
const orgUsers = ref<{ label: string; value: string }[]>([]);
...
const createUserRoles = ref<{ label: string; value: string }[]>([]);
...
const fetchOrgUsers = async () => {
  ...
  try {
    ...
    orgUsers.value = list
      ...
      .map((user: any) => ({ label: user.email, value: user.email }));
...
const fetchCreateUserRoles = async () => {
  try {
    ...
    createUserRoles.value = Array.isArray(res.data) ? res.data : [];
  } catch {
