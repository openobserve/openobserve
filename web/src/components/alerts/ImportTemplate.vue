...
},
{
  ...
  try {
    ...
    const computedWidth = computed (() => {
      console.log(splitterModel.value,'val')
  // Min and Max width of splitter model in pixels
  const minWidth = 250;  // Corresponds to 75vw
  const maxWidth = 1000; // Set your maximum value for the splitter

  // Calculate the dynamic vw based on the splitterModel
  let widthInVW = 75 + ((splitterModel.value - minWidth) / (maxWidth - minWidth)) * 15;

  // Ensure the width stays within the 75vw to 90vw range
  if (widthInVW < 75) widthInVW = 75;
  if (widthInVW > 90) widthInVW = 90;
  console.log(widthInVW,'width')
