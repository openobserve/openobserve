...
<template>
  ...
  >
    ...
    <div data-test="destination-preview-card" class="w-full">
        ...
        <div v-if="type === 'slack'" data-test="slack-preview" class="slack-message max-w-150 mx-auto bg-white border border-border-default rounded-lg p-4 shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
          <div class="slack-message-container flex gap-3">
            <div class="slack-avatar">
              <div class="avatar-circle w-9 h-9 bg-[var(--color-brand-slack-aubergine)] text-white rounded-sm flex items-center justify-center font-bold text-sm">OO</div>
            </div>
            <div class="slack-content flex-1">
              <div class="slack-header flex items-center gap-2 mb-2">
                <strong data-test="slack-bot-name" class="bot-name text-[var(--color-brand-slack-link)] text-[0.9rem]">OpenObserve Bot</strong>
                <span class="slack-timestamp text-[var(--color-brand-slack-meta)] text-xs">{{ getCurrentTime() }}</span>
              </div>
              <div data-test="slack-message-body" class="slack-body">
                <div class="slack-block-header text-lg font-bold mb-3 text-[var(--color-brand-slack-text)]">🚨 High CPU Usage</div>
                <div class="slack-fields grid grid-cols-2 gap-2 mb-3">
                  <div class="slack-field">
                    <div class="field-label font-bold text-[var(--color-brand-slack-text)] text-sm">Stream:</div>
                    <div class="field-value text-[var(--color-brand-slack-meta)] text-sm">system-metrics</div>
                  </div>
                  <div class="slack-field">
                    <div class="field-label font-bold text-[var(--color-brand-slack-text)] text-sm">Type:</div>
                    <div class="field-value text-[var(--color-brand-slack-meta)] text-sm">metrics</div>
                  </div>
                  <div class="slack-field">
                    <div class="field-label font-bold text-[var(--color-brand-slack-text)] text-sm">Status:</div>
                    <div class="field-value text-[var(--color-brand-slack-meta)] text-sm">🔴 Firing</div>
                  </div>
                  <div class="slack-field">
                    <div class="field-label font-bold text-[var(--color-brand-slack-text)] text-sm">Count:</div>
        ...
        <div v-if="type === 'msteams'" data-test="msteams-preview" class="teams-card max-w-150 mx-auto bg-white border border-[var(--color-brand-msg-border-2)] rounded-lg overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
          <div data-test="msteams-card-content" class="teams-card-content">
            ...
            <div class="teams-facts p-4 grid gap-2">
              <div class="teams-fact flex justify-between py-1 border-b border-[var(--color-brand-teams-bg)]">
              ...
              <div class="teams-fact flex justify-between py-1 border-b border-[var(--color-brand-teams-bg)]">
                ...
                <div class="fact-value text-[var(--color-brand-teams-text)]">{{ getCurrentTime() }}</div>
              </div>
            </div>
        ...
        <!-- Email Preview -->
        <div v-if="type === 'email'" data-test="email-preview" class="email-client max-w-150 mx-auto bg-white border border-[var(--color-brand-msg-divider)] rounded-lg overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
          <div class="email-header bg-[var(--color-brand-msg-bg)] p-4 border-b border-[var(--color-brand-msg-border)]">
            ...
            <div class="email-to text-[var(--color-brand-msg-meta)] text-sm mb-1">To: admin@example.com</div>
            <div class="email-time text-[var(--color-brand-msg-meta)] text-sm mb-1">{{ getCurrentTime() }}</div>
          </div>
          <div data-test="email-body" class="email-body p-6">
            <div class="email-alert-header">
              <div class="text-[var(--color-brand-msg-error)] text-center mb-4 text-[1.5rem] font-bold">🚨 Alert Notification</div>
            </div>
            <div class="email-alert-info bg-[var(--color-brand-msg-bg)] border-l-4 border-[var(--color-brand-msg-error)] p-4 my-4">
              <div class="text-[var(--color-brand-msg-error)] m-0 mb-2 text-[1.125rem] font-bold">High CPU Usage</div>
              <p class="m-0 text-[var(--color-brand-msg-meta)]">An alert has been triggered in your OpenObserve monitoring system.</p>
            </div>
            <div class="email-details my-4">
              <div class="email-detail-row flex justify-between py-2 border-b border-[var(--color-brand-msg-border)]">
                <span class="detail-label font-bold text-[var(--color-brand-msg-text-strong)]">Stream:</span>
                <span class="detail-value text-[var(--color-brand-msg-meta)]">system-metrics</span>
              </div>
              <div class="email-detail-row flex justify-between py-2 border-b border-[var(--color-brand-msg-border)]">
                <span class="detail-label font-bold text-[var(--color-brand-msg-text-strong)]">Type:</span>
                <span class="detail-value text-[var(--color-brand-msg-meta)]">metrics</span>
              </div>
              <div class="email-detail-row flex justify-between py-2 border-b border-[var(--color-brand-msg-border)]">
                <span class="detail-label font-bold text-[var(--color-brand-msg-text-strong)]">Status:</span>
                <span class="detail-value text-[var(--color-brand-msg-meta)]">🔴 Firing</span>
              </div>
              <div class="email-detail-row flex justify-between py-2 border-b border-[var(--color-brand-msg-border)]">
                <span class="detail-label font-bold text-[var(--color-brand-msg-text-strong)]">Count:</span>
                <span class="detail-value text-[var(--color-brand-msg-meta)]">15</span>
              </div>
              <div class="email-detail-row flex justify-between py-2 border-b border-[var(--color-brand-msg-border)]">
                <span class="detail-label font-bold text-[var(--color-brand-msg-text-strong)]">Threshold:</span>
                <span class="detail-value text-[var(--color-brand-msg-meta)]">greater than 80%</span>
              </div>
              <div class="email-detail-row flex justify-between py-2 border-b border-[var(--color-brand-msg-border)]">
                <span class="detail-label font-bold text-[var(--color-brand-msg-text-strong)]">Time:</span>
                <span class="detail-value text-[var(--color-brand-msg-meta)]">{{ getCurrentTime() }}</span>
              </div>
            </div>
        ...
        <!-- PagerDuty Preview -->
        <div v-if="type === 'pagerduty'" data-test="pagerduty-preview" class="pagerduty-incident max-w-150 mx-auto bg-white border border-[var(--color-brand-msg-divider)] rounded-lg overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
          <div class="pagerduty-header bg-[var(--color-brand-slack-green)] text-white p-4 flex justify-between items-center">
            <div class="pagerduty-title font-bold text-[1.125rem]">PagerDuty Incident</div>
            <div class="pagerduty-status bg-[var(--color-brand-email-accent)] py-1 px-2 rounded-sm text-xs font-bold">Triggered</div>
          </div>
          <div class="pagerduty-content p-6">
            <div class="m-0 mb-4 text-[var(--color-brand-msg-text-dark)] font-bold text-[1.17rem]">OpenObserve Alert: High CPU Usage</div>
            ...
            <div class="pagerduty-link text-center mt-4">
              <a href="#" class="text-[var(--color-brand-slack-green)] no-underline font-bold">View in OpenObserve</a>
            </div>
          </div>
        </div>

        <!-- ServiceNow Preview -->
        <div v-if="type === 'servicenow'" data-test="servicenow-preview" class="servicenow-incident max-w-150 mx-auto bg-white border border-[var(--color-brand-msg-divider)] rounded-lg overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
          ...
          <div class="servicenow-content p-6">
            ...
            <div class="servicenow-description bg-[var(--color-brand-msg-bg)] p-4 rounded-sm text-[var(--color-brand-msg-text)] [white-space:pre-line] mt-4">
        ...
        <!-- Opsgenie Preview -->
        <div v-if="type === 'opsgenie'" data-test="opsgenie-preview" class="opsgenie-alert max-w-150 mx-auto bg-white border border-[var(--color-brand-msg-divider)] rounded-lg overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
          <div class="opsgenie-header bg-[var(--color-brand-email-ink)] text-white p-4 flex justify-between items-center">
            <div class="opsgenie-title font-bold text-[1.125rem]">Opsgenie Alert</div>
            <div class="opsgenie-priority bg-[var(--color-brand-email-warning)] text-[var(--color-brand-email-ink)] py-1 px-2 rounded-sm font-bold">P3</div>
          </div>
