...
<template>
  <div data-test="destination-test-result" class="mt-3">
    ...
    <div
      ...
      class="flex gap-3 py-3 px-4 rounded-sm border-l-[3px] border-solid border-(--q-positive) bg-[rgba(76,175,80,0.08)] dark:bg-[rgba(76,175,80,0.12)]"
    >
      <div class="shrink-0 pt-0.5 text-(--q-positive)">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div data-test="test-success-timestamp" class="text-2xs text-(--q-text-secondary) flex items-center gap-2 flex-wrap">
    ...
    <div
      ...
      class="flex gap-3 py-3 px-4 rounded-sm border-l-[3px] border-solid border-(--q-negative) bg-[rgba(244,67,54,0.08)] dark:bg-[rgba(244,67,54,0.12)]"
    >
      <div class="shrink-0 pt-0.5 text-(--q-negative)">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div v-if="result.timestamp" data-test="test-failure-timestamp" class="text-2xs text-(--q-text-secondary) flex items-center gap-2 flex-wrap">
          {{ formatTimestamp(result.timestamp) }}
          <OTag v-if="result.statusCode" type="httpStatus" :value="httpStatusBucket(result.statusCode)">
            {{ result.statusCode }}
          </OTag>
        ...
        <!-- Suggested Fix -->
        <div v-if="getSuggestedFix(result)" class="result-suggestion flex items-start gap-2 mt-2 p-2 bg-[rgba(255,193,7,0.1)] dark:bg-[rgba(255,193,7,0.15)] rounded-sm text-2xs text-(--q-text) leading-[1.4]">
        ...
        >
          <template #trigger>
            <div class="flex items-center text-(--q-text-secondary) text-2xs">
    ...
    <div
      ...
      class="flex gap-3 py-3 px-4 rounded-sm border-l-[3px] border-solid border-(--q-primary) bg-[rgba(33,150,243,0.08)] dark:bg-[rgba(33,150,243,0.12)]"
    >
      <div class="shrink-0 pt-0.5 text-(--q-primary)">
        <OSpinner size="xs" />
      </div>
      <div class="flex-1 min-w-0">
        ...
        <div class="text-2xs text-(--q-text-secondary) flex items-center gap-2 flex-wrap">
    ...
    >
      ...
      <span class="text-2xs text-(--q-text-secondary) leading-[1.4]">
