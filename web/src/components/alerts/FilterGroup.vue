<template>
  ...
  <div
    v-if="depth === 0 && previewString"
    class="tw:mb-2 tw:p-2 tw:rounded tw:border tw:w-full"
    :class="
      store.state.theme === 'dark'
        ? 'tw:bg-gray-800 tw:border-gray-700'
        : 'tw:bg-gray-50 tw:border-gray-300'
    "
  >
    <div
      class="tw:flex tw:items-center tw:gap-1 tw:cursor-pointer tw:min-w-0"
    ...
    >
      ...
      />
      <span
        class="tw:font-medium tw:text-xs tw:flex-shrink-0"
        :class="
          store.state.theme === 'dark' ? 'tw:text-gray-300' : 'tw:text-gray-700'
        "
      >
        Preview:
      </span>
  ...
  <div
    :class="[
      `  tw:px-2 tw:mb-2 el-border tw:mt-6 el-border-radius `,
      store.state.isAiChatEnabled
        ? `tw:w-full tw:ml-[${depth * 10}px]`
        : `xl:tw:w-fit tw:ml-[${depth * 20}px]`,
    ]"
    :style="{
  ...
  >
    ...
    <div
      ...
      :class="
        store.state.theme === 'dark' ? 'dark-mode-group' : 'light-mode-group'
      "
    >
      ...
      <div class="flex justify-start items-center tw:ml-4">
        ...
        >
          ...
          <span class="text-bold">{{
            t("alerts.conditions.conditionGroup")
          }}</span>
...
const toggleLabel = (newLabel?: string) => {
  ...
  if (newLabel) {
    ...
    groups.value.logicalOperator =
      groups.value.logicalOperator === "AND" ? "OR" : "AND";
  }
...
const removeCondition = (id: string) => {
  ...
  const itemsAfterRemoval = groups.value.conditions.filter(
    (item: any) => item.id !== id,
  );
  const hasConditionsAfterRemoval = itemsAfterRemoval.some(
    (item: any) => !isGroup(item),
  );

  // Count sub-groups that will be deleted
  const subGroupCount = itemsAfterRemoval.filter((item: any) =>
    isGroup(item),
  ).length;
  ...
  if (!hasConditionsAfterRemoval && subGroupCount > 0) {
    confirmDialog.value = {
      ...
      message:
        "Deleting this condition will remove the entire condition group.",
...
const performRemoveCondition = (id: string) => {
  // V2: Use conditions array instead of items
  groups.value.conditions = groups.value.conditions.filter(
    (item: any) => item.id !== id,
  );

  // Check if there are any conditions left (not sub-groups)
  const hasConditions = groups.value.conditions.some(
    (item: any) => !isGroup(item),
  );
...
const reorderItems = () => {
  ...
  const conditions = groups.value.conditions.filter(
    (item: any) => !isGroup(item),
  );
  const subGroups = groups.value.conditions.filter((item: any) =>
    isGroup(item),
  );
