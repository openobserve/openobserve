  ...
  describe("3. Update Functions", () => {
    beforeEach(() => {
      ...
      if (wrapper.vm.$refs.baseImportRef) {
        ...
        wrapper.vm.$refs.baseImportRef.updateJsonArray(
          [{ name: "test" }],
          false,
        );
      }
    ...
    it("should update destination emails correctly", () => {
      ...
      expect(wrapper.vm.jsonArrayOfObj[0].emails).toEqual([
        "test@example.com",
        "user@test.com",
      ]);
    });
  ...
  describe("8. Import and Processing Functions", () => {
    ...
    describe("importJson", () => {
      ...
      it("should process valid JSON array", async () => {
        ...
        vi.mocked(destinationService.default.create).mockResolvedValueOnce(
          true,
        );
    ...
    describe("createDestination", () => {
      ...
      it("should handle destination creation failure", async () => {
        ...
        const destinationService = await import("@/services/alert_destination");
        vi.mocked(destinationService.default.create).mockClear();
        vi.mocked(destinationService.default.create).mockRejectedValueOnce(
          error,
        );
        ...
        expect(wrapper.vm.destinationCreators[0].message).toContain(
          "Creation failed",
        );
      });
