...
<!--
  ...
  to it, showing that row's dependency chain (DependencyChainPanel). On-demand and
  ...
  <OPopover v-model:open="open" side="bottom" align="end" :side-offset="4" content-class="p-0">
    <template #trigger>
      <OButton
        ...
        @click.stop
      >
        <OIcon name="graph-3" size="sm" />
        <OTooltip side="top" :content="t('alert_dependencies.viewDependencies')" />
      </OButton>
    </template>

    <div
      class="max-h-[70vh] w-100 max-w-[92vw] overflow-y-auto"
      data-test="view-dependencies-popover"
    >
