...
<template>
  <div class="incident-timeline tw:flex tw:flex-col tw:h-full">
    ...
    <div
      v-if="loading"
      class="tw:flex tw:justify-center tw:items-center tw:py-12"
    >
...
class="tw:mb-3 tw:opacity-40" />
      ...
      <div class="tw:text-sm tw:text-gray-400">
        Events and comments will appear here
      </div>
    </div>
    ...
    <div v-else class="tw:flex-1 tw:flex tw:flex-col tw:min-h-0 tw:relative">
      <!-- Scroll buttons -->
      <div
        class="tw:absolute tw:top-2 tw:right-3 tw:flex tw:flex-col tw:gap-1 tw:z-10"
      >
        <q-btn
          round
          unelevated
      ...
      <div
        ...
        class="tw:flex-1 tw:min-h-0 tw:overflow-y-auto tw:px-3 tw:pt-2 tw:pb-4"
      >
        <div class="tw:relative">
          ...
          <div
            ...
            :style="{
              backgroundColor:
                store.state.theme === 'dark' ? '#2d333b' : '#e5e7eb',
              marginTop: '12px',
              marginBottom: '12px',
            }"
          ></div>
                ...
                <div class="tw:flex tw:items-center tw:gap-3">
                  ...
                  <div class="tw:flex-shrink-0">
                    ...
                    <div
                      ...
                      :style="{
                        backgroundColor:
                          store.state.theme === 'dark' ? '#181a1b' : '#ffffff',
                        border:
                    ...
                    <div
                      ...
                      :style="{
                        backgroundColor:
                          store.state.theme === 'dark' ? '#2d333b' : '#f6f8fa',
                        border:
                          store.state.theme === 'dark'
                            ? '1px solid #444c56'
                            : '1px solid #d0d7de',
                  ...
                  <div class="tw:flex-1">
                    <div class="tw:flex tw:items-center tw:gap-2 tw:flex-wrap">
                      ...
                      <template v-if="getUserId(event) !== 'System'">
                        ...
                        <span
                          v-if="
                            event.type !== 'SeverityUpgrade' &&
                            event.type !== 'SeverityOverride'
                          "
                          class="tw:inline-flex tw:items-center tw:px-2 tw:py-0.5 tw:rounded tw:text-xs tw:font-semibold"
                          :style="{
                            backgroundColor:
                              store.state.theme === 'dark'
                                ? getEventBadgeColor(event) + '30'
                                : getEventBadgeColor(event) + '15',
                  ...
                  <div class="tw:flex-1 tw:min-w-0">
                    ...
                    >
                      ...
                      <div class="tw:px-4 tw:py-3">
                        <div
                          class="tw:text-sm tw:whitespace-pre-wrap tw:break-words tw:leading-relaxed"
                          :class="
                            store.state.theme === 'dark'
                              ? 'tw:text-gray-200'
                              : 'tw:text-gray-800'
...
const fetchEvents = async () => {
  ...
  loading.value = true;
  try {
    const response = await incidentsService.getEvents(
      props.orgId,
      props.incidentId,
    );
...
const getInlineEventText = (event: any): string => {
  ...
  const bold = (text: string) =>
    `<span style="font-weight: 600; color: ${eventColor};">${text}</span>`;
  const severityBadge = (severity: string) =>
    `<span style="display: inline-flex; align-items: center; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; background-color: ${getSeverityColor(severity)}${opacity}; color: ${isDark ? "#ffffff" : getSeverityColor(severity)}; border: 1px solid ${getSeverityColor(severity)}${isDark ? "60" : "40"};">${severity}</span>`;
