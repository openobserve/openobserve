    ...
    it("should emit remove-group when removing last condition", () => {
      const singleItemProps = {
        ...
        group: {
          ...
          conditions: [{ id: "only-item", column: "field1", operator: "=", value: "test" }],
        },
      };
        ...
        group: {
          ...
          conditions: [
            { id: "only-condition", column: "field1", operator: "=", value: "test" },
            {
              groupId: "sub-group-1",
              label: "or",
              items: [{ id: "nested-condition", column: "field2", operator: "=", value: "test2" }],
            },
          ],
        },
      };
          ...
          conditions: [
            {
              ...
              items: [
                { id: "condition-1", column: "field1", operator: "=", value: "test1" },
                {
                  groupId: "nested-2",
                  label: "and",
                  items: [{ id: "condition-2", column: "field2", operator: "!=", value: "test2" }],
                },
              ],
            },
...
describe("FilterGroup.vue Form Mode (namePrefix + OForm)", () => {
  ...
  const makeLeaf = (id: string, column: string, operator: string, value: unknown) => ({
    filterType: "condition",
    column,
    operator,
    value,
  ...
  const makeGroup = (groupId: string, conditions: unknown[], logicalOperator = "AND") => ({
    filterType: "group",
    logicalOperator,
    groupId,
    conditions,
  ...
  const renderedValueInputs = (wrapper: ReturnType<typeof mount>) =>
    wrapper.findAllComponents(OFormInput).filter((c) => /\.value$/.test(String(c.props("name"))));

  const renderedColumnSelects = (wrapper: ReturnType<typeof mount>) =>
    wrapper.findAllComponents(OFormSelect).filter((c) => /\.column$/.test(String(c.props("name"))));
  ...
  it("passes the prefix recursively — child at index i binds `${namePrefix}.conditions[${i}]` (incl. nested groups)", () => {
    ...
    expect(renderedColumnSelects(wrapper).map((c) => c.props("name"))).toEqual([
    ...
    expect(
      renderedColumnSelects(wrapper).map((c) => c.findComponent(OSelect).props("modelValue")),
    ).toEqual(["field1", "field2"]);
    expect(
      renderedValueInputs(wrapper).map((c) => c.findComponent(OInput).props("modelValue")),
    ).toEqual(["va", "vn"]);
  });
  ...
  describe("allowCustomColumns passthrough", () => {
    ...
    it("reaches every FilterCondition, including ones in nested groups", () => {
      ...
      conditions.forEach((c) => expect(c.props("allowCustomColumns")).toBe(true));
    });
  ...
  describe("indent-rem", () => {
    ...
    const marginOf = (w: any) =>
      ...
      (w.find(".filter-group-box").attributes("style") || "").match(/margin-left:\s*([^;]+)/)?.[1];
