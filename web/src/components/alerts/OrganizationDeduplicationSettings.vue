...
<template>
  <div
    class="tw:w-full tw:h-[calc(100vh-172px)] tw:px-2 org-dedup-settings tw:flex tw:flex-col"
  >
    <!-- Scrollable content area -->
    <div class="tw:flex-1 tw:overflow-y-auto tw:pr-2">
      <div class="tw:mb-6">
        <GroupHeader
          :title="t('alerts.correlation.title')"
          :showIcon="false"
          class="tw:mb-2"
        />
      ...
      <div class="tw:mb-6" v-if="localConfig.alert_dedup_enabled">
        ...
        <div class="tw:flex tw:flex-col tw:gap-2">
          <q-checkbox
            ...
            :data-test="
              'organizationdeduplication-fingerprint-' + group.id + '-checkbox'
            "
            :key="group.id"
            :model-value="
              localConfig.alert_fingerprint_groups?.includes(group.id)
            "
          ...
          <div
            v-if="
              !localConfig.alert_fingerprint_groups ||
              localConfig.alert_fingerprint_groups.length === 0
            "
    ...
    <div
      class="tw:flex tw:justify-end tw:gap-3 tw:pt-4 tw:pb-2 tw:border-t tw:border-gray-200 dark:tw:border-gray-700 tw:bg-inherit tw:sticky tw:bottom-0"
    >
      <q-btn
        :label="t('alerts.correlation.cancelButton')"
        @click="$emit('cancel')"
  ...
  if (localConfig.value.alert_dedup_enabled) {
    if (
      ...
      $q.notify({
        type: "negative",
        message:
          "Please select at least one semantic group for cross-alert deduplication",
        timeout: 3000,
      });
      return;
    }
...
const loadConfig = async () => {
  if (!props.config) {
    try {
      ...
      const response = await alertsService.getOrganizationDeduplicationConfig(
        props.orgId,
      );
