...
<!--
  ...
  optional "N alerts" count (for destinations), and Open / Delete. Delete uses an
  INLINE confirm (not a modal) so it works inside the dependency popover without
  dismissing it. Every kind (template / destination / alert) uses this row, so the
  same actions are available for all of them.
-->
<template>
  <div
    class="group hover:bg-interactive-hover-bg rounded-default flex items-center gap-2 px-2 py-1"
    :data-test="`dependency-usage-row-${node.name}`"
  >
    ...
    <div class="flex shrink-0 items-center gap-0.5 opacity-0 group-hover:opacity-100">
      <OButton
        ...
        variant="ghost"
        size="icon-sm"
        :data-test="`dependency-usage-open-${node.name}`"
      ...
      <OButton
        ...
        variant="ghost"
        size="icon-sm"
        :data-test="`dependency-usage-delete-${node.name}`"
