<template>
  <div class="tw:flex tw:items-start tw:gap-1 tw:flex-no-wrap">
    ...
    <div
      class="tw:flex tw:items-center tw:justify-center tw:mt-2 tw:min-w-[60px]"
    >
      ...
      <template v-else>
        ...
        >
          <q-tooltip> Toggle between and/or </q-tooltip>
        </q-btn>
      </template>
    </div>
    <div data-test="alert-conditions-select-column" class="q-ml-none tw:mb-2">
      <q-select
    ...
    <div data-test="alert-conditions-operator-select" class="q-ml-none">
      <q-select
    ...
    <div data-test="alert-conditions-value-input" class="q-ml-none">
      <q-input
...
const toggleOperator = () => {
  if (props.condition.logicalOperator) {
    props.condition.logicalOperator =
      props.condition.logicalOperator === "AND" ? "OR" : "AND";
...
const computedInputWidth = computed(() => {
  ...
  return (
    ...
    (store.state.isAiChatEnabled
      ? ""
      : "xl:tw:min-w-[200px] lg:tw:min-w-[90px] lg:tw:w-fit")
  );
});
...
const computedValueWidth = computed(() => {
  ...
  return (
    ...
    (store.state.isAiChatEnabled
      ? "tw:w-[110px]"
      : "xl:tw:min-w-[200px] lg:tw:w-fit lg:tw:min-w-[80px]")
  );
});
...
const validateColumnField = (val: any) => {
  ...
  const fieldExists = props.streamFields.some(
    (field: any) => field.value === val,
  );
