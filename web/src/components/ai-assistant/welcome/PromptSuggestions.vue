...
<template>
  <section class="flex flex-col gap-3 w-full">
    ...
    <div class="suggestions-grid grid w-full gap-2 md:grid-cols-2 lg:grid-cols-3 sm:grid-cols-1">
      ...
      >
        <span class="suggestion-chip__icon inline-flex items-center justify-center shrink-0 w-[22px] h-[22px] rounded-md bg-[color-mix(in_srgb,var(--color-border-default)_40%,transparent)] text-typography-meta [transition:background_0.15s_ease,color_0.15s_ease] group-hover:bg-[rgba(123,97,255,0.15)] group-hover:text-[#7b61ff]">
          <OIcon :name="s.icon" size="sm" />
        </span>
        <span class="flex-1 whitespace-nowrap overflow-hidden text-ellipsis">
          {{ t(`aiAssistant.suggestions.${s.id}`) }}
        </span>
        <span class="suggestion-chip__arrow inline-flex items-center justify-center shrink-0 text-typography-meta opacity-0 -translate-x-1 [transition:opacity_0.15s_ease,transform_0.15s_ease,color_0.15s_ease] group-hover:opacity-100 group-hover:translate-x-0 group-hover:text-[#7b61ff]" aria-hidden="true">
