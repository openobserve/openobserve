...
describe("AddCipherKey.vue", () => {
    ...
    return mount(AddCipherKey, {
      global: {
        ...
        stubs: {
          ...
          "q-form": {
            template:
              '<form class="q-form-stub" @submit.prevent="$emit(\'submit\')"><slot></slot></form>',
            methods: {
          ...
          "q-input": {
            template:
              '<input class="q-input-stub" :data-test="$attrs[\'data-test\']" v-model="modelValue" />',
            props: ["modelValue", "label", "readonly", "disable", "rules"],
          },
          "q-select": {
            template:
              '<select class="q-select-stub" :data-test="$attrs[\'data-test\']" v-model="modelValue"><slot></slot></select>',
          ...
          "q-step": {
            template:
              '<div class="q-step-stub" :data-test="$attrs[\'data-test\']"><slot></slot></div>',
            props: ["name", "title", "icon", "done"],
          },
          "q-stepper-navigation": {
            template:
              '<div class="q-stepper-navigation-stub"><slot></slot></div>',
          },
          "q-btn": {
            template:
              '<button class="q-btn-stub" :data-test="$attrs[\'data-test\']" @click="$emit(\'click\')" :disabled="disable">{{ label }}</button>',
          ...
          ConfirmDialog: {
            template:
              '<div class="confirm-dialog-stub" v-if="modelValue">{{ title }}: {{ message }}</div>',
  ...
  describe("Stepper Functionality", () => {
    ...
    it("shows correct step titles", () => {
      wrapper = createWrapper();
      const step1 = wrapper.find(
        '[data-test="cipher-key-key-store-detils-step"]',
      );
      const step2 = wrapper.find(
