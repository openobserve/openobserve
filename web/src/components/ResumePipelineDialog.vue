...
<template>
  <q-dialog
    :class="
      store.state.theme === 'dark' ? 'dark-theme-dialog' : 'light-theme-dialog'
    "
  >
    <q-card class="resume-pipeline-dialog" data-test="dialog-box">
      <q-card-section
        class="resume-pipeline-dialog-header q-pa-none"
        style="padding: 0px"
      >
        <div class="dialog-title">Resume Pipeline Ingestion</div>
        <div v-if="lastPausedAt" class="last-paused-at-text">
          Last paused: {{ convertUnixToQuasarFormat(lastPausedAt) }}
        </div>
      </q-card-section>
      <q-card-section
        class="resume-pipeline-dialog-body q-pa-none"
        style="padding: 0px"
      >
        <q-radio
        ...
        >
          <div class="resume-radio-label">
            <div
              class="resume-radio-main-text resume-pipeline-dialog-body-text"
            >
              Continue from where it paused
            </div>
            <div
              v-if="lastPausedAt"
              class="resume-radio-sub-text resume-pipeline-dialog-body-text-time"
            >
              {{ convertUnixToQuasarFormat(lastPausedAt) }}.
            </div>
          </div>
        </q-radio>
        <q-radio v-model="resumeFromStart" :val="true"
style="height: 18px">
      ...
      <q-card-actions
        class="resume-pipeline-dialog-actions q-pa-none tw:flex tw:justify-center"
        style="padding: 0px"
      >
        <q-btn
          v-close-popup
