...
const fieldOptions = computed(() =>
  ASSERTION_FIELDS.map((f) => ({
    label: t(`synthetics.protocolConfig.http.fields.${f}`),
    value: f,
  })),
);
...
function addAssertion() {
  update({
    assertions: [...cfg.value.assertions, { field: "status_code", operator: "eq", value: "" }],
  });
}
    ...
    <div class="px-3 py-2 flex flex-col gap-4">
      ...
      <div>
        ...
        <div v-for="(h, i) in cfg.headers" :key="i" class="flex items-center gap-2 mb-2">
          <div class="flex-1 min-w-0">
            <OInput
              ...
              @update:model-value="
                (v: string | number) =>
                  patchHeader(i, { name: typeof v === 'string' ? v : String(v) })
              "
            />
          </div>
          <div class="flex-1 min-w-0">
            <OInput
              ...
              @update:model-value="
                (v: string | number) =>
                  patchHeader(i, { value: typeof v === 'string' ? v : String(v) })
              "
            />
          </div>
          <OButton
        ...
        <OButton
          variant="outline"
          size="sm"
          data-test="synthetics-check-http-add-header-btn"
          @click="addHeader"
        >
      ...
      <div>
        ...
        <div v-for="(a, i) in cfg.assertions" :key="i" class="flex items-center gap-2 mb-2">
          ...
          <div class="flex-1 min-w-0">
            <OInput
              ...
              @update:model-value="
                (v: string | number) =>
                  patchAssertion(i, { value: typeof v === 'string' ? v : String(v) })
              "
            />
          </div>
          <OButton
        ...
        <OButton
          variant="outline"
          size="sm"
          data-test="synthetics-check-http-add-assertion-btn"
          @click="addAssertion"
        >
          <template #icon-left><OIcon name="add" size="xs" /></template>
