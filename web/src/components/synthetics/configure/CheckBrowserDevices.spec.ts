...
describe("CheckBrowserDevices", () => {
  ...
  describe("initial render", () => {
    ...
    it("should render checkboxes for each browser-device combination", () => {
      ...
      const cell = wrapper.find(
        '[data-test="synthetics-check-browser-devices-cell-chromium-laptop_large"]',
      );
      ...
      expect(
        wrapper
          .find('[data-test="synthetics-check-browser-devices-cell-firefox-laptop_large"]')
          .exists(),
      ).toBe(true);
      expect(
        wrapper
          .find('[data-test="synthetics-check-browser-devices-cell-webkit-laptop_large"]')
          .exists(),
      ).toBe(true);
    });
  });
  ...
  describe("with default browsers and devices (no props)", () => {
    ...
    it("should render default checkboxes for chromium+desktop when check.browserDevices uses laptop_large", () => {
      ...
      wrapper = mountDevices({ browsers: undefined, devices: undefined });
      const cell = wrapper.find(
        '[data-test="synthetics-check-browser-devices-cell-chromium-desktop"]',
      );
      expect(cell.exists()).toBe(true);
      ...
      expect(
        wrapper
          .find('[data-test="synthetics-check-browser-devices-cell-chromium-laptop_large"]')
          .exists(),
      ).toBe(false);
    });
  });
  ...
  describe("checkbox state reflects selected browser devices", () => {
    ...
    it("should check checkboxes for devices in browserDevices", () => {
      ...
      expect(
        checkboxChecked(
          wrapper,
          '[data-test="synthetics-check-browser-devices-cell-firefox-laptop_large"]',
        ),
      ).toBe(true);
    });
    ...
    it("should leave unchecked devices that are not in browserDevices", () => {
      expect(
        checkboxChecked(
          wrapper,
          '[data-test="synthetics-check-browser-devices-cell-webkit-laptop_large"]',
        ),
      ).toBe(false);
    });
  });
  ...
  describe("toggle emit behavior", () => {
    ...
    it("should emit update:check when a new device is checked", async () => {
      ...
      const cell = wrapper.find<HTMLInputElement>(
        '[data-test="synthetics-check-browser-devices-cell-webkit-laptop_large"]',
      );
    ...
    it("should emit update:check when a checked device is unchecked", async () => {
      ...
      expect(emitted).toBeTruthy();
      const updatedBrowserDevices = emitted![0][0].browserDevices;
      expect(updatedBrowserDevices).not.toContainEqual({
        browser: "chromium",
        device: "laptop_large",
      });
