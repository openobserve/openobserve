...
const BrowserJourneyStepStub = {
  props: ['step', 'index', 'expanded', 'selected', 'replayDotState', 'replayLocked', 'replayResult'],
  emits: ['update:step', 'update:expanded', 'delete', 'duplicate', 'insert-below', 'toggle-select', 'retry-replay'],
...
describe('RecordJourney', () => {
  ...
  beforeEach(() => {
    ...
    let uuidCounter = 0
    vi.spyOn(globalThis.crypto, 'randomUUID').mockImplementation(() => `mock-uuid-${uuidCounter++}`)
  })
