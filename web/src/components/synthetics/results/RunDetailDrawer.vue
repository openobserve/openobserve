    ...
    <div class="flex flex-col h-full">
      ...
      <div v-if="loading" class="flex flex-col gap-3 p-5">
        <div class="flex gap-6 mb-2">
          <div class="bg-[var(--color-border-default)] animate-pulse h-10 w-28 rounded" />
          <div class="bg-[var(--color-border-default)] animate-pulse h-10 w-20 rounded" />
          <div class="bg-[var(--color-border-default)] animate-pulse h-10 w-40 rounded" />
        </div>
        <div v-for="i in 2" :key="i" class="rounded-lg border border-border-default overflow-hidden">
          ...
          <div class="p-4 flex flex-col gap-2">
            <div class="bg-[var(--color-border-default)] animate-pulse h-4 w-2/3 rounded" />
            <div class="bg-[var(--color-border-default)] animate-pulse h-4 w-1/2 rounded" />
          </div>
        </div>
      </div>
          ...
          >
            ...
            >
              ...
              <div class="ml-auto flex items-center gap-3 shrink-0">
                ...
                <span class="text-sm font-medium tabular-nums text-text-primary">{{ fmtDuration(loc.durationMs) }}</span>
              </div>
            </div>
                ...
                <div v-if="expandedSteps.has(loc.executionId)" class="flex flex-col gap-1">
                  ...
                  >
                    <span
                      ...
                      :class="step.status === 'ok' ? 'bg-[var(--color-success-600)]' : 'bg-[var(--color-error-500)]'"
                    >{{ idx + 1 }}</span>
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        <span class="font-mono text-text-secondary truncate">{{ step.stepId }}</span>
                        <span class="ml-auto tabular-nums text-text-muted shrink-0">{{ fmtDuration(step.durationMs) }}</span>
                      </div>
                      <p v-if="step.error" class="text-status-error-text truncate mt-0.5" :title="step.error">{{ step.error }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-if="loc.steps.some(s => s.screenshotKey)" class="px-4 py-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <div
                    ...
                    :key="step.stepId"
                    class="rounded border border-border-default overflow-hidden bg-surface-panel"
                  >
                    <div class="flex items-center gap-1.5 px-2 py-1 border-b border-border-default">
                      <span
                        class="w-2 h-2 rounded-full shrink-0"
                        :class="step.status === 'ok' ? 'bg-[var(--color-success-500)]' : 'bg-[var(--color-error-500)]'"
                      />
                      <span class="text-[0.65rem] font-mono text-text-muted truncate">{{ step.stepId }}</span>
                    </div>
