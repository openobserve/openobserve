    ...
    it('should not show error banner when steps are present', () => {
      wrapper = mountDrawer({
        execution: makeExecution({
          ...
          steps: [
            makeStepResult({ stepId: 'step-1', status: 'ok', durationMs: 300 }),
            makeStepResult({ stepId: 'step-2', status: 'fail', durationMs: 5000, error: 'Element not found' }),
          ],
        }),
      })
    ...
    it('should show step-level error for failed steps', () => {
      wrapper = mountDrawer({
        execution: makeExecution({
          ...
          steps: [
            makeStepResult({ stepId: 'step-1', status: 'ok', durationMs: 300 }),
            makeStepResult({ stepId: 'step-2', status: 'fail', durationMs: 5000, error: 'Selector not found: #btn' }),
          ],
        }),
      })
