...
describe("RetryJobDialog", () => {
  ...
  it("should apply dark theme when theme is dark", async () => {
    ...
    // Dark mode is driven by the theme-aware `bg-dialog-bg` token on the
    // dialog rather than a runtime `dark-theme` class or a `dark:` variant.
    const dialog = document.body.querySelector(".retry-dialog");
    expect(dialog).not.toBeNull();
    expect(dialog?.classList.contains("bg-dialog-bg")).toBe(true);
    wrapper.unmount();
  });
