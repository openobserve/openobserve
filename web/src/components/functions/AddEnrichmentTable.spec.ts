...
describe("AddEnrichmentTable.vue", () => {
    ...
    return mount(AddEnrichmentTable, {
      ...
      global: {
        ...
        stubs: {
          "q-form": {
            template:
              '<form @submit.prevent="$attrs.onSubmit && $attrs.onSubmit()"><slot /></form>',
          },
          "q-input": {
            template:
              '<input v-model="modelValue" :readonly="readonly" :disabled="disable" />',
          ...
          "q-btn": {
            template:
              '<button @click="$emit(\'click\')" :type="type">{{ label }}<slot /></button>',
            props: ["label", "type"],
            emits: ["click"],
          },
          "q-separator": {
  ...
  describe("Error Handling", () => {
    ...
    it("should handle 403 errors without showing notification", async () => {
      ...
      const createSpy = vi
        .spyOn(jsTransformService.default, "create_enrichment_table")
        .mockRejectedValueOnce(error);
