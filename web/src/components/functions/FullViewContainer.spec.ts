...
describe("FullViewContainer.vue", () => {
    ...
    return mount(FullViewContainer, {
      ...
      global: {
        ...
        stubs: {
          "q-icon": {
            template:
              '<div class="q-icon-stub" :class="$attrs.class" @click.stop="$emit(\'click\', $event)">{{ name }}</div>',
            props: ["name", "size"],
          },
        },
      },
  ...
  describe("Event Handling", () => {
    ...
    it("handles multiple rapid clicks correctly", async () => {
      ...
      expect(wrapper.emitted("update:isExpanded")).toBeTruthy();
      expect(
        wrapper.emitted("update:isExpanded")?.length,
  ...
  describe("CSS Classes and Structure", () => {
    ...
    it("maintains correct component structure", () => {
      ...
      expect(wrapper.find(".tw\\:flex.tw\\:justify-between").exists()).toBe(
        true,
      );
