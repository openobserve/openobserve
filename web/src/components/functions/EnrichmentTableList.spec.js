...
vi.mock("@/services/jstransform", () => ({
  default: {
    get_all_enrichment_table_statuses: vi.fn(() =>
...
describe("EnrichmentTableList Component", () => {
  ...
  describe("Search and Filtering", () => {
    ...
    it("handles empty search terms", () => {
      // For empty string
      expect(wrapper.vm.filterData(wrapper.vm.jsTransforms, "")).toEqual(
        wrapper.vm.jsTransforms,
      );
      ...
      const terms = "";
      expect(
        wrapper.vm.filterData(wrapper.vm.jsTransforms, terms || ""),
      ).toEqual(wrapper.vm.jsTransforms);
    });
  ...
  describe("Form Management", () => {
    ...
    it("refreshes list after form submission", async () => {
      ...
      expect(mockGetStreams).toHaveBeenCalledWith(
        "enrichment_tables",
        false,
        false,
        true,
  ...
  describe("Table Data Formatting", () => {
    ...
    it("handles numeric field sorting", () => {
      const column = wrapper.vm.columns.find((c) => c.name === "doc_num");
      expect(column.sort(null, null, { doc_num: 1000 }, { doc_num: 500 })).toBe(
        500,
      );
    });
  ...
  describe("Data Processing", () => {
    it("handles empty stats in enrichment table data", async () => {
      ...
      expect(wrapper.vm.jsTransforms[0]).toEqual(
        expect.objectContaining({
          name: "test-table",
          doc_num: "",
          storage_size: "",
  ...
  describe("Table Sorting", () => {
    it("handles compressed size sorting", () => {
      const column = wrapper.vm.columns.find(
        (c) => c.name === "compressed_size",
      );
      const result = column.sort(
        null,
        null,
