    ...
    setTimeout(() => {
      try {
        if (typeof req.onupgradeneeded === "function") {
          req.onupgradeneeded({
            target: { result: { objectStoreNames: { contains: () => true } } },
          });
        }
                  ...
                  return {
                    put: (val) => {
                      ...
                      setTimeout(() => {
                        ...
                        const idx = memoryDB.data.findIndex(
                          (d) => d.id === val.id,
                        );
                        if (idx >= 0) memoryDB.data[idx] = val;
                        else memoryDB.data.push(val);
                        if (typeof r.onsuccess === "function")
                          r.onsuccess({ target: { result: val.id } });
                      }, 0);
                      return r;
                    },
                    get: (id) => {
                      ...
                      setTimeout(() => {
                        const row =
                          memoryDB.data.find((d) => d.id === id) || null;
                        if (typeof r.onsuccess === "function")
                    ...
                    index: (/* name */) => ({
                      openCursor: (_range, _dir) => {
                        ...
                        setTimeout(() => {
                          ...
                          const list = [...memoryDB.data].sort((a, b) =>
                    ...
                    delete: (id) =>
                      void (memoryDB.data = memoryDB.data.filter(
                        (d) => d.id !== id,
                      )),
                  };
                },
    ...
    getReader() {
      ...
      return {
        async read() {
          ...
          const v = new TextEncoder().encode(
...
describe("O2AIChat - basic rendering", () => {
  it("renders container when closed and applies theme classes", async () => {
    ...
    expect(
      wrapper.find(".dark-mode").exists() ||
        wrapper.find(".light-mode").exists(),
    ).toBe(true);
  });
...
describe("O2AIChat - header actions", () => {
  ...
  it("addNewChat resets chat state", async () => {
    ...
    expect(
      wrapper.vm.currentChatId === null ||
        typeof wrapper.vm.currentChatId === "number",
    ).toBe(true);
  });
...
describe("O2AIChat - tool confirmation", () => {
  ...
  it("handleToolConfirm resolves pending confirmation block", async () => {
    ...
    expect(
      wrapper.vm.chatMessages[0].contentBlocks[0].pendingConfirmation,
    ).toBe(false);
  });
...
describe("O2AIChat - content block types", () => {
  it("processedMessages handles error blocks", async () => {
    ...
    expect(wrapper.vm.chatMessages[0].contentBlocks[0].message).toBe(
      "Search failed",
    );
  });
  ...
  it("handles tool_call blocks with success/failure states", async () => {
        ...
        contentBlocks: [
          {
            ...
            tool: "SearchSQL",
            success: true,
            resultMessage: "Found 10 records",
          },
          {
            type: "tool_call",
            tool: "CreateAlert",
    ...
    expect(wrapper.vm.chatMessages[0].contentBlocks[1].errorType).toBe(
      "AuthError",
    );
  });
...
describe("O2AIChat - messages with images", () => {
  it("user messages can have images array", async () => {
    ...
    expect(wrapper.vm.chatMessages[0].images[0].filename).toBe(
      "screenshot.png",
    );
  });
});
