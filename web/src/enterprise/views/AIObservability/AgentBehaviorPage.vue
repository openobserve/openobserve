  ...
  >
    ...
    <div class="flex items-center gap-3 px-page-edge py-2">
      ...
      >
        ...
        >
          <!-- On selection show the agent NAME only; env/version are badges.
               Classes mirror OSelect's default label span for visual parity. -->
          <template #trigger>
            <span
              class="flex-1 text-start truncate text-xs leading-4"
              :class="
                selectedAgent ? 'text-select-text' : 'text-select-placeholder'
              "
            >
              {{ selectedAgent?.name || t("aiObservability.agentGraph.agent") }}
            </span>
          </template>
        </OSelect>
      </div>
      ...
      <OAgentBadges
        ...
        data-test="agent-behavior-scope-badges"
      />
    </div>
    </template>
