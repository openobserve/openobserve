    ...
    <div class="bg-card-glass-bg flex h-full min-h-0 flex-col" data-test="ai-discovery-body">
      ...
      >
        ...
        <template #cell-inQueue="{ row }">
          <div v-if="row.queues.length" class="flex flex-wrap items-center gap-1">
            <span v-for="queue in row.queues" :key="queue.queueId">
              <OTag
                :variant="queue.status === 'reviewed' ? 'success-soft' : 'default-soft'"
                shape="rounded"
              >
              ...
              <OTooltip
                side="bottom"
                :content="raw(t(`aiObservability.queues.detail.status.${queue.status}`))"
              />
            </span>
          </div>
