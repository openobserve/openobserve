...
import OTemplateTextarea from "@/lib/forms/TemplateInput/OTemplateTextarea.vue";
...
// `expected_output` is the answer, not an input: completing it into a prompt would leak it.
const suggestions = computed<TemplateSuggestion[]>(() =>
  props.varNames.filter((name) => name !== EXPECTED_OUTPUT_TOKEN).map((name) => ({ name })),
);
