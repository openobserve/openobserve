<template>
    <div v-if="showLicenseExpiryWarning && config.isEnterprise == 'true' && config.isCloud === 'false'" data-test="license-period-container" class="w-full p-3 border border-border-default rounded-md [background:linear-gradient(to_right,transparent_60%,#f7f7ff_70%,#cdf7e4_100%)] dark:[background:linear-gradient(to_right,transparent_60%,#24262F_70%,#2C3934_100%)]">
        <div class="flex" >
