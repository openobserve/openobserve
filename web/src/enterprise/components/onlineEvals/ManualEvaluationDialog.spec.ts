...
import onlineEvalsService, { type EvalJob } from "@/services/online-evals.service";
...
describe("ManualEvaluationDialog", () => {
  ...
  it("submits only the selected target and its authoritative time range", async () => {
    ...
    expect(onlineEvalsService.jobs.manualEval).toHaveBeenCalledWith("org-1", "trace-job", {
      targetId: "trace-1",
      startTime: 1_000,
      endTime: 2_000,
      spanId: undefined,
  ...
  it("shows an empty state when no job matches scope and stream", async () => {
    ...
    expect(document.querySelector('[data-test="manual-evaluation-no-jobs"]')).not.toBeNull();
