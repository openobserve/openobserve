<template>
  <section class="flex flex-col gap-[10px] min-h-0 flex-1" data-test="quality-score-configs-overview">
    <div v-if="isLoading && rows.length === 0" class="flex flex-col items-center gap-2 py-8 px-3 border border-dashed border-[var(--color-dialog-header-border,var(--color-border-default))] rounded-md text-center text-[var(--color-text-secondary,var(--color-text-secondary))]">
      <OSpinner size="sm" />
      ...
      >
        ...
        <template #cell-type="{ row }">
          <OTag
            ...
            :value="row.dataType"
          />
          <span v-else class="text-[var(--color-text-secondary,var(--color-text-secondary))]">—</span>
        </template>
        ...
        <template #cell-updated="{ row }">
          <span v-if="row.lastUpdatedMs" class="text-[11px] text-[var(--color-text-secondary,var(--color-text-secondary))] [font-variant-numeric:tabular-nums]">
...
function sparkClass(status: string): string {
  ...
  if (status === 'healthy') return 'text-[var(--color-status-success-text,#2e7d32)]';
  if (status === 'noData') return 'text-[var(--color-text-secondary,var(--color-text-secondary))] opacity-[0.55]';
  return 'text-[var(--color-text-secondary,var(--color-text-secondary))]';
}
