  ...
  >
    <div class="flex flex-col gap-5">
      <div class="flex flex-col gap-1">
        ...
        <p class="text-text-secondary text-xs break-all">
          {{ targetLabel }}: <span class="font-mono">{{ targetId }}</span>
        </p>
      </div>

      <OBanner v-if="loadError" variant="error-soft" dense data-test="manual-evaluation-jobs-error">
        {{ loadError }}
      </OBanner>
...
const dialogTitle = computed(() => t(`onlineEvals.manualEvaluation.titles.${props.targetScope}`));
...
async function loadJobs(): Promise<void> {
  ...
  form.reset({ jobId: "" });
  try {
    loadedJobs.value = await evalJobsQuery.get(props.orgId);
...
async function onSubmit(value: ManualEvaluationDialogForm): Promise<void> {
  try {
    const result = await onlineEvalsService.jobs.manualEval(props.orgId, value.jobId, {
      targetId: props.targetId,
      startTime: props.startTime,
      endTime: props.endTime,
      spanId: props.targetScope === "span" ? props.targetId : undefined,
      traceId: props.targetScope === "trace" ? props.targetId : props.traceId,
      sessionId: props.targetScope === "session" ? props.targetId : props.sessionId,
    });
    toast({
      variant: "success",
