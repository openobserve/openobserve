<template>
  <aside class="flex-[3.5] min-w-0 overflow-auto p-2 bg-(--o2-card-bg) rounded-md shadow-[0_0_0.313rem_0.063rem_var(--o2-hover-shadow)]">
    <section class="py-[14px] px-4 mb-3 border border-(--color-dialog-header-border,var(--color-border-default)) rounded-md">
      ...
      <div v-else-if="matchedError" class="m-0 text-xs text-(--color-status-error-text,#c62828)">
    ...
    <section class="py-[14px] px-4 mb-0 border border-(--color-dialog-header-border,var(--color-border-default)) rounded-md">
      <header class="flex items-center gap-[6px] mb-[6px] text-(--color-text-secondary,var(--color-text-secondary))">
