...
describe("makeSpanSelectorSchema", () => {
  it("rejects duplicate names and incomplete filters", () => {
    expect(schema.safeParse({ ...validForm, name: "duplicate" }).success).toBe(false);
    expect(schema.safeParse({ ...validForm, filterReady: false }).success).toBe(false);
  });

  it("requires fields for custom schemas", () => {
    expect(schema.safeParse({ ...validForm, fieldMode: "custom", fields: [] }).success).toBe(false);
  });

  it("enforces the output budget", () => {
    expect(schema.safeParse({ ...validForm, maximumSpans: 6 }).success).toBe(false);
