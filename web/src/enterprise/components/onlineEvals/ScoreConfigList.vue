      ...
      >
        ...
        <template #cell-version="{ row }">
          <span class="inline-flex items-center gap-1.5 font-[ui-monospace,SFMono-Regular,Menlo,monospace] text-xs">
            <span class="w-1.5 h-1.5 rounded-full bg-(--color-status-success-text) inline-block" />v{{ row.version }}
          </span>
        </template>
...
function dtypeChipClass(dataType: string): string {
  ...
  if (dataType === 'boolean') return 'bg-[color-mix(in_srgb,var(--color-status-success-text)_14%,transparent)] text-(--color-status-success-text)';
  return '';
}
