      ...
      >
        ...
        <template #cell-version="{ row }">
          <span class="inline-flex items-center gap-1.5 font-[ui-monospace,SFMono-Regular,Menlo,monospace] text-xs">
            <span class="w-1.5 h-1.5 rounded-full bg-success-600 inline-block" />v{{ row.version }}
          </span>
        </template>
...
function dtypeChipClass(dataType: string): string {
  if (dataType === 'numeric') return 'bg-[color-mix(in_srgb,var(--color-info-700)_14%,transparent)] text-info-700';
  if (dataType === 'categorical') return 'bg-[color-mix(in_srgb,var(--color-warning-700)_14%,transparent)] text-warning-700';
  if (dataType === 'boolean') return 'bg-[color-mix(in_srgb,var(--color-success-600)_14%,transparent)] text-success-600';
  return '';
}
