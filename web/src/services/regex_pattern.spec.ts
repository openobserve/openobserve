...
describe("regex_pattern service", () => {
  ...
  describe("list", () => {
    it("should make GET request to list all regex patterns for an org", async () => {
      ...
      expect(mockHttpInstance.get).toHaveBeenCalledWith("/api/test-org/re_patterns");
    });
    ...
    it("should handle different org identifiers", async () => {
      ...
      for (const org of orgs) {
        ...
        expect(mockHttpInstance.get).toHaveBeenCalledWith(`/api/${org}/re_patterns`);
      }
    });
    ...
    it("should propagate errors", async () => {
      ...
      await expect(regexPatterns.list("test-org")).rejects.toThrow("Unauthorized");
    });
  });
  ...
  describe("create", () => {
    it("should make POST request with pattern payload", async () => {
      ...
      expect(mockHttpInstance.post).toHaveBeenCalledWith("/api/test-org/re_patterns", payload);
    });
    ...
    it("should handle minimal payload", async () => {
      ...
      expect(mockHttpInstance.post).toHaveBeenCalledWith("/api/my-org/re_patterns", payload);
    });
  ...
  describe("delete", () => {
    it("should make DELETE request with the correct pattern id", async () => {
      ...
      expect(mockHttpInstance.delete).toHaveBeenCalledWith("/api/test-org/re_patterns/pat-456");
    });
    ...
    it("should handle different org and id combinations", async () => {
      ...
      expect(mockHttpInstance.delete).toHaveBeenCalledWith("/api/prod-org/re_patterns/pattern-xyz");
    });

    it("should propagate errors", async () => {
      mockHttpInstance.delete.mockRejectedValue(new Error("Forbidden"));

      await expect(regexPatterns.delete("test-org", "locked-pattern")).rejects.toThrow("Forbidden");
    });
  });
  ...
  describe("bulkDelete", () => {
    it("should make DELETE request with data payload for bulk deletion", async () => {
      ...
      expect(mockHttpInstance.delete).toHaveBeenCalledWith("/api/test-org/re_patterns/bulk", {
        data,
      });
    });
    ...
    it("should propagate errors", async () => {
      ...
      await expect(regexPatterns.bulkDelete("test-org", { ids: ["pat-1"] })).rejects.toThrow(
        "Partial failure",
      );
    });
  });
  ...
  describe("test", () => {
    it("should make POST request with pattern and test_records", async () => {
      ...
      expect(mockHttpInstance.post).toHaveBeenCalledWith("/api/test-org/re_patterns/test", {
        pattern: "\\d+",
        test_records: ["abc123", "xyz"],
      });
    });
    ...
    it("should include policy in payload when provided", async () => {
      ...
      await regexPatterns.test("test-org", "[a-z]+", ["hello", "world"], "extract");

      expect(mockHttpInstance.post).toHaveBeenCalledWith("/api/test-org/re_patterns/test", {
        pattern: "[a-z]+",
        test_records: ["hello", "world"],
        policy: "extract",
      });
    });
    ...
    it("should propagate errors", async () => {
      ...
      await expect(regexPatterns.test("test-org", "[invalid", ["test"])).rejects.toThrow(
        "Invalid pattern",
      );
    });
  });
  ...
  describe("getBuiltInPatterns", () => {
    it("should make GET request without query params when no params provided", async () => {
      ...
      expect(mockHttpInstance.get).toHaveBeenCalledWith("/api/test-org/re_patterns/built-in");
    });
    ...
    it("should propagate errors", async () => {
      ...
      await expect(regexPatterns.getBuiltInPatterns("test-org")).rejects.toThrow("Server error");
    });
  });
  ...
  describe("error handling", () => {
    it("should propagate network errors from list", async () => {
      ...
      await expect(regexPatterns.list("test-org")).rejects.toThrow("Network error");
    });
    ...
    it("should propagate HTTP 403 error from delete", async () => {
      ...
      await expect(regexPatterns.delete("test-org", "pat-1")).rejects.toEqual(httpError);
    });
  });
});
