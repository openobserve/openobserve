...
describe("Organizations Service", () => {
  ...
  describe("add_members", () => {
    it("should add members to organization", async () => {
      ...
      expect(mockHttp().post).toHaveBeenCalledWith(`api/${mockOrgId}/invites`, memberData);
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe("update_organization_passcode", () => {
    it("should update organization passcode", async () => {
      ...
      expect(mockHttp().put).toHaveBeenCalledWith(`api/${mockOrgId}/passcode`, {});
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe("post_organization_settings", () => {
    it("should update organization settings", async () => {
      ...
      expect(mockHttp().post).toHaveBeenCalledWith(`/api/${mockOrgId}/settings`, settingsData);
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe("set_ai_usage_limit", () => {
    it("should set an organization's AI credit limit through the meta org", async () => {
      ...
      expect(mockHttp().put).toHaveBeenCalledWith(`/api/${mockOrgId}/ai/usage_limit`, data);
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle network errors", async () => {
      ...
      await expect(organizations.list(1, 10, "name", false, "")).rejects.toThrow(
        "Network connection failed",
      );
      await expect(organizations.create({})).rejects.toThrow("Network connection failed");
      await expect(organizations.update_member_role({}, mockOrgId)).rejects.toThrow(
        "Network connection failed",
      );
    });
