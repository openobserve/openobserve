...
// promqlTerms.ts carries Prometheus's own vocabulary — every aggregation,
// function and modifier, with a one-line description each — generated by
// scripts/generate-promql-terms.mjs. This module shapes those terms into the
// entry format the editor already consumes, so PromQL and SQL differ in
...
describe("PROMQL_FUNCTIONS — aggregations and function identifiers", () => {
  ...
  it("marks them Function, so they get the function glyph", () => {
    // The PromQL form of A1. The upstream provider labels EVERY term
...
describe("PROMQL_CATALOG — the merged list the editor receives", () => {
  ...
  it("is derived from the upstream term tables, not hand-written", () => {
    // Deep cuts nobody types from memory: if these are present, the catalog
    // came from Prometheus's published tables rather than from a list someone
