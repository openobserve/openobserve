...
describe("vrlLanguageDefinition.ts", () => {
  ...
  describe("vrlLanguageDefinition - Regular Expressions", () => {
    ...
    it("should have correct regEx pattern", () => {
      ...
      expect(vrlLanguageDefinition.regEx.toString()).toBe(
  ...
  describe("vrlLanguageDefinition - Symbols and Escapes", () => {
    ...
    it("should have correct symbols pattern", () => {
      ...
      expect(vrlLanguageDefinition.symbols.toString()).toBe(
        expectedSymbols.toString(),
      );
    });
  ...
  describe("vrlLanguageDefinition - Tokenizer Structure", () => {
    ...
    it("should have all required tokenizer states", () => {
      ...
      requiredStates.forEach((state) => {
        ...
        expect(Array.isArray(vrlLanguageDefinition.tokenizer[state])).toBe(
          true,
        );
      });
    });
    ...
    it("should have non-empty rule arrays for all states", () => {
      ...
      states.forEach((state) => {
        expect(vrlLanguageDefinition.tokenizer[state].length).toBeGreaterThan(
          0,
        );
      });
    });
    ...
    it("should have function invoke rules", () => {
      ...
      const functionInvokeRule = rootRules.find(
        (rule: any) =>
          ...
          rule[0].toString().includes("([a-zA-Z_!]+)(\\!)(\\()"),
      );
  ...
  describe("vrlLanguageDefinition - String Tokenizer Rules", () => {
    ...
    it("should have escape sequence rule", () => {
      ...
      const escapeRule = stringRules.find(
        (rule: any) =>
          rule[0] instanceof RegExp &&
          rule[0].source === "@escapes" &&
          rule[1] === "string.escape",
      );
  ...
  describe("vrlThemeDefinition - Theme Rules", () => {
    ...
    it("should have keyword styling rules", () => {
      const keywordRule = vrlThemeDefinition.rules.find(
        (rule: any) => rule.token === "keyword",
      );
    ...
    it("should have string styling rules", () => {
      const stringRule = vrlThemeDefinition.rules.find(
        (rule: any) => rule.token === "string",
      );
    ...
    it("should have entity styling rules", () => {
      const entityRule = vrlThemeDefinition.rules.find(
        (rule: any) => rule.token === "entity",
      );
  ...
  describe("vrlThemeDefinition - Color Definitions", () => {
    ...
    it("should have selection colors", () => {
      expect(vrlThemeDefinition.colors).toHaveProperty(
    ...
    it("should have cursor color", () => {
      ...
      expect(vrlThemeDefinition.colors["editorCursor.foreground"]).toBe(
        "#24292e",
      );
    });
    ...
    it("should have whitespace color", () => {
      ...
      expect(vrlThemeDefinition.colors["editorWhitespace.foreground"]).toBe(
        "#959da5",
      );
    });
    ...
    it("should handle regex patterns in different contexts", () => {
      ...
      const regexRules = rootRules.filter(
        (rule: any) =>
          ...
          rule.length >= 2 &&
          (rule[1] === "regexp" ||
            (typeof rule[1] === "object" && rule[1].token === "regexp")),
      );
  ...
  describe("vrlLanguageDefinition - Edge Cases and Validation", () => {
    ...
    it("should have proper tokenizer state transitions", () => {
      ...
      expect(hasValidTransitions(vrlLanguageDefinition.tokenizer.root)).toBe(
        true,
      );
      expect(hasValidTransitions(vrlLanguageDefinition.tokenizer.string)).toBe(
        true,
      );
    });
  });
