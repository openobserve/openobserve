...
vi.mock("@/utils/dashboard/convertDataIntoUnitValue", () => ({
  ...
  getUnitValue: vi.fn((value, unit, customUnit, decimals) => ({
...
describe("convertTableData", () => {
        ...
        override_config: [
          {
            field: { value: "value" },
            config: [
              { type: "unit", value: { unit: "bytes", customUnit: "MB" } },
            ],
          },
        ],
      },
  ...
  it("should handle table_dynamic_columns enabled", () => {
    ...
    const result = convertTableData(
      schemaWithDynamicColumns,
      dataWithExtraColumns,
      mockStore,
    );
  ...
  it("should correctly sort numeric columns with custom sort function", () => {
    ...
    const result = convertTableData(
      schemaWithNumericColumn,
      unsortedData,
      mockStore,
    );

    // Find the score column
    const scoreColumn = result.columns.find(
      (col: any) => col.field === "score",
    );
  ...
  it("should set colorMode to auto when autoColor is enabled in override_config", () => {
    ...
    const statusColumn = result.columns.find(
