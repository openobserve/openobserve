 ...
 *
 * Performance-critical: may be called millions of times in nested loops.
 * Handles numeric keys, camelCase vs lowercase string keys, falsy values
 * (0, false, '', null) which are valid and returned as-is, and null/undefined
 * objects/aliases (returns undefined without throwing).
 *
 * Search strategy: exact (case-sensitive) match first; if not found and alias
 * is a string, falls back to its lowercase form. Uses strict inequality (!==)
 * so falsy values like 0 and false are returned rather than treated as missing.
 *
