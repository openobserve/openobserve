...
export const convertSQLData = async (
  ...
  // Handle colorBySeries logic to prevent duplicate colors
  if (panelSchema?.config?.color?.colorBySeries) {
    console.log(
      "dataaaa------ colorBySeries",
      panelSchema?.config?.color?.colorBySeries,
    );
    ...
    const generateUniqueColor = (
      ...
      // If all palette colors are used, generate a new color
      const hue = (usedColors.size * 137.508) % 360; // Golden angle approximation
      const saturation = 70 + (usedColors.size % 20); // Vary saturation
      const lightness = 45 + (usedColors.size % 20); // Vary lightness
      return `hsl(${hue}, ${saturation}%, ${lightness}%)`;
    };

    // Track used colors and categorize seriese
    const usedColors = new Set<string>();
    const userSelectedSeries = new Set<any>();
    const autoGeneratedSeries = new Set<any>();

    // First pass: categorize series and track used colors
    options.series.forEach((series: any) => {
      // Check if series has a color (either direct color or itemStyle.color)
      const seriesColor = series.color || series.itemStyle?.color;
      if (!seriesColor) return;

      // Check if this series has a user-selected color in config.colorBySeries
      const colorBySeries = panelSchema?.config?.color?.colorBySeries || {};
      const userSelectedColor = colorBySeries[series.name];

      if (userSelectedColor && userSelectedColor === seriesColor) {
        // This series has a user-selected color that matches its current color
        userSelectedSeries.add(series);
        usedColors.add(seriesColor);
      } else {
        // This series either has no user-selected color or the color doesn't match
    ...
    // If an auto-generated color conflicts with any user-selected color, change the auto-generated one
    options.series.forEach((series: any) => {
      const seriesColor = series.color || series.itemStyle?.color;

      if (seriesColor && autoGeneratedSeries.has(series)) {
        // Check if this auto-generated color conflicts with any user-selected color
        const hasConflict = Array.from(userSelectedSeries).some(
          (userSeries: any) => {
            const userColor = userSeries.color || userSeries.itemStyle?.color;
            return userColor === seriesColor;
          },
        );

        // Also check if this color conflicts with any color in colorBySeries config
        const colorBySeries = panelSchema?.config?.color?.colorBySeries || {};
        const conflictsWithConfig =
          Object.values(colorBySeries).includes(seriesColor);

        if (hasConflict || conflictsWithConfig) {
          // Generate a new unique color for this auto-generated series
          const newColor = generateUniqueColor(usedColors, store.state.theme);
          ...
          if (series.itemStyle?.color) {
            series.itemStyle.color = newColor;
          }

          console.log(
            "dataaaa------ Changed auto-generated series color to:",
            newColor,
            "for series:",
            series.name,
          );
