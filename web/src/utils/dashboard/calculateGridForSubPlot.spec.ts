...
import {
  ...
  getTrellisGrid,
} from "@/utils/dashboard/calculateGridForSubPlot";
...
describe("calculateGridForSubPlot", () => {
  ...
  describe("getTrellisGrid", () => {
    it("should throw error for invalid width", () => {
      expect(() => getTrellisGrid(0, 100, 1, 10)).toThrow(
        "Width and height must be positive numbers",
      );
      expect(() => getTrellisGrid(-100, 100, 1, 10)).toThrow(
        "Width and height must be positive numbers",
      );
    });
    ...
    it("should throw error for negative left margin", () => {
      expect(() => getTrellisGrid(100, 100, 1, -10)).toThrow(
        "Left margin must be non-negative",
      );
    });

    it("should throw error for invalid numGrids", () => {
      expect(() => getTrellisGrid(100, 100, "invalid" as any, 10)).toThrow(
        "Number of grids must be a non-negative number",
      );
      expect(() => getTrellisGrid(100, 100, -5, 10)).toThrow(
        "Number of grids must be a non-negative number",
      );
    });
  ...
  describe("calculateOptimalGrid (indirectly tested)", () => {
    ...
    it("should handle very large numbers of grids", () => {
      ...
      expect(result.gridNoOfRow * result.gridNoOfCol).toBeGreaterThanOrEqual(
        100,
      );
    });
  ...
  describe("Additional edge cases", () => {
    ...
    it("should test binary search optimization in calculateOptimalGrid (indirectly)", () => {
      ...
      const efficiency =
        result.gridArray.length / (result.gridNoOfRow * result.gridNoOfCol);
