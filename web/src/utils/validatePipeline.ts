...
export function validatePipeline(
  ...
  pipeline.nodes.forEach((node) => {
    ...
    if (!node.data.node_type) {
      result.errors.push(
        `Node ${node.id} is missing required 'node_type' field`,
      );
    } else {
    ...
    if (
      node.data.stream_type &&
      !validStreamTypes.includes(node.data.stream_type)
    ) {
      result.errors.push(
    ...
    if (["stream", "remote_stream"].includes(node.data.node_type)) {
      if (!node.data.org_id) {
        result.errors.push(
          `Node ${node.id} is missing required 'org_id' field`,
        );
      ...
      if (
        ...
        if (typeof node.data.stream_name === "string") {
          ...
          typeof node.data.stream_name === "object"
        ) {
        ...
        if (streamName) {
          ...
          const originalNode = context.originalPipeline?.nodes?.find(
            (n: any) => n.id === node.id,
          );
          ...
          if (
            context.usedStreamsList.includes(streamName) &&
    ...
    if (node.io_type === "input" && node.data.node_type === "query") {
      ...
      if (!node.data.query_condition) {
        ...
        if (queryCondition.type === "sql") {
          ...
          if (
            queryCondition.promql !== null ||
            queryCondition.promql_condition !== null
          ) {
            result.errors.push(
      ...
      if (!node.data.trigger_condition) {
        result.errors.push(
          `Query node ${node.id} is missing trigger_condition`,
        );
      } else {
        ...
        if (trigger.silence < 0) {
          result.errors.push(
            `Query node ${node.id} has invalid silence value. Must be >= 0`,
          );
        }
        if (trigger.threshold < 0) {
          result.errors.push(
            `Query node ${node.id} has invalid threshold value. Must be >= 0`,
          );
        }
        ...
        if (trigger.frequency_type === "minutes") {
          ...
          if (!trigger.cron || trigger.cron.trim() === "") {
            result.errors.push(
              `Query node ${node.id} has empty cron expression`,
            );
          }
        }
      }
    ...
    if (node.io_type === "default" && node.data.node_type === "function") {
      ...
      if (typeof node.data.after_flatten !== "boolean") {
        result.errors.push(
          `Function node ${node.id} must have 'after_flatten' field as boolean`,
        );
      }
      ...
      if (!node.data.name || node.data.name.trim() === "") {
        ...
        context?.functionsList &&
        !context.functionsList.includes(node.data.name)
      ) {
        result.errors.push(
    ...
    if (node.io_type === "output") {
      if (node.data.node_type === "stream") {
        ...
        if (!streamName || streamName.trim() === "") {
          result.errors.push(
            `Output stream node ${node.id} has empty stream name`,
          );
        }
        ...
        if (
          !node.data.destination_name ||
          node.data.destination_name.trim() === ""
        ) {
          result.errors.push(
            `Remote stream node ${node.id} has empty destination name`,
          );
        } else if (
