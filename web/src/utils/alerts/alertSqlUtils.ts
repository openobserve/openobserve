...
export const addHavingClauseToQuery = (
  ...
  try {
    ...
    if (!ast.groupby || ast.groupby.length === 0) {
      console.warn(
        "Adding HAVING clause to query without GROUP BY may cause SQL error",
      );
    }
    ...
    if (ast.having) {
      ...
      if (conditionPattern.test(sqlified)) {
        console.log(
          "HAVING clause with same condition already exists in AST, skipping addition",
        );
    ...
    console.warn(
      "Failed to parse SQL query for HAVING clause insertion:",
      error,
    );
...
function fallbackHavingInsertion(
  ...
  if (sqlQuery.match(/\(\s*SELECT/i)) {
    console.warn(
      "Query contains subqueries. Fallback HAVING insertion may not work correctly.",
    );
    console.warn(
      "The HAVING clause will be added to the outermost query, which may not be the intended location.",
    );
  }
  ...
  if (!groupByMatch) {
    console.warn(
      "Adding HAVING without GROUP BY - this will likely cause a SQL error",
    );
  }
  ...
  if (sqlLower.includes("having")) {
    ...
    if (existingConditionPattern.test(sqlQuery)) {
      console.log(
        "HAVING clause with same condition already exists, skipping addition",
      );
  ...
  if (orderByMatch && orderByMatch.index !== undefined) {
    return (
      sqlQuery.slice(0, orderByMatch.index) +
      ` ${havingClause}` +
  ...
  if (limitMatch && limitMatch.index !== undefined) {
    return (
      sqlQuery.slice(0, limitMatch.index) +
      ` ${havingClause}` +
  ...
  if (offsetMatch && offsetMatch.index !== undefined) {
    return (
      sqlQuery.slice(0, offsetMatch.index) +
      ` ${havingClause}` +
