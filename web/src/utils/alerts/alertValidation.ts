...
export function validateAlert(
  ...
  if (alert.org_id) {
    if (alert.org_id !== context?.selectedOrgId) {
      result.errors.push(
        `Organization Id should be equal to ${context?.selectedOrgId}`,
      );
    }
  }
  ...
  if (!alert.stream_name || typeof alert.stream_name !== "string") {
    ...
    context?.streamList &&
    !context.streamList.includes(alert.stream_name)
  ) {
    result.errors.push(
      `Stream "${alert.stream_name}" does not exist in the stream list`,
    );
  }
  ...
  if (typeof alert.is_real_time !== "boolean") {
    result.errors.push(
      "Is Real-Time is mandatory and should be a boolean value",
    );
  }
  ...
  if (alert.query_condition) {
      ...
      case "custom":
        ...
        if (alert.query_condition.conditions) {
            ...
            if (!Array.isArray(alert.query_condition.conditions.items)) {
              ...
              alert.query_condition.conditions.items.forEach((item, index) => {
                if (
                  ...
                  result.errors.push(
                    `Query condition item ${index + 1} must have column, operator, and value`,
                  );
                }
                ...
                if (!validOperators.includes(item.operator)) {
                  result.errors.push(
                    `Invalid operator "${item.operator}" in query condition item ${index + 1}`,
                  );
                }
              });
            }
          ...
          // Format 2: Using or/and array
            ...
            alert.query_condition.conditions.or ||
            alert.query_condition.conditions.and
          ) {
            const conditions =
              alert.query_condition.conditions.or ||
              alert.query_condition.conditions.and;
            if (!Array.isArray(conditions)) {
              ...
              conditions.forEach((condition, index) => {
                if (
                  ...
                  result.errors.push(
                    `Query condition ${index + 1} must have column, operator, and value`,
                  );
                }
                ...
                if (
                  ...
                  result.errors.push(
                    `ignore_case must be a boolean in condition ${index + 1}`,
                  );
                }
              });
            }
        ...
        if (
          ...
          result.errors.push(
            "Multi Time Range should be an empty array or null",
          );
        }
    ...
    if (alert.query_condition.aggregation) {
      ...
      if (agg.having && typeof agg.having === "object") {
        ...
        if (
          agg.having.value !== undefined &&
          typeof agg.having.value !== "number"
        ) {
          result.errors.push(
  ...
  if (alert.trigger_condition) {
    ...
    if (!validOperators.includes(trigger.operator)) {
      result.errors.push(
        `Invalid operator "${trigger.operator}" in trigger condition`,
      );
    }
    ...
    if (typeof trigger.frequency !== "number" || trigger.frequency < 1) {
      result.errors.push(
        "Frequency should be a positive number greater than 0",
      );
    }
    ...
    if (typeof trigger.threshold !== "number" || trigger.threshold < 1) {
      result.errors.push(
        "Threshold should be a positive number greater than 0",
      );
    }
    ...
    if (trigger.frequency_type === "cron") {
      if (!trigger.cron || trigger.cron.trim() === "") {
        result.errors.push(
          "Cron expression is required when frequency type is cron",
        );
      }
  ...
  if (!Array.isArray(alert.destinations)) {
    ...
    if (
      !Array.isArray(context.destinationsList) ||
      context.destinationsList.length === 0
    ) {
      ...
      if (invalidDestinations.length > 0) {
        result.errors.push(
          `Invalid destinations: ${invalidDestinations.map((dest) => `"${dest}"`).join(", ")} - must be from available destinations list`,
        );
      }
    }
  }
...
export const validateInputs = (
  ...
  if (input.trigger_condition.frequency_type === "cron") {
    ...
    if (
      ...
      input.trigger_condition.timezone.trim() === ""
    ) {
      notify &&
        q.notify({
          type: "negative",
...
export const validateSqlQuery = async (
  ...
  if (
    !formData.query_condition.sql ||
    formData.query_condition.sql.trim() === ""
  ) {
