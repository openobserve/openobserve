...
describe("alertSqlUtils", () => {
  ...
  describe("addHavingClauseToQuery", () => {
    it("should add HAVING clause before LIMIT", () => {
      const query =
        "SELECT count(*) as cnt FROM table1 GROUP BY field1 ORDER BY cnt DESC LIMIT 10";
    ...
    it("should add HAVING clause before ORDER BY", () => {
      const query =
        "SELECT avg(value) as avg_val FROM table1 GROUP BY region ORDER BY region";
      ...
      expect(result.toLowerCase()).toMatch(
        /having\s+(avg_val|"avg_val")\s+>\s+10/,
      );
    });
    ...
    it("should handle different operators", () => {
      const query =
        "SELECT avg(score) as avg_score FROM tests GROUP BY student";

      const operators = [">=", "<=", ">", "<", "="];
      operators.forEach((op) => {
        const result = addHavingClauseToQuery(
          query,
          "avg_score",
          op,
          75,
    ...
    it("should preserve double quotes in table names", () => {
      const query =
        'SELECT count(*) as cnt FROM "my-table" GROUP BY field1 ORDER BY cnt LIMIT 10';
      const result = addHavingClauseToQuery(query, "cnt", ">=", 5, parser);
    ...
    it("should handle complex aggregation functions", () => {
      ...
      expect(havingIndex).toBeLessThan(orderByIndex);
      expect(result.toLowerCase()).toMatch(
        /having\s+(zo_sql_val|"zo_sql_val")\s+>=\s+10/,
      );
    });
  });
  ...
  describe("Input Validation", () => {
    ...
    it("should throw error for non-numeric threshold", () => {
      const query = "SELECT count(*) as cnt FROM table1 GROUP BY field1";
      expect(() =>
        addHavingClauseToQuery(query, "cnt", ">=", NaN, parser),
    ...
    it("should throw error for infinite threshold", () => {
      const query = "SELECT count(*) as cnt FROM table1 GROUP BY field1";
      expect(() =>
        addHavingClauseToQuery(query, "cnt", ">=", Infinity, parser),
  ...
  describe("Fallback Regex Method", () => {
    it("should use regex fallback when AST parsing fails", () => {
      ...
      const query =
        "SELECT count(*) as cnt FROM users GROUP BY user_id ORDER BY cnt LIMIT 10";
