...
describe("buildAnomalyFilterExpression", () => {
  ...
  describe("comparison operators", () => {
    it("should return field = 'value' when operator is =", () => {
      expect(buildAnomalyFilterExpression("status", "=", "200")).toBe("status = '200'");
    });

    it("should return field <> 'value' when operator is <>", () => {
      expect(buildAnomalyFilterExpression("status", "<>", "500")).toBe("status <> '500'");
    });

    it("should return field >= 'value' when operator is >=", () => {
      expect(buildAnomalyFilterExpression("latency", ">=", "100")).toBe("latency >= '100'");
    });

    it("should return field <= 'value' when operator is <=", () => {
      expect(buildAnomalyFilterExpression("latency", "<=", "500")).toBe("latency <= '500'");
    });

    it("should return field > 'value' when operator is >", () => {
      expect(buildAnomalyFilterExpression("bytes", ">", "1024")).toBe("bytes > '1024'");
    });
  ...
  describe("IN / NOT IN operators", () => {
    it("should return field IN (value) when operator is IN", () => {
      expect(buildAnomalyFilterExpression("status", "IN", "200,404,500")).toBe(
        "status IN (200,404,500)",
      );
    });

    it("should return field NOT IN (value) when operator is NOT IN", () => {
      expect(buildAnomalyFilterExpression("status", "NOT IN", "200,404")).toBe(
  ...
  describe("null check operators", () => {
    it("should return field IS NULL when operator is Is Null", () => {
      expect(buildAnomalyFilterExpression("error_msg", "Is Null", "")).toBe("error_msg IS NULL");
    });

    it("should return field IS NOT NULL when operator is Is Not Null", () => {
      expect(buildAnomalyFilterExpression("error_msg", "Is Not Null", "")).toBe(
  ...
  describe("string match operators", () => {
    it("should return str_match(field, 'value') when operator is str_match", () => {
      expect(buildAnomalyFilterExpression("message", "str_match", "error")).toBe(
        "str_match(message, 'error')",
      );
    });

    it("should return str_match(field, 'value') when operator is Contains", () => {
      expect(buildAnomalyFilterExpression("message", "Contains", "error")).toBe(
        "str_match(message, 'error')",
      );
    });

    it("should return str_match_ignore_case(field, 'value') when operator is str_match_ignore_case", () => {
      expect(buildAnomalyFilterExpression("message", "str_match_ignore_case", "ERROR")).toBe(
        "str_match_ignore_case(message, 'ERROR')",
      );
    });
  ...
  describe("regex operators", () => {
    it("should return re_match(field, 'pattern') when operator is re_match", () => {
      expect(buildAnomalyFilterExpression("level", "re_match", "^(error|warn)$")).toBe(
        "re_match(level, '^(error|warn)$')",
      );
    });

    it("should return re_not_match(field, 'pattern') when operator is re_not_match", () => {
      expect(buildAnomalyFilterExpression("level", "re_not_match", "^debug$")).toBe(
        "re_not_match(level, '^debug$')",
      );
    });
  });
  ...
  describe("LIKE operators", () => {
    it("should return field LIKE 'value%' when operator is Starts With", () => {
      expect(buildAnomalyFilterExpression("path", "Starts With", "/api")).toBe("path LIKE '/api%'");
    });

    it("should return field LIKE '%value' when operator is Ends With", () => {
      expect(buildAnomalyFilterExpression("path", "Ends With", ".json")).toBe("path LIKE '%.json'");
    });

    it("should return field NOT LIKE '%value%' when operator is Not Contains", () => {
      expect(buildAnomalyFilterExpression("message", "Not Contains", "debug")).toBe(
