  ...
  describe("transformFEToBE - OR Operator Transformations", () => {
    it("should transform simple OR group with single condition", () => {
      const input = {
        groupId: "group-1",
        label: "or",
        items: [{ id: "1", column: "status", operator: "=", value: "active", ignore_case: true }],
      };

      const result = transformFEToBE(input);

      expect(result).toEqual({
        or: [{ column: "status", operator: "=", value: "active", ignore_case: true }],
      });
    });
              ...
              items: [
                {
                  groupId: "target-or",
                  label: "or",
                  items: [{ id: "1", column: "a", operator: "=", value: "1", ignore_case: false }],
                },
              ],
            },
          },
...
describe("alertDataTransforms - V2 Structure Tests", () => {
  ...
  describe("detectConditionsVersion", () => {
    ...
    it("should detect V1 structure with and/or keys", () => {
      const v1BEData = {
        and: [{ column: "status", operator: "=", value: "active", ignore_case: true }],
      };
  ...
  describe("convertV0ToV2", () => {
    ...
    it("should handle V0 with existing ids", () => {
      const v0Data = [{ id: "existing-1", column: "field1", operator: "=", value: "val1" }];
  ...
  describe("convertV1ToV2 - Frontend Format", () => {
              ...
              { id: "1", column: "a", operator: "=", value: "1" },
              {
                ...
                items: [
                  { id: "2", column: "b", operator: "=", value: "2" },
                  {
                    groupId: "level-3",
                    label: "or",
                    items: [{ id: "3", column: "c", operator: "=", value: "3" }],
                  },
                ],
              },
            ],
    ...
    it("should preserve existing ids from V1 conditions", () => {
      const v1Data = {
        label: "and",
        items: [{ id: "custom-id-123", column: "field", operator: "=", value: "val" }],
      };
  ...
  describe("convertV1BEToV2 - Backend Format", () => {
    ...
    it("should generate ids for conditions without ids", () => {
      const v1BEData = {
        and: [{ column: "field", operator: "=", value: "val", ignore_case: true }],
      };
    ...
    describe("Complex Real-World Scenarios", () => {
          ...
          or: [
            ...
            { column: "job", operator: "=", value: "swe", ignore_case: true },
            {
              and: [
                ...
                { column: "log", operator: "=", value: "ontime", ignore_case: true },
                {
                  or: [{ column: "_timestamp", operator: "=", value: "12", ignore_case: true }],
                },
              ],
            },
          ],
      ...
      it("SCENARIO 6: Single condition (no grouping)", () => {
        // V1: status = 'active'
        const v1BEData = {
          and: [{ column: "status", operator: "=", value: "active", ignore_case: false }],
        };
