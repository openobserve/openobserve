...
describe("alertDataTransforms - OR Operator Pipeline Tests", () => {
  ...
  describe("removeConditionGroup - OR Operator Context", () => {
    ...
    it("should remove one OR group while keeping others", () => {
      ...
      expect(context.formData.query_condition.conditions.items).toHaveLength(1);
      expect(context.formData.query_condition.conditions.items[0].groupId).toBe(
        "or-keep",
      );
    });
...
describe("alertDataTransforms - V2 Structure Tests", () => {
  ...
  describe("detectConditionsVersion", () => {
    ...
    it("should detect V1 structure with label and items", () => {
      const v1FEData = {
        label: "and",
        items: [{ id: "1", column: "status", operator: "=", value: "active" }],
        groupId: "test",
      };
  ...
  describe("convertV0ToV2", () => {
    ...
    it("should generate ids for V0 items without ids", () => {
      const v0Data = [{ column: "field1", operator: "=", value: "val1" }];
  ...
  describe("convertV1ToV2 - Frontend Format", () => {
    ...
    it("should handle deeply nested V1 structure (3 levels)", () => {
      ...
      expect(result.conditions[0].conditions[1].conditions[1].filterType).toBe(
        "group",
      );
    });
  ...
  describe("convertV1BEToV2 - Backend Format", () => {
    ...
    it("should generate groupId for the root group", () => {
      const v1BEData = {
        and: [{ column: "field", operator: "=", value: "val" }],
      };
    ...
    describe("Complex Real-World Scenarios", () => {
      ...
      it("SCENARIO 14: Only nested groups (no direct conditions)", () => {
        ...
        expect(
          (result.conditions[0] as any).conditions[0].logicalOperator,
        ).toBe("AND");
        expect(
          (result.conditions[0] as any).conditions[1].logicalOperator,
        ).toBe("AND");
        ...
        expect(
          (result.conditions[1] as any).conditions[0].logicalOperator,
        ).toBe("AND");
        expect(
          (result.conditions[1] as any).conditions[1].logicalOperator,
        ).toBe("AND");
      });
  ...
  describe("updateGroup - V2 Structure", () => {
    it("should update root V2 group", () => {
      ...
      expect(context.formData.query_condition.conditions.logicalOperator).toBe(
        "OR",
      );
      expect(
        context.formData.query_condition.conditions.conditions[0].column,
      ).toBe("new");
    });
    ...
    it("should update nested V2 group", () => {
      ...
      const nestedGroup =
        context.formData.query_condition.conditions.conditions[1];
    ...
    it("should handle deeply nested V2 update (3 levels)", () => {
      ...
      const deepGroup =
        context.formData.query_condition.conditions.conditions[0].conditions[0];
    ...
    it("should handle V2 group with multiple conditions", () => {
      ...
      const targetGroup =
        context.formData.query_condition.conditions.conditions[0];
  ...
  describe("removeConditionGroup - V2 Structure", () => {
    it("should remove V2 condition from root group", () => {
      ...
      expect(context.formData.query_condition.conditions.conditions[0].id).toBe(
        "keep-id",
      );
    });
