...
describe("treeTooltipHelpers", () => {
  ...
  describe("generateNodeTooltipContent", () => {
    it("should generate correct HTML for node tooltip", () => {
      const html = generateNodeTooltipContent(
        "user-service",
        5500000,
        387200,
        7.07,
  ...
  describe("findIncomingEdgeForNode", () => {
    ...
    it("should find edge by exact parent and child match", () => {
      const edge = findIncomingEdgeForNode(
        "user-service",
        "api-gateway",
        edges,
      );
  ...
  describe("calculateRootNodeMetrics", () => {
    const edges = [
      {
        from: "api-gateway",
        to: "user-service",
        total_requests: 5500000,
        failed_requests: 387200,
      },
      {
        from: "api-gateway",
        to: "order-service",
        total_requests: 3100000,
        failed_requests: 12000,
      },
      {
        from: "user-service",
        to: "database",
        total_requests: 2700000,
        failed_requests: 61000,
      },
    ];
    ...
    it("should calculate correct error rate when total is zero", () => {
      const edges = [
        { from: "node", to: "child", total_requests: 0, failed_requests: 0 },
      ];
