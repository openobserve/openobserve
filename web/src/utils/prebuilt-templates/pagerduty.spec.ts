...
describe("pagerduty template", () => {
  ...
  describe("pagerdutyConfig", () => {
    ...
    it("validates PagerDuty URLs", () => {
      ...
      expect(
        pagerdutyConfig.urlValidator("https://events.pagerduty.com/v2/other"),
      ).toBe(false);
      expect(pagerdutyConfig.urlValidator("invalid-url")).toBe(false);
    });

    it("integration key validator validates key length", () => {
      const integrationKeyField = pagerdutyConfig.credentialFields.find(
    ...
    it("has severity field", () => {
      const severityField = pagerdutyConfig.credentialFields.find(
        (f) => f.key === "severity",
      );
