...
describe("discord template", () => {
  ...
  describe("discordConfig", () => {
    ...
    it("URL validator accepts valid Discord webhook URLs", () => {
      expect(
        discordConfig.urlValidator(
          "https://discord.com/api/webhooks/123456789/abcdef",
        ),
      ).toBe(true);
      expect(
        discordConfig.urlValidator("https://discord.com/api/webhooks/123/abc"),
      ).toBe(true);
    });

    it("URL validator rejects invalid URLs", () => {
      expect(discordConfig.urlValidator("https://example.com/webhook")).toBe(
        false,
      );
      expect(discordConfig.urlValidator("https://discord.com/api/other")).toBe(
        false,
      );
      expect(
        discordConfig.urlValidator("https://evil.com?discord.com=fake"),
      ).toBe(false);
    ...
    it("webhook URL field is required", () => {
      const webhookField = discordConfig.credentialFields.find(
        (f) => f.key === "webhookUrl",
      );
    ...
    it("username field is optional", () => {
      const usernameField = discordConfig.credentialFields.find(
        (f) => f.key === "username",
      );
    ...
    it("webhook URL validator validates Discord URLs", () => {
      ...
      if (validator) {
        // Valid URLs
        expect(validator("https://discord.com/api/webhooks/123/abc")).toBe(
          true,
        );
        expect(
          validator("https://discord.com/api/webhooks/123456789/abcdef"),
        ).toBe(true);
