...
describe("msteams template", () => {
  ...
  describe("msteamsConfig", () => {
    ...
    it("URL validator rejects invalid URLs", () => {
      ...
      expect(
        msteamsConfig.urlValidator(
          "http://prod-10.westus.logic.azure.com/workflows/abc/triggers/manual/paths/invoke",
        ),
      ).toBe(false);
      expect(msteamsConfig.urlValidator("https://evil.com?outlook.office.com=fake")).toBe(false);
      expect(
        msteamsConfig.urlValidator("https://prod-10.westus.logic.azure.com/not-a-workflow"),
      ).toBe(false);
