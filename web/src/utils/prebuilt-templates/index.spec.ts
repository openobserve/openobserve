...
describe("Prebuilt Templates Index", () => {
  describe("PREBUILT_DESTINATION_TYPES", () => {
    ...
    it("should have proper structure for each type", () => {
      PREBUILT_DESTINATION_TYPES.forEach((type) => {
        ...
        expect(["messaging", "incident", "email", "custom"]).toContain(
          type.category,
        );
      });
    });
  ...
  describe("PREBUILT_CONFIGS", () => {
    it("should have configs for all destination types", () => {
      ...
      const destinationTypes = PREBUILT_DESTINATION_TYPES.map(
        (type) => type.id,
      );
  ...
  describe("detectPrebuiltTypeFromUrl", () => {
    ...
    it("should detect ServiceNow URLs", () => {
      const servicenowUrl =
        "https://dev123.service-now.com/api/now/table/incident";
  ...
  describe("generateDestinationUrl", () => {
    it("should generate correct URLs for different types", () => {
      ...
      expect(generateDestinationUrl("slack", slackCredentials)).toBe(
        "https://hooks.slack.com/test",
      );

      const pagerdutyCredentials = {};
      expect(generateDestinationUrl("pagerduty", pagerdutyCredentials)).toBe(
        "https://events.pagerduty.com/v2/enqueue",
      );

      const opsgenieCredentials = { euRegion: true };
      expect(generateDestinationUrl("opsgenie", opsgenieCredentials)).toBe(
        "https://api.eu.opsgenie.com/v2/alerts",
      );

      const opsgenieCredentialsUs = { euRegion: false };
      expect(generateDestinationUrl("opsgenie", opsgenieCredentialsUs)).toBe(
    ...
    it("should generate URLs for all webhook types", () => {
      expect(
        generateDestinationUrl("discord", {
          webhookUrl: "https://discord.com/api/webhooks/123/abc",
        }),
      ).toBe("https://discord.com/api/webhooks/123/abc");
      expect(
        generateDestinationUrl("msteams", {
          webhookUrl: "https://outlook.office.com/webhook/xxx",
        }),
      ).toBe("https://outlook.office.com/webhook/xxx");
      expect(
        generateDestinationUrl("servicenow", {
          instanceUrl: "https://dev.service-now.com/api/now/table/incident",
        }),
      ).toBe("https://dev.service-now.com/api/now/table/incident");
    });
  });
  ...
  describe("generateDestinationHeaders", () => {
    ...
    it("should not mutate original config headers", () => {
      ...
      const headers2 = generateDestinationHeaders("opsgenie", {
    ...
    it("should have proper credential field structure", () => {
      Object.values(PREBUILT_CONFIGS).forEach((config) => {
        config.credentialFields.forEach((field) => {
          ...
          expect(["text", "password", "email", "select", "toggle"]).toContain(
            field.type,
          );
