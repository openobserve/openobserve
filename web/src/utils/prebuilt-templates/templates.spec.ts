...
} from "@/utils/prebuilt-templates/discord";
import {
  emailTemplate,
  emailConfig,
  emailDestinationType,
} from "@/utils/prebuilt-templates/email";
import {
  msteamsTemplate,
  msteamsConfig,
  msteamsDestinationType,
} from "@/utils/prebuilt-templates/msteams";
import {
  pagerdutyTemplate,
  pagerdutyConfig,
  pagerdutyDestinationType,
} from "@/utils/prebuilt-templates/pagerduty";
import {
  servicenowTemplate,
  servicenowConfig,
...
} from "@/utils/prebuilt-templates/opsgenie";
...
describe("Email template", () => {
  ...
  it("has email credential field", () => {
    const emailField = emailConfig.credentialFields.find(
      (f) => f.key === "recipients",
    );
...
describe("Microsoft Teams template", () => {
  ...
  it("config validates MS Teams webhook URLs", () => {
    expect(
      msteamsConfig.urlValidator("https://outlook.office.com/webhook/"),
    ).toBe(true);
...
describe("PagerDuty template", () => {
  ...
  it("validates PagerDuty URLs", () => {
    expect(
      pagerdutyConfig.urlValidator("https://events.pagerduty.com/v2/enqueue"),
    ).toBe(true);
    expect(pagerdutyConfig.urlValidator("https://example.com")).toBe(false);
  });

  it("has routing key credential field", () => {
    const routingKeyField = pagerdutyConfig.credentialFields.find(
      (f) => f.key === "integrationKey",
    );
...
describe("Opsgenie template", () => {
  ...
  it("config has API key field", () => {
    const apiKeyField = opsgenieConfig.credentialFields.find(
      (f) => f.key === "apiKey",
    );
  ...
  it("validates Opsgenie URLs", () => {
    expect(
      opsgenieConfig.urlValidator("https://api.opsgenie.com/v2/alerts"),
    ).toBe(true);
...
describe("Template consistency", () => {
  const templates = [
    ...
    },
    {
      ...
      config: emailConfig,
      type: emailDestinationType,
    },
    {
      name: "msteams",
      template: msteamsTemplate,
      config: msteamsConfig,
      type: msteamsDestinationType,
    },
    {
      name: "pagerduty",
      template: pagerdutyTemplate,
      config: pagerdutyConfig,
      type: pagerdutyDestinationType,
    },
    {
      name: "servicenow",
      template: servicenowTemplate,
      config: servicenowConfig,
      type: servicenowDestinationType,
    },
    {
      name: "opsgenie",
      template: opsgenieTemplate,
      config: opsgenieConfig,
      type: opsgenieDestinationType,
    },
  ];
