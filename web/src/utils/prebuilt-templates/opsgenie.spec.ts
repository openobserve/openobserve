...
describe("opsgenie template", () => {
  ...
  describe("opsgenieConfig", () => {
    ...
    it("validates Opsgenie URLs", () => {
      ...
      expect(
        opsgenieConfig.urlValidator("https://api.eu.opsgenie.com/v2/alerts"),
      ).toBe(true);
      expect(opsgenieConfig.urlValidator("https://example.com")).toBe(false);
      expect(
        opsgenieConfig.urlValidator("https://api.opsgenie.com/v2/other"),
      ).toBe(false);
    ...
    it("has priority field", () => {
      const priorityField = opsgenieConfig.credentialFields.find(
        (f) => f.key === "priority",
      );
