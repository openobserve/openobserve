...
<template>
  <q-page
    class="tw:w-full tw:h-full tw:px-[0.625rem] tw:pb-[0.625rem] aboutPage q-pt-xs"
  >
    <div class="card-container tw:h-[calc(100vh-50px)] tw:overflow-auto">
      <div class="q-px-sm q-py-sm tw:h-full">
        ...
        <div class="hero-section">
          <div
            class="tw:flex tw:flex-col tw:flex-row tw:items-center tw:justify-between tw:gap-8"
          >
            <div class="tw:flex-1">
              <img
                :src="
              ...
              <div class="tw:flex tw:gap-3 tw:mt-6">
                ...
                <div
                  class="build-badge tw:capitalize"
                  :class="
                    store.state.theme === 'dark'
                      ? 'build-badge-dark'
                      : 'build-badge-light'
            ...
            <div class="stats-grid">
              <div
                class="stat-card stat-card-commit"
                :class="
                  store.state.theme === 'dark'
              ...
              >
                <q-icon name="code"
size="32px" class="stat-icon" />
                <div class="stat-label">{{ t("about.commit_lbl") }}</div>
                <div class="stat-value tw:font-mono">
                  {{ store.state.zoConfig.commit_hash }}
                </div>
              </div>
              <div
                class="stat-card stat-card-built"
                :class="
                  store.state.theme === 'dark'
...
size="16px" class="tw:mr-1" />
          ...
          >
            ...
            <div
              class="tw:mt-4 tw:p-3 tw:rounded tw:bg-opacity-10"
              :class="
                store.state.theme === 'dark'
                  ? 'tw:bg-blue-400'
                  : 'tw:bg-blue-500'
        ...
        >
          <div class="feature-card">
            ...
            <div v-if="loadingLicense" class="tw:text-center tw:py-8">
              ...
              <div class="tw:mt-3 tw:text-sm tw:opacity-70">
                {{ t("about.loading_license_info") }}
              </div>
            </div>

            <div
              v-else-if="!licenseData || !licenseData.license"
              class="tw:py-4"
            >
              <div
                class="tw:flex tw:items-start tw:gap-3 tw:p-4 tw:rounded tw:bg-opacity-10"
                :class="
                  store.state.theme === 'dark'
                    ? 'tw:bg-yellow-400'
                    : 'tw:bg-yellow-500'
              ...
              >
                <q-icon name="warning"
size="24px" class="tw:text-yellow-500" />
                <div>
                  <div class="tw:font-semibold tw:mb-1">
                    {{ t("about.no_license_installed_lbl") }}
                  </div>
                  ...
                  <div
                    ...
                    class="tw:text-xs tw:opacity-70 tw:mb-2"
                  >
                    {{ t("about.installation_id_lbl") }}:
                    <code
                      class="tw:px-2 tw:py-1 tw:rounded tw:bg-black tw:bg-opacity-10"
                      >{{ licenseData.installation_id }}</code
                    >
                  </div>
                </div>
              </div>
...
dense class="compact-table">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          <q-badge
                            :color="licenseData?.expired ? 'red' : 'green'"
                          >
                            {{
                              licenseData?.expired
                                ? t("about.expired_lbl")
                      ...
                      <tr>
                        ...
                        <td>
                          <div
                            class="tw:flex tw:items-center tw:justify-start tw:gap-4"
                          >
                            <span>{{
...
dense class="compact-table">
                    <thead>
                      <tr>
                        <th colspan="2" class="tw:text-center tw:font-semibold">
                    ...
                    <tbody>
                      <tr>
                        <td class="tw:font-semibold">
                          {{ t("about.ingestion_type") }}
                        </td>
                        <td>
                          {{
                            !licenseData?.expired &&
                            licenseData.license.limits?.Ingestion?.typ
                              ? licenseData.license.limits.Ingestion.typ
                              : "PerDayCount"
                          }}
                        </td>
                      ...
                      <tr>
                        <td class="tw:font-semibold">
                          {{ t("about.ingestion_limit") }}
                        </td>
                        <td>
                          {{
                            !licenseData?.expired &&
                            licenseData.license.limits?.Ingestion?.value
                              ? `${licenseData.license.limits.Ingestion.value} GB / day`
                              : "50 GB / day"
                          }}
                        </td>
                      </tr>
