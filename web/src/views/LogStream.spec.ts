...
describe("LogStream Component", () => {
  ...
  describe("User Interactions", () => {
    ...
    it("should handle search input changes", async () => {
      const searchInput = wrapper.find(
        '[data-test="streams-search-stream-input"] input',
      );
  ...
  describe("Stream Management", () => {
    ...
    it("should handle batch deletion", async () => {
      ...
      const positiveCalls = mockNotify.mock.calls.filter(
        (call) =>
  ...
  describe("Error Handling", () => {
    ...
    it("should not show error for 403 responses", async () => {
      ...
      const errorNotifications = mockNotify.mock.calls.filter(
        (call) =>
          call[0].type === "negative" &&
          call[0].message !== "Error while fetching streams.",
      );
  ...
  describe("Time Range Handling", () => {
    it("should handle stream exploration with time ranges", async () => {
      ...
      if (typeof wrapper.vm.exploreStream === "function") {
        ...
        const exploreButton = wrapper.find(
          '[data-test="stream-explore-button"]',
        );
  ...
  describe("Data Transformation and Formatting", () => {
    ...
    it("should filter streams by search query", async () => {
      ...
      const logsStreams = testStreams.filter((s: any) =>
        s.name.includes("logs"),
      );
