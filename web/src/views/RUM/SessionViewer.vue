...
const handleErrorEvent = (event: any) => {
  ...
  _event.name = event.error_message || "Custom Error";
  return _event;
};

const handleActionEvent = (event: any) => {
  const _event = getDefaultEvent(event);
  _event.name = event.action_type + ' on "' + event.action_target_name + '"' || "Custom Action";
...
const handleViewEvent = (event: any) => {
  ...
  // }
  _event.name = event.view_loading_type + " : " + event.view_url || "Custom View";
  return _event;
};
