...
const MockAppTable = {
  name: "AppTable",
  template:
    '<div class="app-table"><slot name="error_details" :column="{ row: mockRow }" /></div>',
...
describe("AppErrors.vue", () => {
    ...
    router = createRouter({
      ...
      routes: [
        { path: "/", name: "home", component: { template: "<div>Home</div>" } },
        {
          path: "/error/:id",
          name: "ErrorViewer",
          component: { template: "<div>Error Viewer</div>" },
        },
      ],
    });
  ...
  describe("Query Execution", () => {
    ...
    it("should reset errors and current page when running query", async () => {
      ...
      expect(
        (wrapper.vm as any).errorTrackingState.data.resultGrid.currentPage,
      ).toBe(0);
    });
  });
  ...
  describe("URL Query Parameters", () => {
    it("should restore URL query parameters on mount", async () => {
      ...
      expect(
        (wrapper.vm as any).errorTrackingState.data.datetime.startTime,
      ).toBe(1640995200000);
      expect((wrapper.vm as any).errorTrackingState.data.datetime.endTime).toBe(
        1640998800000,
      );
    });
    ...
    it("should handle relative time period in URL", async () => {
      ...
      expect(
        (wrapper.vm as any).errorTrackingState.data.datetime.relativeTimePeriod,
      ).toBe("1h");
      expect(
        (wrapper.vm as any).errorTrackingState.data.datetime.valueType,
      ).toBe("relative");
    });
  });
  ...
  describe("Schema Mapping", () => {
    it("should filter user data fields correctly", async () => {
      ...
      expect((wrapper.vm as any).streamFields[0].name).toBe(
        "user_agent_device_brand",
      );
  ...
  describe("SQL Query Building", () => {
    it("should build correct SQL query with error fields", async () => {
      ...
      const getErrorLogsSpy = vi
        .spyOn(wrapper.vm as any, "getErrorLogs")
        .mockImplementation(() => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle missing stream schema", async () => {
      ...
      vi.mocked((wrapper.vm as any).getStream).mockResolvedValueOnce({
        schema: [],
      });
  ...
  describe("User Interactions", () => {
    it("should handle error cell click correctly", async () => {
      ...
      const mockPayload = {
        row: {
          latest_error_id: "test_error_123",
          zo_sql_timestamp: 1640995200000,
        },
      };
      ...
      expect((wrapper.vm as any).errorTrackingState.data.selectedError).toEqual(
        mockPayload.row,
      );
      expect(routerPushSpy).toHaveBeenCalledWith({
        name: "ErrorViewer",
