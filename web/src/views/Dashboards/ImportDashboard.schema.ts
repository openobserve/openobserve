...
//
// Validation schema for ImportDashboard.vue — a deliberate PARTIAL (per the
// dashboards migration brief). The OForm covers only the input-collection
// fields: `url` and `jsonFiles`. The deep JSON-content validation
// (title / stream_type / structure via `validateDashboardJson`) cannot be
// expressed per-field in Zod and stays imperative — it drives the error
// side-panel + recovery inputs, which remain outside this form. Monaco
