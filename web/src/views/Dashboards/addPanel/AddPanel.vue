<template>
	<div style="height: calc(100vh - 57px); overflow-y: auto;" class="scroll">
		<div class="flex justify-between items-center q-pa-sm">
			<div class="q-table__title q-mr-md">{{ editMode ? t("panel.editPanel"): t("panel.addPanel") }}</div>
			<div class="flex items-baseline q-gutter-sm">
				<date-time @date-change="updateDateTime" />
				<q-btn class="q-ml-md q-mb-xs text-bold" outline padding="sm lg" color="red" no-caps :label="t(`Discard`)"
					@click="goBackToDashboardList" />
				<q-btn class="q-ml-md q-mb-xs text-bold" outline padding="sm lg" color="white" text-color="black" no-caps
					:label="t(`Save`)" @click="savePanelOnClick" />
				<q-btn class="q-ml-md q-mb-xs text-bold no-border" padding="sm lg" color="secondary" no-caps :label="t(`Apply`)"
					@click="runQuery" />
			</div>
		</div>
		...
		<div class="row" style="height: calc(100vh - 118px); overflow-y: auto;">
			...
			<div class="col" style="overflow: hidden; min-width: 75px; max-width: 75px;">
...
<script lang="ts">
import { defineComponent, ref, computed, toRaw, onActivated, nextTick } from "vue"
...
import { addPanel, getConsumableDateTime, getPanel, updatePanel } from "../../../utils/commons";
...
export default defineComponent({
	...
	setup() {
		...
		const { dashboardPanelData, resetDashboardPanelData } = useDashboardPanelData()
		...
		onActivated(async () => {
			...
			if(route.query.panelId) {
				editMode.value = true
				const panelData = await getPanel(store, route.query.dashboard, route.query.panelId)
		...
		const savePanelChangesToDashboard = async (
			...
			if(editMode.value) {
				await updatePanel(store, dashId, JSON.parse(JSON.stringify(dashboardPanelData.data)))
				...
				await addPanel(store, dashId, JSON.parse(JSON.stringify(dashboardPanelData.data)))
			}
