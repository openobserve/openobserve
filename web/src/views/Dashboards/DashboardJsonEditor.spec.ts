...
vi.mock("@/components/CodeQueryEditor.vue", async () => {
  ...
  const component = defineComponent({
    name: "QueryEditor",
    template:
      '<div data-test="dashboard-json-editor" class="mocked-query-editor"><textarea v-model="query" @input="onInput"></textarea></div>',
...
describe("DashboardJsonEditor", () => {
    ...
    return mount(DashboardJsonEditor, {
      ...
      global: {
        stubs: {
          ...
          "q-btn": {
            template:
              '<button class="q-btn" @click="$emit(\'click\')" :disabled="disable || disabled"><slot>{{ label }}</slot></button>',
          ...
          "q-icon": {
            template:
              '<i class="q-icon" :data-test="$attrs[\'data-test\']"></i>',
            props: ["name", "size"],
          },
          ...
          QueryEditor: {
            name: "QueryEditor",
            template:
              '<div data-test="dashboard-json-editor" class="query-editor"><slot /></div>',
            props: ["debounceTime", "language", "editorId", "query"],
            emits: ["update:query"],
          },
        },
  ...
  it("should mount successfully with required props", () => {
    ...
    expect(wrapper.find('[data-test="dashboard-json-editor"]').exists()).toBe(
      true,
    );
  });
  ...
  it("should render component with correct structure", () => {
    ...
    expect(wrapper.find('[data-test="json-editor-cancel"]').exists()).toBe(
      true,
    );
  ...
  it("should prevent dashboardId changes", () => {
    ...
    expect(wrapper.vm.validationErrors).toContain(
      "Dashboard ID cannot be modified",
    );
  });
  ...
  it("should successfully save valid JSON", async () => {
    ...
    expect(mockSaveJsonDashboard.execute).toHaveBeenCalledWith(
      mockDashboardData,
    );
  });
  ...
  it("should handle save execution errors", async () => {
    ...
    expect(wrapper.vm.validationErrors).toContain(
      "Failed during JSON save: Save failed",
    );
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Failed during JSON save:",
      saveError,
    );
  ...
  it("should handle non-Error exceptions during save", async () => {
    ...
    expect(wrapper.vm.validationErrors).toContain(
      "Failed during JSON save: Unknown error",
    );
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Failed during JSON save:",
      "String error",
  ...
  it("should handle JSON parsing errors during save", async () => {
    ...
    expect(wrapper.vm.validationErrors[0]).toMatch(
      /Failed during JSON save: .*/,
    );
  ...
  it("should disable save button when JSON is invalid", async () => {
    ...
    expect(
      !wrapper.vm.isValidJson ||
        wrapper.vm.validationErrors.length > 0 ||
        wrapper.vm.saveJsonLoading,
    ).toBe(true);
  });
