...
<template>
  <ODrawer
    ...
    :sub-title="prompt ? t('aiObservability.promptManagement.promptSubtitle', { type: prompt.type }) : raw('')"
    data-test="prompt-detail-drawer"
    @update:open="emit('update:open', $event)"
  >
    <div v-if="prompt" class="flex h-full min-h-0 flex-col">
      <header class="border-b-dialog-header-border flex shrink-0 flex-wrap items-center gap-2 border-b px-5 py-3">
        <OSelect
        ...
        <div class="flex min-w-0 flex-1 flex-wrap gap-1">
          <OTag
            ...
            shape="rounded"
          >
            {{ t("aiObservability.promptManagement.versionLabel", { name: label.name, version: label.version }) }}
          </OTag>
        </div>
        <OButton
          variant="outline"
      ...
      <section class="border-b-dialog-header-border grid shrink-0 grid-cols-4 gap-2.5 border-b px-5 py-4 max-md:grid-cols-2">
        <div v-for="stat in stats" :key="stat.label" class="rounded-default border-border-default bg-surface-base border px-3 py-2">
      ...
      <div class="min-h-0 flex-1 overflow-auto p-5">
        ...
        <template v-else-if="activeVersion && activeTab === 'configuration'">
          <div class="flex flex-col gap-5">
            <section>
              ...
              <pre class="rounded-default border-border-default bg-surface-base max-h-96 overflow-auto border p-3 font-mono text-xs whitespace-pre-wrap">{{ payloadText(activeVersion.payload) }}</pre>
            </section>
            <section>
              <h4 class="text-text-heading mb-2 text-sm font-semibold">
              ...
              <dl class="grid grid-cols-[9rem_minmax(0,1fr)] gap-x-3 gap-y-2 text-xs">
                <dt class="text-text-secondary">{{ t("aiObservability.promptManagement.promptId") }}</dt><dd class="font-mono break-all">{{ prompt.entityId }}</dd>
                <dt class="text-text-secondary">{{ t("aiObservability.promptManagement.versionId") }}</dt><dd class="font-mono break-all">{{ activeVersion.id }}</dd>
                <dt class="text-text-secondary">{{ t("aiObservability.promptManagement.model") }}</dt><dd>{{ activeVersion.config.model || raw("—") }}</dd>
                <dt class="text-text-secondary">{{ t("aiObservability.promptManagement.parameters") }}</dt><dd><OCode block>{{ jsonText(activeVersion.config.params) }}</OCode></dd>
                <dt class="text-text-secondary">{{ t("aiObservability.promptManagement.tools") }}</dt><dd><OCode block>{{ jsonText(activeVersion.config.tools) }}</OCode></dd>
                <dt class="text-text-secondary">{{ t("aiObservability.promptManagement.responseFormat") }}</dt><dd><OCode block>{{ jsonText(activeVersion.config.responseFormat) }}</OCode></dd>
              </dl>
            </section>
            <section>
              ...
              <div v-if="showLabelEditor" class="rounded-default border-border-default flex flex-col gap-2 border p-3">
                <div class="grid grid-cols-[minmax(0,1fr)_8rem_auto] gap-2">
                <OInput v-model="labelName" :placeholder="t('aiObservability.promptManagement.labelName')" data-test="prompt-label-name" />
                  <OSelect v-model="labelVersion" :options="versionOptions" label-key="label" value-key="value" />
                <OButton variant="primary" size="sm" :disabled="!labelName.trim()" @click="moveLabel">
                  {{ t("aiObservability.promptManagement.assign") }}
                </OButton>
                </div>
                <div v-for="label in movableLabels" :key="label.name" class="flex items-center gap-2 text-xs">
                <span class="min-w-0 flex-1">{{ t("aiObservability.promptManagement.versionLabel", { name: label.name, version: label.version }) }}</span>
                  <OButton
            ...
            <section>
              ...
              <div v-for="entry in activity" :key="entry.id" class="border-b-border-default flex gap-2 border-b py-2 text-xs">
        ...
        <template v-else-if="activeTab === 'versions'">
          <div class="mb-3 flex justify-end">
            <OSelect v-model="sourceFilter" :options="sourceOptions" label-key="label" value-key="value" class="w-48" />
          </div>
          <div class="flex flex-col gap-2">
            ...
            >
              <button type="button" class="text-accent font-semibold" @click="selectedVersion = version.version">
              ...
              <div class="min-w-0 flex-1">
                ...
                {{ version.source }}{{ raw(" · ") }}{{ version.createdBy }}<template v-if="version.baseVersion">{{ t("aiObservability.promptManagement.fromVersion", { version: version.baseVersion }) }}</template>
                </div>
              </div>
              <div class="flex flex-wrap justify-end gap-1">
                <OTag v-for="label in labelsForVersion(version.version)" :key="label" variant="default-soft">{{ label }}</OTag>
        ...
        <template v-else-if="activeTab === 'compare'">
          <div class="mb-4 flex items-end gap-2">
            <OSelect v-model="compareVersions" multiple :options="versionOptions" label-key="label" value-key="value" class="max-w-md flex-1" />
            <span class="text-text-secondary text-xs">
              {{ t("aiObservability.promptManagement.selectTwoVersions") }}
            </span>
          </div>
          <PromptVersionDiff v-if="compared[0] && compared[1]" :left="compared[0]" :right="compared[1]" />
    ...
    <template v-if="prompt" #footer>
      <div class="flex w-full items-center justify-between">
        ...
        <OButton v-if="prompt.status === 'active'" variant="outline" size="sm" @click="emit('archive')">
...
const visibleLabels = computed(() => props.prompt?.labels.filter((label) => label.version != null) ?? []);
const movableLabels = computed(() => visibleLabels.value.filter((label) => label.name !== "latest"));
...
const comparisonVersionNumbers = computed<[number, number] | null>(() =>
  compared.value.length === 2
    ? [compared.value[0].version, compared.value[1].version]
    : null,
);
...
function labelsForVersion(version: number): string[] {
  return visibleLabels.value.filter((label) => label.version === version).map((label) => label.name);
}

function confirmProtectedLabel(label: string, action: "move" | "delete"): boolean {
  if (!props.protectedLabels.includes(label)) return true;
  return window.confirm(
    `${label} is a protected label. Confirm that you want to ${action} it.`,
  );
}
...
async function load() {
  ...
  try {
    ...
    toast({ variant: "error", message: raw(error instanceof Error ? error.message : "Failed to load prompt.") });
  } finally {
    loading.value = false;
  }
}
...
async function moveLabel() {
  ...
  const existing = props.prompt.labels.find((label) => label.name === labelName.value.trim());
  if (!confirmProtectedLabel(labelName.value.trim(), "move")) return;
  try {
    ...
    toast({ variant: "error", message: raw(error instanceof Error ? error.message : "Failed to move label.") });
  }
}

async function deleteLabel(label: PromptLabel) {
  if (!props.prompt || label.version == null || label.name === "latest") return;
  if (!confirmProtectedLabel(label.name, "delete")) return;
  try {
    ...
    toast({ variant: "error", message: raw(error instanceof Error ? error.message : "Failed to delete label.") });
  }
}
