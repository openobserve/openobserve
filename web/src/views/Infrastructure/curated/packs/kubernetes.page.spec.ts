...
describe("drift variants (§10 pass-1 finding 1)", () => {
  it("node CPU and node memory each carry exactly 2 variants of the right families", () => {
    for (const id of ["k8s_nd_cpu", "k8s_ov_node_cpu_top", "k8s_ov_nodes", "k8s_ov_fleet_cpu"]) {
  ...
  it("the usage variant overrides the unit — cores in use is not a ratio", () => {
    ...
    expect(panel("k8s_ov_fleet_cpu").variants[1].unit).toBe("numbers");
  });
});
...
describe("DRY-RUN finding 7 — topk bounds rows only on an instant vector", () => {
  ...
  it("the four tables are the ONLY table panels — every other panel is a chart", () => {
    const tables = allPanels().filter((p: any) => p.type === "table").map((p: any) => p.id);
...
describe("DRY-RUN finding 10 — five phases exist, so the tiles are not a partition", () => {
  ...
  it("the Running/Pending/Failed tiles each declare a *Sub subtitle key", () => {
    expect(panel("k8s_ov_pods_running").subtitleKey).toBe("infra.k8s.panel.podsRunningSub");
    expect(panel("k8s_ov_pods_pending").subtitleKey).toBe("infra.k8s.panel.podsPendingSub");
    expect(panel("k8s_ov_pods_failed").subtitleKey).toBe("infra.k8s.panel.podsFailedSub");
  });
});
...
describe("declarative scoping (pass-1 finding 3)", () => {
  it("Overview and Nodes declare `cluster` only — fleet tiles stay fleet-wide BY DECLARATION", () => {
    ...
    // The fleet phase chart carries no scope token even though its section has one.
    expect(queriesOf("k8s_ov_pods_by_phase")[0]).not.toContain("${scope:");
  });
