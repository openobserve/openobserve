    ...
    <div class="ov-body">
      ...
      <div class="ov-kpi-strip">
        ...
        >
          ...
          <div class="ov-kpi-val" :class="{ 'ov-kpi-val--red': kpi.severity === 'critical', 'ov-kpi-val--orange': kpi.severity === 'medium' }">
            ...
            <span v-else>{{ formatNum(Number(kpi.value)) }}</span>
          </div>
      ...
      <div class="ov-row-2">
        ...
        <div class="ov-card ov-card--third">
          ...
          <div class="ov-card-body">
            <div v-if="loading" class="ov-skeleton-rows" />
            ...
            <div v-else class="ov-empty-state">
              <OIcon name="check-circle-outline" size="xl" class="text-green-500 opacity-50" />
          ...
          <div class="ov-card-body">
            ...
            <template v-else>
              ...
              <div class="flex items-end gap-3 mb-4">
                <div>
                  <div class="text-3xl font-extrabold text-text-primary">{{ enabledAlerts.length }}</div>
                  <div class="text-text-tertiary text-xs">active of {{ alerts.length }} total</div>
                </div>
                <div class="flex-1">
                  <div class="ov-rule-bar-track">
                    <div class="ov-rule-bar-fill" :style="{ width: alerts.length ? `${Math.round(enabledAlerts.length / alerts.length * 100)}%` : '0%' }" />
                  </div>
                  <div class="text-text-tertiary text-xs mt-1">
                    {{ alerts.length ? Math.round(enabledAlerts.length / alerts.length * 100) : 0 }}% coverage
                  </div>
                </div>
              </div>
              <!-- Top rules -->
              <div class="flex flex-col gap-1.5">
                <div v-for="rule in alerts.slice(0, 5)" :key="rule.uuid ?? rule.name" class="flex items-center gap-2">
                  <div :class="['ov-rule-dot', rule.enabled ? 'ov-rule-dot--on' : 'ov-rule-dot--off']" />
                  <span class="text-text-secondary text-xs truncate flex-1">{{ rule.name }}</span>
            ...
            <template v-else>
              ...
              <div class="flex items-center gap-4 mb-4">
                <div class="text-center">
                  <div class="text-2xl font-extrabold" :class="silentSources > 0 ? 'text-orange-500' : 'text-green-500'">
                    {{ sourceHealth.filter(s => s.healthy).length }}
                  </div>
                  <div class="text-text-tertiary text-xs">healthy</div>
                </div>
                <div class="h-10 w-px bg-border-default" />
                <div class="text-center">
                  <div class="text-2xl font-extrabold" :class="silentSources > 0 ? 'text-orange-500' : 'text-text-tertiary'">
                    {{ silentSources }}
                  </div>
                  <div class="text-text-tertiary text-xs">silent</div>
                </div>
                <div class="flex-1" />
                <div v-if="silentSources === 0" class="text-green-500 text-xs font-semibold">All OK</div>
              </div>
              <!-- Source list -->
              <div class="flex flex-col gap-1.5">
                <div
                  v-for="src in sourceHealth.slice(0, 6)" :key="src.name"
                ...
                >
                  ...
                  <span class="text-text-tertiary text-xs tabular-nums">{{ formatNum(src.docCount) }}</span>
                </div>
      ...
      <div class="ov-card">
        ...
        <template v-else-if="secStreams.length">
          <div
            v-for="s in secStreams.slice(0, 8)" :key="s.name"
            class="ov-stream-row"
            @click="nav('/security/events', { stream: s.name })"
          >
            <div :class="['ov-health-dot', (s.stats?.doc_num ?? 0) > 0 ? 'ov-health-dot--ok' : 'ov-health-dot--warn']" />
            <span class="text-text-primary text-xs font-medium truncate flex-1">{{ s.name }}</span>
            <span class="text-text-tertiary text-xs tabular-nums">{{ formatNum(s.stats?.doc_num ?? 0) }} events</span>
            <span class="text-text-tertiary text-xs ml-2">{{ fmtBytes(s.stats?.storage_size) }}</span>
            <OIcon name="arrow-forward" size="xs" class="text-text-tertiary opacity-30 ml-2" />
          </div>
          <div v-if="secStreams.length > 8" class="ov-more-row" @click="nav('/security/events')">
            +{{ secStreams.length - 8 }} more streams
          </div>
        </template>
        <div v-else class="ov-empty-state py-8">
...
.ov-card--third { /* no extra styles needed */ }
...
/* ── Detection rule bar ──────────────────────────────────────────────────── */
.ov-rule-bar-track { height: 6px; border-radius: 99px; background: rgba(0,0,0,0.07); overflow: hidden; }
.dark .ov-rule-bar-track { background: rgba(255,255,255,0.07); }
.ov-rule-bar-fill { height: 100%; background: #6d5ce0; border-radius: 99px; transition: width 0.4s ease; }
...
.ov-more-row {
  padding: 8px 14px; font-size: 11px; color: #6d5ce0; cursor: pointer; text-align: center;
  border-top: 1px dashed var(--color-border-default, rgba(0,0,0,0.06));
  font-weight: 600;
}
.ov-more-row:hover { background: rgba(109,92,224,0.05); }
