  ...
  <q-page class="q-pa-lg">
    ...
    <div v-if="no_data_ingest" class="q-pa-md row items-start q-gutter-md">
      ...
      <q-card class="my-card">
        ...
        <!--<q-card-actions align="center">
          <q-btn no-caps color="primary" @click="() => $router.push('/ingestion/fluentbit')" flat>{{ t('home.findIngestion')
          }}      
          </q-btn>
        </q-card-actions>-->
      </q-card>
    ...
    <!--<div class="row">
      <div class="col-6">
        <q-btn label="Find how to ingest logs" color="primary" @click="() => $router.push('/ingestion/fluentbit')" />
      </div>
    </div>-->
  </q-page>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getSummary = (org_id: any) => {
      ...
      orgService.get_organization_summary(org_id).then((res) => {
        if (res.data.streams.length == 0 && res.data.functions.length == 0 && res.data.alerts.length == 0) {
        ...
        if (res.data.streams.length > 0) {
          sum = res.data.streams.reduce((acc: number, val: {
            [x: string]: any; storage_size: any;
          }) => {
