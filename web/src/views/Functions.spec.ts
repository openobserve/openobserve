...
const MockRouterView = {
  name: "RouterView",
  template:
    '<div class="mock-router-view"><slot v-bind="{ Component: MockComponent }" /></div>',
  setup() {
    const MockComponent = {
      name: "MockComponent",
...
describe("Functions.vue", () => {
    ...
    return mount(Functions, {
      global: {
        ...
        stubs: {
          ...
          "q-tabs": {
            template:
              '<div class="q-tabs" :class="$attrs.class"><slot /></div>',
            props: ["modelValue", "indicatorColor", "inlineLabel", "vertical"],
          },
          "q-route-tab": {
            template:
              '<div class="q-route-tab" :class="{ active: name === activeTab }">{{ label }}</div>',
  ...
  describe("Store Integration", () => {
    it("should access store state correctly", async () => {
      ...
      expect(
        (wrapper.vm as any).store.state.selectedOrganization.identifier,
      ).toBe("test-org-id");
      expect((wrapper.vm as any).store.state.selectedOrganization.name).toBe(
        "Test Organization",
      );
    });
