...
vi.mock("@/components/SanitizedHtmlRenderer.vue", () => ({
  default: {
    ...
    props: ["htmlContent"],
    template:
      '<div class="sanitized-html" data-test="sanitized-html">{{ htmlContent }}</div>',
  },
}));
...
describe("MemberSubscription.vue", () => {
  ...
  const createWrapper = (
    routeHash = "#token=test-token-123",
    preventAutoProcess = false,
  ) => {
    // Create store
    store = createStore({
      state: {
    ...
    if (preventAutoProcess) {
      vi.spyOn(wrapperInstance.vm, "ProcessSubscription").mockImplementation(
        () => Promise.resolve(),
      );
    }
  ...
  describe("Template Rendering", () => {
    it("should show processing message when status is processing", async () => {
      ...
      mockOrganizationsService.process_subscription.mockReturnValue(
        delayedPromise,
      );
  ...
  describe("ProcessSubscription Method", () => {
    ...
    it("should handle successful subscription without data", async () => {
      ...
      const processSubscriptionSpy = vi.spyOn(
        wrapper.vm,
        "ProcessSubscription",
      );
      ...
      expect(
        mockOrganizationsService.process_subscription,
      ).toHaveBeenCalledWith("test-token", "confirm", "test-org-id");
      expect(wrapper.vm.status).toBe("completed");
    });
    ...
    it("should handle successful subscription with organization data", async () => {
      ...
      expect(window.location.href).toBe(
        "/organizations?org_identifier=test-org-id",
      );
    });
    ...
    it("should handle subscription error", async () => {
      ...
      mockOrganizationsService.process_subscription.mockRejectedValue(
        mockError,
      );
      ...
      expect(wrapper.vm.status).toBe("error");
      expect(wrapper.vm.error).toBe(
        "Subscription failed with http://localhost:3000 error",
      );
    });
    ...
    it("should replace [BASE_URL] placeholder in error message", async () => {
      ...
      expect(wrapper.vm.error).toBe(
        "Error occurred at http://localhost:3000/api/endpoint",
      );
    });
  });
