# Quasar Stepper Components → O2 Migration Guide
...
| ---------------------- | --------------------------------------------- | ----------------- | ---------------- | -------------------- |
| `q-stepper`            | `OStepper`                                    | `StepperRoot`     | 5 files          | ⏳ Pending build     |
| `q-step`               | `OStep`                                       | `StepperItem`     | 9 step instances | ⏳ Pending build     |
| `q-stepper-navigation` | **Removed** — buttons go outside `<OStepper>` | —                 | 9 instances      | N/A — pattern change |

> All O2 stepper components will live in `web/src/lib/navigation/Stepper/`.

---

## Architecture Decision — Navigation Placement
...
```
┌─ BEFORE (Quasar)                 ┌─ AFTER (O2)
│  <q-stepper v-model="step">      │  <OStepper v-model="step">
│    <q-step :name="1">            │    <OStep :name="1">
│      <!-- content -->            │      <!-- content -->
│      <q-stepper-navigation>      │    </OStep>
│        <OButton @click="step=2"> │    <OStep :name="2">
│      </q-stepper-navigation>     │      <!-- content -->
│    </q-step>                     │    </OStep>
│    <q-step :name="2"> ... </q-step> │  </OStepper>
│  </q-stepper>                    │
│                                  │  <!-- navigation BELOW stepper -->
│                                  │  <div class="tw:flex tw:gap-2 ...">
│                                  │    <OButton v-if="step > 1" @click="step--">Back</OButton>
│                                  │    <OButton v-if="!isLastStep" @click="step++">Continue</OButton>
│                                  │    <OButton v-if="isLastStep" @click="save">Save</OButton>
│                                  │  </div>
```
...
### 1. `q-stepper` → `OStepper`

**Family**: `OStepper` + `OStep` (compound — build together)  
...
| ------------------ | ------------------------ | ----------------------------------------------------- |
| `v-model` (number) | `v-model` (number)       | Direct — current step index                           |
| `vertical`         | `orientation="vertical"` | Rename — `"horizontal"` is default                    |
| `animated`         | `animated`               | Direct — `true` by default in O2                      |
| `flat`             | —                        | **Drop** — O2 has one clean style (no raised variant) |
| `header-nav`       | `navigable`              | Rename — allows clicking completed step headers       |
| `color`            | —                        | **Drop** — design tokens control indicator color      |
| `ref`              | `ref`                    | Direct — same template ref pattern                    |
...
| ------------------------------------------ | ------------------------------------------------------------------ |
| `flat`                                     | O2 uses a borderless design by default — no raised / flat variants |
| `color`                                    | Design tokens (`--o2-color-primary`) control the indicator color   |
| `keep-color`                               | Design tokens always active                                        |
| `contracted`                               | Not needed — O2 uses a clean compact header design                 |
...
| Quasar prop        | O2 prop                    | Action                                                |
| ------------------ | -------------------------- | ----------------------------------------------------- |
| `:name`            | `:name`                    | Direct — step index number                            |
| `:title` / `title` | `:title`                   | Direct                                                |
| `icon`             | `:icon` (Lucide component) | **See Icon Migration below**                          |
| `:done`            | `:done`                    | Direct — pass `step > N` expression                   |
| `data-test`        | `data-test`                | Direct pass-through                                   |
| `:header-nav`      | `:navigable`               | Rename — per-step override of root `navigable`        |
| `v-if`             | `v-if`                     | Direct — conditional steps work the same way          |
| `caption`          | `description`              | Rename — subtitle below the step title in header      |
| `class`            | `class`                    | Direct pass-through                                   |
| `error`            | `error`                    | Direct — shows error state in header indicator        |
| `error-icon`       | —                          | **Drop** — error icon is baked into the design        |
| `active-icon`      | —                          | **Drop** — active state icon is baked into the design |
| `done-icon`        | —                          | **Drop** — done state icon (checkmark) is baked in    |
...
### 3. `q-stepper-navigation` → **Remove** (no O2 equivalent)
...
```vue
<!-- BEFORE — buttons inside each q-step via q-stepper-navigation -->
<q-step :name="1" ...>
  <!-- step content -->
...
<!-- AFTER — single navigation block below OStepper -->
...
| `animated`   | `animated`               | unchanged — default `true`          |
| `flat`       | —                        | drop                                |
...
| `icon="material-name"`                     | `:icon="LucideComponent"` | icon → Lucide component ref |
| `:done`                                    | `:done`                   | unchanged                   |
...
> Already used by OrgStorageEditor and CreateDestinationForm — minimal changes needed.
...
<!-- Form buttons — unchanged, already in correct position -->
<div class="tw:flex tw:gap-2">
  <OButton v-if="step === 1" @click="nextStep">Continue</OButton>
  <OButton v-if="step === 2" @click="step--">Back</OButton>
  <OButton v-if="step === 2" @click="save">Save</OButton>
...
```vue
<!-- BEFORE (CreateReport pattern — 3 steps, each with q-stepper-navigation) -->
<q-stepper
  v-model="step"
  vertical
  color="primary"
...
<OStepper v-model="step" orientation="vertical" animated navigable>
  <OStep data-test="add-report-select-dashboard-step" :name="1" title="Select Dashboard"
    ...
    <!-- dashboard selection content only — no navigation here -->
  </OStep>
...
```
Tier 1 — Horizontal steppers (smallest blast radius, navigation already outside)
...
Tier 2 — Vertical steppers (requires consolidating q-stepper-navigation)
3. AddCipherKey.vue            (2 steps — simplest vertical case)
4. CreateReport.vue            (3 steps — has conditional step 3)
5. EditScript.vue              (4 steps — most complex, conditional step 2)
