...
describe('OStepper', () => {
  ...
  it('renders a checkmark indicator on completed steps', async () => {
    const wrapper = mountStepper({ modelValue: 2 })
    await wrapper.vm.$nextTick()
    // Step 1 is done (active > 1 → done=true), so its indicator should have a Check icon
  ...
  it('disables step header buttons when navigable=false and step is done', async () => {
    ...
    // Step 1 is done but navigable=false → its trigger should be disabled
  ...
  it('navigating to a done step changes the active step', async () => {
    // Start at step 2 — step 1 is done (active > 1 = true), navigable=true
    ...
    // Also check triggerClasses — if 'cursor-pointer' is set but disabled is still true,
    ...
    // v-model on the Wrapper updates active → step 1 content now visible
    expect(wrapper.text()).toContain('Step 1 content')
  })
  ...
  it('does not render the horizontal header list in vertical orientation', async () => {
    ...
    await wrapper.vm.$nextTick()
    // Horizontal header uses role="list" — should not be present in vertical mode
