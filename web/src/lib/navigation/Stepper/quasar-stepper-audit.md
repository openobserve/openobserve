# Quasar Stepper Components — Props & Emits Audit
...
| `vertical`        | 3     | EditScript, AddCipherKey, CreateReport                   | Vertical layout — steps stacked top-to-bottom                   |
...
| Prop               | Count | Notes                                                                                            |
| ------------------ | ----- | ------------------------------------------------------------------------------------------------ |
| `:name`            | 9     | Required — numeric step index (1, 2, 3, 4)                                                       |
| `:title` / `title` | 9     | Step header label — static string or dynamic `:title` expression                                 |
| `icon`             | 9     | Material icon name displayed in the step indicator dot                                           |
| `:done`            | 9     | Boolean — `step > N` pattern marks step as completed (shows checkmark)                           |
| `data-test`        | 7     | Static test attribute for E2E selectors                                                          |
| `:data-test`       | 0     | Not used — only static `data-test`                                                               |
| `:header-nav`      | 2     | On individual step — conditionally allows/disallows clicking that step header                    |
| `v-if`             | 2     | Conditional step — shown only when certain state is true (e.g., `formData.type === 'scheduled'`) |
...
`<q-stepper-navigation>` is a thin layout wrapper placed at the bottom of a `<q-step>`'s default slot. It provides no props or events of its own — its only purpose is to group the Back / Continue / Save buttons for a given step.

| Attribute           | Count | Notes                                         |
| ------------------- | ----- | --------------------------------------------- |
| `class="q-pa-none"` | 1     | AddCipherKey step 2 — removes default padding |
| (no attributes)     | 8     | Plain wrapper with no attributes              |

Total instances: **9** across 3 files (EditScript ×4, AddCipherKey ×2, CreateReport ×3).

> **OrgStorageEditor** and **CreateDestinationForm** do **not** use `<q-stepper-navigation>` — their navigation buttons are placed **outside** the `<q-stepper>` block entirely (below the closing `</q-stepper>` tag). This is the preferred pattern for O2 migration.
...
| `src/components/pipeline/NodeForm/CreateDestinationForm.vue` | `.q-stepper__header`, `.q-stepper__tab`, `.q-stepper__tab--active`, `.q-stepper__tab--done`, `.q-stepper__dot`, `.q-stepper__step-inner`                                                                       | Scoped under `.modern-stepper` — overrides applied to its own `<q-stepper flat class="modern-stepper">`. These will be replaced by Tailwind/O2 design tokens.      |
...
| ------------------------------------------------------------ | ----------------- | ------------------------------------------- | --------------------------- | ---------------------------------------------------------------------- |
| `src/components/actionScripts/EditScript.vue`                | vertical          | 4 (step 2 conditional on `formData.type`)   | Yes (×4)                    | `header-nav` enabled, 4-step wizard                                    |
| `src/components/cipherkeys/AddCipherKey.vue`                 | vertical          | 2                                           | Yes (×2)                    | `header-nav` enabled, 2-step wizard                                    |
...
| `src/components/reports/CreateReport.vue`                    | vertical          | 3 (step 3 conditional on `!isCachedReport`) | Yes (×3)                    | `header-nav` enabled, 3-step wizard                                    |
