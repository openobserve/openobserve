...
export default meta;
type Story = StoryObj<typeof OFormTextarea>;

export const Playground: Story = {
  render: (args) => ({
    components: { OFormTextarea, OForm },
    setup() {
      const demoSchema = z.object({ demo: z.any() });
      return { args, demoSchema };
    },
    template: `<OForm :schema="demoSchema" class="w-full max-w-md"><OFormTextarea name="demo" :label="'Notes'" v-bind="args"></OFormTextarea></OForm>`,
  }),
};
