...
describe("OFormSelect", () => {
  ...
  it("renders inside OForm without errors", () => {
    wrapper = mount(OForm, {
      ...
      slots: {
        default: "<OFormSelect name=\"kind\" :options=\"[{label:'A',value:'a'}]\" />",
      },
      global: {
  ...
  it("forwards `loading` through to OSelect", () => {
    ...
    expect(wrapper.findComponent(OSelect).findComponent(OSpinner).exists()).toBe(true);
  });
  ...
  it("paints the select from a schema issue at a nested path (submit-gated)", async () => {
    ...
    expect(wrapper.text()).toContain("Column is required when using an aggregate function.");
  });
  ...
  it("forwards other re-declared props (optionTooltip, labelPosition) through to OSelect", () => {
    ...
    expect(wrapper.findComponent(OSelect).props("labelPosition")).toBe("inside");
  });
});
