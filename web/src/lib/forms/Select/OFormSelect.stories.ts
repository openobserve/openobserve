...
export default meta;
type Story = StoryObj<typeof OFormSelect>;

export const Playground: Story = {
  render: (args) => ({
    components: { OFormSelect, OForm },
    setup() {
      const demoSchema = z.object({ demo: z.any() });
      const sampleOptions = [{ label: 'Logs', value: 'logs' }, { label: 'Metrics', value: 'metrics' }];
      return { args, sampleOptions, demoSchema };
    },
    template: `<OForm :schema="demoSchema" class="w-full max-w-md"><OFormSelect name="demo" :options="sampleOptions" v-bind="args"></OFormSelect></OForm>`,
  }),
};
