...
const getOptionAt = (i: number) => document.querySelectorAll('.o2-select__option')[i] as HTMLElement
...
describe('O2Select', () => {
  ...
  afterEach(() => {
    ...
    document.querySelectorAll('.o2-select__dropdown').forEach((el) => el.remove())
  })
  ...
  describe('Pattern D — simple dropdown', () => {
    ...
    it('should show placeholder when no value selected', () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, placeholder: 'Pick one' })
    ...
    it('should close dropdown on Escape key', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS })
      await openDropdown(wrapper)
      await wrapper.find('.o2-select__control').trigger('keydown', { key: 'Escape' })
      await flushPromises()
      expect(getDropdown()).toBeNull()
    })

    it('should apply data-test to root element', () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, 'data-test': 'my-select' })
      expect(wrapper.find('[data-test="my-select"]').exists()).toBe(true)
      expect(wrapper.find('.o2-select__control[data-test]').exists()).toBe(false)
    })
  ...
  describe('emit-value + map-options', () => {
    ...
    it('should mark the matching option as selected', async () => {
      ...
      await openDropdown(wrapper)
      expect(getOptionAt(2).classList.contains('o2-select__option--selected')).toBe(true)
      expect(getOptionAt(0).classList.contains('o2-select__option--selected')).toBe(false)
    })
  ...
  describe('Pattern B — multiple selection', () => {
    it('should not close dropdown after selecting when multiple', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, multiple: true, modelValue: [] })
      await openDropdown(wrapper)
      getOptionAt(0).click()
      await flushPromises()
      expect(getDropdown()).not.toBeNull()
    ...
    it('should remove from array when selecting an already-selected option', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, multiple: true, modelValue: ['Alpha', 'Beta'] })
      await openDropdown(wrapper)
    ...
    it('should mark multiple selected options', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, multiple: true, modelValue: ['Alpha', 'Gamma'] })
      await openDropdown(wrapper)
      expect(getOptionAt(0).classList.contains('o2-select__option--selected')).toBe(true)
      expect(getOptionAt(1).classList.contains('o2-select__option--selected')).toBe(false)
      expect(getOptionAt(2).classList.contains('o2-select__option--selected')).toBe(true)
    })
  ...
  describe('Pattern B — chips display', () => {
    ...
    it('should remove last chip on Backspace with empty input', async () => {
      ...
      await wrapper.find('.o2-select__control').trigger('keydown', { key: 'Backspace' })
      await flushPromises()
      expect(wrapper.emitted('update:modelValue')![0]).toEqual([['Alpha']])
    })
  })
  ...
  describe('Pattern A — search and filter', () => {
    ...
    it('should emit @filter event with input value and update callback', async () => {
      const filterHandler = vi.fn((val: string, update: (fn: () => void) => void) => {
      ...
      await input.trigger('input')
      await flushPromises()
      expect(filterHandler).toHaveBeenCalledWith('alp', expect.any(Function), expect.any(Function))
    })

    it('should emit @filter with empty string on open (Quasar-compatible)', async () => {
      const filterHandler = vi.fn((_val: string, update: (fn: () => void) => void) => {
        update(() => {})
      })
      wrapper = mount(O2Select, {
        props: { options: SIMPLE_OPTIONS, useInput: true },
      ...
      await openDropdown(wrapper)
      expect(filterHandler).toHaveBeenCalledWith('', expect.any(Function), expect.any(Function))
    })
  ...
  describe('input debounce', () => {
    ...
    it('should debounce @filter emission', async () => {
      ...
      expect(filterHandler).toHaveBeenCalledWith('al', expect.any(Function), expect.any(Function))
    })
  })
  ...
  describe('Pattern C — tagging and new value creation', () => {
    it('should emit @new-value when Enter pressed with no matching option', async () => {
      ...
      wrapper = mount(O2Select, {
        props: { options: SIMPLE_OPTIONS, useInput: true, newValueMode: 'add-unique' },
      ...
      await input.trigger('input')
      await wrapper.find('.o2-select__control').trigger('keydown', { key: 'Enter' })
      await flushPromises()
      expect(newValueHandler).toHaveBeenCalledWith('NewTag', expect.any(Function))
    })
  })
  ...
  describe('Pattern F — validation', () => {
    ...
    it('should show error after validate() called with no value', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, rules: [required], modelValue: null })
      const exposed = wrapper.vm as unknown as { validate: () => boolean }
      exposed.validate()
      await flushPromises()
      expect(wrapper.find('.o2-select--error').exists()).toBe(true)
      expect(wrapper.find('.o2-select__error-text').text()).toContain('Required')
    })

    it('should return true from validate() when value is valid', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, rules: [required], modelValue: 'Alpha' })
      const exposed = wrapper.vm as unknown as { validate: () => boolean }
  ...
  describe('clearable', () => {
    it('should show clear button when value present and clearable', () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, modelValue: 'Alpha', clearable: true })
      expect(wrapper.find('.o2-select__clear').exists()).toBe(true)
    })

    it('should hide clear button when no value', () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, modelValue: null, clearable: true })
  ...
  describe('disabled and loading states', () => {
    ...
    it('spinner should have aria-hidden', () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS, loading: true })
      expect(wrapper.find('.o2-select__spinner').attributes('aria-hidden')).toBe('true')
    })
  })
  ...
  describe('option-disable', () => {
    it('should apply disabled class to disabled options', async () => {
      wrapper = mountSelect({ options: DISABLED_OPTIONS, emitValue: true, mapOptions: true })
      await openDropdown(wrapper)
      expect(getOptionAt(1).classList.contains('o2-select__option--disabled')).toBe(true)
      expect(getOptionAt(0).classList.contains('o2-select__option--disabled')).toBe(false)
    })
  ...
  describe('Pattern E — custom slots', () => {
    ...
    it('should render default no-option text when options empty and no slot', async () => {
      ...
      await openDropdown(wrapper)
      expect(document.querySelector('.o2-select__no-option')?.textContent?.trim()).toBe('No options')
    })

    it('should render #hint slot', () => {
      wrapper = mountSelect({}, { hint: '<span data-test="hint">Helper text</span>' })
  ...
  describe('keyboard navigation', () => {
    it('should open dropdown on ArrowDown key', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS })
      await wrapper.find('.o2-select__control').trigger('keydown', { key: 'ArrowDown' })
      await flushPromises()
      expect(getDropdown()).not.toBeNull()
    })
    ...
    it('should navigate options with ArrowDown', async () => {
      ...
      await wrapper.find('.o2-select__control').trigger('keydown', { key: 'ArrowDown' })
      await flushPromises()
      expect(getOptionAt(0).classList.contains('o2-select__option--focused')).toBe(true)
    })
  ...
  describe('exposed API', () => {
    it('should expose validate()', () => {
      wrapper = mountSelect()
      expect(typeof (wrapper.vm as unknown as { validate: () => boolean }).validate).toBe('function')
    })

    it('should expose resetValidation()', () => {
      wrapper = mountSelect()
      expect(
        typeof (wrapper.vm as unknown as { resetValidation: () => void }).resetValidation,
      ).toBe('function')
    })
  ...
  describe('accessibility', () => {
    it('should have role=combobox on control', () => {
      wrapper = mountSelect()
      expect(wrapper.find('.o2-select__control').attributes('role')).toBe('combobox')
    })

    it('should have aria-expanded=false when closed', () => {
      wrapper = mountSelect()
      expect(wrapper.find('.o2-select__control').attributes('aria-expanded')).toBe('false')
    })

    it('should have aria-expanded=true when open', async () => {
      wrapper = mountSelect({ options: SIMPLE_OPTIONS })
      await openDropdown(wrapper)
      expect(wrapper.find('.o2-select__control').attributes('aria-expanded')).toBe('true')
    })
