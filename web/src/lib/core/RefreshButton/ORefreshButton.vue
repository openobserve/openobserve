...
const updateRelativeTime = () => {
  if (!props.lastRunAt) { relativeTime.value = ''; return }
  relativeTime.value = getRelativeTime(props.lastRunAt)
}

// green < 30s, amber 30s–5min, red > 5min
const dotColor = computed(() => {
  if (props.loading) return 'tw:bg-[var(--color-text-disabled)]'
  const s = diffSeconds()
  if (s === Infinity) return 'tw:bg-[var(--color-text-disabled)]'
  if (s < 30)   return 'tw:bg-[var(--color-success-500)]'
  if (s < 300)  return 'tw:bg-[var(--color-warning-500)]'
  return 'tw:bg-[var(--color-error-500)]'
})
...
<template>
  <div class="tw:inline-flex tw:items-center tw:gap-1.5">
    <!-- staleness dot -->
    <span
      :class="['tw:size-2 tw:rounded-full tw:shrink-0 tw:transition-colors tw:duration-700', dotColor]"
      :title="dotTitle"
    />
    <!-- relative timestamp -->
    <span
      v-if="lastRunAt"
      class="tw:text-xs tw:text-[var(--color-text-secondary)] tw:tabular-nums tw:whitespace-nowrap tw:select-none"
      :title="exactTime"
    >
