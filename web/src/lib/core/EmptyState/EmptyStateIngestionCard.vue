...
<template>
  <button
    type="button"
    class="es-ing-card group relative flex items-center gap-3 flex-1 basis-56 min-w-0 max-w-72 min-h-16 py-2.5 pr-3.5 pl-3 rounded-xl border border-border-default bg-surface-base shadow-sm text-left cursor-pointer outline-none transition-[color,background-color,border-color,box-shadow] duration-150 hover:shadow-md hover:border-primary-400 hover:bg-tabs-hover-bg focus-visible:ring-2 focus-visible:ring-primary-500/40"
    @click="emit('click')"
  >
    ...
    <span class="es-ing-card__body flex-1 min-w-0 flex flex-col gap-0.5">
      <span class="text-compact font-semibold text-text-primary truncate">{{ label }}</span>
