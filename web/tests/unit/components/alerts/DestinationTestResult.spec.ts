...
describe("DestinationTestResult", () => {
  ...
  it("shows loading state when testing", () => {
    ...
    expect(wrapper.find('[data-test="test-result-loading"]').exists()).toBe(
      true,
    );
  ...
  it("shows success state for successful test", () => {
    ...
    expect(wrapper.find('[data-test="test-success-timestamp"]').exists()).toBe(
      true,
    );
  ...
  it("shows error details when available", () => {
    ...
    expect(wrapper.find('[data-test="test-failure-details"]').exists()).toBe(
      true,
    );
  });
  ...
  describe("error messages", () => {
    ...
    it("shows connection error for ECONNREFUSED", () => {
      ...
      expect(component.getFailureMessage(result)).toBe(
        "alerts.testErrorConnection",
      );
    });
  ...
  describe("suggested fixes", () => {
    ...
    it("suggests checking credentials for 401 error", () => {
      ...
      expect(component.getSuggestedFix(result)).toBe(
        "alerts.suggestCheckCredentials",
      );
    });
