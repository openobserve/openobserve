...
describe("PrebuiltDestinationSelector", () => {
  ...
  it("renders correctly", () => {
    ...
    expect(
      wrapper.find('[data-test="prebuilt-destination-selector"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-test="destination-search-input"]').exists(),
    ).toBe(true);
  });
  ...
  it("shows popular badge for popular destinations", () => {
    ...
    const slackCard = wrapper.find('[data-type="slack"]');
    expect(
      slackCard.find('[data-test="destination-popular-badge"]').exists(),
    ).toBe(true);
  });
  ...
  it("filters destinations based on search query", async () => {
    ...
    const visibleCards = wrapper.findAll(
      '[data-test="destination-type-card"]:not(.q-field)',
    );
  ...
  it("shows empty state when no results found", async () => {
    ...
    await wrapper
      .find('[data-test="destination-search-input"]')
      .setValue("nonexistent");

    expect(
      wrapper.find('[data-test="destination-search-empty"]').exists(),
    ).toBe(true);
  ...
  it("clears search when clear search button is clicked", async () => {
    ...
    await wrapper
      .find('[data-test="destination-search-empty"] .q-btn')
      .trigger("click");
