...
- `OTemplateInput` wraps `OInput`. `OTemplateTextarea` wraps `OTextarea`. Both ship together with two shared sub-components — `OTemplateOverlay` (the peek box and the list, rendered by each wrapper as one element) and `OTemplateSuggestList` — and a co-located composable `useTemplateSuggest`, which owns the whole token grammar including the `{{name}}` text both overlays render.
...
- New `VariableSuggestionRow` component fills the `suggestion` slot: scope icon `public` when global and defined in no environment, else `layers`, with the `aria-label` following the same either/or (the global label, or the environment names, never both); the name in mono; environment names in secondary text, truncated; a `warning` icon with the coverage-gap tooltip when `gap` is not empty; a `lock` icon with the existing secret tooltip. No new Synthetics i18n keys. The one new key in this work is the library's `components.templateInput.suggestions`.
