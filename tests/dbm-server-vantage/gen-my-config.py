...
#
# Set to "" to switch the fan-out off.
...
def exporters_block(lane) -> str:
  ...
  otlphttp/my_traces:
    ...
    # The SECOND destination. Same signals, same stream name; only the org
    # differs. Written as separate exporters rather than a second endpoint on
    # the existing ones because an otlphttp exporter has exactly one URL.
    fanout_exporters = f"""
  otlphttp/fan_logs:
...
def service_block(lane, recipe_receivers: str) -> str:
    ...
    # rather than replacing is what makes this additive: the lane's org keeps
    # receiving exactly what it did before.
    ...
    traces:
      ...
      exporters: [otlphttp/my_traces{fan_traces}]""" if traces else """
  ...
  pipelines:
    ...
    logs/receiver_events:
      ...
      exporters: [otlphttp/my_logs{fan_logs}, file/raw_events]
    ...
    logs:
      ...
      exporters: [otlphttp/my_logs{fan_logs}]
    ...
    metrics:
      receivers: [mysql]
      processors: [memory_limiter/my, resource/ident, batch]
      exporters: [otlphttp/my_metrics{fan_metrics}]{traces_pipeline}
"""
