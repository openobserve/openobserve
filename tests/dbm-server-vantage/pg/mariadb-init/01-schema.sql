-- MariaDB fixture — deliberately MIRRORS pg/mysql-init/01-schema.sql.
--
-- WHY A SEPARATE FILE rather than sharing MySQL's: the point of the MariaDB
-- container is to answer ONE question the MySQL rig cannot
-- (dbm-engine-support.md §3) — does MariaDB write a deadlock as ONE multi-line
-- block, or split it across entries the way MySQL 8 does? Answering that needs
-- the SAME workload shape on both engines so the only variable is the server.
-- Keeping the schema identical is what makes the two error logs comparable.
--
-- Divergences from the MySQL file are marked; each one is a real MariaDB
...
-- Monitoring user. NOTE the MariaDB divergence: there is no `mysqlreceiver`
-- consumer-enabling step here because MariaDB's performance_schema is off by
-- default and enabled via --performance-schema=ON in compose instead, so the
-- UPDATE grant MySQL needs is pointless. PROCESS is what the blocking query
-- needs to see other sessions' SQL.
