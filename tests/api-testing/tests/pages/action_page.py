...
class ActionPage:
    ...
    def delete_action(self, session, base_url, user_email, user_password, action_name):
        """Delete an action."""
        session.auth = HTTPBasicAuth(user_email, user_password)
        response = session.delete(
            f"{base_url}api/{self.org_id}/actions/{action_name}"
        )
        assert response.status_code == 200, f"Failed to delete action: {response.content}"
        return response

    def retrieve_actions(self, session, base_url, user_email, user_password):
        """Retrieve all actions."""
        session.auth = HTTPBasicAuth(user_email, user_password)
        response = session.get(f"{base_url}api/{self.org_id}/actions")
        assert response.status_code == 200, f"Failed to retrieve actions: {response.content}"
        ...
        filtered_actions = [
            ...
            isinstance(action.get('name'), str) and
            action['name'].startswith(f"action{ActionPage.Unique_value_action}")
        ]
    ...
    def validate_deleted_action(self, session, base_url, user_email, user_password, action_name):
        """Validate that an action has been deleted."""
        session.auth = HTTPBasicAuth(user_email, user_password)
        response = session.get(f"{base_url}api/{self.org_id}/actions")
        assert response.status_code == 200, f"Failed to retrieve actions: {response.content}"
        ...
        if len(matching_actions) == 0:
            print(f"Action '{action_name}' not found (expected for deletion verification)")
            return True
        else:
            print(f"Action '{action_name}' still exists")
            return False
