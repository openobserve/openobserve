...
def test_create_dashboard(create_session, base_url):
    ...
    body = resp.json()
    # Dashboard ID is nested in v7.dashboardId
    assert "v7" in body and "dashboardId" in body["v7"], "Response should contain dashboard ID in v7.dashboardId"

    # Clean up: delete the created dashboard
    dashboard_id = body["v7"]["dashboardId"]
    if dashboard_id:
...
def test_get_dashboard_by_id(create_session, base_url):
    ...
    dashboard_id = create_body["v7"]["dashboardId"]
    ...
    body = resp.json()
    assert body["v7"]["title"] == "Test Get Dashboard", "Dashboard title should match"
...
def test_update_dashboard(create_session, base_url):
    ...
    body = get_resp.json()
    assert body["v7"]["title"] == "Dashboard for Update Test", "Dashboard title should match"
    assert body["v7"]["description"] == "Testing dashboard lifecycle", "Dashboard description should match"
...
def test_delete_dashboard(create_session, base_url):
    ...
    create_resp = session.post(create_url, json=dashboard_data)
    dashboard_id = create_resp.json()["v7"]["dashboardId"]
...
def test_create_dashboard_with_empty_panels(create_session, base_url):
    ...
    dashboard_id = resp.json()["v7"]["dashboardId"]

    # Verify dashboard was created
    get_resp = session.get(f"{base_url}api/{ORG_ID}/dashboards/{dashboard_id}")
    assert get_resp.status_code == 200
    assert get_resp.json()["v7"]["title"] == "Dashboard with Empty Panels"
...
def test_dashboard_duplicate_title(create_session, base_url):
    ...
    dashboard_id1 = resp1.json()["v7"]["dashboardId"]
    ...
    if resp2.status_code in [200, 201]:
        dashboard_id2 = resp2.json()["v7"]["dashboardId"]
        if dashboard_id2:
...
def test_dashboard_with_variables(create_session, base_url):
    ...
    assert get_resp.status_code == 200
    body = get_resp.json()
    assert "variables" in body["v7"], "Dashboard should have variables"
    assert len(body["v7"]["variables"]["list"]) == 2, "Should have 2 variables"
...
def test_dashboard_with_multiple_tabs(create_session, base_url):
    ...
    assert get_resp.status_code == 200
    body = get_resp.json()
    assert len(body["v7"]["tabs"]) == 3, "Dashboard should have 3 tabs"
    assert body["v7"]["tabs"][0]["name"] == "Overview"
    assert body["v7"]["tabs"][1]["name"] == "Details"
    assert body["v7"]["tabs"][2]["name"] == "Analytics"
