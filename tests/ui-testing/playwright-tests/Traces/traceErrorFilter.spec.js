...
test.describe("Trace Error Filter testcases", () => {
  ...
  test("P0: View traces with error indicators", {
    ...
    if (hasErrorTraces) {
      ...
      expect(errorText).toMatch(/\d+\s+Error\s+(Trace|Span)s?/);

      // Extract and validate error count
      const match = errorText.match(/(\d+)\s+Error\s+(Trace|Span)s?/);
      if (match) {
      ...
      if (errorTraceCount > 1) {
        ...
        expect(secondErrorText).toMatch(/\d+\s+Error\s+(Trace|Span)s?/);
