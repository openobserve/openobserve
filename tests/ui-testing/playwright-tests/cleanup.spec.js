...
test.describe("Pre-Test Cleanup", () => {
  ...
  test('Clean up all test data via API', {
    ...
    await pm.apiCleanup.cleanupEnrichmentTables([
      ...
      /^url_table_[a-f0-9]{8}$/,                                                             // url_table_<uuid> (URL creation tests)
      /^invalid_url_[a-f0-9]{8}$/,                                                           // invalid_url_<uuid> (validation tests)
      /^list_check_[a-f0-9]{8}$/,                                                            // list_check_<uuid> (list visibility tests)
      /^reload_test_[a-f0-9]{8}$/,                                                           // reload_test_<uuid> (reload mode tests)
      /^append_test_[a-f0-9]{8}$/,                                                           // append_test_<uuid> (append mode tests)
      /^replace_test_[a-f0-9]{8}$/,                                                          // replace_test_<uuid> (replace mode tests)
      /^cancel_test_[a-f0-9]{8}$/,                                                           // cancel_test_<uuid> (cancel form tests)
      /^search_test_[a-f0-9]{8}$/,                                                           // search_test_<uuid> (search functionality tests)
      /^delete_test_[a-f0-9]{8}$/,                                                           // delete_test_<uuid> (delete operation tests)
      /^toggle_test_[a-f0-9]{8}$/,                                                           // toggle_test_<uuid> (source toggle tests)
      /^nonexistent_url_[a-f0-9]{8}$/,                                                       // nonexistent_url_<uuid> (URL validation edge case)
      /^explore_test_[a-f0-9]{8}$/,                                                          // explore_test_<uuid> (data exploration tests)
      /^explore_log_[a-f0-9]{8}$/                                                            // explore_log_<uuid> (logs page navigation test)
    ]);
