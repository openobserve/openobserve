...
test.describe(" VRL UI testcases", () => {
    ...
    test('should display the VRL function in the field list after it has been written.', async ({ page }) => {
      ...
      //  await page.locator('#fnEditor > .monaco-editor > .overflow-guard > div:nth-child(2) > .lines-content > .view-lines').click();
        await page.locator('[data-test="dashboard-vrl-function-editor"]').getByLabel('Editor content;Press Alt+F1').fill('.vrl=123');
        await page.locator('[data-test="dashboard-apply"]').click();
        await page.waitForTimeout(200);
        const locator = page.locator('[data-test="field-list-item-logs-e2e_automate-vrl"]');

        // Get the locator and ensure it is visible
       // await page.locator('[data-test="field-list-item-logs-e2e_automate-vrl"]').waitFor({ state: 'visible' });
     //   await expect(page.getByText('drag_indicatortext_fields vrl')).toBeVisible({timeout:30000});
       // await expect(page.locator('[data-test="field-list-item-logs-default-vrl"]')).toBeVisible({timeout:30000});


    });

    test('should be able to successfully add multiple VRL functions.', async ({ page }) => {
        ...
        await page.locator('label').filter({ hasText: 'Streamarrow_drop_down' }).locator('i').click();
        await page.getByText('e2e_automate').click();
        await page.locator('[data-test="field-list-item-logs-e2e_automate-kubernetes_annotations_kubectl_kubernetes_io_default_container"] [data-test="dashboard-add-b-data"]').click();
    ...
    test('The chart should be successfully saved by adding the VRL function.', async ({ page }) => {
        ...
        await page.locator('[data-test="dashboard-if-no-panel-add-panel-btn"]').click();
        await page.locator('div').filter({ hasText: /^arrow_drop_down$/ }).nth(2).click();
    ...
    test('The VRL function query should not vanish after changing from custom SQL mode to auto mode.', async ({ page }) => {
      ...
      //  await page.locator('[data-test="chart-renderer"] canvas').click
        ...
        await expect(page.getByText('Are you sure you want to change the query mode? The data saved for X-Axis, Y-Axis and Filters will be wiped off.')).toBeVisible();
        await page.locator('[data-test="confirm-button"]').click();
        await expect(page.getByText('.percentage , err = .histogram*.kubernetes_container_hash/')).getByText('.percentage , err = .histogram*.kubernetes_container_hash/100');
    });



    test('should display an error message when changing the VRL function if the existing VRL function is not updated or changed', async ({ page }) => {
      ...
      await page.locator('[data-test="dashboard-vrl-function-editor"]').getByLabel('Editor content;Press Alt+F1').fill('.vrl="Hello"');
        await page.locator('[data-test="dashboard-apply"]').click();
      ...
      //  await page.locator('[data-test="field-list-item-logs-e2e_automate-vrl"] [data-test="dashboard-add-y-data"]').click();
        ...
        await page.locator('[data-test="dashboard-vrl-function-editor"]').getByLabel('Editor content;Press Alt+F1').press('ArrowLeft');
        await page.locator('[data-test="dashboard-vrl-function-editor"]').getByLabel('Editor content;Press Alt+F1').press('ArrowLeft');
        await page.locator('[data-test="dashboard-vrl-function-editor"]').getByLabel('Editor content;Press Alt+F1').fill('.Test="Hello"');
        await page.locator('[data-test="dashboard-apply"]').click();
        await page.getByText('drag_indicatortext_fields test').click();
      
        await page.locator('[data-test="field-list-item-logs-e2e_automate-test"] [data-test="dashboard-add-y-data"]').click();
        await page.locator('[data-test="dashboard-apply"]').click();
        ...
        await page.locator('[data-test="dashboard-panel-save"]').click();
        await page.locator('[data-test="dashboard-edit-panel-VRL-dropdown"]').click();
      ...
      test('should display an error message if an invalid VRL function is added.', async ({ page }) => {
        ...
        await expect(page.getByRole('list').getByText('Function error: error[E202]: syntax error ┌─ :1:1 │ 1 │ ╭ .VRL="Hello 2 │ │ . │ ╰──^ unexpected error: invalid string literal │ = see language documentation at https://vrl.dev = try your code in the VRL REPL, learn more at https://vrl.dev/examples')).toBeVisible();
      ...
      test('function should not disappear when changing the chart type.', async ({ page }) => {
        ...
        await expect(page.locator('div').filter({ hasText: /^\.Vrl=123$/ }).nth(2)).toBeVisible();
      ...
      test('should not show an error if the VRL is null on the older panel.', async ({ page }) => {
        ...
        await page.locator('[data-test="dashboard-panel-name"]').click();
        await page.locator('[data-test="dashboard-panel-name"]').fill('Dashboared ');
        await page.locator('[data-test="dashboard-panel-save"]').click();
        await page.locator('[data-test="dashboard-edit-panel-Dashboared -dropdown"]').click();
      ...
      test('should display the VRL filed in table chart', async ({ page }) => {
        ...
        await page.locator('[data-test="dashboard-vrl-function-editor"]').getByLabel('Editor content;Press Alt+F1').fill('.vrl ,err =.y_axis_1/.y_axis_2*100');
        await page.locator('[data-test="dashboard-apply"]').click();
        await page.locator('[data-test="date-time-btn"]').click();

        await page.locator('[data-test="date-time-relative-6-w-btn"]').click();
        await page.locator('[data-test="field-list-item-logs-e2e_automate-vrl"] [data-test="dashboard-add-x-data"]').click();
        await page.locator('[data-test="dashboard-apply"]').click();
        await expect(page.locator('[data-test="dashboard-panel-table"]').getByRole('cell', { name: 'Vrl' })).toBeVisible();
        await page.locator('[data-test="dashboard-panel-name"]').click();
        await page.locator('[data-test="dashboard-panel-name"]').fill('VRL');
        await page.locator('[data-test="dashboard-panel-save"]').click();
      });

      test('should able to select the VRL from saved Function list ', async ({ page }) => {
        ...
        await page.locator('[data-test="dashboard-use-saved-vrl-function"]').fill('VRL_01');
        ...
        await expect(page.getByText('drag_indicatortext_fields percentage')).toBeVisible();
        await page.locator('[data-test="field-list-item-logs-e2e_automate-percentage"] [data-test="dashboard-add-b-data"]').click();
        await page.locator('[data-test="dashboard-apply"]').click();
        await page.locator('[data-test="dashboard-panel-name"]').click();
        await page.locator('[data-test="dashboard-panel-name"]').fill('VRL');
...
test('should be  VRL function is  updated in the function editor when a different function is selected from the saved function list.', async ({ page }) => {
...
test('should update the VRL function in the function editor when a different function is selected from the saved function list', async ({ page }) => {
    ...
    await page.getByRole('option', { name: 'vrltest' }).locator('div').nth(2).click();

    // Verify the VRL function is applied
    const vrlTestAppliedText = await page.locator('text=vrltest function applied').textContent();
