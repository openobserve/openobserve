...
[.]`;
  ...
  test.describe('JavaScript Row Expansion Tests', () => {
    ...
    // All 3 validations preserved in sequential flow
    // SKIP on cloud: QuickJS on alpha1 fails to compile JS with #ResultArray# + array manipulation
    // Error: "JS compilation failed: Exception generated by QuickJS" — works on self-hosted
    ...
    test('P0: JS row expansion creation and validation flow', { tag: ['@smoke', '@P0', '@jsRowExpansion'] }, async ({ page }) => {
      test.skip(isCloudEnvironment(), 'QuickJS on cloud cannot compile JS #ResultArray# functions — pending backend fix');
