...
// ============================================================================
// Multi-Window Alert VRL Tests - Bug #10872
// ============================================================================
test.describe("Multi-Window Alert VRL Processing @bug-10872", {
  tag: ['@vrl', '@alerts', '@multiWindow', '@regression']
}, () => {
  let pm;
  ...
  /**
   * Bug #10872: Result array VRL does not work correctly for multi-window alerts
   *
   * When creating a multi-window alert with "Compare with Past" enabled,
   * the VRL script should correctly process the 2D array payload structure.
   */
  test("VRL Apply button should process multi-window result array correctly", {
    tag: ['@vrlMultiWindow', '@P2', '@regression']
  }, async ({ page }) => {
    testLogger.info('Testing VRL processing for multi-window alert result arrays');
    ...
    if (await vrlEditor.first().isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Try to input a simple VRL function that processes array
  ...
  [.count]
    ...
    // Clean up - go back or close
    await pm.alertsPage.clickBackButton().catch(() => {});

    testLogger.info('✓ Multi-window VRL test completed');
  });
});
