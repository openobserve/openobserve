...
test.describe("Users and Organizations", () => {
    ...
    test("should render the role summary strip for the root org", { tag: ['@users-rbac-custom-roles', '@all'] }, async ({ page }, testInfo) => {
        testLogger.testStart(testInfo.title, testInfo.file);
        ...
        await pageManager.userPage.expectRoleTileVisible('root');
        await pageManager.userPage.expectSummaryTotalVisible();
        await pageManager.userPage.verifyUserExists(process.env["ZO_ROOT_USER_EMAIL"]);

        testLogger.info('Test completed successfully');
    });

    test("should skip the custom-roles fetch in OSS (diff regression guard)", { tag: ['@users-rbac-custom-roles', '@all'] }, async ({ page }, testInfo) => {
        testLogger.testStart(testInfo.title, testInfo.file);

        await navigateToBase(page);
        pageManager = new PageManager(page);

        const tracker = pageManager.userPage.registerCustomRolesRequestTracker();
        await pageManager.userPage.gotoIamPage();
        await pageManager.userPage.waitForUsersList();
        await pageManager.userPage.openAddUserDialog();
        await pageManager.userPage.cancelUserDrawer();

        expect(tracker.list).toBe(0);
        expect(tracker.batched).toBe(0);

        testLogger.info('Test completed successfully');
    });

    test("should narrow and restore the users list via the role facet filter", { tag: ['@users-rbac-custom-roles', '@all'] }, async ({ page }, testInfo) => {
        const uniqueEmail = `email${Date.now()}_${Math.floor(Math.random() * 10000)}@gmail.com`;
        testLogger.testStart(testInfo.title, testInfo.file);
        ...
        await pageManager.userPage.clickRoleTile('root');
        await pageManager.userPage.verifyUserExists(process.env["ZO_ROOT_USER_EMAIL"]);
        await pageManager.userPage.expectRowAbsent(uniqueEmail);

        await pageManager.userPage.clickSummaryTotal();
        await pageManager.userPage.verifyUserExists(uniqueEmail);
    ...
    test("should not render the custom-role select in the Add-user dialog (OSS)", { tag: ['@users-rbac-custom-roles', '@all'] }, async ({ page }, testInfo) => {
        const uniqueEmail = `email${Date.now()}_${Math.floor(Math.random() * 10000)}@gmail.com`;
        testLogger.testStart(testInfo.title, testInfo.file);
        ...
        await pageManager.userPage.addUser(uniqueEmail);
        await pageManager.userPage.selectUserRole('Admin');
        await pageManager.userPage.expectCustomRoleFieldCount(0);
        await pageManager.userPage.cancelUserDrawer();

        testLogger.info('Test completed successfully');
    });

    test("should render a role tile for a created admin user", { tag: ['@users-rbac-custom-roles', '@all'] }, async ({ page }, testInfo) => {
        const uniqueEmail = `email${Date.now()}_${Math.floor(Math.random() * 10000)}@gmail.com`;
        testLogger.testStart(testInfo.title, testInfo.file);
        ...
        await pageManager.userPage.gotoIamPage();
        await pageManager.userPage.waitForUsersList();
        await pageManager.userPage.expectRoleTileVisible('admin');
    ...
    test.fixme("should render custom roles — not wired: enterprise/cloud + rbac_enabled required (web/src/aws-exports.ts:30; web/src/stores/index.ts:100)", { tag: ['@users-rbac-custom-roles', '@all'] }, async ({ page }, testInfo) => {
        ...
        await pageManager.userPage.openAddUserDialog();
        await pageManager.userPage.selectUserRole('Admin');
        await pageManager.userPage.expectCustomRoleFieldCount(1);
