...
test.describe("Report Bulk Operations", () => {
  ...
  test("P1: Bulk pause selected reports", {
    ...
    testLogger.info('Creating reports for bulk pause test');
    const r1 = await createReportViaApi(pm.apiCleanup, `${REPORT_1}_pause`);
    if (!r1.success) {
      throw new Error('Setup: failed to create report for bulk pause test');
    }

    await pm.reportFoldersPage.navigateToReports();
    await pm.reportFoldersPage.expectReportVisibleInTable(`${REPORT_1}_pause`);

    testLogger.info('Selecting all reports and pausing');
    await pm.reportFoldersPage.selectAllReports();
    await pm.reportFoldersPage.expectBulkButtonsVisible();
  ...
  test("P1: Bulk resume previously paused reports", {
    tag: ['@reportBulkOps', '@functional', '@P1']
  }, async ({ page }) => {
    testLogger.info('Creating reports for bulk resume test');
    const r1 = await createReportViaApi(pm.apiCleanup, `${REPORT_1}_resume`);
    if (!r1.success) {
      throw new Error('Setup: failed to create report for bulk resume test');
    }

    await pm.reportFoldersPage.navigateToReports();
    await pm.reportFoldersPage.expectReportVisibleInTable(`${REPORT_1}_resume`);
