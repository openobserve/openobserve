...
test.describe("Logs Table Field Management - Complete Test Suite", () => {
  ...
  test("should place include/exclude WHERE clause before GROUP BY, not ORDER BY, in SQL aggregation queries", {
    tag: ['@logsTable', '@all', '@logs', '@includeExclude', '@sqlMode', '@aggregation']
  }, async ({ page }) => {
    testLogger.info('Testing include/exclude WHERE clause placement in SQL aggregation queries with GROUP BY + ORDER BY');

    // Step 1: Enable SQL mode
    await pageManager.logsPage.enableSqlModeIfNeeded();
    await page.waitForTimeout(1000);
    testLogger.info('✓ SQL mode enabled');

    // Step 2: Type an aggregation query with both GROUP BY and ORDER BY
    const aggregationQuery = 'SELECT code, count(*) as cnt FROM "e2e_automate" GROUP BY code ORDER BY cnt DESC';
    await pageManager.logsPage.clearAndFillQueryEditor(aggregationQuery);
    await page.waitForTimeout(500);
    testLogger.info('✓ Aggregation query typed: ' + aggregationQuery);

    // Step 3: Run the query
    await pageManager.logsPage.clickSearchBarRefreshButton();
    await page.waitForTimeout(2000);

    // Step 4: Verify results are returned (no SQL error)
    await pageManager.logsPage.expectLogsSearchResultLogsTableVisible();
    testLogger.info('✓ Aggregation query returned results');

    // Step 5: Open first log details
    await pageManager.logsPage.openFirstLogDetails();
    await page.waitForTimeout(1000);
    testLogger.info('✓ Log details opened');

    // Step 6: Add an include search term
    await pageManager.logsPage.addIncludeSearchTermFromLogDetails();
    await page.waitForTimeout(1500);
    testLogger.info('✓ Include search term added');

    // Step 7: Verify query was rewritten correctly — WHERE must come before GROUP BY
    const updatedQuery = await pageManager.logsPage.getQueryEditorText();
    testLogger.info('Query after include term: ' + updatedQuery);

    // Critical assertions: WHERE must appear before GROUP BY, not after it
    expect(updatedQuery.toLowerCase()).toContain('where');
    expect(updatedQuery.toLowerCase()).toContain('group by');
    expect(updatedQuery.toLowerCase()).toContain('order by');

    const whereIndex = updatedQuery.toLowerCase().indexOf('where');
    const groupByIndex = updatedQuery.toLowerCase().indexOf('group by');
    const orderByIndex = updatedQuery.toLowerCase().indexOf('order by');

    expect(whereIndex).toBeLessThan(groupByIndex);
    expect(groupByIndex).toBeLessThan(orderByIndex);
    testLogger.info('✓ WHERE clause correctly placed before GROUP BY (not before ORDER BY)');

    // Step 8: Run the query again to verify it executes without SQL error
    await pageManager.logsPage.clickSearchBarRefreshButton();
    await page.waitForTimeout(2000);

    // Step 9: Verify results are still returned after include term
    await pageManager.logsPage.expectLogsSearchResultLogsTableVisible();
    testLogger.info('✓ Query with include term returned results (no SQL syntax error)');

    // Step 10: Verify include/exclude buttons are still visible in log details
    await pageManager.logsPage.expectIncludeExcludeButtonsVisibleInLogDetails();
    testLogger.info('✓ Include/exclude buttons remain visible after query with include term');

    testLogger.info('✓ Include/exclude WHERE clause placement in aggregation queries verified successfully');
  });

  test("should place exclude WHERE clause correctly in SQL aggregation queries", {
    tag: ['@logsTable', '@all', '@logs', '@includeExclude', '@sqlMode', '@aggregation']
  }, async ({ page }) => {
    testLogger.info('Testing exclude WHERE clause placement in SQL aggregation queries with GROUP BY');
    ...
    // Step 2: Type an aggregation query with GROUP BY + ORDER BY
    const aggregationQuery = 'SELECT code, count(*) as cnt FROM "e2e_automate" GROUP BY code ORDER BY cnt DESC';
    await pageManager.logsPage.clearAndFillQueryEditor(aggregationQuery);
    await page.waitForTimeout(500);
    testLogger.info('✓ Aggregation query typed');

    // Step 3: Run the query
    await pageManager.logsPage.clickSearchBarRefreshButton();
    await page.waitForTimeout(2000);
    await pageManager.logsPage.expectLogsSearchResultLogsTableVisible();
    testLogger.info('✓ Initial aggregation query succeeded');

    // Step 4: Open first log details and add an exclude search term
    await pageManager.logsPage.openFirstLogDetails();
    await page.waitForTimeout(1000);

    // Add exclude term via the include/exclude dropdown
    ...
    testLogger.info(`Found ${buttonCount} include/exclude buttons`);
    expect(buttonCount).toBeGreaterThanOrEqual(1);

    await includeExcludeButtons.nth(Math.min(1, buttonCount - 1)).click();
    await page.waitForTimeout(500);

    const excludeMenuItem = page.getByText('Exclude Search Term', { exact: true });
    await excludeMenuItem.waitFor({ state: 'visible', timeout: 5000 });
    await excludeMenuItem.click();
    await page.waitForTimeout(1500);
    testLogger.info('✓ Exclude search term added');

    // Step 5: Verify query was rewritten with WHERE before GROUP BY
    const updatedQuery = await pageManager.logsPage.getQueryEditorText();
    testLogger.info('Query after exclude term: ' + updatedQuery);

    expect(updatedQuery.toLowerCase()).toContain('where');
    expect(updatedQuery.toLowerCase()).toContain('group by');

    const whereIdx = updatedQuery.toLowerCase().indexOf('where');
    const groupByIdx = updatedQuery.toLowerCase().indexOf('group by');
    expect(whereIdx).toBeLessThan(groupByIdx);
    testLogger.info('✓ WHERE clause correctly placed before GROUP BY for exclude term');

    // Step 6: Run query and verify no errors
    await pageManager.logsPage.clickSearchBarRefreshButton();
    await page.waitForTimeout(2000);
    await pageManager.logsPage.expectLogsSearchResultLogsTableVisible();
    testLogger.info('✓ Query with exclude term returned results (no SQL syntax error)');

    testLogger.info('✓ Exclude WHERE clause placement in aggregation queries verified successfully');
  });
