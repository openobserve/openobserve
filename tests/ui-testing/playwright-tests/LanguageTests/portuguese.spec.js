/**
 * Portuguese (Português) Translation Coverage Test
 */
const { test, expect, navigateToBase } = require('../utils/enhanced-baseFixtures.js');
const testLogger = require('../utils/test-logger.js');
const PageManager = require('../../pages/page-manager.js');

test("Portuguese (Português) translation coverage", {
  tag: ['@languageTranslation', '@P1', '@pt']
}, async ({ page }) => {
  ...
  const result = await pm.languagePage.testLanguageTranslation(pm, 'pt', true);
  ...
  const statusMsg = result.status === 'ideal' ? 'PASS (ideal)'
    ...
    : 'FAIL';
  testLogger.info(`\nPORTUGUESE RESULT: ${result.overallPercentage}% - ${statusMsg}`);

  // Test passes if coverage >= acceptable minimum (ideal - delta)
  expect(result.overallPercentage,
    `Portuguese translation coverage should be >= ${pm.languagePage.getAcceptableMinCoverage()}%`)
