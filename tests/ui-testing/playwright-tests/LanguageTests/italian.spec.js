/**
 * Italian (Italiano) Translation Coverage Test
 */
const { test, expect, navigateToBase } = require('../utils/enhanced-baseFixtures.js');
const testLogger = require('../utils/test-logger.js');
const PageManager = require('../../pages/page-manager.js');

test("Italian (Italiano) translation coverage", {
  tag: ['@languageTranslation', '@P1', '@it']
}, async ({ page }) => {
  ...
  const result = await pm.languagePage.testLanguageTranslation(pm, 'it', true);
  ...
  const statusMsg = result.status === 'ideal' ? 'PASS (ideal)'
    ...
    : 'FAIL';
  testLogger.info(`\nITALIAN RESULT: ${result.overallPercentage}% - ${statusMsg}`);

  // Test passes if coverage >= acceptable minimum (ideal - delta)
  expect(result.overallPercentage,
    `Italian translation coverage should be >= ${pm.languagePage.getAcceptableMinCoverage()}%`)
