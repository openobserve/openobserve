/**
 * Chinese Simplified (简体中文) Translation Coverage Test
 */
const { test, expect, navigateToBase } = require('../utils/enhanced-baseFixtures.js');
const testLogger = require('../utils/test-logger.js');
const PageManager = require('../../pages/page-manager.js');

test("Chinese Simplified (简体中文) translation coverage", {
  tag: ['@languageTranslation', '@P1', '@zh']
}, async ({ page }) => {
  ...
  const result = await pm.languagePage.testLanguageTranslation(pm, 'zh-cn', true);
  ...
  const statusMsg = result.status === 'ideal' ? 'PASS (ideal)'
    ...
    : 'FAIL';
  testLogger.info(`\nCHINESE RESULT: ${result.overallPercentage}% - ${statusMsg}`);

  // Test passes if coverage >= acceptable minimum (ideal - delta)
  expect(result.overallPercentage,
    `Chinese translation coverage should be >= ${pm.languagePage.getAcceptableMinCoverage()}%`)
