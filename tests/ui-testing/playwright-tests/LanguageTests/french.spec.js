/**
 * French (Français) Translation Coverage Test
 */
const { test, expect, navigateToBase } = require('../utils/enhanced-baseFixtures.js');
const testLogger = require('../utils/test-logger.js');
const PageManager = require('../../pages/page-manager.js');

test("French (Français) translation coverage", {
  tag: ['@languageTranslation', '@P1', '@fr']
}, async ({ page }) => {
  ...
  const result = await pm.languagePage.testLanguageTranslation(pm, 'fr', true);
  ...
  const statusMsg = result.status === 'ideal' ? 'PASS (ideal)'
    ...
    : 'FAIL';
  testLogger.info(`\nFRENCH RESULT: ${result.overallPercentage}% - ${statusMsg}`);

  // Test passes if coverage >= acceptable minimum (ideal - delta)
  expect(result.overallPercentage,
    `French translation coverage should be >= ${pm.languagePage.getAcceptableMinCoverage()}%`)
