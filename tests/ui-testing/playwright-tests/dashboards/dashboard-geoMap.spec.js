...
test.describe("dashboard filter testcases", () => {
 ...
 test("should correctly apply the filter conditions with different operators, and successfully apply them to the query", async ({
  ...
  await page.locator('[data-test="field-list-item-logs-geojson-country"] [data-test="dashboard-add-weight-data"]').click();
    ...
    await page.locator('[data-test="dashboard-config-latitude"]').click();
    await page.locator('[data-test="dashboard-config-latitude"]').fill('26.1206');

    await page.locator('[data-test="dashboard-config-longitude"]').click();
    await page.locator('[data-test="dashboard-config-longitude"]').fill('091.6523');

    await page.locator('[data-test="dashboard-config-zoom"]').click();
    await page.locator('[data-test="dashboard-config-zoom"]').fill('10');
    await page.locator('[data-test="dashboard-apply"]').click();
