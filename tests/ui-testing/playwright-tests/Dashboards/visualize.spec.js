...
test.describe("logs testcases", () => {
  ...
  test("Should display the correct query in the dashboard when saved from a Table chart.", async ({
    ...
    // Wait for the panel to fully render before clicking the dropdown.
    // Scope to the panel bar containing the panel title, then find the
    // dropdown trigger by its ARIA attribute (data-test on the button requires
    // a rebuilt binary after the OButton dataTest prop fix).
    const panelBar = page
      .locator('[data-test="dashboard-panel-bar"]')
      .filter({ hasText: panelName });
    const panelDropdown = panelBar.locator('button[aria-haspopup="menu"]');
    await panelDropdown.waitFor({ state: "visible", timeout: 20000 });
  ...
  test("should display the correct query in the dashboard when saved from a Line chart.", async ({
    ...
    // dropdown trigger by its ARIA attribute (data-test on the button requires
    // a rebuilt binary after the OButton dataTest prop fix).
    const panelBar2 = page
      .locator('[data-test="dashboard-panel-bar"]')
      .filter({ hasText: panelName });
    const panelDropdown2 = panelBar2.locator('button[aria-haspopup="menu"]');
    await panelDropdown2.waitFor({ state: "visible", timeout: 20000 });
    await panelDropdown2.click();
    await page.locator('[data-test="dashboard-query-inspector-panel"]').click();
  ...
  test.skip("should render line chart for SELECT * query and save to dashboard with correct query in inspector", async ({
    page,
  }) => {
    // TODO: Query Inspector is not available for SELECT * → bar-chart panels
    // because metaData.queries is never populated in this flow. Likely a bug
    // in PanelSchemaRenderer — skip until fixed.
    const pm = new PageManager(page);
    ...
    // Expect bar chart to be the selected/default visualization
    // NOTE: default panel type was changed from "line" to "bar" in useDashboardPanelDefaults.ts
    ...
    // dropdown trigger by its ARIA attribute (data-test on the button requires
    // a rebuilt binary after the OButton dataTest prop fix).
    const panelBar3 = page
      .locator('[data-test="dashboard-panel-bar"]')
      .filter({ hasText: panelName });
    const panelDropdown3 = panelBar3.locator('button[aria-haspopup="menu"]');
    await panelDropdown3.waitFor({ state: "visible", timeout: 20000 });
    await panelDropdown3.click();
    await page.locator('[data-test="dashboard-query-inspector-panel"]').click();
  ...
  test("should show quick mode as true when toggling to visualize tab", async ({
    ...
    // After switching to the visualize tab, updateUrlQueryParams() is called
    // which writes quick_mode=true into the URL when quick mode is enabled.
    // This is more reliable than opening the utilities menu and inspecting the
    // q-toggle aria attribute (the menu uses reka-ui OButton which doesn't
    // reliably trigger Quasar's q-menu in Playwright).
    await expect(page).toHaveURL(/quick_mode=true/, { timeout: 10000 });
  });
 test("should apply override config after running histogram query", async ({
    ...
    // NOTE: The table uses TenstackTable which renders plain <td> elements, not Quasar .q-td.
    const coloredCells = page.locator(
      '[data-test="dashboard-panel-table"] tbody td[style*="background-color"]'
    );
