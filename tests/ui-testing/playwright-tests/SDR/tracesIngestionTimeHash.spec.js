...
// Traces variant of the SDR ingestion-time hash suite. Matching values are
// replaced with a [REDACTED:<hash>] marker at write time. Traces read back via
// ?type=traces.
test.describe("Traces Ingestion Time Hash - Combined Test", { tag: '@enterprise' }, () => {
  ...
  const patternsToTest = [
    {
      name: `traces_url_hash_${testRunId}`,
      description: 'URL validation pattern (for traces hash tests)',
      pattern: '^https?://[\\w.-]+\\.[\\w.-]+.*$',
      field: 'website_url',
      value: 'https://example.com'
    },
    {
      name: `traces_password_hash_${testRunId}`,
      description: 'Strong password pattern (for traces hash tests)',
      pattern: '^[A-Za-z0-9@$!%*?&]{8,}$',
      field: 'user_password',
      value: 'P@ssw0rd!123'
    },
    {
      name: `traces_ipv4_hash_${testRunId}`,
      description: 'IPv4 address format (for traces hash tests)',
      pattern: '^(\\d{1,3}\\.){3}\\d{1,3}$',
      field: 'server_ip',
      value: '192.168.1.1'
    },
    {
      name: `traces_hex_color_hash_${testRunId}`,
      description: 'Hexadecimal color code (for traces hash tests)',
      pattern: '^#[0-9a-fA-F]{6}$',
      field: 'theme_color',
      value: '#FF5733'
    }
  ...
  const testStreamName = `sdr_traces_hash_${testRunId}`;
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    testLogger.info('Combined traces ingestion time hash test setup completed');
  });

  // Cleanup test - runs first
  test('setup: cleanup patterns', {
    tag: ['@sdr', '@traces', '@cleanup', '@ingestionHash']
  }, async ({ page }) => {
    testLogger.info('=== SPEC-LEVEL CLEANUP: Cleaning up patterns for traces hash test ===');
  ...
  test("should hash multiple trace fields at ingestion time when patterns are applied", {
    tag: ['@sdr', '@traces', '@ingestionHash', '@all', '@combined']
  }, async ({ page }, testInfo) => {
    testInfo.annotations.push({ type: 'dependency', description: 'setup: cleanup patterns' });
    testLogger.info('=== Testing combined traces HASH for 4 patterns at ingestion time ===');
    ...
    // STEP 3: Link all 4 patterns to their respective trace fields with HASH action
    ...
    // STEP 4: Ingest trace data WITH SDR patterns linked - all fields should be HASHED
    // Ingestion-time hash is baked in at write time, so this is a NEW batch
    ...
    testLogger.info('✓ STEP 4 PASSED: All trace fields are HASHED');

    testLogger.info('=== ✓ COMBINED TRACES HASH TEST COMPLETED SUCCESSFULLY ===');
  });
});
