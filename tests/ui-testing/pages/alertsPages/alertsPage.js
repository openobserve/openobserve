...
export class AlertsPage {
    ...
    _initializeLocators() {
        return {
            ...
            // Alert Setup selectors (V3 UI: alert type is a dropdown, not radio buttons)
            ...
            // V3 revamp: alert type is now a q-select dropdown next to "Alert Type" label
            alertTypeSelect: '.alert-v3-inline-label:has-text("Alert Type") + .q-select, .alert-v3-inline-label:has-text("Alert Type") ~ div .q-select',
            alertTypeScheduledOption: 'Scheduled',
            alertTypeRealtimeOption: 'Real-time',

            // V3 Tab selectors (Alert Rules / Advanced)
            alertRulesTab: 'div:has-text("Alert Rules")',
            advancedTab: '[data-test="alert-list-page"] div:text-is("Advanced")',

            // Query/Conditions selectors (Alert Rules tab)
            ...
            step2ViewEditorButton: '[data-test="step2-view-editor-btn"], button:has-text("View Editor")',
            ...
            // Alert Settings (within Alert Rules tab)
            alertDestinationsMultiSelect: '[data-test="alert-destinations-select"]',
            stepAlertConditionsSection: '.step-alert-conditions',
            ...
            // Query mode tab selectors (V3: custom buttons in a query-mode-tabs container)
            tabSql: 'div[data-test="step2-query-tabs"] button:has-text("SQL")',
            tabPromql: 'div[data-test="step2-query-tabs"] button:has-text("PromQL")',
            tabBuilder: 'div[data-test="step2-query-tabs"] button:has-text("Builder")',
            ...
            // Evaluation Status Indicator (PR #10470 - "Would Trigger" badge)
    ...
    async verifyAlertCounts() {
        // Helper: find a count number near a text label on the home page
        async function getCountNearLabel(page, labelText) {
            // Strategy 1: .column with .text-subtitle containing the label, find .results-count
            try {
                const column = page.locator(`.column:has(.text-subtitle:text-is("${labelText}"))`).first();
                const count = await column.locator('.results-count, .count, .stat-value, .kpi-value').first().textContent({ timeout: 5000 }).catch(() => null);
                if (count !== null) return count.trim();
            } catch { /* fall through */ }

            // Strategy 2: Find the label, then look for nearby number
            try {
                const label = page.getByText(labelText, { exact: true }).first();
                const parent = label.locator('..');
                const numText = await parent.textContent({ timeout: 3000 }).catch(() => null);
                if (numText) {
                    const nums = numText.match(/\d+/);
                    if (nums) return nums[0];
                }
            } catch { /* fall through */ }

            // Strategy 3: Broader search — any element containing the label, find any number nearby
            try {
                const el = page.locator(`*:has-text("${labelText}")`).first();
            ...
            testLogger.warn(`Could not find count for label: ${labelText}`);
            return '0';
        }

        const scheduledAlertsCount = await getCountNearLabel(this.page, 'Scheduled');
        const realTimeAlertsCount = await getCountNearLabel(this.page, 'Real time');

        testLogger.info('Alert counts retrieved', { scheduledAlertsCount, realTimeAlertsCount });
    ...
    async verifyFieldRequiredValidation() {
        ...
        // In V3 UI, leave alert name empty and click Save to trigger validation
        await this.page.locator(this.locators.alertNameInput).click();

        // Try Save button first; if not enabled, check for pre-existing validation
        try {
            await this.page.locator(this.locators.alertSubmitButton).click({ timeout: 3000 });
            await this.page.waitForTimeout(500);
        } catch (e) {
            testLogger.warn('Save button not clickable, validation may already be shown');
        }

        // Check for required field validation message
        await expect(this.page.getByText('Field is required!').or(this.page.getByText('is required'))).toBeVisible({ timeout: 5000 });
     ...
     *
     * Creates the validation destination via a direct API call with the
     * Authorization header included from the start. This avoids a V3 UI issue
     * where destinations created via the UI flow (even without headers) do not
     * always appear in the alert wizard's destination dropdown immediately.
     *
    ...
    async ensureValidationInfrastructure(pm, uniqueSuffix) {
        ...
        const authHeader = headers['Authorization'] || (() => {
            if (isCloudEnvironment()) {
                testLogger.warn('alertsPage: no cloud passcode available for destination auth - Basic auth will not work on cloud OIDC endpoints');
            }
        ...
        // Create validation destination via direct API call with Authorization header included from the start.
        // We use POST (create) or PUT (create/update) — POST works for new destinations.
        const apiUrl = `${baseUrl}/api/${orgName}/alerts/destinations`;
        const payload = {
        ...
        try {
            ...
            if (response.ok()) {
                testLogger.info('Validation destination created via direct API', { destinationName });
            } else if (response.status() === 409) {
                // Destination already exists — update it via PUT
                testLogger.info('Validation destination already exists, updating via API', { destinationName });
                const updateUrl = `${apiUrl}/${encodeURIComponent(destinationName)}`;
                const updateResponse = await this.page.request.put(updateUrl, {
                    headers: { 'Authorization': authHeader, 'Content-Type': 'application/json' },
                    data: payload,
                });
                if (updateResponse.ok()) {
                    testLogger.info('Validation destination updated via API', { destinationName });
                } else {
                    const errText = await updateResponse.text().catch(() => 'unknown');
                    testLogger.error('Failed to update validation destination via API', {
                        destinationName, status: updateResponse.status(), error: errText.substring(0, 500)
                    });
                }
            } else {
                const errorText = await response.text().catch(() => 'unknown');
                testLogger.error('Failed to create validation destination via API', {
                    destinationName, status: response.status(), error: errorText.substring(0, 500)
                });
            }
        } catch (apiError) {
            testLogger.warn('API call to create validation destination failed, falling back to UI flow', {
                destinationName, error: apiError.message
            });
            // Fallback: create via UI flow
    ...
    /**
     * Select scheduled alert type from the dropdown (V3 UI: q-select, not radio buttons)
     */
    async selectScheduledAlertType() {
        await this.page.locator('[data-test="add-alert-stream-name-select-dropdown"]').first().click();
        await this.page.waitForTimeout(500);
        await this.page.getByRole('option', { name: 'Scheduled' }).first().click();
        testLogger.info('Selected scheduled alert type');
    }

    /**
     * Select real-time alert type from the dropdown (V3 UI: q-select, not radio buttons)
     */
    async selectRealtimeAlertType() {
        await this.page.locator('[data-test="add-alert-stream-name-select-dropdown"]').first().click();
        await this.page.waitForTimeout(500);
        await this.page.getByRole('option', { name: 'Real-time' }).first().click();
        testLogger.info('Selected real-time alert type');
    }

    /**
     * Expect the alert type dropdown to be visible
     */
    async expectAlertTypeVisible(timeout = 5000) {
        await expect(this.page.locator('[data-test="add-alert-stream-name-select-dropdown"]')).toBeVisible({ timeout });
    }
     ...
     * In V3 UI, this button no longer exists. Only clicks if present (legacy compatibility).
     */
    async clickContinueButton() {
        ...
        if (await continueBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
            ...
            await this.page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
            testLogger.info('Clicked Continue button (legacy)');
        } else {
            testLogger.warn('Continue button not found (V3 UI has no wizard steps)');
        }
    }
    ...
    /**
     * Open a new scheduled alert and set it up.
     * In V3 UI, no step navigation needed — alert setup, conditions, and settings
     * are all on the "Alert Rules" tab (default).
    ...
    async setupScheduledAlertWizardToStep2(streamName, alertName) {
        ...
        // V3 UI: no step navigation needed — conditions/config are on Alert Rules tab (default)
        testLogger.info('Wizard setup complete', { alertName, streamName });
    }

    /**
     * Click a specific step indicator in the wizard (0-indexed)
     * In V3 UI, there is no q-stepper — use tab switching instead.
     * Fallback: try tab buttons if stepper not found.
     */
    async clickStepIndicator(stepIndex) {
        const tabs = this.page.locator('button[role="tab"]');
        const stepperTabs = this.page.locator('.q-stepper__tab').nth(stepIndex);

        if (await stepperTabs.isVisible({ timeout: 2000 }).catch(() => false)) {
            await stepperTabs.click();
        } else if (await tabs.nth(stepIndex).isVisible({ timeout: 2000 }).catch(() => false)) {
            await tabs.nth(stepIndex).click();
        } else {
            testLogger.warn('No step indicator or tab found for index', { stepIndex });
        }
        await this.page.waitForTimeout(1000);
    ...
    async navigateToVrlEditor() {
        ...
        const viewEditorSelectors = [
            '[data-test="step2-view-editor-btn"]',    // Primary selector
