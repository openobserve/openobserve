...
async deleteJobSearch(trace_id) {
  ...
  async restartJobSearch(trace_id) {
    ...
    try {
        ...
        console.error(`Error: Unable to find job row for trace ID ${trace_id}.`, error);
...
async exploreJob(trace_id) {
    ...
    try {
        await this.page.waitForSelector(exploreButtonSelector, { timeout: 10000 }); // Adjust timeout as necessary
    } catch (error) {
        console.error(`Error: Unable to find explore button for trace ID ${trace_id}.`, error);
        throw new Error(`Explore button for trace ID ${trace_id} not found.`);
    }

    // Click the explore button for the specified job row
    await this.page.locator(exploreButtonSelector).click();

    // Verify the success message
    await expect(this.page.locator('#q-notify')).toContainText('check_circleSearch Job have been applied successfully');
}
...
async viewJobDetails(trace_id) {
    ...
    try {
        ...
        console.error(`Error: Unable to find or click expand button for trace ID ${trace_id}.`, error);
    ...
    try {
        ...
        console.error(`Error: Unable to find or click More Details tab for trace ID ${trace_id}.`, error);
        throw new Error(`More Details tab for trace ID ${trace_id} not found or not clickable.`);
    }

    // Verify that the expected text is present in the code block
    await expect(this.page.getByRole('code')).toContainText(`"${trace_id}"`);
}
