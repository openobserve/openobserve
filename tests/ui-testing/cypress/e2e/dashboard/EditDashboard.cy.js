...
describe("Create a new dashboard", () => {
  ...
  it("should update the value automatically when trying to save the chart after switching from the auto to custom mode", () => {
    ...
    cy.get('[data-test="dashboard-panel-query-editor"]')
      ...
      .click()
      .type(
        `{ctrl+d}{ctrl+d}{ctrl+d}${dashboardData.editCustomQueryValue.field1}`
    ...
    // );
    const updatedQuery = `SELECT histogram(_timestamp) as "${dashboardData.editCustomQueryValue.field1}", count(log) as "${dashboardData.customQueryValue.field2}"  FROM "default"  GROUP BY ${dashboardData.editCustomQueryValue.field1} ORDER BY ${dashboardData.editCustomQueryValue.field1}`;
