...
mod tests {
    ...
    use common::{infra::config::ENRICHMENT_TABLES, meta::user::UserList};
    use config::{
        get_config,
        meta::{
    ...
    async fn e2e_delete_alert_with_sns_destination() {
        ...
        let alert = openobserve_alerts::repository::alert::get_by_name(
            "e2e",
    ...
    async fn e2e_post_alert_multirange() {
        ...
        let id = id.unwrap();
        // Check the trigger
        let trigger = openobserve_scheduler::exists(
            "e2e",
    ...
    async fn e2e_handle_alert_after_destination_retries() {
        ...
        let trigger = openobserve_scheduler::get(
            "e2e",
            config::meta::triggers::TriggerModule::Alert,
            &id.to_string(),
        )
    ...
    async fn e2e_handle_alert_after_evaluation_retries() {
        ...
        let res = openobserve_alerts::repository::alert::set("e2e", alert, true).await;
        assert!(res.is_ok());
        ...
        let res = openobserve_alerts::repository::alert::delete_by_name(
            "e2e",
            config::meta::stream::StreamType::Logs,
            "olympics_schema",
    ...
    async fn e2e_handle_derived_stream_success() {
        ...
        while attempts < max_attempts {
            let trigger =
                openobserve_scheduler::get("e2e", TriggerModule::DerivedStream, &module_key).await;
