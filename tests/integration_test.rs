...
mod tests {
    ...
    async fn e2e_test() {
        ...
        // register node
        openobserve_core::common::infra::cluster::register_and_keep_alive()
            .await
            .unwrap();
        // init config
    ...
    async fn e2e_delete_alert_with_sns_destination() {
        ...
        let alert = openobserve_core::db::alerts::alert::get_by_name(
            "e2e",
    ...
    async fn e2e_post_alert_multirange() {
        ...
        let id = id.unwrap();
        // Check the trigger
        let trigger = openobserve_core::db::scheduler::exists(
            "e2e",
    ...
    async fn e2e_handle_alert_after_destination_retries() {
        ...
        let trigger = openobserve_core::db::scheduler::get(
            "e2e",
            config::meta::triggers::TriggerModule::Alert,
            &id.to_string(),
        )
    ...
    async fn e2e_handle_alert_after_evaluation_retries() {
        ...
        let res = openobserve_core::db::alerts::alert::set("e2e", alert, true).await;
        assert!(res.is_ok());
        ...
        let res = openobserve_core::db::alerts::alert::delete_by_name(
            "e2e",
            config::meta::stream::StreamType::Logs,
            "olympics_schema",
    ...
    async fn e2e_handle_derived_stream_max_retries() {
        ...
        let trigger =
            openobserve_core::db::scheduler::get("e2e", TriggerModule::DerivedStream, &module_key)
                .await;
        assert!(trigger.is_ok());
        let trigger = trigger.unwrap();
        assert!(trigger.next_run_at > now && trigger.retries == 0);
    ...
    async fn test_derived_stream_invalid_timerange_delay_scenario() {
        ...
        let _ = openobserve_core::db::scheduler::delete(
            "e2e",
            TriggerModule::DerivedStream,
            &format!("logs/e2e/test_invalid_timerange_pipeline/{}", pipeline.id),
        )
