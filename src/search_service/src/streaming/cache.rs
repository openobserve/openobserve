...
use ::search::{CachedQueryResponse, MultiCachedQueryResponse, QueryDelta, SearchResultType};
use config::{
    cluster::LOCAL_NODE,
    meta::{
...
pub async fn handle_cache_responses_and_deltas(
    ...
    match cache_order_by {
        OrderBy::Desc => {
            deltas.sort_by_key(|k| std::cmp::Reverse(k.delta_start_time));
            cached_resp.sort_by_key(|k| std::cmp::Reverse(k.response_start_time));
        }
        OrderBy::Asc => {
...
mod tests {
    ...
    fn test_cache_duration_calculation_descending_order() {
        ...
        cached_resp.sort_by_key(|item| std::cmp::Reverse(item.response_start_time));
