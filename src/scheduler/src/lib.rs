...
/// Replace a wildcard seconds field with the supplied current second.
///
/// Alert, derived-stream, and report schedules use the same normalization rule. Keeping it in
/// the scheduler crate prevents those domains from depending on one another for cron handling.
...
mod tests {
    ...
    #[test]
    fn replaces_wildcard_seconds_in_cron_expression() {
        assert_eq!(
            update_cron_expression("* */5 * * * *", 15),
            "15 */5 * * * *"
        );
        assert_eq!(update_cron_expression("0 */5 * * * *", 15), "0 */5 * * * *");
    }
