        ...
        match self {
            ...
            Values::Vrl(data) => {
                ...
                let json_data = pool.install(|| {
                    data.par_iter()
                        .map(crate::db::enrichment_table::convert_from_vrl)
                        .collect()
                });
        ...
        match self {
            ...
            Values::Json(data) => {
                ...
                let vrl_data = pool.install(|| {
                    data.par_iter()
                        .map(crate::db::enrichment_table::convert_to_vrl)
                        .collect()
                });
                Ok(Arc::new(vrl_data))
            }
...
pub mod remote {
    ...
    pub async fn upload_to_remote(
        ...
        openobserve_catalog::file_list::set(&account, &remote_key, Some(file_meta), false).await?;
