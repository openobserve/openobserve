...
pub async fn organizations(
    ...
    for org in all_orgs {
        ...
        #[cfg(feature = "cloud")]
        if organization_domain::org_status::is_blocked(&org.identifier) {
            continue;
        }
        id += 1;
        #[cfg(feature = "cloud")]
...
async fn get_super_cluster_nodes(regions: &[String]) -> Result<NodeListResponse, anyhow::Error> {
    ...
    for cluster in clusters {
        ...
        match runtime_services::node::get_node_list(&trace_id, cluster).await {
            Ok(cluster_nodes) => {
...
async fn get_super_cluster_info(regions: &[String]) -> Result<ClusterInfoResponse, anyhow::Error> {
    ...
    for cluster in clusters {
        ...
        match runtime_services::cluster_info::get_super_cluster_info(&trace_id, cluster).await {
            Ok(cluster_info_obj) => {
