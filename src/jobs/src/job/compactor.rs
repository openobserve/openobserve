...
pub async fn run() -> Result<(), anyhow::Error> {
    ...
    let mut scheduler =
        JobScheduler::new(cfg.limit.file_merge_thread_num, worker.tx(), merge_service);
    ...
    spawn_pausable_job!(
        ...
        get_config().compact.sync_to_db_interval,
        {
            ...
            if let Err(e) = openobserve_compactor::repository::files::sync_cache_to_db().await {
    ...
    spawn_pausable_job!(
        ...
        get_config().compact.sync_to_db_interval,
        {
            ...
            if let Err(e) =
                openobserve_compactor::repository::downsampling::sync_cache_to_db().await
            {
                log::error!(
