...
use openobserve_api::handler::http::router::*;
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // init http server
    if let Err(e) = openobserve_api::server::run(web::ui_routes).await {
...
async fn init_action_server() -> Result<(), anyhow::Error> {
    let cfg = get_config();
    let haddr = openobserve_api::server::server_addr()?;
    ...
    let app = openobserve_api::server::apply_common_middlewares(create_action_server_router())
        .layer(TraceLayer::new_for_http());

    openobserve_api::server::serve(haddr, app).await?;
