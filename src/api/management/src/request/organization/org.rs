...
mod tests {
    ...
    /// `browser_steps_used: quota.browser_used`.
    const SYNTHETICS_FIELDS: [(&str, &str); 2] =
        [("browser_steps", "browser"), ("protocol_steps", "protocol")];
    ...
    /// The field-prefix half of `SYNTHETICS_FIELDS` for a pool. Exhaustive, so a third pool
