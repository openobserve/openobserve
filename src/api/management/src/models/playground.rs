...
#[derive(Clone, Debug, Deserialize, Serialize, ToSchema)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct SeedFromSpanRequestBody {
    /// Traces stream holding the span.
    pub stream: String,
    pub trace_id: String,
    pub span_id: String,
    /// Search window in microseconds. A trace is found by scanning a time
    /// range, so the caller must say roughly when it happened.
    pub start_time: i64,
    pub end_time: i64,
}
...
#[derive(Clone, Debug, Serialize, ToSchema)]
#[serde(rename_all = "camelCase")]
pub struct SeededColumnBody {
    pub messages: Vec<PlaygroundMessageBody>,
    pub model: Option<String>,
    /// Set when a configured provider serves this model. Absent means the
    /// caller must pick one before running.
...
#[derive(Clone, Debug, Serialize, ToSchema)]
#[serde(rename_all = "camelCase")]
pub struct SeedFromSpanResponseBody {
    pub column: SeededColumnBody,
    /// What the span actually answered. Kept out of the editable messages on
    /// purpose: it is a reference to compare against, not a prompt to resend.
