...
`openobserve-api-http` is the HTTP transport composition root for OpenObserve. It composes the
...
- `openobserve-api-ingest` for log, metric, trace/OTLP, RUM, and cluster ingestion.
...
- `openobserve-api-management` for alerts, dashboards, organizations, users, streams, actions, AI,
  node and platform administration, and other control-plane APIs.

Two further crates support them rather than owning a domain:

- `openobserve-api-common` for shared HTTP types, extractors, and authentication.
...
such as `openobserve-core`, `common`, `audit`, or `openobserve-api-common`. This crate is the only
...
4. CRUD, administration, automation, alerts, dashboards, stream and node management, health,
   authentication, and configuration go to `src/api/management`.

Code used by two or more API crates goes to `openobserve-api-common` only when it is
