...
pub async fn cache_status() -> impl IntoResponse {
    ...
    let (len, cap, mem_size) = ::search::file_list::DELETED_FILES.stats();
    stats.insert(
        "DELETED_FILES",
        json::json!({"len": len, "cap": cap, "mem_size": mem_size}),
    );

    let (len, cap, mem_size) = ::search::file_list::DEDUPLICATE_FILES.stats();
    stats.insert(
        "DEDUPLICATE_FILES",
        json::json!({"len": len, "cap": cap, "mem_size": mem_size}),
    );
    ...
    let (len, cap, mem_size) = maintenance::compact::organization::STREAMS.stats();
    stats.insert(
...
async fn reload_module_cache(module: &str) -> Result<(), anyhow::Error> {
    match module {
        ...
        "functions" => catalog::functions::cache(openobserve_core::telemetry::error_sink()).await,
