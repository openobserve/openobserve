...
async fn update(msg: Message) -> Result<()> {
    ...
    match trigger.module {
        ...
        TriggerModule::PublicDashboard => {
            // Only sync if the public dashboard still exists in this region.
            if infra::table::public_dashboards::get(conn, &trigger.module_key)
                .await
                .unwrap_or(None)
                .is_some()
            {
                ...
                log::warn!(
                    "[SUPER_CLUSTER:sync] Public dashboard not found for module_key: {}. No need to sync this trigger",
                    trigger.module_key
                );
            }
        }
