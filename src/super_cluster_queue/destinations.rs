...
pub(crate) async fn process(msg: Message) -> Result<()> {
    match msg.message_type {
        MessageType::DestinationPut => {
            let bytes = msg
                .value
                .ok_or(Error::Message("Message missing value".to_string()))?;
            let destination: Destination = json::from_slice(&bytes).inspect_err(|e| {
                log::error!(
                    "[SUPER_CLUSTER:DESTINATION] Failed to deserialize message value to destination: {}",
                    e
                );
            })?;
            infra::table::destinations::put(destination).await?;
            infra::cluster_coordinator::destinations::emit_put_event(&msg.key).await?;
        }
        MessageType::DestinationDelete => {
            infra::cluster_coordinator::destinations::emit_delete_event(&msg.key).await?;
            let (org_id, name) = parse_event_key(&msg.key)?;
            infra::table::destinations::delete(org_id, name).await?;
        }
        _ => {
            log::error!(
                "[SUPER_CLUSTER:DESTINATION] Invalid message: type: {:?}, key: {}",
                msg.message_type,
                msg.key
            );
...
fn parse_event_key(event_key: &str) -> Result<(&str, &str)> {
    let item_key = event_key
        .strip_prefix(DESTINATION_WATCHER_PREFIX)
