...
pub async fn merge_files(
    ...
    // an open-hour batch is sized in files by the planner, so it is never cut by bytes here
    let merge_whole_batch = mode.merges_whole_batch() || mode.merges_open_hour_pending();
