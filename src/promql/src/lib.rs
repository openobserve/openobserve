...
pub trait TableProvider: Sync + Send + 'static {
    ...
    /// and standalone providers use the default implementation, while the
    /// OpenObserve provider supplies a receiver backed by its query manager.
...
mod tests {
    ...
    fn test_adjust_start_end_preserves_point_count_and_aligns() {
        ...
        let start = 105i64;
        let end = 205i64;
        let original_points = (end - start) / step + 1;
