...
pub(crate) fn histogram_quantile(phi: f64, data: Value, eval_ctx: &EvalContext) -> Result<Value> {
    ...
    let input_series = in_matrix.len();
    ...
    log::info!(
        "[trace_id: {trace_id}] [PromQL Timing] histogram_quantile processed {input_series} bucket series into {} groups across {} time points in {:?}",
        range_values.len(),
        timestamps.len(),
        start.elapsed(),
    );
