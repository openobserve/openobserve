...
pub(crate) fn round(data: Value, to_nearest: &ScalarArg) -> Result<Value> {
...
mod tests {
    ...
    fn test_math_operations_values() {
        ...
        let round = |input| apply(|data| super::round(data, &ScalarArg::Value(1.0)), input);
        assert_eq!(round(3.2), 3.0);
        assert_eq!(round(3.5), 4.0);
        assert_eq!(round(3.7), 4.0);
        assert_eq!(round(-3.2), -3.0);
        assert_eq!(round(-3.5), -3.0);
        let round = |input| apply(|data| super::round(data, &ScalarArg::Value(0.5)), input);
        assert_eq!(round(3.2), 3.0);
        assert_eq!(round(3.25), 3.5);
        assert_eq!(round(3.7), 3.5);
        assert_eq!(round(-3.2), -3.0);
        let round = |input| apply(|data| super::round(data, &ScalarArg::Value(0.1)), input);
        assert_eq!(round(2.345), 2.3);
        assert_eq!(round(0.3), 0.3);
        let round = |input| apply(|data| super::round(data, &ScalarArg::Value(5.0)), input);
    ...
    fn test_round() {
        ...
        let result = round(value, &ScalarArg::Value(1.0)).unwrap();
