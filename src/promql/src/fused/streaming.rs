...
/// Runs the fused aggregation as parallel per-hash-band ordered streams; every
/// context (sorted storage, hash-sorted WAL) contributes chains to the same
/// bands, so a series split across them is still merged whole.
...
pub(crate) async fn streaming_fused_agg(
    ctxs: &[(SessionContext, Arc<Schema>, bool)],
    ...
    let (first_ctx, first_schema, _) = &ctxs[0];
    if ctxs.iter().any(|(_, schema, _)| {
        schema
    ...
    let Some(group_cols) = group_label_columns(modifier, first_schema, shape.func.name()) else {
        return Ok(None);
    };
    ...
    let bands = first_ctx.state().config().target_partitions();
    let mut band_inputs: Vec<Vec<_>> = (0..bands).map(|_| Vec::new()).collect();
    let mut band0_plan = None;
    for (ctx, schema, keep_filters) in ctxs {
        // a WAL projection may lack a group column the full schema has
        if group_cols
            .iter()
        ...
        // an exact index selection already dropped the matched rows' filters
        let df = if *keep_filters {
            apply_matchers(df, selector.matchers)?
        } else {
            df
        };
        let Some((inputs, plan)) = build_band_inputs(&df, &columns, bands, &trace_id).await? else {
            return Ok(None);
        };
        band0_plan.get_or_insert(plan);
        for (band, streams) in inputs.into_iter().enumerate() {
            band_inputs[band].extend(streams);
        }
    }
    let band0_plan = band0_plan.expect("at least one context");
...
mod tests {
    ...
    async fn run_streaming_ctxs(
        ctxs: Vec<SessionContext>,
        modifier: &Option<LabelModifier>,
        func_name: &str,
        op: FusedAggOp,
        range: Duration,
    ...
    async fn run_streaming(
        ...
        run_streaming_ctxs(vec![ctx.clone()], modifier, func_name, op, range).await
    }
    ...
    fn sorted_ctx_from_rows(rows: Vec<Row>) -> SessionContext {
        let ctx = session_ctx();
        let sort_order: Vec<SortExpr> = vec![
            col(HASH_LABEL).sort(true, false),
            col(TIMESTAMP_COL_NAME).sort(true, false),
        ];
        let table = MemTable::try_new(arrow_schema(), vec![vec![rows_to_batch(rows)]])
            .unwrap()
            .with_sort_order(vec![sort_order]);
        ctx.register_table(format!("m{STREAMING_AGG_TABLE_SUFFIX}"), Arc::new(table))
            .unwrap();
    ...
    /// Early samples in a "storage" context, late ones in a "WAL" context:
    /// every multi-sample series — including the counter reset at the split —
    /// crosses the context boundary and must be merged whole.
    #[tokio::test]
    async fn test_streaming_merges_wal_context_chains() {
        let (early, late): (Vec<Row>, Vec<Row>) =
            test_rows().into_iter().partition(|row| row.1 < 110);
        let storage_ctx = sorted_ctx_from_rows(early);
        let wal_ctx = sorted_ctx_from_rows(late);
        let range = Duration::from_secs(60);
        let eval_ctx = eval_ctx();

        for op in [FusedAggOp::Sum, FusedAggOp::Avg, FusedAggOp::Count] {
            for func_name in ["rate", "increase", "last_over_time"] {
                for modifier in [None, by(&["path"])] {
                    ...
                    let actual = run_streaming_ctxs(
                        vec![storage_ctx.clone(), wal_ctx.clone()],
                        &modifier,
                        func_name,
                        op,
                        range,
                    ...
                    .await
                    .expect("streaming path must not fall back on sorted contexts");

                    assert_matrix_close(
                        canonical_matrix(expected),
                        canonical_matrix(actual),
                        &format!(
                            "wal-merged streaming {}({func_name}) (modifier: {modifier:?})",
                            op.name()
                        ),
                    );
                }
    ...
    #[tokio::test]
    async fn test_streaming_falls_back_when_one_context_lacks_the_sorted_table() {
        let (early, late): (Vec<Row>, Vec<Row>) =
            test_rows().into_iter().partition(|row| row.1 < 110);
        let storage_ctx = sorted_ctx_from_rows(early);
        let wal_ctx = session_ctx();
        let table = MemTable::try_new(arrow_schema(), vec![vec![rows_to_batch(late)]]).unwrap();
        wal_ctx.register_table("m", Arc::new(table)).unwrap();

        let result = run_streaming_ctxs(
            vec![storage_ctx, wal_ctx],
            &None,
            "rate",
            FusedAggOp::Sum,
