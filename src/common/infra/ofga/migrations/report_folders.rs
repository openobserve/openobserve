...
///
/// 1. Establishes ownership tuples for every report folder that exists in the database, so that the
///    `rfolder` type is properly represented in OpenFGA.
///
/// 2. For every existing report, establishes a `parent` relation between `report:{report_id}` and
///    `rfolder:{folder_id}`.  Before this migration all reports lived in the default folder, so
///    this effectively links every report to `rfolder:default`.
///
/// 3. For every role in every organisation: replaces the org-level `report:_all_<org_id>`
///    permission with the equivalent `rfolder:_all_<org_id>` permission.  Individual
...
pub async fn migrate_report_folders<C: ConnectionTrait>(db: &C) -> Result<(), anyhow::Error> {
    ...
    while let Some(page) = folder_pages.fetch_and_next().await? {
        ...
        if !tuples.is_empty() {
            match authorizer::authz::update_tuples(tuples, vec![]).await {
                Ok(_) => {
                    log::debug!("{folder_len} report folders migrated to openfga");
                }
                Err(e) => {
                    log::error!("Error migrating report folders in openfga: {e}");
                }
            }
        }
    }
    ...
    #[cfg(not(feature = "cloud"))]
    {
        ...
        // ------------------------------------------------------------------
        // 3. Migrate org-level role assignments: report:_all_<org_id>  →  rfolder:_all_<org_id>
        //
            ...
            for role in roles.iter() {
                ...
                if !add_roles.is_empty() {
                        ...
                        .await
                    {
                        Ok(_) => {
                            log::debug!(
                                "{count} report-folder role tuples added for role {role} in org \
                                 {org}"
                            );
                        }
                        Err(e) => {
