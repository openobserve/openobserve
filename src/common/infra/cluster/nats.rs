...
async fn register() -> Result<()> {
    ...
    for node in node_list {
        ...
        super::NODES.pin().insert(node.uuid.clone(), node);
    }
    ...
    super::NODES.pin().insert(LOCAL_NODE.uuid.clone(), node);
...
pub(crate) async fn set_status(status: NodeStatus) -> Result<()> {
    ...
    let mut node = match super::NODES.pin().get(LOCAL_NODE.uuid.as_str()) {
        Some(node) => {
