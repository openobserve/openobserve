...
impl From<LokiError> for actix_web::HttpResponse {
    fn from(error: LokiError) -> Self {
        ...
        match error {
            // Client errors (400)
            LokiError::InvalidTimestamp { message } => {
                HttpResponse::BadRequest()
                    .content_type("text/plain")
                    .body(format!("invalid timestamp: {}", message))
            },
            LokiError::InvalidLabels { message } => {
                HttpResponse::BadRequest()
                    .content_type("text/plain")
...
mod tests {
    ...
    fn test_error_to_http_response() {
        let error = LokiError::InvalidTimestamp { message: "bad timestamp".to_string() };
    ...
    fn test_client_vs_server_errors() {
        let client_errors = vec![
            LokiError::InvalidTimestamp { message: "test".to_string() },
            LokiError::InvalidLabels { message: "test".to_string() },
            LokiError::EmptyStream,
            LokiError::UnsupportedContentType { content_type: "test".to_string() },
        ];
        ...
        let server_error = LokiError::Ingestion { source: anyhow::anyhow!("internal error") };
    ...
    fn test_loki_entry_deserialize_array_formats() {
        ...
        let entry: LokiEntry = serde_json::from_str(r#"["1609459200000000000", "test message"]"#).unwrap();
        ...
        let entry: LokiEntry = serde_json::from_str(r#"["1609459200000000000", "test message", {"trace": "123"}]"#).unwrap();
        assert_eq!(entry.timestamp, "1609459200000000000");
        assert_eq!(entry.line, "test message");
        assert_eq!(entry.structured_metadata.unwrap().get("trace"), Some(&"123".to_string()));
    }
