...
impl FromRequest for AuthExtractor {
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        ...
        async move {
            ...
            let auth_str = extract_auth_str_async(&req).await;
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        ...
        Box::pin(async move { Err(actix_web::error::ErrorUnauthorized("Unauthorized Access")) })
    }
}

#[cfg(feature = "enterprise")]
pub async fn extract_auth_str_async(req: &HttpRequest) -> String {
            ...
            match USER_SESSIONS.get(&session_key) {
                ...
                None => {
                    ...
                    match crate::service::db::session::get(&session_key).await {
                        Ok(token) => {
                            // Update cache with the fetched token
                            USER_SESSIONS.insert(session_key.clone(), token.clone());
                            format!("Bearer {}", token)
                        }
                        Err(e) => {
                            log::warn!(
