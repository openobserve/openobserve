...
pub async fn update_stats_from_file_list(catalog: &dyn StreamCatalog) -> Result<(), anyhow::Error> {
    ...
    // get last offset
    let (mut last_updated_at, node) = repository::stats::get_offset().await;
    ...
    repository::stats::set_offset(latest_updated_at, Some(&LOCAL_NODE.uuid.clone()))
        .await
...
async fn update_stats_lock_node() -> Result<Option<i64>, anyhow::Error> {
    ...
    // first
    let (offset, node) = repository::stats::get_offset().await;
    if !node.is_empty() && LOCAL_NODE.uuid.ne(&node) && get_node_by_uuid(&node).await.is_some() {
    ...
    let ret = repository::stats::set_offset(offset, Some(&LOCAL_NODE.uuid.clone())).await;
