...
/// Port implemented by the composition layer that owns the heavy merge
/// algorithm. Worker lifecycle and scheduling remain in this crate.
#[async_trait]
...
impl MergeExecutor for NoopMergeExecutor {
    async fn merge_by_stream(
        &self,
        _worker_tx: mpsc::Sender<(MergeSender, MergeBatch)>,
        _org_id: &str,
                ...
                loop {
                    ...
                    match ret {
                        ...
                        Some(job) => {
                            ...
                            crate::repository::stream::clear_running(&key);
                        }
                    }
                }
            });
...
mod job_scheduler_tests {
    ...
    fn test_job_scheduler_new_and_tx() {
        ...
        let scheduler = JobScheduler::new(3, worker_tx, Arc::new(NoopMergeExecutor));
        let tx = scheduler.tx();
        drop(tx);
    }
    ...
    fn test_job_scheduler_multiple_tx_clones() {
        let (worker_tx, _rx) = mpsc::channel::<(MergeSender, MergeBatch)>(1);
        let scheduler = JobScheduler::new(1, worker_tx, Arc::new(NoopMergeExecutor));
...
impl MergeWorker {
    pub fn new(num: usize, executor: Arc<dyn MergeExecutor>) -> Self {
...
mod merge_worker_tests {
    ...
    fn test_merge_worker_new_and_tx() {
        let worker = MergeWorker::new(4, Arc::new(NoopMergeExecutor));
        let tx = worker.tx();
        drop(tx);
    }
    ...
    fn test_merge_worker_new_single() {
        let _worker = MergeWorker::new(1, Arc::new(NoopMergeExecutor));
    }
}
