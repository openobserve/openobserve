...
use super::{
    auth::{
        token::token_validator, validator, validator_aws, validator_gcp, validator_proxy_url,
        validator_rum,
    },
    request::{
        dashboards::{folders::*, *},
...
pub fn get_service_routes(cfg: &mut web::ServiceConfig) {
    ...
    let service_config = if !CONFIG.common.enable_jwt_auth {
        cfg.service(
            web::scope("/api")
