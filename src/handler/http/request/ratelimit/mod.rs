...
const RULE_MAX_RESOURCE_LENGTH: usize = 1024;
const RESOURCE_LENGTH_ERROR: &str = "Resource length must be between 1 and 1024 characters.";

fn validate_resource(resource: &str) -> Result<(), HttpResponse> {
    if resource.is_empty() || resource.len() > RULE_MAX_RESOURCE_LENGTH {
        return Err(HttpResponse::BadRequest().json(MetaHttpResponse::error(
            http::StatusCode::BAD_REQUEST.into(),
            RESOURCE_LENGTH_ERROR.to_string(),
        )));
    }
    Ok(())
}
...
pub async fn save_ratelimit(
    ...
    if let Err(resp) = validate_resource(&rr.resource) {
        return Ok(resp);
    }
