...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Backfill job cancelled successfully"),
        (status = 404, description = "Job not found"),
        (status = 500, description = "Internal server error"),
    ),
)]
...
pub async fn cancel_backfill(
    ...
    log::info!(
        "[BACKFILL API] Cancel backfill job {} for org {}",
        job_id,
        org_id
    );
    ...
    match backfill::cancel_backfill_job(&org_id, &job_id).await {
        Ok(_) => {
            log::info!(
                "[BACKFILL API] Cancelled backfill job {} for org {}",
                job_id,
                org_id
            );
        ...
        Err(e) => {
            log::error!(
                "[BACKFILL API] Failed to cancel backfill job {} for org {}: {}",
                job_id,
                org_id,
                e
            );
