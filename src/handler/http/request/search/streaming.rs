...
use std::{cmp::Reverse, collections::BinaryHeap, io::Error, time::Instant};
use actix::ActorFutureExt;
use actix_web::{HttpRequest, HttpResponse, get, http::StatusCode, post, web, web::Bytes};
...
use crate::{
    ...
    service::{
        ...
        search::{
            cache, datafusion::distributed_plan::streaming_aggs_exec,
            sql::Sql,
        },
        self_reporting::{http_report_metrics, report_request_usage_stats},
        setup_tracing_with_trace_id,
        websocket_events::search::{process_delta, send_cached_responses},
        websocket_events::sort::order_search_results,
    },
};

/// Search with HTTP/2 Streaming
...
pub async fn search_http2_stream(
    ...
    let (tx, rx) = futures::channel::mpsc::channel::<Result<Bytes, actix_web::Error>>(100);
    let sender = tx.clone();
    ...
    // Spawn the search handling in a separate task
    tokio::spawn(async move {
        // Create a callback for sending results to client
        let send_message = |event_type: &str, payload: serde_json::Value| {
            ...
            Box::pin(async move {
                match json::to_vec(&message) {
                    Ok(json_bytes) => {
                        ...
                        let _ = sender.send(Ok(Bytes::from(bytes_with_newline))).await;
                    },
                    Err(e) => {
        ...
        let req_size = if req.query.size == 0 {
            req.query.size = cfg.limit.query_default_limit;
            req.query.size
        } else {
            req.query.size
        };
        ...
        if let Some(interval) = sql.histogram_interval {
            ...
            if let Some(updated_query) = update_histogram_interval_in_query(&req.query.sql, interval) {
        ...
        if req.payload.query.from == 0 {
            let c_resp =
                cache::check_cache_v2(&trace_id, &org_id, stream_type, &req.query, req.use_cache)
        ...
        send_message(req.trace_id.clone(), end_res.to_json()).await?;
        ...
        http_report_metrics(
            ...
            &org_id_clone,
            stream_type_clone,
            "200", // Assume success for metrics
            "_search_stream",
            "",
            "",
        );
...
pub async fn do_partitioned_search(
    ...
    send_message: &mut impl Fn(&str, serde_json::Value),
) -> Result<(), Error> {
...
pub async fn handle_cache_responses_and_deltas(
    ...
    let cache_order_by =
        if req.search_type.unwrap() == SearchEventType::Dashboards || req.size == -1 {
            &OrderBy::Desc
        } else {
            req_order_by
        };
        ...
        *curr_res_size += search_res.hits.len() as i64;
        ...
        if !search_res.hits.is_empty() {
            ...
            *start_timer = Instant::now();
            ...
            send_message(trace_id.as_str(), ws_search_res.to_json());
        }
...
async fn send_partial_search_resp(
    ...
    send_message(trace_id, ws_search_res.to_json());
