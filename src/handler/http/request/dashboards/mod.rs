...
#[utoipa::path(
    ...
    extensions(
        ...
        ("x-o2-mcp" = json!({
            "description": "Create a dashboard with visualization panels. LAYOUT: 192-column grid with {x, y, w, h, i} where x=column(0-191), y=row, w=width, h=height, i=panel_id. Common widths: full=192, half=96, third=64. Min width=12, min height=8. PANEL QUERIES: Each panel query needs 'fields' with x/y/z/breakdown arrays populated EVEN when customQuery=true. Field alias values MUST match the SQL SELECT column aliases exactly (case-insensitive). AXIS RULES: x=dimension/time field, y=metric/aggregate field(s), z=ONLY for heatmaps (color intensity value) - leave z=[] for all other chart types. BREAKDOWN FIELD (CRITICAL): Use the separate 'breakdown' array (NOT z) to split data into multiple series by a categorical field. Example: to show 'errors by service over time', use x=time, y=count, breakdown=service_name. Breakdown works with: bar, stacked, h-bar, h-stacked, line, area, area-stacked, scatter. Breakdown does NOT work with: pie, donut, metric, gauge, heatmap, table, geomap, sankey. When breakdown is used, the SQL must GROUP BY both x and breakdown columns. Example with breakdown: 'SELECT histogram(_timestamp) as x_axis_1, service as breakdown_1, COUNT(*) as y_axis_1 FROM stream GROUP BY x_axis_1, breakdown_1 ORDER BY x_axis_1' needs x=[{label:'Time',alias:'x_axis_1',column:'x_axis_1'}], y=[{label:'Count',alias:'y_axis_1',column:'y_axis_1'}], breakdown=[{label:'Service',alias:'breakdown_1',column:'breakdown_1'}]. CHART TYPE SELECTION: line/area/area-stacked=time series with continuous data (AVOID for single data points - renders invisible); bar/stacked=categories or time buckets (works with any count of data points); metric=single KPI number; gauge=single value with min/max range; pie/donut=proportions (no breakdown, x=categories y=values); heatmap=2D matrix (needs x, y, z); table=raw records or aggregated rows. SINGLE DATA POINT WARNING: line, area, area-stacked, and scatter charts render a nearly invisible dot with only 1 data point - use bar or metric instead. FILTER: The 'filter' field in 'fields' MUST be an object (NOT an array). Use: {type:'list',values:[],logicalOperator:'AND',filterType:'list'} for no filters. GEOMAP FIELDS: Use separate latitude, longitude, and weight fields (not z). SANKEY FIELDS: Use separate source, target, and value fields. TABLE PANELS: When type='table' with customQuery=true and empty x/y fields, MUST set config.table_dynamic_columns=true, otherwise no columns appear. VRL FUNCTION: Always set vrlFunctionQuery='' (empty string, NOT null) in each query object.",
            "category": "dashboards"
        }))
    )
)]
...
#[utoipa::path(
    ...
    extensions(
        ...
        ("x-o2-mcp" = json!({"description": "Update an existing dashboard. WORKFLOW: Always call GetDashboard first to get the current dashboard JSON and its hash, then modify the JSON, then call UpdateDashboard with the full modified body. This is a full PUT replacement - you must send the complete dashboard body including all tabs, panels, and config (not just changed fields). HASH PARAMETER: Pass the hash from GetDashboard response in the 'hash' query parameter for conflict detection - omitting it may overwrite concurrent changes. FIELD RULES: Same as CreateDashboard - all field alias/breakdown/chart-type rules apply. See CreateDashboard description for details on breakdown fields, chart type selection, axis rules, and filter format.", "category": "dashboards"}))
    )
)]
