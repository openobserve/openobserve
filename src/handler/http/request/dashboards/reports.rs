...
#[utoipa::path(
    ...
    extensions(
        ...
        ("x-o2-mcp" = json!({"description": "Create a scheduled report for a dashboard. Requires: title, dashboards array (each with dashboard/folder/tabs/timerange), frequency (cron/once), destinations (email/slack). Reports can be PDF screenshots or data exports.", "category": "dashboards"}))
    )
)]
#[post("/{org_id}/reports")]
