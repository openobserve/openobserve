...
pub async fn get_latest_traces(
    ...
    let query_sql = format!(
        "SELECT {TIMESTAMP_COL_NAME}, trace_id, start_time, end_time, duration, service_name, operation_name, span_status, reference_parent_span_id FROM \"{stream_name}\" WHERE trace_id IN ('{trace_ids}') ORDER BY {TIMESTAMP_COL_NAME} ASC"
    );
    req.query.from = 0;
    req.query.size = 9999;
    ...
    loop {
        ...
        for item in resp_search.hits {
            ...
            // Prefer the true root span (empty reference_parent_span_id) over first-by-timestamp.
            let is_root = item
                .get("reference_parent_span_id")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .is_empty();
            if trace.first_event.is_null() || is_root {
...
async fn process_latest_traces_stream(
    ...
    for (idx, partition) in partitions_desc.iter().enumerate() {
        ...
        let detail_sql = format!(
            "SELECT {TIMESTAMP_COL_NAME}, trace_id, start_time, end_time, duration, service_name, operation_name, span_status, reference_parent_span_id \
        ...
        loop {
            ...
            for item in detail_res.hits {
                ...
                if let Some(trace) = traces_data.get_mut(&tid) {
                    // Prefer the true root span (empty reference_parent_span_id) over
                    // first-by-timestamp.
                    let is_root = item
                        .get("reference_parent_span_id")
                        .and_then(|v| v.as_str())
