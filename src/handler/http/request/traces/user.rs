...
#[utoipa::path(
    get,
    path = "/{org_id}/{stream_name}/traces/user",
    context_path = "/api",
    tag = "Traces",
    operation_id = "GetLatestUsers",
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            metrics::HTTP_INCOMING_REQUESTS
                ...
                .inc();
            log::error!("get users latest data error: {err:?}");
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            metrics::HTTP_INCOMING_REQUESTS
                ...
                .inc();
            log::error!("get users trace details error: {err:?}");
            return map_error_to_http_response(&err, Some(trace_id));
        }
    };
...
fn aggregate_users(
    ...
    users_data.sort_by_key(|k| k.last_event);
    users_data
}
