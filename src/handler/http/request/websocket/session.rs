...
use crate::{
    ...
    service::websocket_events::{
        ...
        sessions_cache_utils, handle_values_request
    },
};
        ...
        Ok(client_msg) => {
            match client_msg {
                ...
                WsClientEvents::Values(ref values_req) => {
                    ...
                    let values_result = handle_values_event(
                        values_req,
                        org_id,
                        user_id,
                        req_id,
                    ...
                    if let Err(e) = values_result {
                        log::error!("[WS_HANDLER]: Error processing values request: {}", e);
...
/// Handle the values WebSocket event
/// Creates a cancellable task to process values request
async fn handle_values_event(
    values_req: &config::meta::websocket::ValuesEventReq,
    org_id: &str,
    user_id: &str,
    req_id: &str,
    #[allow(unused_variables)] path: String,
    ...
    // Register running values search
    WS_SEARCH_REGISTRY.insert(
        trace_id.clone(),
        SearchState::Running {
            cancel_tx,
