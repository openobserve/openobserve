...
#[utoipa::path(
    ...
    description = "Updates incident title, severity, or status. Only one field can be updated per request.",
    security(("Authorization" = [])),
    params(
        ("org_id" = String, Path, description = "Organization name"),
        ("incident_id" = String, Path, description = "Incident ID"),
    ...
    responses(
        ...
        (status = 404, description = "Not found", content_type = "application/json", body = ()),
        (status = 400, description = "Invalid value", content_type = "application/json", body = ()),
    ),
    extensions(
        ("x-o2-ratelimit" = json!({"module": "Alerts", "operation": "update"}))
    )
)]
#[patch("/v2/{org_id}/alerts/incidents/{incident_id}/update")]
