        ...
        if openfga_cfg.enabled {
            for (index, org) in source_orgs.iter().enumerate() {
                ...
                if index == 0 {
                    ...
                    tuples.push(get_user_org_tuple(&user_email, &user_email, Some(&org.role.to_string())));
                }
...
async fn map_group_to_custom_role(
    ...
    if db_user.is_none() {
        ...
        if openfga_cfg.enabled {
            ...
            .await;
            tuples.push(get_user_org_tuple(&dex_cfg.default_org, user_email, Some(&role.to_string())));
            // this check added to avoid service accounts from logging in
            if !role.eq(&UserRole::ServiceAccount) {
                tuples.push(get_user_org_tuple(user_email, user_email, Some(&role.to_string())));
            }
