...
use o2_enterprise::enterprise::{dex::service::auth::get_jwks, common::infra::config::O2_CONFIG};
...
pub async fn token_validator(
    ...
    match jwt::verify_decode_token(token.token(), &keys, &O2_CONFIG.dex.client_id, false).await {
        Ok(res) => {
            if res.0.is_valid {
