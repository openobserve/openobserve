...
const WAL_FILE_MAX_SIZE: usize = 1024 * 1024 * 256; // 256MB
...
pub fn get_writer(org_id: &str, stream_type: &str) -> Arc<Writer> {
    let key = WriterKey::new(org_id, stream_type);
    let mut rw = WRITERS.write();
    let w = rw
        .entry(key.clone())
...
pub fn get_reader(org_id: &str, stream_type: &str) -> Option<Arc<Writer>> {
    let key = WriterKey::new(org_id, stream_type);
    WRITERS.read().get(&key).cloned()
}
...
impl Writer {
    pub(crate) fn new(key: WriterKey) -> Self {
        ...
        Self {
            ...
            wal: Arc::new(Mutex::new(
                WalWriter::new(WAL_DIR, &key.org_id, &key.stream_type, wal_id)
    ...
    pub fn write(&self, schema: Arc<Schema>, entry: Entry) -> Result<()> {
        let data = entry.into_bytes()?;
        if self.check_threshold(data.len()) {
            // rotation wal
            let mut wal = self.wal.lock();
            let id = self.next_seq.fetch_add(1, Ordering::SeqCst);
            println!("wal rotation: {}", id);
            let new_wal_writer =
                WalWriter::new(WAL_DIR, &self.key.org_id, &self.key.stream_type, id)
                    .context(WalSnafu)?;
            ...
            drop(wal);
            drop(memtable);
            IMMUTABLES.lock().push(immutable::Entry::new(
                self.key.clone(),
                old_wal_writer.path().clone(),
                old_mem_table,
            ));
        }

        // write into wal
        self.wal.lock().write(&data).context(WalSnafu)?;

        // write into memtable
        self.memtable.write().write(schema, entry)?;
        Ok(())
    }
    ...
    fn check_threshold(&self, data_size: usize) -> bool {
        let wal_size = self.wal.lock().size();
        wal_size + data_size > WAL_FILE_MAX_SIZE
            || self.created_at.load(Ordering::Relaxed)
                + Duration::seconds(WAL_FILE_ROTATION_INTERVAL)
                    .num_microseconds()
                    .unwrap()
