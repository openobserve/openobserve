...
async fn move_files_to_storage() -> Result<(), anyhow::Error> {
    ...
    move_arrow_files_to_storage().await?;
...
async fn move_arrow_files_to_storage() -> Result<(), anyhow::Error> {
    let data_dir = Path::new(&CONFIG.common.data_wal_dir)
        .canonicalize()
        .unwrap();

    let pattern = format!("{}/files/*/*/*/*.arrow", &CONFIG.common.data_wal_dir);
    let files = scan_files(&pattern);
