...
pub(crate) async fn generate_index_on_ingester(
    ...
    // /home/runner/work/o2-enterprise/o2-enterprise/openobserve/src/job/files/parquet.rs:625:26:
    // index out of bounds: the len is 0 but the index is 0
    if buf.is_empty() {
        log::debug!("buf len is 0");
    }

    let index_record_batches = prepare_index_record_batches_v1(
        buf,
        org_id,
        stream_name,
        &new_file_key,
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "prepare_index_record_batches_v1 error, org/stream {}/{}. new_file_key, {}. Error {}",
            org_id,
            stream_name,
            new_file_key,
            e
    ...
    if record_batches.is_empty() {
        log::debug!("index_record_batches len is 0",);
    }
    let json_rows = record_batches_to_json_rows(&record_batches).map_err(|e| {
        anyhow::anyhow!(
            "record_batches_to_json_rows error, org/stream {}/{}. new_file_key, {}. Error {}",
            org_id,
            stream_name,
            new_file_key,
            e
