...
use crate::infra::{cluster, config::CONFIG, wal, metrics, storage};
...
async fn move_files_to_storage() -> Result<(), anyhow::Error> {
    let files = wal::get_waiting_to_move_files();
    ...
    for (file, data) in files {
        ...
        if db::compact::delete::is_deleting_stream(&org_id, &stream_name, stream_type, None) {
            ...
            if wal::FILES.write().unwrap().remove(&file).is_none() {
            ...
            Ok(ret) => match ret {
                Ok((path, key, meta, _stream_type)) => {
                    match db::file_list::local::set(&key, meta, false).await {
                        Ok(_) => {
                            if wal::FILES.write().unwrap().remove(&path).is_none() {
...
async fn upload_file(
    ...
    if file_size == 0 {
        if wal::FILES.write().unwrap().remove(path_str).is_none() {
