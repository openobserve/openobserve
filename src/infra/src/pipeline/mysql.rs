...
(
    ...
    async fn create_table_index(&self) -> Result<()> {
        ...
        let queries = vec![
            "CREATE INDEX pipeline_org_idx ON pipeline (org_id);",
            "CREATE INDEX pipeline_org_id_idx ON pipeline (org_id, id);",
            "CREATE UNIQUE INDEX pipeline_src_type_stream_params_idx ON pipeline (source_type, org_id, stream_name, stream_type);",
        ];
    ...
    async fn put(&self, org_id: &str, pipeline: Pipeline) -> Result<()> {
        ...
        if let Err(e) = match pipeline.source {
            PipelineSource::Stream(stream_params) => {
                ...
                sqlx::query(
                    r#"
INSERT IGNORE INTO pipeline (id, version, name, description, source_type, org_id, stream_name, stream_type, nodes, edges)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                    "#,
                )
                .bind(pipeline.id)
            ...
            PipelineSource::Query(query_inner) => {
                ...
                sqlx::query(
                    r#"
INSERT IGNORE INTO pipeline (id, version, name, description, source_type, org_id, query_inner, nodes, edges)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
                    "#,
                )
                .bind(pipeline.id)
    ...
    async fn get_by_stream(
        ...
        let pool = CLIENT.clone();
        let query = r#"SELECT * FROM pipeline WHERE source_type = ? AND org_id = ? AND stream_name = ? AND stream_type = ?;"#;
    ...
    async fn get_by_id(&self, org_id: &str, pipeline_id: &str) -> Result<Pipeline> {
        let pool = CLIENT.clone();
        let query = r#"SELECT * FROM pipeline WHERE id = ? AND org_id = ?;"#;
        let pipeline = match sqlx::query_as::<_, Pipeline>(query)
            .bind(pipeline_id)
            .bind(org_id)
            .fetch_one(&pool)
    ...
    async fn list(&self, org_id: &str) -> Result<Vec<Pipeline>> {
        let pool = CLIENT.clone();
        let query = r#"SELECT * FROM pipeline WHERE org_id = ? ORDER BY id;"#;
            ...
            .await
        {
            ...
            Err(e) => {
                log::debug!("[MYSQL] list pipeline by org_id error: {}", e);
