...
impl FileData {
    ...
    async fn set(&mut self, trace_id: &str, file: &str, data: Bytes) -> Result<(), anyhow::Error> {
        ...
        if self.cur_size + data_size >= self.max_size {
            log::info!(
                "[trace_id {trace_id}] [CacheType:{}] File disk cache is full, can't cache extra {} bytes",
                self.file_type,
                data_size
            );
    ...
    async fn gc(&mut self, trace_id: &str, need_release_size: usize) -> Result<(), anyhow::Error> {
        let cfg = get_config();
        log::info!(
            "[trace_id {trace_id}] [CacheType:{}] File disk cache start gc {}/{}, need to release {} bytes",
            self.file_type,
            self.cur_size,
            self.max_size,
            need_release_size
        ...
        loop {
            ...
            if let Err(e) = fs::remove_file(&file_path) {
                log::error!(
                    "[trace_id {trace_id}] [CacheType:{}] File disk cache gc remove file: {}, error: {}",
                    self.file_type,
                    file_path,
                    e
                );
