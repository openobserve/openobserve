...
pub struct Model {
    ...
    pub id: String,
    pub org_id: String,
    /// Canonical `k=v/k=v` of the dimensions that matched no rule. The queue is
    /// keyed on it, so a signal that keeps firing into the same gap is one row
    /// with a count rather than a row per firing.
    pub path: String,
    ...
    /// means it paged nobody — what every row written before the default-team
    /// tier existed meant, so the two agree without a backfill.
