...
mod tests {
    ...
    #[tokio::test]
    async fn test_add_and_get_dimension_values() {
        // Clear test data
        let _ = clear_org("test_org").await;

        let org_id = "test_org";
        let dimension = "environment";

        // Add some values
        add(DimensionValueRecord::new(
            org_id, dimension, "hash1", "prod",
        ))
        .await
        .unwrap();
        add(DimensionValueRecord::new(
            org_id, dimension, "hash2", "staging",
        ))
        .await
        .unwrap();
        add(DimensionValueRecord::new(org_id, dimension, "hash3", "dev"))
            .await
            .unwrap();

        // Get values
        let values = get_dimension_values(org_id, dimension).await.unwrap();
        assert_eq!(values.len(), 3);
        assert!(values.contains(&"hash1".to_string()));
        assert!(values.contains(&"hash2".to_string()));
        assert!(values.contains(&"hash3".to_string()));
    }

    #[tokio::test]
    async fn test_dimension_cardinality() {
        let _ = clear_org("test_org2").await;

        let org_id = "test_org2";

        // Add values for multiple dimensions
        add(DimensionValueRecord::new(org_id, "env", "h1", "prod"))
            .await
            .unwrap();
        add(DimensionValueRecord::new(org_id, "env", "h2", "dev"))
            .await
            .unwrap();
        add(DimensionValueRecord::new(org_id, "region", "h3", "us-west"))
            .await
            .unwrap();

        // Check cardinality
        let env_card = get_dimension_cardinality(org_id, "env").await.unwrap();
        assert_eq!(env_card, 2);

        let region_card = get_dimension_cardinality(org_id, "region").await.unwrap();
        assert_eq!(region_card, 1);
    }

    #[tokio::test]
    async fn test_duplicate_insertion() {
        let _ = clear_org("test_org3").await;

        let org_id = "test_org3";
        let record = DimensionValueRecord::new(org_id, "version", "hashX", "1.0");

        // Insert same record twice - should not error
        add(record.clone()).await.unwrap();
        add(record).await.unwrap();

        // Should only have 1 record
        let card = get_dimension_cardinality(org_id, "version").await.unwrap();
        assert_eq!(card, 1);
    }
}
