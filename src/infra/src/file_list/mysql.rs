            ...
            tasks.push(tokio::task::spawn(async move {
                ...
                let cfg = get_config();
                 if cfg.limit.use_upper_bound_for_max_ts {
                    ...
                    let query = format!("SELECT id, records, original_size FROM file_list WHERE stream = ? AND max_ts >= ? AND max_ts <= ? AND min_ts <= ? {};",cond);
                    ...
                    let query = format!("SELECT id, records, original_size FROM file_list WHERE stream = ? AND max_ts >= ? AND min_ts <= ? {};",cond); 
                    sqlx::query_as::<_, super::FileId>(&query)
                    .bind(stream_key)
                    .bind(time_start)
                    .bind(time_end)
