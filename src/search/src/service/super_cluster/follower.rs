...
pub async fn search(
    ...
    // check work group
    let _lock = crate::service::work_group::acquire_work_group_lock(
        &trace_id,
        &req,
        &mut took_watch,
...
pub async fn get_file_id_lists(
    ...
    {
        let start = runtime::enrichment_table_start_time(org_id, &stream_name).await;
        let end = config::utils::time::now_micros();
        time_range = (start, end);
    }
    let file_id_list =
        runtime::query_file_ids(trace_id, org_id, stream_type, &stream_name, time_range).await?;
    Ok(file_id_list)
}
