...
//! Search orchestration, cluster execution, caching, partitioning, and streaming APIs.
...
use openobserve_vrl::{
    apply_vrl_fn, compile_vrl_function, get_all_transform_keys, init_vrl_runtime,
};
...
use {
    crate::service::partition::aggregate::prepare_streaming_aggregate,
...
use crate::service::{
    ...
    runtime::report_request_usage_stats,
};
...
pub use crate::{bloom_pruner, datafusion, index, inspector, sql, tantivy, utils};
    ...
    match res {
        Ok(mut res) => {
            if in_req.query.streaming_output && meta.order_by.is_empty() {
                res = crate::service::streaming::order_search_results(res, None);
            }
