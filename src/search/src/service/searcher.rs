    ...
    pub async fn get_task_status(&self) -> Vec<proto::cluster_rpc::QueryStatus> {
        ...
        for query_status in &mut status {
            if let Some(ref mut ctx) = query_status.search_event_context
                ...
                && let Some(dashboard) =
                    crate::service::runtime::dashboard_context(org_id, dashboard_id).await
            {
                ctx.dashboard_name = Some(dashboard.dashboard_name);
                ctx.dashboard_folder_name = Some(dashboard.folder_name);
                ctx.dashboard_folder_id = Some(dashboard.folder_id);
            }
        }
