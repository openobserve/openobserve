...
use crate::service::{partition::sql_context::PartitionSqlContext, runtime};
...
pub async fn collect_stream_files(
    ...
    for (stream, schema) in schemas.iter() {
        ...
        if !use_single_partition && !use_stream_stats_for_partition {
            let stream_files =
                runtime::query_file_ids(trace_id, org_id, stream_type, &stream_name, time_range)
                    .await?;
            let settings_max_query_range =
                runtime::settings_max_query_range(stream_settings.max_query_range, org_id, user_id)
                    .await;
