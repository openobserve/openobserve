...
use {
    crate::service::partition::sql_context::PartitionSqlContext,
...
pub async fn prepare_streaming_aggregate(
    ...
    let streaming_id = if req.streaming_output && ctx.is_streaming_aggregate {
        ...
        let group_by_fields =
            crate::service::sql::visitor::group_by::get_group_by_fields(&ctx.sql).await?;
        let cardinality_map = crate::service::cardinality::check_cardinality(
            org_id,
            stream_type,
            &stream_name,
            &group_by_fields,
