...
#[cfg(feature = "enterprise")]
use crate::service::cache::cacher::delete_cache;
use crate::{
    QueryDelta, SearchResultType,
    service::{self as SearchService},
};
...
pub async fn do_partitioned_search(
    ...
    for (idx, &[start_time, end_time]) in partitions.iter().enumerate() {
        ...
        if is_result_array_skip_vrl {
            search_res.hits = crate::service::cache::apply_vrl_to_response(
    ...
    if is_non_ts_order_by {
        ...
        if is_result_array_skip_vrl {
            final_res.hits = crate::service::cache::apply_vrl_to_response(
                backup_query_fn.clone(),
                &mut final_res,
                org_id,
                stream_name,
...
async fn send_partial_search_resp(
    ...
    .await?;
    if is_result_array_skip_vrl {
        s_resp.hits = crate::service::cache::apply_vrl_to_response(
            backup_query_fn.clone(),
            &mut s_resp,
            org_id,
            stream_name,
