...
pub async fn get_file_id_lists(
    ...
    for stream in stream_names {
        ...
        {
            let start = runtime::enrichment_table_start_time(org_id, &name).await;
            let end = now_micros();
            time_range = (start, end);
        }
        // get file list
        let file_id_list =
            runtime::query_file_ids(trace_id, org_id, stream_type, &name, time_range).await?;
