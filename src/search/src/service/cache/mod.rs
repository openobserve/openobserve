...
use openobserve_vrl::{self as functions, apply_vrl_fn, compile_vrl_function, init_vrl_runtime};
use proto::cluster_rpc::SearchQuery;
use result_utils::get_ts_value;
use tracing::Instrument;

use crate::{
    CachedQueryResponse, MultiCachedQueryResponse, QueryDelta,
    service::{
        self as SearchService,
        cache::{cacher::check_cache, result_utils::extract_timestamp_range},
        inspector::{SearchInspectorFieldsBuilder, search_inspector_fields},
        runtime::{http_report_metrics, report_request_usage_stats},
        sql::RE_SELECT_FROM,
    },
};
...
pub async fn search(
    ...
    #[cfg(feature = "vectorscan")]
    crate::service::cache::apply_regex_to_response(
        &req,
        org_id,
        &stream_name,
        stream_type,
...
pub async fn prepare_cache_response(
    ...
    let sql = match crate::service::Sql::new(&query, org_id, stream_type, req.search_type).await {
        Ok(v) => v,
        Err(e) => {
    ...
    if is_complex_query && sql.histogram_interval.is_some() {
        ...
        crate::service::sql::histogram::handle_histogram(
            &mut origin_sql,
            q_time_range,
...
pub async fn apply_regex_to_response(
    ...
    let sql =
        match crate::service::sql::Sql::new(&query, org_id, stream_type, req.search_type).await {
            Ok(v) => v,
            Err(e) => {
                log::error!("Error parsing sql: {e}");
                return Ok(());
    ...
    let projections: std::collections::HashMap<String, Vec<ProjectionColumnMapping>> =
        crate::service::datafusion::plan::regex_projections::get_columns_from_projections(sql)
            .await?;
