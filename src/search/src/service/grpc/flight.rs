...
pub async fn search(
    ...
    if runtime::is_deleting_stream(&org_id, stream_type, &stream_name) {
...
async fn get_file_list_by_ids(
    ...
    let file_list =
        runtime::query_files_by_ids(trace_id, org_id, stream_type, stream_name, time_range, ids)
            .await?;
...
async fn update_index_updated_at(
    ...
    let ttv_timestamp_updated_at = runtime::ttv_timestamp_updated_at().await;
    ...
    if matches!(
        ...
        let ttv_secondary_index_updated_at = runtime::ttv_secondary_index_updated_at().await;
