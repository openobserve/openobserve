...
async fn get_data(
    ...
    // db_stats.end_time
    let end_time_exclusive = infra::table::enrichment_tables::get_meta_stats(&org_id, clean_name)
        .await
        .map(|stats| stats.end_time + 1); // +1 because search end_time is exclusive

    let data = match infra::table::enrichment_tables::get_data_by_org_and_name(
        &org_id,
        clean_name,
        end_time_exclusive,
    )
