...
mod tests {
    ...
    #[test]
    fn test_deriv_value_none_input() {
        let result = deriv_test_helper(Value::None).unwrap();
        assert!(matches!(result, Value::None));
    }

    #[test]
    fn test_deriv_invalid_input_returns_err() {
        let result = deriv_test_helper(Value::Float(1.0));
