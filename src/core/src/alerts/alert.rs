...
impl AlertExt for Alert {
    ...
    async fn send_notification(
        ...
        let dest_result = send_to_destinations(
            self,
            &self.destinations,
            rows,
            rows_end_time,
        ...
        // Workflows are alert-level: composite alerts route their `on_composite` /
        // `on_term` destination subsets through `send_to_destinations` directly, so
        // triggering workflows here keeps them firing exactly once per alert.
        ...
        if !self.workflows.is_empty() {
            ...
            // A workflow-only alert (no destinations) fails only when every
            // workflow trigger failed.
            if self.destinations.is_empty() && workflow_error == self.workflows.len() {
                return Err(AlertError::SendNotificationError {
