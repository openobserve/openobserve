...
/// without a restart.
/// Refuses to bring up a super-cluster region that cannot encrypt portably.
///
/// The whole fix for o2-enterprise#2451 rests on every region holding the same
/// `ZO_SYNTHETICS_ENCRYPTION_KEY`, and nothing else checks that. Left empty,
/// this region falls back to the per-org DEK — minted locally, different
/// everywhere — and reproduces the original bug exactly: checks it creates
/// carry secrets no other region can read, with no error at any point. That is
/// worth refusing to start over, because the alternative is discovering it from
/// silently lost runs.
///
/// Resolving the key here also pins it before any encrypt path can run, so the
/// value is fixed for the life of the process.
///
/// Only meaningful with the super cluster on; the caller gates on that.
pub fn super_cluster_preflight() -> Result<(), String> {
    preflight_verdict(
        config::get_config().synthetics.enabled,
        service::configured_key(),
    )
}

/// The decision itself, split from the globals it reads so both branches can be
/// tested — the key cache is process-wide and fixed by whatever resolves it
/// first.
fn preflight_verdict(
    synthetics_enabled: bool,
    key: &Result<Option<Vec<u8>>, String>,
) -> Result<(), String> {
    ...
    match key {
        Ok(Some(_)) => Ok(()),
        Ok(None) => Err(
            "ZO_SYNTHETICS_ENCRYPTION_KEY is required when the super cluster is enabled: \
             without it each region encrypts check secrets under its own key, so a check \
             created in one region cannot run in another (o2-enterprise#2451). Set the same \
             base64 64-byte value in every region."
                .to_string(),
        ),
        Err(e) => Err(format!(
            "ZO_SYNTHETICS_ENCRYPTION_KEY is set but unusable: {e}"
        )),
    }
}
...
mod preflight_tests {
    ...
    #[test]
    fn a_configured_key_starts() {
        assert!(preflight_verdict(true, &Ok(Some(vec![1u8; 64]))).is_ok());
    }

    /// The case the fix exists for: without a shared key this region silently
    /// falls back to a locally-minted DEK and reproduces o2-enterprise#2451.
    #[test]
    fn a_missing_key_refuses_to_start() {
        let err = preflight_verdict(true, &Ok(None)).unwrap_err();
        assert!(err.contains("ZO_SYNTHETICS_ENCRYPTION_KEY is required"));
    }

    #[test]
    fn an_unusable_key_refuses_to_start() {
        let err = preflight_verdict(true, &Err("bad base64".into())).unwrap_err();
        assert!(err.contains("unusable"));
    }

    /// Synthetics off means nothing encrypts, so the key is irrelevant and must
    /// not block an unrelated deployment from starting.
    #[test]
    fn synthetics_disabled_does_not_require_a_key() {
        assert!(preflight_verdict(false, &Ok(None)).is_ok());
    }
}
