...
pub struct Common {
    ...
    #[env_config(
        ...
        default = false,
        help = "Persist memtables into packed wal files (one file per rotation instead of one file per stream). NOTE: requires the pack-aware file mover; keep disabled until the full pipeline supports packs"
    )]
