...
use crate::service::{
    db::background_job::{
        get_job, get_partition_jobs_by_job_id, is_have_partition_jobs, set_error_message,
        set_partition_error_message, set_partition_status, submit_partitions,
    },
...
pub async fn run() -> Result<(), anyhow::Error> {
    ...
    // 2. check if the job is previous running (get error then retry, be cancel then retry) (case 1)
    ...
    if !have_partition_job {
        ...
        if let Err(e) = res {
            set_error_message(job.id, &e.to_string()).await?;
            log::error!("handle_search_partition error: {}", e);
            return Err(e);
        }
    }
    ...
    for partition_job in partition_jobs.iter() {
        ...
        if let Err(e) = res {
            set_error_message(job.id, &e.to_string()).await?;
            log::error!("run_partition_job error: {}", e);
            return Err(e);
        }
    }
    ...
    // 5. update `background_jobs` table
    set_status(job.id, 2).await?;
...
// TODO: swap the other thread to update updated_at
...
async fn run_partition_job(job: &Job, partition_job: &PartitionJob) -> Result<(), anyhow::Error> {
    ...
    // 2. set the partition status to running
    set_partition_status(job.id, partition_job.partition_id, 1).await?;
    ...
    .await;
    if let Err(e) = res {
        set_partition_error_message(job.id, partition_job.partition_id, &e.to_string()).await?;
        return Err(e.into());
    }
    ...
    // 5. set the partition status to finish
    set_partition_status(job.id, partition_job.partition_id, 2).await?;
    Ok(())
}
