...
pub async fn set(org_id: &str, key: &str, val: Bytes) -> Result<(), anyhow::Error> {
    ...
    // This is necessary because the coordinator (NATS or meta table) is the event bus.
    // The actual KV data is stored in kv_store table; watchers fetch from there.
    // The migration will clean up these markers from the meta table.
    let coord_key = format!("/kv/{}", cache_key);
    db::put(
        &coord_key,
        Bytes::from("__kv_marker__"),
        db::NEED_WATCH,
        None,
    )
    .await?;
...
pub async fn delete(org_id: &str, key: &str) -> Result<(), anyhow::Error> {
    ...
    let coord_key = format!("/kv/{}", cache_key);
    db::delete(&coord_key, false, db::NEED_WATCH, None).await?;
