...
async fn ingest_evaluator_traces(org_id: &str, traces: Vec<json::Value>, node_idx: usize) {
    if traces.is_empty() {
        return;
    }

    let traces_json = serde_json::to_vec(&traces).unwrap_or_default();
    if traces_json.is_empty() {
        return;
    }

    let ingest_req = proto::cluster_rpc::IngestionRequest {
        org_id: org_id.to_string(),
        stream_name: config::meta::self_reporting::evaluator::EVALUATOR_STREAM.to_string(),
        stream_type: config::meta::stream::StreamType::Traces.to_string(),
        data: Some(proto::cluster_rpc::IngestionData { data: traces_json }),
        ingestion_type: Some(proto::cluster_rpc::IngestionType::Json as i32),
        ..Default::default()
    };
    if let Err(e) = crate::service::ingestion::ingestion_service::ingest(ingest_req).await {
        log::error!(
            "[Pipeline]: LLM evaluation node {node_idx} failed to self-ingest evaluator traces: {e}"
        );
    }
}
            ...
            while let Some(item) = receiver.recv().await {
                    ...
                    )
                {
                    ...
                    if !sampled {
                        ...
                        ingest_evaluator_traces(&org_id, vec![skipped_trace.span_json], node_idx)
                            .await;
                        count += 1;
                        continue;
                    }
