...
pub(crate) fn avg_over_time_range(data: Value, eval_ctx: &EvalContext) -> Result<Value> {
    let start = std::time::Instant::now();
    log::info!(
        "[trace_id: {}] [PromQL Timing] avg_over_time_range() started",
        eval_ctx.trace_id
    );
    let result = super::eval_range(data, AvgOverTimeFunc::new(), eval_ctx);
    log::info!(
        "[trace_id: {}] [PromQL Timing] avg_over_time_range() execution took: {:?}",
        eval_ctx.trace_id,
        start.elapsed()
    );
    result
...
mod tests {
    ...
    fn avg_over_time_test_helper(data: Value) -> Result<Value> {
        ...
        avg_over_time_range(data, &eval_ctx)
    }
    ...
    fn test_avg_over_time_function() {
        ...
        // Should return a vector with average value
        match result {
            Value::Vector(v) => {
                assert_eq!(v.len(), 1);
                // Average should be (10+20+30)/3 = 20.0
                assert!((v[0].sample.value - 20.0).abs() < 0.001);
