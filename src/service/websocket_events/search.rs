...
pub async fn handle_search_request(
    ...
    let stream_names = match resolve_stream_names(&req.payload.query.sql) {
        ...
        Err(e) => {
            ...
            send_message(req_id, err_res.to_json(), Some(trace_id)).await?;
            return Ok(());
        }
    };
    ...
    send_message(req_id, end_res.to_json(), Some(trace_id)).await?;
        ...
        *curr_res_size += search_res.hits.len() as i64;
        ...
        if !search_res.hits.is_empty() {
            ...
            *remaining_query_range -= queried_range;
            ...
            send_message(req_id, ws_search_res.to_json(), Some(trace_id.to_string())).await?;
        }
