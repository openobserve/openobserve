...
mod tests {
    ...
    #[test]
    fn test_combine_already_combined() {
    ...
    #[test]
    fn test_combine_no_field() {
        // Non-count without a field → returns function name as-is
    ...
    #[test]
    fn test_validate_bad_query_mode() {
        let req = make_req("sql", "1h", "1h");
        assert!(validate_config_request(&req).is_err());
    }

    #[test]
    fn test_validate_filters_mode_missing_filters() {
        let mut req = make_req("filters", "1h", "1h");
        req.filters = None;
        assert!(validate_config_request(&req).is_err());
    }
    ...
    fn test_validate_custom_sql_mode_missing_sql() {
        let mut req = make_req("custom_sql", "1h", "1h");
        req.custom_sql = None;
        assert!(validate_config_request(&req).is_err());
    }

    #[test]
    fn test_validate_filters_mode_ok() {
        let req = make_req("filters", "5m", "1h");
        assert!(validate_config_request(&req).is_ok());
    }

    #[test]
    fn test_validate_custom_sql_mode_ok() {
        let req = make_req("custom_sql", "1h", "30m");
        assert!(validate_config_request(&req).is_ok());
    }

    #[test]
    fn test_validate_invalid_histogram_interval() {
        let req = make_req("filters", "1d", "1h");
        assert!(validate_config_request(&req).is_err());
    }

    #[test]
    fn test_validate_invalid_schedule_interval() {
        let req = make_req("filters", "5m", "60s");
        assert!(validate_config_request(&req).is_err());
    }
}
