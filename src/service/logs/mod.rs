...
use crate::common::utils::hasher::get_fields_key_xxh3;
...
pub fn cast_to_type(mut value: Value, delta: Vec<Field>) -> (Option<String>, Option<String>) {
    ...
    if parse_error.is_empty() {
        (
            Some(common::utils::json::to_string(&local_map).unwrap()),
            None,
        )
    } else {
...
async fn add_valid_record(
    ...
    let mut value_str = common::utils::json::to_string(&local_val).unwrap();
    // check schema
    ...
    if schema_evolution.schema_compatible {
        let valid_record = if schema_evolution.types_delta.is_some() {
            ...
            let loc_value: Value = common::utils::json::from_slice(value_str.as_bytes()).unwrap();
