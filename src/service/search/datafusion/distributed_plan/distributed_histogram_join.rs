...
use super::{
    ...
    node::RemoteScanNode, remote_scan::RemoteScanExec,
};
...
impl DistributedTimeBinJoinExec {
    fn execute_local_processing(
        ...
        for node in worker_nodes {
            ...
            let remote_exec = Arc::new(RemoteScanExec::new(local_join_plan, remote_node)?);
...
impl TimeBinCoordinatorStream {
    ...
    fn add_batch_to_time_bins(&mut self, batch: RecordBatch) -> Result<()> {
        ...
        let time_array = batch
            ...
            .ok_or_else(|| {
                DataFusionError::Internal("Invalid timestamp column type".to_string())
            })?;
    ...
    fn merge_time_bin(&mut self, time_bin: i64) -> Result<RecordBatch> {
        ...
        if batches.is_empty() {
            return Err(DataFusionError::Internal(
                "No batches for time bin".to_string(),
            ));
        }
        ...
        let concatenated =
            arrow::compute::concat_batches(&self.schema, batch_refs.iter().copied())?;
