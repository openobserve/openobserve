...
mod tests {
    ...
    async fn test_get_segment_ids_strips_account_prefix() {
        ...
        set(trace_id, "schema", vec![file]).await;

        let location = get(&format!("{trace_id}/schema=schema")).unwrap()[0]
            .location
            .to_string();
        assert!(location.contains("/acc1/::/"));
        assert_eq!(get_segment_ids(&location).as_deref(), Some(&*bits));
        assert_eq!(
            get_segment_ids(&format!("{trace_id}/schema=schema/$$/{file_key}")).as_deref(),
            Some(&*bits)
        );
        clear(trace_id);
    }
