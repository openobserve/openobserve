...
use datafusion::{
    common::{
        Result,
        tree_node::{TreeNodeRecursion, TreeNode, TreeNodeVisitor},
    },
...
impl HistogramJoinPlanner {
    ...
    /// Replace NewEmptyExec in either side with the data-bearing side in distributed histogram joins
    fn fix_empty_exec_sides(
        ...
        if is_right_empty && !is_left_empty {
            log::info!("HistogramJoinPlanner: Right side is empty ({}), using left side data for both sides", right.name());
        ...
        if is_left_empty && !is_right_empty {
            log::info!("HistogramJoinPlanner: Left side is empty ({}), using right side data for both sides", left.name());
    ...
    fn find_or_create_data_source(&self, plan: &Arc<dyn ExecutionPlan>) -> Result<Arc<dyn ExecutionPlan>> {
        ...
        if let Some(data_source) = visitor.data_source {
            log::info!("HistogramJoinPlanner: Found data source: {}", data_source.name());
            Ok(data_source)
        } else {
...
impl TreeNodeVisitor<'_> for DataSourceFinder {
    ...
    fn f_down(&mut self, node: &Self::Node) -> Result<TreeNodeRecursion> {
        ...
        if node.name().contains("DataSourceExec") ||
           ...
           (node.name() != "NewEmptyExec" && node.name() != "EmptyExec" && node.children().is_empty()) {
            log::debug!("DataSourceFinder: Found potential data source: {}", node.name());
...
impl ExtensionPlanner for HistogramJoinPlanner {
    async fn plan_extension(
        ...
        log::info!("HistogramJoinPlanner: plan_extension called for node type: {}", node.name());
        ...
        if let Some(histogram_join) = node.as_any().downcast_ref::<HistogramJoinNode>() {
            ...
            return self.create_histogram_join_exec(histogram_join, physical_inputs, session_state).await;
        }

        log::info!("HistogramJoinPlanner: Node is not a HistogramJoinNode, delegating to default planner");
...
impl HistogramJoinPlanner {
    async fn create_histogram_join_exec(
        ...
        log::info!(
            ...
            (histogram_join.left_time_column.clone(), histogram_join.right_time_column.clone()),
        ...
        log::debug!("Left schema fields: {}", physical_inputs[0].schema().fields().len());
        log::debug!("Right schema fields: {}", physical_inputs[1].schema().fields().len());
        log::debug!("Join schema fields: {}", histogram_join.schema.fields().len());
        ...
        let (left_plan, right_plan) = self.fix_empty_exec_sides(
        ...
        log::debug!("Using original join schema with {} fields", arrow_schema.fields().len());
        ...
        let histogram_join_exec = HistogramSortMergeJoinExec::new_with_schema(
            ...
            histogram_join.join_columns.clone(),
            histogram_join.time_bin_interval.clone(),
            None, // No remote scan nodes for now
            arrow_schema,
        )?;
