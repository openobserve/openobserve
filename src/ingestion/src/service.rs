...
use common::{infra::config::STREAM_ALERTS, meta::stream::SchemaRecords};
use config::{
...
use crate::types::IngestionRequest;
...
pub async fn evaluate_trigger(triggers: TriggerAlertData) {
    ...
    for (alert, val) in triggers.iter() {
        let module_key = alert::scheduler_key(alert.id);
    ...
    for trigger_data_stream in trigger_usage_reports {
        crate::ports::publish_trigger_usage(trigger_data_stream);
    }
}
...
pub async fn check_ingestion_allowed(
    ...
    #[cfg(feature = "cloud")]
    {
        if !crate::ports::is_org_in_free_trial_period(org_id).await? {
