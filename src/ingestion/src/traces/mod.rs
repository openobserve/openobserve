...
use self::otel::{OtelIngestionProcessor, is_llm_trace};
use crate::{
    grpc::get_val,
    logs::O2IngestJsonData,
    ports::{self, DistinctValue, TraceListValue},
    repository::model_pricing,
    service::{
...
pub async fn handle_otlp_request(
    ...
    #[cfg(feature = "cloud")]
    {
        match ports::is_org_in_free_trial_period(org_id).await {
            Ok(false) => {
    ...
    if infra::schema::get_is_llm_stream(org_id, &traces_stream_name, StreamType::Traces).await {
        is_llm_stream = true;
        if let Err(e) =
            ports::ensure_gen_ai_fields_in_schema(org_id, &traces_stream_name, StreamType::Traces)
                .await
        {
            log::warn!(
                ...
                Ok(pl_results) => {
                    ...
                    for (stream_params, stream_pl_results) in pl_results {
                        ...
                        for (_idx, mut res) in stream_pl_results {
                            ...
                            {
                                record_val = crate::service::refactor_map(record_val, fields);
                            }
    ...
    if need_mark_llm_stream
        && let Err(e) =
            ports::set_stream_is_llm(org_id, &traces_stream_name, StreamType::Traces, true).await
    {
...
async fn write_traces_by_stream(
    ...
    for (traces_stream_name, (json_data, fn_num)) in json_data_by_stream {
        // For cloud, emit an event when a new trace stream is created.
        #[cfg(feature = "cloud")]
        ports::report_stream_created_if_new(org_id, &traces_stream_name, StreamType::Traces, None)
            .await
...
async fn write_traces(
    ...
    if stream_schema.has_partition_keys {
        partition_keys =
            crate::service::get_stream_partition_keys(org_id, &StreamType::Traces, stream_name)
                .await
    }
    ...
    for (timestamp, record_val) in json_data {
        ...
        let trace_id = record_val
            ...
            .unwrap()
            .to_string();
        trace_index_values.push(TraceListValue {
            timestamp,
            stream_name: stream_name.to_string(),
    ...
    if cfg.common.traces_list_index_enabled
        ...
        && let Err(e) = ports::write_trace_list(org_id, trace_index_values).await
    {
...
mod tests {
    ...
    use crate::grpc::get_val_for_attr;
