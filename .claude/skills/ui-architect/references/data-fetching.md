...
| --- | --- | --- |
| 1 min | `LIVE_STALE_TIME` | alerts (list, detail, history, dependencies), alert sources, anomaly detection, incidents, SLOs, synthetics monitors, enrichment-table job statuses, on-call **state** (who is on call, team overview, escalation preview + progress, responses, deliveries, coverage gaps, unrouted signals, my on-call), DB monitoring tab counts on a relative range |
