...
| --- | --- | --- |
| Fetching data for a page | Never call a service from a component. Read through `web/src/composables/query/` — pick a **tier**, never a `staleTime`. | [.claude/rules/fe-data-fetching.md](../../rules/fe-data-fetching.md) |
...
- [ ] Any data the page reads goes through `composables/query/` with a **tier**,
      not a service call with a hand-rolled `loading` ref. Writes invalidate a
      key prefix rather than re-calling the page loader. Anything carrying a
      token or key material sets `persist: "none"`.
      See [.claude/rules/fe-data-fetching.md](../../rules/fe-data-fetching.md).
