...
| **Whole-page layout** | Prefer **`AppPage`** — it owns the whole skeleton (full-height column + `AppPageHeader` + optional `#subnav` tab strip + a body already wrapped in `OContent`), so you plug in title/icon/actions/tabs/body and CANNOT hand-roll a padded `<div>` that drifts. Pass `bleed` when the body is a full-bleed `OTable`. Drop to raw `AppPageHeader` + `flex flex-col h-full p-0` only when `AppPage` can't express the layout (then change `AppPage`, don't fork it). Sidebar pages use `PageLayout`. | [page-recipes](references/page-recipes.md) |
| **Content inset** | inside `AppPage` the body is already inset. Anywhere else (a panel, a dialog section, one tab's content) wrap it in **`OContent`** (bakes the one `px-page-edge` grid line) instead of hand-picking `px-2`/`px-4`/`p-2.5`; pass `bleed` (or `bleed-x`/`bleed-y`) for full-bleed content that owns its own edge — same escape-hatch idea as `ODrawer`/`ODialog` `bleed`. Never hand-roll a content inset. | [conventions](references/conventions.md) |
