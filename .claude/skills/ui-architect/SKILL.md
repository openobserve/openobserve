...
| **Content inset** | wrap every page body / panel / tab-panel / dialog section in **`OContent`** (bakes the one `px-page-edge` grid line) instead of hand-picking `px-2`/`px-4`/`p-2.5`; pass `bleed` (or `bleed-x`/`bleed-y`) for full-bleed content (an `OTable`, a chart, a flow canvas) that owns its own edge — same escape-hatch idea as `ODrawer`/`ODialog` `bleed`. Never hand-roll a content inset. | [conventions](references/conventions.md) |
