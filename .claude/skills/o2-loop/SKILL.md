...
| coder | `o2-coder` subagent (`.claude/agents/o2-coder.md`) | implements the spec, runs the gates, answers findings; continued across rounds with `SendMessage` so it keeps its context, or respawned from the ledger when that tool is unavailable |
...
```bash
while IFS= read -r line; do echo "$line"; case "$line" in *" question: "*|*" step: finished"*) break ;; esac; done < <(tail -n 0 -f <ledger>/round-N/coder.log); pkill -f "^tail -n 0 -f <ledger>/round-N/coder.log"
```
The watcher ends only on the coder's fixed last line `step: finished` or on a `question:` line; a looser pattern such as `done` matches ordinary step lines ("edits done; running fmt") and ends the watch early. If it does end early, start it again with the same command. When the coder reports back, post its summary to the user in one short paragraph. If it wrote a `question:` line, take the question to the user, answer it via `SendMessage` to the coder, and continue.
